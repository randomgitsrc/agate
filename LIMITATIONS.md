...
这不是假设性担忧——PeekView 真实发生过的 T005/T006 生产环境数据污染事故（详见 `docs/design-notes/main-agent-oversight.md`），根因正是主 Agent 接受了一个不该被接受的裁剪建议，自己没有做独立判断。事后的修复是"以后主 Agent 要更谨慎"，这是要求同一个单点更努力，不是结构性修复。

**现状**：部分缓解，未根治。已落地的应对是"针对具体已知风险做强制客观信号"（如 `[PROD_TOUCHED]` 标记），这能拦住已经发生过、已知的风险模式，但不是通用解法——下一个没被预见到的风险模式，仍然完全依赖主 Agent 当时的判断力。值得探索但尚未落地的方向是"确定性脚本扫描历史生成异常模式报告供人工审阅"，详见同一份决策记录。
