...
1. 第 1 次空返回：
   ...
   - 调整策略后重派：
     ...
     d. 加分阶段落盘指令（见派发 prompt 模板「分阶段落盘」——验证实测：空返回最有效的缓解）
...
2. 空返回后检查中间产物文件（subagent 是否写过任何文件 → 判断是否动过）
3. 从任务本身分析（任务是否涉及多异构产出、输入是否过多——见「任务粒度指引」）

**空返回的最可能根因**（验证实测）：不是上下文窗口被输入占满，而是**任务结构导致认知过载**——subagent 读完所有输入后面对"从零开始写一篇大报告"的推理复杂度过高，模型在单次推理中放弃。`steps` 上限无法缓解（已验证 steps:15/30 均无效）。有效的是**分阶段落盘**（见派发 prompt 模板的落盘指令）——把"一次性大产出"拆成"逐步小产出"，每步认知闭合，降低单次推理复杂度。
