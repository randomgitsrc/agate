...
| P6→P7 | BDD 验收通过 ⚠️ self-authored | `scripts/check-gate.sh P6` → exit 2（FAIL=0/NC=0/证据非空已验）+ 主 Agent 手动核实 `grep -cE '^\s*- (PASS\|FAIL)' P6-acceptance.md` = P1 BDD 总数（UI 条件须截图 + vision-analyst YAML 引用 + `summary.blocker_count → =0`）。**截图质量标准**：操作类 BDD 截图必须互不相同（md5 去重），查询类 BDD 可不截图（断言值是唯一证据）。任何 BDD 标 FAIL → gate 不通过 → 回 P4 |
