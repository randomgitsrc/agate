...
| 步骤 | 预计 | 交付 |
|------|------|------|
| 0. 基准验证 | 先跑（10min）| CHECK 5/9 baseline + state-machine.md:506 定位 |
| 1. rules/ 文件 | 提取+精简现有内容 | 2 个 rules 文件 |
| 2. 先抽 P4 卡片实测行数 | 用实际数据校正模板（P4 要从 953 行文件抽取，不要假设 ~60 行）| 1 张卡片 |
| 2. 其余 9 张卡片 | 依次写，不再预判行数 | 8 张卡片 + README |
...
建议顺序：0 → 1 → 2(先 P4 实测) → 2(其余 8 张) → 3 → 4 → 5 → 6 → 7。
