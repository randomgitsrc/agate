...
2. **state-machine.md:506**：
   - 当前："重读 orchestrator-template 列出的 8 个协议文件"
   - 改为："重读 orchestrator-template.md 的 mapping 表查当前阶段卡片，按卡片指引执行"

3. **check-protocol-consistency.py FILE_COUNT_ANCHORS**：
   - 去掉 `orchestrator-template.md expected=8` 锚点（理由：orchestrator-template 不再列 8 文件为必读）
   - 去掉 `state-machine.md expected=8` 锚点（理由：state-machine.md:506 不再引用 8 文件清单）
   - 保留 CHECK 5 其他锚点（如 v0.6 关键词、README badge 等）
