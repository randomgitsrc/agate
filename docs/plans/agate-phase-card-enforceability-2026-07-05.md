...
方案 C 实施成本可控——本质上是 A + 一个 hook 检查。所有改动在 hook 层（CI 可验证），不在 agent 内部（agent 不可靠）。
...
以下内容必须由 `agate-next-card.sh P{N}` 输出原样粘贴。hook 会校验 sha256 一致。

<!-- AGATE_CARD_HASH: <CLI 输出的 sha256> -->
...
if [ -f "$DC_FILE" ]; then
    # 提取嵌入卡片块
    EMBEDDED=$(awk '/<!-- AGATE_CARD_HASH: /,/<!-- END_AGATE_CARD -->/' "$DC_FILE" \
               | grep -v '^<!--' | grep -v '^```' | sed '/^$/d')
    ...
    if [ "$EMBEDDED_HASH" != "$EXPECTED_HASH" ]; then
        ...
        echo "      提示：重新调 agate-next-card.sh P${PHASE} 复制到 dispatch-context.md" >&2
        exit 1
    fi
fi
```

触发条件：仅当 `P${PHASE}-dispatch-context.md` 存在时才检查。如果 agent 没生成（且没派发 subagent），检查跳过——但这意味着 agent 跳过 subagent 派发就不受强制，这是已知边界。
...
| CLI 输出过长（如果某张卡片 >500 行） | 实测卡片行数（97/102/72/85/88 行），单张 ~100 行，远低于窗口 |
