...
> 定位：与 `design-independent-judge.md` / `design-structured-layer.md` 同级的设计文档，作为 v1.0 之后立项的地基，**非当前立项条目**。
...
1. **时机**：门户是"协议 v1.0 稳定之后"的事。协议还在补强（TAG0023 修 gate 缺口），急着做门户 = 在晃动的基座上盖楼。
