...
- **三类活动节奏（2026-09-03 补充，驱动 §3.4.2 两级冻结判据设计）**：subagent 运行 = 思考/输出/执行三类活动。实测：思考间隙（result→下个 call）p50=9.8s / p95=94s / **p99=742s / max≈1239s（20 分钟）**，40 次 >60s、12 次 >300s；执行阶段（call→result）p95=12s / p99=189s / **max≈925s（15 分钟）**，18 次 >60s；model 思考 chunk 内部间隔 p95=3.8s（**思考时事件流持续流动**）；活动事件（思考/输出/工具任一）静默间隔 p99=7.4s / p99.9=154s。**推论：subagent 合法"只思考不调工具"最长 20 分钟、"长命令执行中"最长 15 分钟，"最后一条命令开始距今"不能作冻结判据**，必须区分"卡住了"与"在思考/在跑长命令"（两级判据：调用冻结看未结束 call 时长 vs expected×2；活动冻结看无 call 悬挂时的最后活动事件）
