...
> 日期：2026-06-29（v2：2026-06-30 修订——基于 PeekView 12 任务实战评审）
> 状态：设计文档（待评审）
> 关联：LIMITATIONS.md 局限 3（主 Agent 判断力是单点故障）、局限 4（subagent 不可观测）
> 修订说明：v2 纳入 PeekView 实战评审发现（5 个结构性问题 + 4 个执行层问题），增加"流程选择硬约束"维度
...
| 编号 | 名称 | 做什么 | 解决什么 | 来源 |
|------|------|--------|---------|------|
| P1.1 | pre-commit-gate.sh | git commit 时自动跑 check-gate.sh，结果写入 .gate-result.json | gate 被跳过 | v1 |
| P1.2 | PROD_TOUCHED 检测 | 整合进 pre-commit hook，命中则中止 commit | 忘了扫描生产环境接触标记 | v1 |
| P1.3 | CI gate backstop | GitHub Action：push 时重跑 gate，对照 .gate-result.json | 故意 --no-verify 绕过 | v1 |
| P1.4 | .gate-result.json + .gate-history.jsonl | gate 结果存储 + 历史记录 | gate 结果可追溯、可对照 | v1 |
| P1.5 | READY 收尾检查脚本化 | git status 干净 + git tag 存在 检查脚本化 | 忘了检查发布前清理 | v1 |
| P1.6 | CHANGELOG 检查 | hook 检查 CHANGELOG.md [Unreleased] 含 task_id | P8 裁剪后 CHANGELOG 无写入时机 | v2 (S1) |
| P1.7 | P6 证据格式检查 | hook 检查 P6-acceptance.md 每条 BDD 有 Evidence 引用 | 证据存在不等于证据对应 | v2 (E1) |
...
| 编号 | 名称 | 做什么 | 解决什么 | 来源 |
|------|------|--------|---------|------|
| P2.1 | P6 验收独立化 | verifier subagent 独立执行 BDD 验收，直接 commit 产出（独立 author）；主 Agent 只读结论 | T026 根因：主 Agent 编造验收结果 | v1 |
| P2.2 | BDD 格式约定 + 总数对照 | P1 BDD 条目格式约定；check-gate.sh P6 扩展：从 P1 计数 vs P6 结果计数 | P6 exit 2 -> exit 0/1 | v1 |

#### 2B. 状态一致性强制

| 编号 | 名称 | 做什么 | 解决什么 | 来源 |
|------|------|--------|---------|------|
| P2.3 | 状态转移强制 | pre-commit hook 检查 .state.yaml phase 变更合法性 | 违规跳阶段 | v1 |
| P2.4 | 重试计数强制 | pre-commit hook 检查 retries >= MAX -> phase 必须是 PAUSED | 超限不暂停 | v1 |
| P2.5 | 回退跳变检测脚本化 | pre-commit hook 检查 phase 变更差值 >= 2 -> 必须有 PAUSED 记录 | T019 教训：跨阶段回退未暂停 | v1 |
| P2.6 | 修复后全量重跑验证 | .state.yaml 记录 last_fix_phase；hook 检查 .gate-result.json 是 full run | 修复引入回归 | v1 |

#### 2C. 流程选择硬约束（v2 新增——Agent 默认倾向于"不走"）

| 编号 | 名称 | 做什么 | 解决什么 | 来源 |
|------|------|--------|---------|------|
| P2.7 | 风险等级字段 | P1-requirements.md 增加 risk_level: low/medium/high + 量化条件 | 裁剪决策依赖临场判断 | v2 (S4) |
| P2.8 | 裁剪条件 hook 检查 | hook 检查裁剪需满足条件（风险等级 + BDD 数等） | Agent 默认倾向裁剪 | v2 (S4) |
| P2.9 | 裁剪声明回写 | 裁剪声明与实际执行不一致时必须回写 P1 override 字段 | P1 声明与执行不符 | v2 (C1) |
| P2.10 | P2 评审派发强制 | risk_level=high -> P2-review.md git author 非主 Agent | Agent 默认自审 | v2 (S3) |
| P2.11 | SCOPE+ 处理追踪 | 协议定义 [SCOPE_RESOLVED] 标记；hook 检查对应关系 | 忘了处理 SCOPE+ | v1 |
| P2.12 | 复盘异常触发 | 脚本检测异常模式 -> 强制提醒复盘 | Agent 默认不复盘 | v2 (E3) |
...
Phase 1 (纯加工具)          Phase 2 (协议改动)         Phase 3 (平台支持)
     ...
     |- P1.2 PROD_TOUCHED       |- P2.2 BDD 格式 + 总数      |- P3.2 subagent 可观测
     |- P1.3 CI backstop        |- P2.3 状态转移强制          |- P3.3 结果独立存储
     |- P1.4 gate-result        |- P2.4 重试计数强制          |- P3.4 gate 平台化
     |- P1.5 READY 检查         |- P2.5 回退跳变检测
     |- P1.6 CHANGELOG 检查      |- P2.6 修复全量重跑验证
     |- P1.7 P6 证据格式        |- P2.7 风险等级字段
                                |- P2.8 裁剪条件 hook
                                |- P2.9 裁剪声明回写
                                |- P2.10 P2 评审派发强制
                                |- P2.11 SCOPE+ 追踪
...
| Phase | 优先级 | 理由 | 预估工作量 |
|-------|--------|------|-----------|
| Phase 1 | 高 | 成本最低、收益最大、不改协议 | 3-4 个实现任务 |
| Phase 2 | 中 | 解决 T026 根因 + 流程选择硬约束，需协议改动 + 评审 | 6-8 个实现任务 |
...
4. Phase 2 内部顺序：先做 2A（产出独立化）+ 2B（状态一致性），再做 2C（流程选择硬约束）——2C 依赖 risk_level 字段，需要协议改动先行
