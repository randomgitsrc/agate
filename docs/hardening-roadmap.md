...
| 编号 | 名称 | 做什么 | 解决什么 | 来源 | 状态 |
|------|------|--------|---------|------|------|
| P2.1 | P6 验收独立化 | verifier subagent 独立执行 BDD 验收，直接 commit 产出（独立 author）；主 Agent 只读结论 | T026 根因：主 Agent 编造验收结果 | v1 | 降级方案 v2 已实现（客观行为审计：证据-结论对应 + dispatch-context 审计 + BDD 总数对照） |
...
| 编号 | 名称 | 做什么 | 解决什么 | 来源 | 状态 |
|------|------|--------|---------|------|------|
| P2.7 | 风险等级字段 | P1-requirements.md 增加 risk_level: low/medium/high + 量化条件 | 裁剪决策依赖临场判断 | v2 (S4) | 已实现 |
| P2.8 | 裁剪条件 hook 检查 | hook 检查裁剪需满足条件（risk_level + 裁剪阶段匹配） | Agent 默认倾向裁剪 | v2 (S4) | 已实现（BDD≤10 移到流程层，C2） |
| P2.9 | 裁剪声明回写 | 裁剪声明与实际执行不一致时必须回写 P1 override 字段 | P1 声明与执行不符 | v2 (C1) | 已实现 |
| P2.10 | P2 评审派发强制 | risk_level=high -> P2-review.md git author 非主 Agent | Agent 默认自审 | v2 (S3) | 降级方案 v2 已实现（agent 字段软提醒 + dispatch-context 审计，硬拦截待 Phase 3 平台支持独立 git author） |
| P2.11 | SCOPE+ 处理追踪 | 协议定义 [SCOPE_RESOLVED] 标记；hook 检查对应关系 | 忘了处理 SCOPE+ | v1 | 已实现 |
| P2.12 | 复盘异常触发 | 脚本检测异常模式 -> 强制提醒复盘 | Agent 默认不复盘 | v2 (E3) | 已实现 |

##### 2B-3. 非 agate 任务最低门槛（v3 新增，来自评审 S2 裁决）

| 编号 | 名称 | 做什么 | 解决什么 | 来源 | 状态 |
|------|------|--------|---------|------|------|
| P2.13 | 非 agate 任务风险矩阵 | 适用边界表增加"复杂度 × 风险"维度；高风险微改动必须走精简 agate（P1+P4+P5） | 安全修复裸 commit 无质量保障 | v3 (S2) | 已实现 |
| P2.14 | "直接做"最低要求 | commit message 必须声明改了什么 + 为什么安全 | 不走 agate = 没有任何记录 | v3 (S2) | 已实现 |
