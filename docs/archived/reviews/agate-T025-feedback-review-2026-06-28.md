...
### G. CHANGELOG-ENTRY.md — 延期

**原始建议**：READY 阶段产出 `CHANGELOG-ENTRY.md`，每个任务完成时记录面向用户的变更描述，批量 bump 时合并。

**评审判定**：方向正确，信息在任务完成时最新鲜，bump 时回忆必然遗漏。但当前 P8 gate 已增加 CHANGELOG 覆盖率检查（`git log v{prev_version}..HEAD --oneline` 对照），信息断层有了部分缓解。完整解决方案需要：
1. 定义 CHANGELOG-ENTRY.md 格式和产出时机（READY？P8？）
2. P8 subagent prompt 追加"合并所有 CHANGELOG-ENTRY.md"指令
3. 裁剪 P8 时的替代产出路径

延期到下一个跳 P8 的多任务批次遇到实际 CHANGELOG 遗漏时再落地。当前 P8 gate 的 git log 对照已提供兜底。
...
| # | 建议 | 裁决 | 理由 |
|---|------|------|------|
| A | gate-cheatsheet.md | ✅ 采纳（修正版已落地为 check-gate.sh） | T025 v3 实施时 gate-cheatsheet 演化为 check-gate.sh 脚本化 |
| B | dispatch-base.md | ❌ 拒绝 | 不省 subagent 上下文，引入版本一致性风险 |
| C | skip_reviews | ❌ 拒绝 | Agent 自信判断不可信，评审不可跳过 |
| D | 分层 P5 gate | ❌ 拒绝 | 质量环节不可裁减，增量测试假设影响范围可预知 |
| E | verification_env | ❌ 拒绝 | 写跑分离隔离 hang 风险，当前无 timeout kill 机制 |
| F | 信任等级 | ❌ 合并进 A | check-gate.sh exit 0/1/2 已隐含此分类 |
| G | CHANGELOG-ENTRY.md | ⏳ 延期 | P8 gate git log 对照已部分缓解，待实际遗漏再落地 |

**7 条建议中采纳 1 条（已落地），拒绝 4 条，合并 1 条，延期 1 条。**
