# P5 技术验证结果（retry1 — 全量重跑）

- 任务：T001（agate 协议 v0.40.0 结构化数据改造）
- 阶段：P5 技术验证，第 2 次派发（重跑全量，非仅测修复部分）
- 工作目录：/home/kity/oclab/agate/.worktrees/v2.0（worktree，分支 feat/v2.0）
- HEAD：fade134（P6 回退修复 + 增量 self-gate 审查完成后）
- 触发原因：上一轮 P5（commit f4bd942）通过后，P6 验收发现 check-p6-format.sh --fix 的 bug，回退 P4 修复（commit afe758a）。本轮按"如果是重试"规则全量重跑 gate_commands.P5* 全部 4 条命令，验证修复本身正确且未引入回归。

[PROD_NOT_TOUCHED] 全程只操作 worktree 内 agate 协议脚本/测试/文档，未连接任何生产环境/生产数据库/生产 API。

## 命令与结果总览

| 命令 | exit code | 结果 |
|---|---|---|
| P5（bats 主命令，全量：sanity+unit+regression+integration） | 0 | 603/603 通过，0 个 `not ok` |
| P5_consistency（check-protocol-consistency.py） | 0 | CHECK 1-4/6-9 全部 PASS，0 ERROR |
| P5_shellcheck（shellcheck -S warning agate/scripts/*.sh） | 0 | 无输出，0 警告 |
| P5_count（count-tests.sh） | 0 | 总计 **597** 个测试用例（与本轮预期基线一致：594 + 3 条新增回归测试 F_P6FMFIX.1/2/3） |

**基线核对**：597（count-tests.sh，不含 sanity.bats）+ 6（sanity.bats）= 603，与 bats 主命令 `1..603` 计划行、`ok` 计数 603 一致。较上一轮基线 594/600 各 +3/+3，增量为 P4 修复引入的 3 条回归测试，符合派发指引"本次新增要求 1"的说明，非漂移。

**failed 数量：0**（4 个命令 exit code 均为 0，bats `not ok` 计数为 0）

**未发现预存失败**，无需登记 known-failures.md。
...
完整原始输出（TAP 格式，供签名校验 `grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)'` 识别）：
...
ok 317 F_P6FMFIX.1 check-p6-format.sh --fix: frontmatter 的 pass:/fail: 字段不被正文归一化 sed 误伤，仍为合法 YAML
ok 318 F_P6FMFIX.2 check-p6-format.sh --fix: frontmatter 存在时正文总结行仍被归一化为 **Summary** 格式
ok 319 F_P6FMFIX.3 check-p6-format.sh --fix: 无 frontmatter 闭合边界的畸形文件回退按正文整体处理（不误判为已切分）
ok 320 PV.1 check-p6-provenance.sh 无 P6 文件 期望 exit 0
ok 321 PV.2 check-p6-provenance.sh PASS 引用不存在的文件 期望 exit 1
ok 322 PV.3 check-p6-provenance.sh (vision: ...) 引用被剥离不当文件路径
ok 323 PV.4 check-p6-provenance.sh 行末多个括号取最后一个（a.png 不存在但 b.png 存在）
ok 324 PV.4b check-p6-provenance.sh 行末多括号 + 全部不存在 期望 exit 1
ok 325 PV_BDD19.1 BDD-19: check-gate.sh P7 frontmatter blocker_count/deviation_count 均 0 时判定通过（不再用非计数行排除正则）
ok 326 PV.5b check-p6-provenance.sh 14 PASS 引用 8 共享证据文件 期望 exit 0
ok 327 PV.6 check-p6-provenance.sh 证据文件未被引用（充数文件）期望 exit 1
ok 328 PV.7 check-p6-provenance.sh .gitkeep 算隐藏文件不计入证据（exit 0）
ok 329 PV.8 check-p6-provenance.sh dispatch-context 含 PASS 预判 期望 exit 1
ok 330 PV.9 check-p6-provenance.sh P1 BDD 标题数 > P6 总数 期望 exit 1
ok 331 PV.10 check-p6-provenance.sh P1 无标准 BDD 标题 期望 exit 1（无过渡期兜底）
ok 332 PV_BDD_COUNT.1 P1 含 3 条 #### BDD-NN，P6 有 3 条 PASS 期望 exit 0
ok 333 PV_BDD_COUNT.4 P1 含 1 条带 Examples 表的 BDD-NN，P6 有 1 条 PASS 期望 exit 0（数据驱动共享编号）
ok 334 PV_BDD_COUNT.5 P1 BDD 编号有间隔（BDD-1,BDD-3，无 BDD-2），P6 有 2 条 PASS 期望 exit 0（按标题计数非 max 编号）
ok 335 PV.11 check-p6-provenance.sh UI + 截图 PASS 缺 vision 引用 期望 exit 1
ok 336 PV.12 check-p6-provenance.sh vision YAML 文件不存在 期望 exit 1
ok 337 PV.13 check-p6-provenance.sh vision YAML blocker_count != 0 期望 exit 1
ok 338 PV.14 check-p6-provenance.sh P6 缺 agent 字段 期望 exit 2（WARNING）
ok 339 PV.15 check-p6-provenance.sh risk=high + P2-review agent=main 期望 exit 0（agent=main 检查已移至 check-gate.sh）
ok 340 PV.17 dispatch-context 含任务上下文节 → 审计 2 放行
ok 341 PV.18 check-p6-provenance.sh PASS 行含嵌套括号描述如 nth(1) → 提取 screenshots/ 路径（exit 0）
ok 342 PV.19 check-p6-provenance.sh PASS 行含嵌套括号 + vision 引用 → 提取 screenshots/ 路径（exit 0）
ok 343 PV.20 check-p6-provenance.sh PASS 行含嵌套括号且路径不存在 → exit 1 + 含具体路径
ok 344 PV.21 审计5: 日志 EXIT_CODE=1 但 P6 声明 PASS → exit 1
ok 345 PV.22 审计5: 日志 EXIT_CODE=0 配 PASS → exit 0
ok 346 PV.23 审计5: 日志缺少 EXIT_CODE 尾行 → WARNING 不阻断
ok 347 PROV_MULTI.1 PASS 行引用 2 个逗号分隔的证据文件，均存在 → exit 0
ok 348 PROV_MULTI.2 PASS 行引用 2 个逗号分隔的证据文件，其中 1 个不存在 → exit 1 + 报告缺失
ok 349 PV_BDD20.1 BDD-20: check-gate.sh P7 frontmatter design_gap_reviewed_count < design_gap_count 时拦截（不再用数量相减的 0-vs-0 歧义判定）
ok 350 PV.DP1: dispatch-prompt file excluded from agent field check
ok 351 PV.24 审计6: evidence JSON shows FAIL but P6 says PASS → exit 1
ok 352 PV.25 审计6: evidence JSON all pass + P6 all PASS → exit 0
ok 353 PV.26 审计6: non-standard evidence JSON (no bdd_results) → silent skip, exit 0
ok 354 PV.27 审计6: P6 says FAIL + evidence JSON says fail → consistent, exit 0
ok 355 PV.28 审计6: agent 字段缺失 + evidence 矛盾 → 仍应 exit 1（不被 agent 检查短路）
ok 356 CHECK 9: EXIT_CODE 锚点存在且关键词匹配
ok 357 CHECK 9: AGATE_ALIGNMENT_REVIEW_THRESHOLD 锚点存在
ok 358 CHECK 9: ci-gate-backstop.py 被纳入 anchor coverage 扫描范围
ok 359 P2.1 check-pruning.sh 缺 risk_level 期望 exit 1
ok 360 P2.2 check-pruning.sh 裁剪 P2 期望 exit 1
ok 361 P2.3a check-pruning.sh 裁剪 P2 + legacy_p2_pruned 期望 exit 1
ok 362 P2.3b check-pruning.sh 裁剪 P2 + design_trivial 期望 exit 1
ok 363 P2.3c check-pruning.sh 裁剪 P2 + follows_existing_pattern 期望 exit 1
ok 364 P2.4 check-pruning.sh 裁剪 P6 期望 exit 1
ok 365 P2.4a check-pruning.sh 裁剪 P6 + no_behavior_change 期望 exit 1
ok 366 P2.5c check-pruning.sh P4 裁剪 期望 exit 1
ok 367 P2.5d check-pruning.sh P5 裁剪 期望 exit 1
ok 368 P2.5 BDD-9: check-pruning.sh 旧格式（--legacy-fields，risk_level 在正文非 frontmatter）risk=high 裁剪 P3 期望 exit 1（回退路径行为与 v0.35 一致）
ok 369 P2.5b check-pruning.sh risk=medium 裁剪 P3 期望 exit 1（P1-8: 仅 low 可裁）
ok 370 P2.6a check-pruning.sh 裁剪 P7，源文件数 > 5 期望 exit 1
ok 371 P2.6b check-pruning.sh 裁剪 P7，源文件数 ≤ 5 + coupling_checklist 通过
ok 372 P2.6c BDD-1: check-pruning.sh 裁剪 P7 + frontmatter implicit_coupling 字段 期望 exit 1
ok 373 P2.6d check-pruning.sh 裁剪 P7 无 coupling_checklist 期望 exit 1
ok 374 P2.6e check-pruning.sh 裁剪 P7 + coupling_checklist 放行
ok 375 P2.7 check-pruning.sh 裁剪 P8 无 internal_only 期望 exit 1
ok 376 P2.7a BDD-1: check-pruning.sh 裁剪 P8 + frontmatter internal_only: true + internal_only_reason 放行
ok 377 P2.8 check-pruning.sh 裁剪理由缺'跳过风险' 期望 exit 1
ok 378 P2.12 check-pruning.sh P6 裁剪无跳过风险 期望 exit 1
ok 379 P2.12a check-pruning.sh P6 裁剪 + no_behavior_change + 跳过风险 期望 exit 1
ok 380 P2.13 check-pruning.sh 裁剪 P8 有 internal_only 无 reason 期望 exit 1
ok 381 P2.14 check-pruning.sh 裁剪 P8 + internal_only + internal_only_reason 期望 exit 0
ok 382 P2.9 check-pruning.sh 裁剪声明 vs 实际有产出文件 + 无 override 期望 exit 1
ok 383 P2.9a check-pruning.sh 裁剪 P2 + override + 产出文件 期望 exit 1 (P2 不可裁)
ok 384 P2.10 check-pruning.sh 无 P1 文件 期望 exit 2
ok 385 P2.11 check-pruning.sh 全合规 (happy path) 期望 exit 0
ok 386 P2.52: YAML list format phases: - P1\n - P2 → parsed correctly
ok 387 P2.52b: YAML list format phases with P3 pruned (risk=low) → pass
ok 388 RT.1 check-retrospective.sh 无异常 期望 exit 0 + 无输出
ok 389 RT.2 check-retrospective.sh retries 超限 期望 exit 0 + 含'重试超限'
ok 390 RT_BDD21.1 BDD-21: check-gate.sh P1 frontmatter need_confirm_resolved 已覆盖具体描述时该 NEED_CONFIRM 项不再阻塞
ok 391 RT.DP1: dispatch-prompt file excluded from SCOPE+ scan
ok 392 RT.4 check-retrospective.sh override 触发 期望 exit 0 + 含'override'
ok 393 RT.5 check-retrospective.sh retries[P3]=2 触发超限（P3 MAX=2）
ok 394 RT.6 check-retrospective.sh retries[P3]=1 不触发（P3 MAX=2 未达）
ok 395 RT.7 句中 [SCOPE+]（非行首）不触发复盘提醒 期望 exit 0 + 无输出
ok 396 RETRO_SCOPE_DC.1 dispatch-context 含 [SCOPE+] 不触发复盘提醒
ok 397 RETRO_SCOPE_CARD.1 AGATE_CARD 块内 [SCOPE+] 不触发复盘提醒
ok 398 SC.1 check-scope-resolved.sh 不存在的 task 目录 期望 exit 2
ok 399 SC.2 check-scope-resolved.sh 无 SCOPE+ 触发 期望 exit 0
ok 400 SC.3 check-scope-resolved.sh 有 SCOPE+ 但无 P1 文件 期望 exit 1
ok 401 P2.53: progress file with [SCOPE+] text does not trigger SCOPE check
ok 402 SC.DP1: dispatch-prompt file excluded from SCOPE+ scan
ok 403 SC.4 check-scope-resolved.sh 有 SCOPE+ 但 P1 无 SCOPE_RESOLVED 期望 exit 1
ok 404 SC.5 check-scope-resolved.sh 有 SCOPE+ + P1 有 [SCOPE_RESOLVED] 期望 exit 0
ok 405 SC_BDD22.1 BDD-22: check-scope-resolved.sh 有 SCOPE+ + P1 frontmatter scope_resolved 非空列表 → 闭环判定通过
ok 406 SC.6 dispatch-context 文件中的 [SCOPE+] 字面引用不触发检查
ok 407 SC.7 句中 [SCOPE+]（非行首）不触发检查 期望 exit 0
ok 408 ST.1 check-state-transition.sh 无 .state.yaml 暂存 期望 exit 0
ok 409 ST.2 check-state-transition.sh 新 phase: P1（首次）期望 exit 0
ok 410 ST.3 check-state-transition.sh 顺序跳 P1→P3（差 2）期望 exit 0
ok 411 ST.4 check-state-transition.sh 回退 P3→P1（差 2）期望 exit 1（强制 PAUSED）
ok 412 ST.5 check-state-transition.sh 回退 P4→P2（差 2）期望 exit 1（强制 PAUSED）
ok 413 ST.6 check-state-transition.sh retries[P2]>=3 + phase 非 PAUSED 期望 exit 1
ok 414 ST.7 check-state-transition.sh retries[P2]>=3 + phase: PAUSED 期望 exit 0
ok 415 ST.8 check-state-transition.sh 终止态 PAUSED/READY/DONE 期望 exit 0
ok 416 ST.9 check-state-transition.sh retries[P3]>=2 + phase 非 PAUSED 期望 exit 1（P3 MAX=2）
ok 417 ST.10 check-state-transition.sh retries[P5]>=2 + phase 非 PAUSED 期望 exit 1（P5 MAX=2）
ok 418 ST.11 check-state-transition.sh 多阶段 retries 不同阈值 期望 exit 0（P2:2 不超, P3:1 不超）
ok 419 ST.12 check-state-transition.sh retries[P2]=3 + retries[P3]=2 期望 exit 1（任一超限）
ok 420 ST.13 check-state-transition.sh 回退 P3→P1（差 2）期望 exit 1（恢复强制 PAUSED）
ok 421 ST.14 check-state-transition.sh 回退 P4→P2（差 2）期望 exit 1（恢复强制 PAUSED）
ok 422 ST.15 check-state-transition.sh PAUSED→P3 恢复 期望 exit 0（验证 old_num 守卫）
ok 423 ST.16 commit gate: P1→P2 推进，P1 产出已 commit → exit 0
ok 424 ST.17 commit gate: P1→P2 推进，P1 产出与 phase 推进在同一 commit → exit 0（模式 B）
ok 425 ST.18 commit gate: P1→P2 推进，P1 产出不存在 → exit 0（产出存在性由 check-gate.sh 检查）
ok 426 ST.19 commit gate: PAUSED→P3 恢复 → 跳过 commit gate
ok 427 ST.20 commit gate: P3→P1 回退 → 跳过 commit gate
ok 428 ST_ARCHIVE.1 回退 P6→P5，P6-acceptance.md 仍在原位（未归档）期望 exit 1
ok 429 ST_ARCHIVE.2 回退 P6→P5，P6-acceptance.md 已被归档（原位不存在）期望 exit 0
ok 430 ST_ARCHIVE.3 回退 P5→P4（P5 不在 self-authored 名单）不受归档检查影响
ok 431 ST_ARCHIVE.4 前进 P4→P5（非回退方向）不触发归档检查
ok 432 ST_ARCHIVE.5 回退 P1->P0（退到起始阶段），P1-review.md 仍在原位 -> 不触发归档检查（与检查 1 一致，P0 是起始阶段）
ok 433 ST_ARCHIVE.6 回退 P2->P1，P2-design.md 已归档但 P2-review.md 仍在原位 期望 exit 1
ok 434 SY.1 check-state-yaml.sh 无 .state.yaml 期望 exit 2
ok 435 SY.2 check-state-yaml.sh 空文件 期望 exit 1
ok 436 SY.3 check-state-yaml.sh 缺 task_id 期望 exit 1
ok 437 SY.4 check-state-yaml.sh task_id 格式错 期望 exit 1
ok 438 SY.5 check-state-yaml.sh phase 非法值 期望 exit 1
ok 439 SY.6 check-state-yaml.sh retries 非 dict 期望 exit 1
ok 440 SY.7 check-state-yaml.sh retries[P1] 非 list 期望 exit 1
ok 441 SY.8 check-state-yaml.sh 全合规 期望 exit 0
ok 442 SY.9 check-state-yaml.sh YAML 语法错 期望 exit 1
ok 443 TD.1 check-tdd-red.sh TEST_RUNNER 指向不存在 + 无 pytest 期望 exit 1
ok 444 TD.1b check-tdd-red.sh 无 TEST_RUNNER + 无 pytest（无 PATH 找不到 pytest）期望 exit 3
ok 445 TD.2 check-tdd-red.sh 测试全绿 期望 exit 2（实现先于测试）
ok 446 TD.3 check-tdd-red.sh 经典红灯（assertion failure）期望 exit 0
ok 447 TD.4 check-tdd-red.sh B 类：项目内 import 失败 期望 exit 0
ok 448 TD.5 check-tdd-red.sh A 类：第三方 import 失败 期望 exit 1
ok 449 TD.6 check-tdd-red.sh A 类：SyntaxError 期望 exit 1
ok 450 TD.7 check-tdd-red.sh 混合：1 failed + 1 B 类 error 期望 exit 0
ok 451 TD.8 check-tdd-red.sh 无 PROJECT_MODULE + ImportError 期望 exit 0（启发式）
ok 452 TDD.N1: TEST_RUNNER without formatter does not add -q
ok 453 TDD.N2: vitest pure assertion failure → red-light exit 0
ok 454 TDD.N3: vitest B-class → exit 0
ok 455 TDD.N4: vitest A-class → exit 0 (exit-code-only without formatter)
ok 456 TDD.G1: BDD-15 回归：gate_commands.P3 保持正文（不迁移 frontmatter）→ auto-read, red-light exit 0
ok 457 TDD.G2: no gate_commands.P3 → TEST_RUNNER still works (backward compat)
ok 458 TDD.G3: TEST_RUNNER env var takes priority over gate_commands.P3
ok 459 TDD.G4: no TASK_DIR → skip gate_commands read, fall back to TEST_RUNNER
ok 460 TDD.G5: gate_commands.P3 with double-quoted value → strip quotes
ok 461 TDD.F1: gate_commands.P3 + P3_formatter → auto-read both, classic red-light exit 0
ok 462 TDD.F2: gate_commands.P3 without formatter → exit-code-only, red-light exit 0
ok 463 TDD.F3: formatter detects B-class (import from project_module) → exit 0 + B-class
ok 464 TDD.F4: formatter detects A-class (SyntaxError) → exit 1 + A-class
ok 465 TDD.F11: absolute path formatter works
ok 466 TDD.F12: PROJECT_MODULE env var overrides gate_commands project_module
ok 467 TDD.F5: formatter detects A-class (import NOT from project_module) → exit 1 + A-class
ok 468 TDD.F6: green light (exit 0) → exit 2
ok 469 TDD.F7: TEST_RUNNER env var still works (backward compat, exit-code-only)
ok 470 TDD.F8: no TEST_RUNNER, no gate_commands.P3, no pytest → exit 3
ok 471 TDD.F9: no formatter → command runs without -q
ok 472 TDD.F10: multi-stack P3 + P3_js → both run, combined result → exit 0
ok 473 TD.FAIL_HINT: classic red-light outputs assertion-mismatch hint
ok 474 TDD.TIMEOUT: 测试命令超时 → exit 0 + 超时提示
ok 475 PYX.1 agate-read-gate-commands.py P2 含 P3 + P3_html_formatter + project_module
ok 476 PYX.2 agate-read-gate-commands.py P2 无 gate_commands → 空 JSON
ok 477 PYX.3 agate-read-gate-commands.py P2 双引号值被去除
ok 478 PYX.4 agate-read-gate-commands.py P2 单引号值被去除
ok 479 PYX.5 agate-read-gate-commands.py P2 末行无换行也能解析
ok 480 PYX.6 agate-read-gate-commands.py GATE_FILE 不存在 → 非零退出
ok 481 FMT.1: generic-exit-only.sh exit 1 → exit_code=1, empty arrays
ok 482 FMT.2: generic-exit-only.sh exit 0 → exit_code=0
ok 483 FMT.3: pytest.sh (2 failed, 5 passed) → failed=2, passed=5, errors=0, failed_tests has 2
ok 484 FMT.4: pytest.sh B-class (ImportError from myapp.foo) → import_errors[0].module=='myapp.foo'
ok 485 FMT.5: pytest.sh A-class (SyntaxError) → syntax_errors non-empty
ok 486 FMT.6: pytest.sh all passed → passed=5, failed=0
ok 487 FMT.7: vitest.sh (11 failed, 6 passed) → failed=11, errors=0, import_errors=[]
ok 488 FMT.8: vitest.sh B-class (Cannot find module '../src/bar') → import_errors[0].module=='../src/bar'
ok 489 FMT.9: vitest.sh A-class (Cannot find module 'react') → import_errors[0].module=='react'
ok 490 FMT.10: go-test.sh cargo format (2 passed, 1 failed) → failed=1, failed_tests contains 'foo::test_bar'
ok 491 FMT.11: generic-tap.sh (2 ok, 1 not ok) → passed=2, failed=1, failed_tests contains 'test gamma'
ok 492 FMT.12: generic-junit-xml.sh (tests=3, failures=1, errors=1) → total=3, failed=1, errors=1, passed=1
ok 493 detect_ci_platform: Gitea 优先于 GitHub 被识别
ok 494 detect_ci_platform: GitLab CI 正确识别
ok 495 detect_ci_platform: 无可识别平台时 SKIP 而非误判
ok 496 backstop P3: 真红灯（exit 0）→ PASS
ok 497 backstop P3: 绿灯（exit 2）→ FAIL
ok 498 backstop P3: 假红灯（exit 1）→ FAIL
ok 499 backstop P3: 无运行器（exit 3）→ WARN 不 FAIL
ok 500 backstop P3: 无 .gate-result.json（--no-verify）时仍执行 check-tdd-red.sh
ok 501 commit-msg-self-gate: .sh 文件触发 self-gate WARNING
ok 502 commit-msg-self-gate: .py 文件触发 self-gate WARNING
ok 503 commit-msg-self-gate: 非 agate .py 文件不触发
ok 504 commit-msg-self-gate: self-gate-review: 路径消除 WARNING
ok 505 B3-warning: 产出暂存缺 dispatch-context → WARNING
ok 506 install-hook: .gitignore 忽略 .state.yaml → WARNING 提醒
ok 507 install-hook: 无 .gitignore → 无 WARNING
ok 508 install-hook: pre-push 是软链指向 pre-push-gate.sh
ok 509 install-hook: 已有非软链 pre-push → 备份并替换为软链
ok 510 install-hook: ln 退化为复制时打印升级提醒（Windows 兼容）
ok 511 R2.1 BDD-20: frontmatter design_gap_count == design_gap_reviewed_count（已全部配对）→ exit 0
ok 512 R2.2 BDD-20: frontmatter design_gap_reviewed_count(0) < design_gap_count(1) → exit 1（未配对）
ok 513 R2.3 P4 有 DESIGN_GAP 但 P7 frontmatter design_gap_count 为 0（未转抄）→ exit 1（交叉核对，回归 R2.3）
ok 514 R2.3b BDD-20: P4 DESIGN_GAP 数量 ≤ P7 frontmatter design_gap_count 且已 REVIEWED → exit 0
ok 515 R5.1 P8 gate 暂存区有 version + CHANGELOG → exit 2（脚本化通过）
ok 516 R5.2 P8 gate 暂存区无 version 文件 → WARNING（P1-6: 降级非阻断）
ok 517 R5.3 P8 gate 暂存区有 version 但 CHANGELOG 无变更 → exit 2 (WARNING)
ok 518 R4.1 裁剪 P8 无 internal_only → exit 1
ok 519 R4.2 BDD-1: 裁剪 P8 + frontmatter internal_only: true + internal_only_reason → exit 0
ok 520 R4.3 BDD-1: 裁剪 P8 + frontmatter internal_only: true 但无 internal_only_reason → exit 1
ok 521 R3.1 裁剪 P7 + 暂存区 6 个源文件 → exit 1（源码文件数 > 5 拦截）
ok 522 R3.2 BDD-1: 裁剪 P7 + 暂存区 3 个源文件 + frontmatter coupling_checklist → exit 0（≤ 5）
ok 523 R1.1 task-files.md executor_env 块 YAML 可解析
ok 524 R1.2 task-files.md executor_env: 顶格（无前导空格）
ok 525 R1.3 task-files.md executor_env 子字段 2 空格缩进
ok 526 CSG.1 非触发文件改动 → 无 WARNING
ok 527 CSG.2 触发文件改动 + 无 review 路径 → WARNING
ok 528 CSG.3 触发文件改动 + 有 review 路径 → 无 WARNING
ok 529 CSG.4 触发文件改动 + self-gate-skip → 无 WARNING
ok 530 CSG.5 agate/scripts/*.sh 改动触发
ok 531 CSG.6 agate/*.md 改动触发
ok 532 CON.1 CHECK 1: YAML 代码块可解析
ok 533 CON.2 CHECK 2: 文件引用存在
ok 534 CON.3 CHECK 3: 无硬编码行号
ok 535 CON.4 CHECK 4: gate_commands 键集合一致
ok 536 CON.5 CHECK 6: LICENSE 归属
ok 537 CON.6 CHECK 7: version badge 同步
ok 538 CON.8 BDD-13: CHECK 9 协议-脚本结构对齐（含新增 check-frontmatter.sh 锚点，37→38）
ok 539 CON.9 CHECK 9: md5 去重锚点已实现
ok 540 CON.10 CHECK 8: v0.6 关键词存在性
ok 541 CON.11 CHECK 9: PROD_TOUCHED 锚点含 PROD_NOT_TOUCHED
ok 542 CON.12 CHECK 9: NEED_CONFIRM 三值锚点存在（v0.30.2 起 SUGGEST）
ok 543 DC.1 dispatch-context-{role}.md 含正确卡片 hash → commit 不因 hash mismatch 被拦
ok 544 DC.2 dispatch-context-{role}.md 卡片被篡改 → hash mismatch
ok 545 DC.3 dispatch-context-{role}.md 空卡片块 → hash mismatch
ok 546 DC.4 派发阶段 (P2) 产出 commit 缺 dispatch-context → exit 1
ok 547 DC.5 P5 产出 commit 缺 dispatch-context → 拦截
ok 548 DC.6 P7 产出 commit 缺 dispatch-context → 拦截
ok 549 DC.7 P8 产出 commit 缺 dispatch-context → 拦截
ok 550 DC.multi 同一阶段多个 dispatch-context 文件 → 逐个校验 hash
ok 551 IT.1 pre-commit-hook 无 .state.yaml 变更 不触发
ok 552 IT.2 pre-commit-hook phase 变更 + gate 通过
ok 553 IT.3 pre-commit-hook 句中提及 [PROD_TOUCHED]（非行首声明）→ 不中止（T090 修复）
ok 554 IT.4 pre-commit-hook .state.yaml phase 变更触发 state-yaml 校验
ok 555 IT.5 pre-commit-hook .state.yaml 格式校验（任何变更都触发）
ok 556 IT.6 pre-commit-hook 多任务：任务级 .state.yaml + P1 产出 → 正常 commit
ok 557 IT.7 pre-commit-hook 多任务：P4 产出但 phase 仍 P3 → WARNING 不拦截
ok 558 IT.8 pre-commit-hook 多任务：phase 变更到 P2 但无 P2-design.md → 拦截
ok 559 IT.9 pre-commit-hook 多任务：裁剪跳阶 P2→P5 无 P3 产出（low 风险）→ 不拦截
ok 560 IT.9b pre-commit-hook 裁剪跳阶 P3 medium 风险 → 拦截（P1-8: 仅 low 可裁 P3）
ok 561 IT.10 pre-commit-hook 向后兼容：根 .state.yaml 仍工作
ok 562 IT.11 pre-commit-hook P2 阶段暂存代码文件 → WARNING
ok 563 IT_PT_BINARY.1 暂存 diff 含行首 [PROD_TOUCHED] 描述 → 中止 commit（步骤 1）
ok 564 IT_PT_BINARY.2 暂存 diff 含 [PROD_NOT_TOUCHED] → 不中止
ok 565 IT_PT_BINARY.3 暂存 diff 含删除行 [PROD_TOUCHED] → 不中止（只扫 ^+ 行）
ok 566 IT_PT_BINARY.4 暂存 diff 含句中引用 [PROD_TOUCHED]（非行首声明）→ 不中止（T090 修复）
ok 567 IT_PT_BINARY.5 暂存 diff 含句中引用 [PROD_TOUCHED]（非行首声明）→ 不中止（T090 修复）
ok 568 IT_PT_BINARY.6 暂存 diff 既无正向也无负向 → 不中止 + 无 WARNING（步骤 3 静默通过）
ok 569 IT_PHASE_SPAN.1 新增 P1/P2 产出文件 phase=P3（历史产出晚提交）→ 不报 WARNING
ok 570 IT_PHASE_SPAN.2 已存在 P1 产出被重新暂存 phase=P3 → 报 WARNING（真实过期）
ok 571 IT_PHASE_SPAN.3 新增 P4 产出文件 phase=P3（提前产出）→ 报 WARNING
ok 572 IT_PHASE_SPAN.4 多任务场景：T001 历史产出晚提交不 WARNING / T002 已存在产出修改报 WARNING / T003 提前产出报 WARNING
ok 573 IT_PHASE_SPAN.5 phase=PAUSED 暂存阶段号不符文件 → 不崩溃、报 WARNING、无 integer expression expected
ok 574 IT_PT_BINARY.7 暂存 diff 含 [PROD_NOT_TOUCHED] 确认未接触（负向+描述）→ 不中止
ok 575 IT_PT_MENTION.1 正文句中提及 [PROD_TOUCHED]（非行首声明）→ 不误报（T090 修复）
ok 576 IT_P6_CODE.1 phase=P6，暂存 P6-evidence/ 下截图 → 不拦（证据文件例外）
ok 577 IT_P6_CODE.1b phase=P6，暂存 evidences/ 下截图 → 不拦（T090 白名单修复）
ok 578 IT_P6_CODE.2 phase=P6，暂存项目源码文件 → exit 1 硬拦截
ok 579 IT_P6_CODE.3 phase=P4，暂存源码文件 → 不拦（回归）
ok 580 IT_P6_CODE.4 phase=P5，暂存源码文件 → 不拦（回归）
ok 581 IT_P6_CODE.5 phase=P2，暂存源码文件 → WARNING 而非硬拦截（回归，现有行为不变）
ok 582 IT_RETREAT.1 agate-retreat-to.sh 在装了真实 hook 的仓库里，每一步都真的过 hook 校验
ok 583 IT_RETREAT.2 中途一步的 commit 被 hook 拒绝时，agate-retreat-to.sh 明确报告停在哪步且不继续
ok 584 IT_PT_T6.1 P8 dispatch-context 含 AGATE_CARD 注入块（[PROD_TOUCHED] 说明文本）→ 不误拦
ok 585 IT_PT_T6.2 任务产出文件含句中 [PROD_TOUCHED]（非 AGATE_CARD 块内）→ 不拦截（T090 修复）
ok 586 IT_PT_T6.3 任务产出文件含行首 [PROD_TOUCHED]（步骤1）→ 拦截（回归）
ok 587 IT_PT_T6.4 任务产出文件含 [PROD_NOT_TOUCHED]（负向声明）→ 不拦截（回归）
ok 588 IT_CHANGELOG_P54: P4 commit without CHANGELOG → no CHANGELOG WARNING
ok 589 IT_CHANGELOG_P54b: P8 commit without CHANGELOG → CHANGELOG WARNING
ok 590 IT_GATE_REAL.1: hook runs check-gate.sh and writes real .gate-result.json
ok 591 HOOK_EVIDENCE_WARNING: P6 截图触发低方差 WARNING → commit 不应被拦截（T086 修复）
ok 592 pre-commit hook: AGATE_ROOT 未设时自定位到脚本自身本体（worktree 支持，T086）
ok 593 pre-push hook: 新分支首次推送提示跳过检测
ok 594 pre-push hook: 大改动触发提示
ok 595 pre-push hook: 无 agate/*.md 改动时零匹配 → 不报整数表达式错误（T086 回归）
ok 596 SG.1 角色文件 protocol-alignment-review.md 存在且含必需 frontmatter
ok 597 SG.2 角色文件含 A1-A6 审查清单
ok 598 SG.3 角色文件含 NEEDS_HUMAN_REVIEW 闭环规则 + HUMAN_CONFIRMED 标记
ok 599 SG.4 SELF-GATE.md 含派发模板
ok 600 SG.5 SELF-GATE.md 含检查清单
ok 601 SG.6 CHECK 9 锚点表覆盖全部 11 个 gate 脚本
ok 602 SG.7 commit-msg-self-gate.sh 存在且可执行
ok 603 SG.8 SELF-GATE.md 含递归终止条件
EXITCODE:0
```

## 命令 2：P5_consistency（协议一致性检查）
...
完整原始输出（无输出即为通过，仅记录 EXITCODE）：

```
EXITCODE:0
```

## 命令 4：P5_count（count-tests.sh）
...
=== 测试用例覆盖度自检 ===
  ...
  unit/check-p6-format.bats                           13 个 @test
...
4 个 gate_commands.P5* 命令全部 exit 0，failed=0。P4 修复（check-p6-format.sh 的 bug 修复，新增 F_P6FMFIX.1/2/3 三条回归测试）未引入任何回归，测试基线合法更新为 597（+6 sanity = 603），与本次派发指引的预期基线一致。
