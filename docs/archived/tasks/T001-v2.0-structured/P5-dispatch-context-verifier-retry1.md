...
上一轮 P5（commit `f4bd942`）已通过，但随后 P6 验收发现 `check-p6-format.sh --fix` 的 bug（详见 `docs/tasks/T001-v2.0-structured/P6-gate-diagnosis.md`），退回 P4 修复（commit `afe758a`）。按 P5 阶段卡片"如果是重试"规则："修复后重跑 gate_commands.P5 **全量**（T027 教训：修复可能引入回归，不能只检查修复项）"——本次是全量重跑，不是只测修复的那部分。

完整任务目标/约束/输入文件清单请直接读上一轮的派发指引（内容仍然适用，只是这次要重新执行）：
`docs/tasks/T001-v2.0-structured/P5-dispatch-context-verifier.md`

### 本次新增要求

1. **测试基线已更新**：`count-tests.sh` 现在应输出 **597**（594 + P4 修复新增的 3 条回归测试 `F_P6FMFIX.1/2/3`），不再是 594——这是合法更新，不是漂移。
2. **产出到新路径**：本次产出写 `docs/tasks/T001-v2.0-structured/P5-test-results-retry1/unit.md` + `fail-list.txt`（用 `-retry1` 后缀区分于上一轮的 `P5-test-results/`，避免覆盖历史记录——两轮结果都有留档价值，尤其这次要证明"P4 修复没有引入新回归"）。
3. 其余约束（4 个 gate_commands.P5* 命令、PROD_NOT_TOUCHED、非 UI 任务、纯只读验证）与上一轮完全相同。

### 输入文件（自己读）

- `agate/assets/execution-roles/verifier.md`
- `docs/tasks/T001-v2.0-structured/P5-dispatch-context-verifier.md`（上一轮完整指引，本次沿用）
...
<objective_info>
- 环境状态：worktree `feat/v2.0`，HEAD `fade134`（P6 回退修复 + 增量 self-gate 审查完成后）。`.state.yaml` working tree 已改为 phase=P5（待与本次产出一起 commit）。
- 上一轮 P5（f4bd942）：600/600、594 基线。本轮预期：603/603、597 基线（新增 3 条回归测试）。
</objective_info>
