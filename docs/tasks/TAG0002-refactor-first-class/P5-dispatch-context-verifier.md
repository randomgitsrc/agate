...
### 目标
执行 TAG0002 P4 实现的技术验证（P5 模式）：从 P2-design.md 读取 gate_commands.P5 全部命令并执行，产出 P5-test-results/（unit.md + fail-list.txt + 各补充命令结果）。你**执行命令并记录结果**，不修改任何代码。

### 约束
- **只读验证**：不修改 agate/ 任何文件、不修改测试、不修复 bug。发现失败只记录，不回改。
- 本任务是 **agate 协议自身改造**（dogfooding）：验证对象是 worktree `agate/`（已含 TAG0003 工作区架构 + TAG0002 refactor 机制）；**禁止触碰 `~/.agate`**（稳定版 v0.40.2 开发工具）。跑测试用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
...
- **预存失败处理**：若发现与本次改动无关的预存失败，在 unit.md 标注"预存失败：X（与本次改动无关）"并记录，不视为本任务失败。
- **自查≠gate**：你执行记录，最终 gate 由主 Agent 亲自跑。不要声称"P5 已过"。
- 禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。

### 上游关联
- P4 已 commit：refactor 机制实现（change_type frontmatter-only + P6 分流 + P3 refactor 感知 + 文档同步），review 三审 approved（R1 BLOCKER + R2 文档残留闭环）。
- P3 测试：check-gate.bats / agate-md-field-get.bats / check-frontmatter.bats / ci-gate-backstop.bats 新增 19 回归用例。
- 测试基线：全量 bats 654 用例（TAG0003 后 631 + P3 新增 + P4 修复新增）；consistency 0 ERROR；shellcheck 0。

### 输入文件
- docs/tasks/TAG0002-refactor-first-class/P2-design.md（gate_commands §5——必读，命令权威来源）
- docs/tasks/TAG0002-refactor-first-class/P0-brief.md（环境约束——必读）
- AGENTS.md（测试约定/脚本约定——必读）
- docs/tasks/TAG0002-refactor-first-class/P4-review.md（实现评审记录——选读）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=TAG0002-P4 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
...
- 已核实查证：全量 bats 654 用例（从根跑）；count-tests.sh 基线 644（informational）+ sanity 6。
</objective_info>
