...
### 目标
产出 docs/tasks/TAG0001-tech-debt-closure/P8-release.md——TAG0001（agate 技术债登记闭环 + debt/ 归类修正）的发布准备记录：bump_type + 版本号变更确认 + CHANGELOG 更新确认 + 临时资源清单。**你只产出文件，不执行 git commit / git tag / bump-version**（主 Agent 在 gate 验证后亲自执行）。

### 约束
- **只产出 P8-release.md**：不执行 `git commit` / `git tag` / bump-version——这些是主 Agent 的专属职责。
- 本任务是 **agate 协议自身改造**（dogfooding）：发布对象是 worktree `agate/`（已含 TAG0003 工作区架构 + TAG0002 refactor 机制 + TAG0001 技术债闭环）；`~/.agate` 是稳定版 v0.40.2 开发工具（禁止改动）。
- **版本 bump 判定**（主 Agent 已决策，遵循里程碑策略）：TAG0003=v0.41.0、TAG0002=v0.42.0（均已打本地 tag）。TAG0001 为**规则新增/调整**（技术债登记闭环 + debt/ 目录归类修正，非破坏性——默认无 tech-debt.md 时行为不变）→ **minor bump → v0.43.0**。
- **版本号变更确认**：当前版本 v0.42.0（README.md L6 badge + 本地 tag v0.42.0）。P8-release.md 中给出：旧版本号 → 建议新版本号（v0.43.0）、bump_type（minor）、理由。
- **CHANGELOG 更新确认**：给出 CHANGELOG.md 的 [0.42.0] → [0.43.0] 新节内容建议（Keep a Changelog 格式：新增/变更/修复分类；本任务 = 技术债登记闭环：DEBT 条目模板/schema 校验/回退强制/P8 debt_check 留痕/debt/ 目录归类修正）。
...
### 上游关联
- P7 已通过（gate exit 0）：DESIGN_GAP 1/1 REVIEWED，SCOPE+ 2/2 闭环，BLOCKER=0。
- P1..P6 全部通过：20 BDD（P6 20/20 PASS）、P5 验证绿（bats 676/0 + consistency 0 ERROR + shellcheck 0）。
- P2 packages=[agate]，ui_affected=false。
- 里程碑策略（主 Agent 决策）：TAG0003=v0.41.0、TAG0002=v0.42.0（已打本地 tag），TAG0001=v0.43.0，最终一起 push/merge。

### 输入文件
- docs/tasks/TAG0001-tech-debt-closure/P2-design.md（packages 声明 + gate_commands——必读）
...
- docs/tasks/TAG0001-tech-debt-closure/P7-consistency.md（一致性结论——选读）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=TAG0001-P7 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 当前版本：v0.42.0（README.md L6 badge + 本地 tag v0.42.0，未 push）；git describe --tags = v0.42.0。
- 测试基线：全量 bats 676 用例（count 670 + sanity 6）；consistency 0 ERROR；shellcheck 0。
- 任务进度：TAG0001 P1 20 BDD → P2 D1-D4 定案 → P3 22 测试红灯 → P4 实现（review approved + serialize_evidence 修复）→ P5 验证绿 → P6 验收 20/20 PASS → P7 一致性通过。
- 里程碑：TAG0003（工作区架构）v0.41.0 + TAG0002（重构一等任务）v0.42.0 已完成并打本地 tag；TAG0001（技术债闭环）本次发布 v0.43.0。
