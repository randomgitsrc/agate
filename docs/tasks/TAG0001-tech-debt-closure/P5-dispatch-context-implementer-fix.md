...
### 目标
**P5 修复轮（真 bug）**：修复 `agate-debt-check.py::serialize_evidence` 的 YAML int 边界 bug（丢弃全数字 int 哈希，导致 test_bdd_15 偶发红）。修复后重跑全量验证。

### 约束
- **增量模式**：上轮产出见 P4-implementation-core.md；上轮 dispatch-context（P4-dispatch-context-implementer-core.md）复用其约束。本文件只列修复目标。
- **必须修复**：`agate/scripts/agate-debt-check.py` 的 `serialize_evidence`——verifier 定位：**全数字 int 哈希被 YAML 序列化丢弃/边界错误**，导致 `test_bdd_15_real_retreat_records_fixture_reproducible` 偶发红（1/4 全量运行）。修复方向（按 verifier 诊断 + 读代码确认）：
  - serialize_evidence 在序列化 evidence 字段时，对全数字的值（如 commit 哈希 `023b28b`、`29301ad`——YAML 会当 int）处理不正确，导致 round-trip 后与原始 fixture 不一致
  - 修复：确保全数字哈希在序列化时保持字符串语义（如显式引号/类型保持），round-trip 稳定
- **修复要求**：
  1. 读 `agate/tests/unit/agate-debt-check.bats` test_bdd_15 断言，理解偶发根因（YAML int vs str 边界）
  2. 读 `agate/scripts/agate-debt-check.py` serialize_evidence 实现
  3. 修复 serialize_evidence（最小改动，不改测试语义）
  4. 自跑 test_bdd_15 多次（≥5 次）+ 全量 bats 确认不再偶发
- **低危顺手项**（verifier 观察，按成本评估）：无其他阻塞项。
- 改完后自跑验证：`bats agate/tests/unit/agate-debt-check.bats`（20 用例多轮）+ 全量 bats（676 基线）+ consistency 0 ERROR + shellcheck 0。
- 遵守 AGENTS.md 脚本约定。禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。

### 上游关联
- P5 verifier 发现：test_bdd_15 偶发红（1/4 全量），根因 = P4 交付的 serialize_evidence 丢弃全数字 int 哈希（YAML int 边界），实测定界。非预存失败、非环境问题。
- P5-test-results/unit.md §2 有完整诊断（必读）。

### 输入文件
- docs/tasks/TAG0001-tech-debt-closure/P5-test-results/unit.md（P5 验证结果——必读，含失败诊断 §2）
- agate/scripts/agate-debt-check.py（修复对象——必读）
- agate/tests/unit/agate-debt-check.bats（test_bdd_15 断言——必读，理解偶发根因）
- docs/tasks/TAG0001-tech-debt-closure/P4-implementation-core.md（实现记录——必读）
...
- 已核实查证（verifier）：test_bdd_15 偶发红根因 = agate-debt-check.py::serialize_evidence 对全数字值（如 commit 哈希 023b28b/29301ad）的 YAML int 边界处理错误；4 次全量 1 次红，单文件/单目录复跑绿。
- 测试基线：全量 bats 676 用例；consistency 0 ERROR；shellcheck 0。
</objective_info>
