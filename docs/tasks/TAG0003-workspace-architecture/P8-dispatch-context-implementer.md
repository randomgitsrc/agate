...
### 目标
产出 docs/tasks/TAG0003-workspace-architecture/P8-release.md——TAG0003（agate 工作区架构）的发布准备记录：bump_type + 版本号变更确认 + CHANGELOG 更新确认 + 临时资源清单。**你只产出文件，不执行 git commit / git tag / bump-version**（主 Agent 在 gate 验证后亲自执行）。

### 约束
- **只产出 P8-release.md**：不执行 `git commit` / `git tag` / bump-version——这些是主 Agent 的专属职责，你在 P8 产出中给出建议版本号与 CHANGELOG 更新内容，主 Agent 验证后执行。
- 本任务是 **agate 协议自身改造**（dogfooding）：发布对象是 worktree `agate/`（新版协议，含本任务全部改动）；`~/.agate` 是稳定版 v0.40.2 开发工具（禁止改动）。
- **版本 bump 判定**（P8 派发追加）：
  - 本任务：公共 API 行为变化 / 破坏性变更 → major；加功能 / 内部重构改 API（向后兼容）→ minor；修 bug / 不改 API 行为 → patch。
  - 本任务性质：**破坏性变更**（docs/tasks 强制迁移到 agate-workspace/，A 策略已确认；orchestrator 路径改动影响所有接入项目）→ 建议 **major**（v0.41.0？请按项目版本规范判定——agate 历史版本 v0.40.x 是连续 minor，是否遵循 semver major 需结合 UPGRADING.md 的既有惯例；给出明确建议 + 理由，供主 Agent 决策）。
  - 测试缺陷不应影响版本号决策。
- **版本号变更确认**：当前版本 v0.40.2（README.md L6 badge）。P8-release.md 中给出：旧版本号 → 建议新版本号、bump_type、理由。
- **CHANGELOG 更新确认**：给出 CHANGELOG.md 的 [Unreleased] → 新版本号节的内容建议（按 Keep a Changelog 格式：变更/修复/破坏性变更分类；本任务含 UPGRADING.md 迁移指引，破坏性变更必须显著标注）。
- **临时资源清单**：列出本任务执行期间启动的临时服务/进程/数据/开发安装（若无可写「无」）。本任务主要是文档+脚本改动，注意 fixture 仓库（/tmp/opencode/ 下）、P6-evidence 等。
- **Lessons Learned**：P8-release.md 增加「Lessons Learned」节（2-3 条关键教训），主 Agent 汇入 docs/notes/lessons.md。
- **READMEY 收尾提示**：P8-release.md 列出需主 Agent 清理的临时资源（供 READY 收尾检查用）。
- 读取 P2-design.md packages 声明（[agate] 单包）——bump 范围 = agate 协议本体。
- 禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。

### 上游关联
- P7 已通过（gate exit 0）：DESIGN_GAP 3/3 REVIEWED，SCOPE+ 闭环，BLOCKER=0。
- P1..P6 全部通过：20 BDD（P6 20/20 PASS）、P5 验证绿（bats 631/0 + consistency 0 ERROR + shellcheck 0 + count 625）。
- P2 packages=[agate]，ui_affected=false。

### 输入文件
- docs/tasks/TAG0003-workspace-architecture/P2-design.md（packages 声明 + gate_commands——必读）
...
- docs/tasks/TAG0003-workspace-architecture/P7-consistency.md（一致性结论——选读）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=2c833f3=P7 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 当前版本：v0.40.2（README.md L6 badge）；git describe --tags = v0.40.2。
- 测试基线：全量 bats 631 用例（count 625 + sanity 6）；consistency 0 ERROR；shellcheck 0。
- 任务进度：P1 20 BDD → P2 方案 A → P3 21 测试红灯 → P4 实现（9 脚本 + 16 文档 + roadmap-template + 8 测试换血）→ P5 验证绿 → P6 验收 20/20 PASS → P7 一致性通过。
