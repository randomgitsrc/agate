...
### 目标
产出 docs/tasks/TAG0003-workspace-architecture/P7-consistency.md——对 TAG0003 P1-P6 产出做跨文件一致性审查：DESIGN_GAP 配对、SCOPE+ 闭环、跨文件一致性、未决项清零。你是独立视角（agent 必须是 consistency-reviewer）。

### 约束
- **只审不写**：不修改任何产出文件，只产出 P7-consistency.md。
- 本任务是 **agate 协议自身改造**（dogfooding）：审查对象是 worktree `agate/`（新版协议）；`~/.agate` 是稳定版 v0.40.2 开发工具（禁止改动）。
- **检查清单**（P7 卡片）：
  1. **DESIGN_GAP 配对**：P4-implementation-core.md 中的 [DESIGN_GAP]（迁移工具自动 commit）→ 必须在 P7-consistency.md 中逐条转抄 + 配 [DESIGN_GAP_REVIEWED] 标记（P4 已标 REVIEWED，P7 要转抄确认）。未配对 → gate 不通过。
  2. **SCOPE+ 闭环**：P1-requirements.md frontmatter 有 scope_resolved（3 项）——确认 3 项 SCOPE+（check-state-transition.sh / check-pruning.sh 去硬编码 + worktree 自身不迁移）已纳入 P2/P4 实现。
  3. **跨文件一致性**：
     - P2 packages=[agate] 与 P8 release 的 bump 范围一致？
     - P1 的 20 条 BDD 与 P6 的 20 条验收结果数量匹配 + 内容对应（每条 BDD-NN 都映射到对应验收）？
     - P4 实现路径（9 脚本 + 16 文档 + roadmap-template + 8 测试换血）与 P2 方案设计（§1.1 改动清单）吻合？
     - P4-implementation.md 各文件（core/docs/tests/fix/review-fix）声明与 git 实际改动一致？
...
### 上游关联
- P4-implementation-core.md 有 1 条 [DESIGN_GAP]（迁移工具自动 commit）+ [DESIGN_GAP_REVIEWED: 已确认]（主 Agent 已标）。
- P1 frontmatter scope_resolved 3 项（SCOPE+ 回补）。
- P6 验收 20/20 PASS（P6-acceptance.md，provenance 审计 0）。
- P5 全量验证绿（bats 631/0 + consistency 0 ERROR + shellcheck 0 + count 625）。

### 输入文件
- docs/tasks/TAG0003-workspace-architecture/P1-requirements.md（需求基线 + BDD + scope_resolved——必读）
- docs/tasks/TAG0003-workspace-architecture/P2-design.md（方案设计 + packages/domains——必读）
- docs/tasks/TAG0003-workspace-architecture/P4-implementation-core.md / -docs.md / -tests.md / -fix.md / -review-fix.md（实现记录——必读，含 DESIGN_GAP）
- docs/tasks/TAG0003-workspace-architecture/P4-review.md（实现评审——必读）
- docs/tasks/TAG0003-workspace-architecture/P5-test-results/unit.md（验证结果——必读）
- docs/tasks/TAG0003-workspace-architecture/P6-acceptance.md（验收结果——必读）
- docs/tasks/TAG0003-workspace-architecture/P0-brief.md（任务简报——必读）
- AGENTS.md（项目约定——必读）
- git log（实际改动核对——按需）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=6bf3110=P6 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 任务进度：P1 20 BDD → P2 方案 A → P3 21 测试红灯 → P4 实现（9 脚本 + 16 文档 + roadmap-template + 8 测试换血）→ P5 验证绿 → P6 验收 20/20 PASS。
- 已核实查证：git log 显示 P1..P6 六个 commit（4dd7fec/a2e85b3/80c30d5/9aaab53/2bf9221/6bf3110）；P4-implementation-core.md 含 DESIGN_GAP + REVIEWED；P1 scope_resolved 3 项。
</objective_info>
