...
| 教训 | 来源 | 日期 |
|------|------|------|
| P2-design 的显式取舍声明是 P8 发布说明的免费草稿：设计期把取舍写成面向用户的语言（历史形态退役 + 存量证据），P8 的 CHANGELOG 转正与 UPGRADING 草案逐字复用零翻译成本——设计时只写实现语言，P8 还要二次翻译且易漏关键取舍 | TAG0029 | 2026-09-04 |

### 测试

| 教训 | 来源 | 日期 |
|------|------|------|
| fail-closed 校验必须配"残渣零产出"断言：值清洗类修复若只断言"报错了"而不验证"没产出残渣命令串"，消费方仍可能拿到半截命令执行——把"exit 非 0 + stderr + 不产出残渣"绑成一条验收原子项，避免 fail-half-open 式半修复通过门槛 | TAG0029 | 2026-09-04 |

### 架构

| 教训 | 来源 | 日期 |
|------|------|------|
| 豁免机制优先选"目录声明绑定"而非"内容特征匹配"：宽匹配豁免会被真代码借用逃逸（改一行字即可），绑定目录路径后借用方须搬家才逃逸且搬家在 review 可见——豁免设计的强度 = 逃逸成本，评审豁免先问"借用它要付出什么" | TAG0029 | 2026-09-04 |
