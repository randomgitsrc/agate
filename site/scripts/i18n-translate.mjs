...
// 中文化自动化管道 —— 把英文博客/首页/配图翻译生成中文版（site/zh/）。
...
function fmSet(fm, key, value) {
  const esc = value.includes('"') ? value.replace(/"/g, '\\"') : value
  const line = `${key}: "${esc}"`
  return fm.replace(new RegExp(`^${key}:.*$`, 'm'), line)
}

// ---- 只翻译真文本，返回可直用的全文 ----
async function translateMarkdown(src, wantFrontmatter = false) {
  const { fm, body } = splitFrontmatter(src)
  const p = protect(body)
  let chunks = []
  if (p.text.length <= 4000) chunks = [p.text]
  else {
    // 按段落拆块，避免超长
    const paras = p.text.split(/\n\n/)
    let cur = ''
    for (const para of paras) {
      if ((cur + '\n\n' + para).length > 3500 && cur) { chunks.push(cur); cur = para }
      else cur = cur ? `${cur}\n\n${para}` : para
    }
...
async function translatePost(enFile, rel) {
  ...
  if (newerThan(zhFile, enFile)) { log(`跳过（已最新）: zh/${rel}`); return }
  log(`翻译文章: ${rel}`)
  if (dryRun) { log('  [dry-run] 将调用 API 生成 zh/' + rel); return }
  const src = readFileSync(enFile, 'utf8')
  const zh = await translateMarkdown(src, true)
  mkdirSync(path.dirname(zhFile), { recursive: true })
