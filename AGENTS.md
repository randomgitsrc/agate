...
- `site/`：产品 Web 层（VitePress 站点源码：首页/博客）。属于产品对外内容，**在协议 gate 治理之外**——改它不触发 SELF-GATE，detect-docs-only 视其改动为 docs-only（跳过全量 pytest/shellcheck）。唯一硬校验 = `npm run build` 通过。品牌唯一权威源在 `docs/brand/`，`site/public/` 是构建快照（`npm run sync:brand` 生成，不入库）。**接博客任务先读**：机械流程见 `site/CONTRIBUTING.md`，质量标准见 `site/BLOG-STANDARDS.md`（发布前必须过独立评审）
