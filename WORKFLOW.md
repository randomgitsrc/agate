...
| P2 | 方案设计 | architect | plan-eng-review（risk_level=high 时必须派发独立 subagent）/ plan-design-review（domains 含 frontend 时追加）/ plan-ceo-review（涉及商业模式判断时可选）| P2-review.md 的 status == approved；`grep -cE '^(packages|domains|ui_affected|gate_commands):' P2-design.md` → =4 |
...
| P6 | 验收 | verifier（验收模式）| — | `scripts/check-gate.sh P6` exit 2（FAIL=0/NC=0/证据非空）；主 Agent 手动核实 BDD 总数 = P1 BDD 总数；UI 条件须 vision-analyst YAML `summary.blocker_count==0` ⚠️ self-authored |
