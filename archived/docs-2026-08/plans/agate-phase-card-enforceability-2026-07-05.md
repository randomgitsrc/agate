...
### 步骤 0：把"8 文件必读"改为 mapping 入口（连锁简化）

**目的**：当前 orchestrator-template.md 把"8 个协议文件依次读完"作为会话启动的必读清单。这是 Phase Card 之前的旧设计遗产——现在 Phase Card 已经覆盖了执行所需信息，8 文件应是**reference** 而非**必读**。

**改动清单**：

1. **orchestrator-template.md**：
   - 删去 line 108-117 的"8 个协议文件依次读完"小节
   - 把"按阶段渐进加载（推荐）"小节（line 123-138）提升为默认入口，标记为"推荐方式（CLI 优先）"
   - 保留 8 文件列表作为 reference（在底部 "Fallback：完整协议文件列表" 小节）

2. **state-machine.md:506**（基于真实现状，2026-07-04 phase-cards 实施时已部分修订）：
   - 当前内容（实测）：
     ```
     依次重读：orchestrator-template.md 的 mapping 表查当前阶段卡片，
     然后按卡片指引加载协议文件（推荐），或回退到 orchestrator-template.md
     「工作流规则」列出的 8 个协议文件全量重读。
     （WORKFLOW.md / dispatch-protocol.md / state-machine.md / ...）
     ```
   - 改后内容（去掉回退到 8 文件的悬空指针——8 文件在 step 0 后不再是"工作流规则"必读列表，而是 reference）：
     ```
     依次重读：orchestrator-template.md 的 mapping 表查当前阶段卡片，
     按卡片指引执行。卡片查不到的信息回退到 orchestrator-template.md
     「Fallback：完整协议文件列表」节（reference，非必读）。
     ```
   - **同步**：删除 :507-508 的 8 文件枚举清单（与 orchestrator-template reference 节对得上）
   - 关键防悬空：不能让 :506 指向一个不存在的列表——必须指向 step 0 后的新位置

3. **check-protocol-consistency.py FILE_COUNT_ANCHORS**：
   - 去掉 `orchestrator-template.md expected=8` 锚点
   - 去掉 `state-machine.md expected=8` 锚点
   - **后果**：FILE_COUNT_ANCHORS 删完变空列表，CHECK 5 函数体循环 0 次 → 永远 PASS → **绿色空壳 theater check**
   - **强制二选一**（S1 评审发现）：
     - **(a) 删 CHECK 5 + CON.5 测试**：检查数从 9 减到 8，README 同步
     - **(b) 改 CHECK 5 去守别的真东西**：如 phase-cards 数量 = 9、rules 文件数 = 2

**推荐 (a)**：诚实路径。agate 不再计数校验任何文件清单，CHECK 5 + CON.5 一起删。理由是"该计数不再有意义"，不是"CHECK 5 在强制阅读"——S3 评审纠正了之前高估 CHECK 5 的论证。

**风险**：

- 新用户首次接入 agate 时，如果按 mapping 表直接进入卡片，可能不知道还有完整 reference 可用
- 缓解：README.md 增加"完整协议 reference" 章节，列出 8 文件作为参考（不强制读）
...
**为什么这是 Phase Card 防漂移方案的必要前置**：

旧设计的"8 文件必读 → 渐进加载（推荐）"是个矛盾体：
- "必读"是强约束 → CHECK 5 锚点强制 → agent 必须读
- "渐进加载（推荐）"是弱约束 → agent 可以忽略
- 两者并存：CHECK 5 保证 8 文件被读完，agent 读完才发现 mapping 表可以省读

**步骤 0 之后**：
- 8 文件不再是必读 → CHECK 5 锚点可以安全移除
- mapping 表 / CLI 是默认入口
- 8 文件保留作为 reference（agent 按需查阅）
...
**实施纪律**：step 4 测试必须真跑到"嵌入 → hook → hash 相等 → commit 成功"。仅"文件生成"不够，必须到 hash 相等这一步。这是项目反复强调的"实跑纪律"——伪代码层面看不出 marker/fence 错位。
...
方案 C 涉及多个改动（8文件必读简化 + CLI + 模板 + hook + 测试 + self-gate），一次性实施约 1-2 天工作量。建议：

1. **步骤 0**（8 文件必读简化）独立提交——CHECK 5 锚点变更，是 CLI/hook 的必要前置
2. **步骤 1**（CLI 命令）独立提交——基础工具，可单独用
3. **步骤 2-3**（dispatch-context 模板 + hook 强制）独立提交——可独立验证
4. **步骤 4**（测试）+ **步骤 5**（self-gate）+ **步骤 6**（验证）合并提交

4 个 commit，每个都可独立回滚。

## 与 Phase Card 实施计划的关系

步骤 0 的"8 文件必读简化"是 Phase Card 实施计划（`agate-phase-cards-implementation-2026-07-05.md`）的逻辑延伸。原 Phase Card 计划保留 8 文件必读是为了不让 CHECK 5 报错——本计划步骤 0 直接改 CHECK 5 锚点白名单，移除这两个锚点，让"8 文件必读"从协议层面消失。

这是一个**架构级简化**：Phase Card 不再是"叠加在 8 文件必读之上的可选优化"，而是**取代**8 文件必读作为默认入口。8 文件彻底降级为 reference。