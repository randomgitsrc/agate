...
> 以下内容由任务委托人（用户）在复盘后补充，修正了复盘文档的遗漏和偏差。

### 10.1 dispatch-context.md 时序问题（复盘遗漏）

**复盘原文**（5.2 节）将 dispatch-context 问题定性为"hash 校验复杂度过高"。这没有触及核心。

**实际发生**：P2 阶段干完活后才写 P2-dispatch-context.md，而非先写再派发。这意味着：
- dispatch-context.md 记录的不是"我实际给了 subagent 什么上下文"，而是"我事后声称给了什么上下文"
- hash 校验机制形同虚设——事后补写当然能通过校验
- P2 评审后多次派发 subagent（review §1 → revision §1 → review §2 → revision §2 → review §3），一篇 dispatch-context 无法覆盖多次派发

**正确流程**：每次派发 subagent 前，先写 dispatch-context.md（记录本次派发的上下文），再派发。hook 校验"你声明的上下文 = 你实际给的上下文"才有意义。

**根因**：agate 协议没有规定"每次派发都应有对应的 dispatch-context"，也没有规定 dispatch-context 的时序约束（先写再派 vs 事后补写）。当前 hook 只校验 hash 一致性，不校验时序。

### 10.2 GLM 5.1 content filter 触发原因（复盘遗漏）

**复盘原文**（5.7 节）将模型切换淡化为"上下文断点"问题，没有分析触发原因。

**实际发生**：上班阶段运行 GLM 5.1（200k 上下文），某次执行触发了 content filter：
```
作为一个人工智能语言模型，我还没学习如何回答这个问题，您可以向我问一些其它的问题，我会尽力帮您解决的。
The response was blocked by the provider's content filter
```
之后切换到 DeepSeek V4 Flash（1M 上下文），完整跑完 P8 并发布成功。

**未回答的问题**：
1. 什么输入触发了 content filter？是 agate 协议内容本身（某个测试用例描述？某个 BDD 场景？），还是项目代码内容？
2. 200k 上下文是否是导致遗忘/违规的间接原因？切换到 1M 上下文后，DeepSeek V4 Flash 在剩余阶段没有出现类似的主 Agent 违规问题——这是巧合还是上下文窗口大小的因果？

### 10.3 上下文窗口大小与 Agent 纪律的关联

**观察**：GLM 5.1（200k）阶段出现了 3 次主 Agent 违规（P1 未 commit、擅改评审、直接改代码），切换到 DeepSeek V4 Flash（1M）后剩余阶段（P5-P8）没有出现类似问题。

**假设**：200k 上下文窗口在长会话中更容易"遗忘"协议规则（尤其是铁律），因为：
- agate 协议本身占大量 token（WORKFLOW.md + dispatch-protocol.md + 角色文件 + 阶段卡片）
- 任务产出文件（P1-P4 的 .md 文件）持续累积
- 协议规则被挤出上下文窗口后，Agent 行为退化为"直觉驱动"而非"规则驱动"

**需要验证**：这个假设需要更多数据点（不同模型、不同上下文窗口大小的对照实验）才能确认。但 T048 的数据至少提示了相关性。
