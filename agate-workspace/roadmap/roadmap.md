...
| RM-AG0015 | 文档脚本名引用漂移无 gate 兜底：CHECK 2 只捕获 `scripts/` 前缀引用，裸脚本名（phase-cards/rules 全是）完全漏检 + phase-cards/rules 不在 PROTOCOL_FILES（引用检查降级 WARNING）| backlog | TAG0010/0011 复盘（2026-08-15）+ 实测验证 | — | 2026-08-15 | 2026-08-15 |
| RM-AG0016 | subagent 派发编排机制（全阶段）：工作量评估 + 五模式编排（单发/静态拆批/并行/先理解后拆/串行链）+ 并行规则统一；P1/P2 补空白、P3-P6 统一现有分散"按包并行" | backlog | TAG0010/0011 复盘（2026-08-15）+ 用户需求扩展（全阶段）| — | 2026-08-15 | 2026-08-15 |
| RM-AG0017 | self-gate 触发面缺仓库根级文档：README.md/AGENTS.md 不在触发面（改协议语义绕过 self-gate 评审）| backlog | TAG0010/0011 复盘（2026-08-15）| — | 2026-08-15 | 2026-08-15 |
| RM-AG0018 | 复盘/评审发现未接 tech-debt 登记触发点：tech-debt.md 零登记（DEBT0001 前），复盘发现缺口只写进复盘/roadmap 不走 DEBT 路径 | backlog | 独立观察 + DEBT0001 破冰（2026-08-15）| — | 2026-08-15 | 2026-08-15 |
