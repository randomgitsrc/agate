...
| RM-AG0057 | 验收盲区机制补强批（TPV0095 跨项目复盘，peekview v0.22.0 交付后暴露 4 类问题查证）：①**测试副作用/环境还原 gate**——BDD 只有正向路径，创建型 E2E（建团队/条目）跑完不清理累积污染共享环境，P6 验收全 PASS 时环境尚干净、残留验收后才暴露（agate 卡 P3 只有测试前失败基线 capture-env-baseline，无测试后残留检查/清理钩子要求；peekview DEBT0008 实证）；②**P1 人工体验路径验收节**——排除 seed/数据改动时 BDD 全用 fixture 验收，"用户按文档 seed 后页面应有内容"成隐性无人验路径（peekview DEBT0009 实证：make debug-seed 后 Teams tab 空）；③**布局方案 ≥2 候选进 P2 设计评审必审**——P2 候选≥2 现为架构级（candidate_count），"行 vs 卡"布局决策无候选权衡必审，plan-design-review 已有视觉打分维度但非布局候选 gate（TPV0095 实测 N 卡片堆叠过全部 44 BDD）；④**视觉契约断言收录**——"dropdown ≥ trigger"类可量化协调性无 BDD 表达机制，建议收录为可表达子集（E2E DOM 度量断言）+ P2/P6 指南提及（vision-analyst 现为被动截图翻译） | backlog | TPV0095 复盘（2026-09-03，peekview team-visibility 交付后暴露层；RM-AG0021 跨项目反馈机制汇入）| — | 2026-09-03 | 2026-09-03 |
## 状态标识
