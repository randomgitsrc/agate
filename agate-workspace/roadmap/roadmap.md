...
| RM-AG0035 | 品牌改名执行（Agateon，2026-08-23 决策已定）：agate 名称与 4+ 活跃项目撞车（GitHub in:name 1223）——**用户拍板改名 Agateon**（"agent gates on"闸门开启/验证激活；2026-08-23 四源核验：npm/PyPI 空闲、.com/.dev/.io 疑似可注册[WHOIS 待确认]、GitHub 0 真实撞名；保留 agate 前缀迁移成本最低；候选对比与淘汰记录于 dsh-workspace/agate-research/rename-recommendation.md[gatewise 因域名被占淘汰/agaton 造词感/turngate 有 4 撞名]）。**剩余工作（转执行型）**：①域名注册（WHOIS 确认）+ 商标查询 ②仓库改名（agate→agateon，原 URL 301）③`agate-*.py` 脚本前缀 + `~/.agate`/`AGATE_*` 环境变量兼容别名策略 ④8000+ 行 md 引用迁移 + 一致性 gate ⑤v1.0 迁移窗口；验收锚=仓库/文档/脚本品牌统一 agateon + AGATE_* 兼容别名生效、旧 URL 301 跳转正常 | backlog | 用户决策（2026-08-23 拍板 Agateon）| — | 2026-08-21 | 2026-08-23 |
