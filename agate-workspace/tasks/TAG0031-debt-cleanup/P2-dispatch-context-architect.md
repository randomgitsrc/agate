...
> 本次是 P2 第 2 轮（重试）：上一版 P2-design.md 已被 plan-eng-review 判 rejected，两处阻塞见 P2-gate-diagnosis.md。

### 目标
修改（不是重写）已有的 P2-design.md，只修正 review 指出的两处阻塞，方案主体（三簇拆批、候选方案选择、gate_commands、影响面梳理其余部分、BDD 覆盖）已被 review 确认通过，**不要改动**。

### 约束
- **只改两处，不重写全文**：
  1. **R1 pyyaml checksum 顺序缺口**：`_ensure_agate_common(bundle_dir)` 会在做任何 checksum 校验之前就 `pip install` + `import` bundle 里的 pyyaml wheel，导致 pyyaml 是 manifest 里唯一"先执行、后校验"的组件（其余组件均全部 checksum 通过后才被使用）。**推荐修复**：在 `pip install` 之前，用一行内联 `hashlib.sha256(Path(wheel_path).read_bytes()).hexdigest()` 单独校验 manifest 里 `components["pyyaml"]["sha256"]` 这一个文件级 hash（不违反 BDD-1「全仓仅 1 处 `compute_sha256` 定义」——这是一次性、单文件、不对外暴露为函数的内联校验，属于 `_ensure_agate_common` 自身的引导前置检查，不是"compute_sha256 的另一份实现"）。校验通过再 `pip install`，不通过则 stderr 报错 + exit 非 0，不执行 `pip install`。约 5-8 行改动量。同时在「回归覆盖」段补一条对应用例（构造 checksum 不匹配的 pyyaml wheel，断言 `_ensure_agate_common` 在 `pip install` 之前就报错拒绝，用 mock `subprocess.run` 断言其未被调用来验证"校验先于安装"这一顺序本身）。**弱化方案**（若判断补校验成本过高）：至少在「代价确认」段明确、无歧义地写清"pyyaml 组件的 checksum 保护顺序弱于其余组件，损坏的 pyyaml wheel 会在校验前被 pip install 并 import"这一具体残留风险及可接受理由，不能只停留在"不引入外部信任源"这一层论证。两条路径任选其一，但必须让读者能准确判断实际风险状态。
  2. **`[SCOPE+]` 行首格式**：第 75 行 `**R1（新发现，[SCOPE+]）：...**` 把标记嵌在加粗标题句中段，`check-scope-resolved.py` 的正则 `^\s*-?\s*\[SCOPE\+\]`（要求行首）不会匹配，导致这一新发现的隐含需求脱离"主 Agent 增补 P1 基线 + `[SCOPE_RESOLVED]` 闭环"的既定机械追踪流程。改写为行首独立格式（参照 `assets/execution-roles/architect.md`「设计中发现新隐含需求 → 标 [SCOPE+]」节给出的样例结构：`[SCOPE+] 发现：...` / `必须做的理由：...` / `影响：...` 另起一段放在 R1 小节开头），使正则能正确检出。纯格式修正，不改变 R1 的技术内容本身（技术内容改动见①）。
- **方案主体不动**：三簇拆批（`dispatch_plan`）、候选方案 1/2 及选择理由、gate_commands 四个 P5 系列 key、影响面梳理其余部分（1.1 改什么/1.2 不改什么/1.3 R2-R5）、files_to_read、env_constraints、minimal_validation、实现完成标志——均已被 review 确认通过，**重新论证是浪费**。
- **两条非阻塞建议可顺手采纳**（不强制，视改动成本）：`agate_common.py` L30-34 `import yaml`/`sys.exit(1)` 分支旁补一行代码注释留痕；R5 标题句"agate_common.py 被簇 A 与簇 C 同时触达"措辞澄清（避免误读为两簇同改一文件）。

### 上游关联
- 第 1 轮 review 结论：rejected，两处阻塞，完整诊断见 {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-gate-diagnosis.md（主 Agent 已独立复核两处均成立：`agate-pack-offline.py:129` 确认 pyyaml 是独立文件级 checksum 组件；`check-scope-resolved.py:17` 正则确认要求行首格式，P2-design.md 第 75 行确实不匹配）
- P2-review.md 本体也可读（{AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-review.md）——但诊断文件已提炼出行动项，无需重读全文找问题点

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-design.md（待修改的现有产出，改动前请先读一遍避免推翻已通过的部分）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-gate-diagnosis.md（本轮修改的权威诊断，两处问题的具体描述与修正路径）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0031/agate/assets/execution-roles/architect.md（`[SCOPE+]` 标记的行首格式样例结构）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0031/agate/scripts/install-offline.py（R1 涉及的现状代码，修复落点）
- /home/kity/oclab/agateon/.worktrees/agate-TAG0031/agate/scripts/agate-pack-offline.py（pyyaml 组件的 manifest 结构，L129 附近）
</dispatch_guide>
...
- 关键标识：`check-scope-resolved.py:17` 正则 `^\s*-?\s*\[SCOPE\+\]`；`agate-pack-offline.py:129` `components = {"agate": agate_dir, "wheels": wheels_dir, "pyyaml": pyyaml_wheels[0]}`
- 查证结果：主 Agent 已实测确认两处阻塞均真实存在，见「上游关联」节
</objective_info>
