...
### 目标
逐条验收 P1-requirements.md 的全部 15 条 BDD（BDD-1~BDD-15），产出 `P6-acceptance.md` + `P6-evidence/`。本任务 `ui_affected: false`（非 UI 任务），不涉及 vision-helper/截图证据，全部走断言/日志类证据。本任务未声明 `change_type: refactor`，走标准验收口径（不是回归口径）。

### 约束
- **PASS/FAIL 只能二选一**，不允许"调整/跳过/覆盖"；**验收报告记录的是验收时的事实**——如果某条 BDD 此刻验证为 FAIL 就写 FAIL，不能因为"理论上应该没问题"就写 PASS。
- **每条 PASS 必须有证据文件引用**：`- PASS BDD-NN: {描述} ({证据路径})`。证据来源优先复用 P3/P4/P5 阶段已产出的测试文件（不是重新臆造），具体做法：为每条 BDD 实际执行对应的 pytest 用例（`-v` 模式，保留断言输出），把输出保存到 `P6-evidence/bdd-{NN}.log`，PASS 行引用该文件。
- **不是简单复制 P5 的汇总数字**：P5 验证的是"测试全部转绿"这一技术事实，P6 验收的是"每条 BDD 对应的具体断言是否满足"——请为每条 BDD 单独跑一次对应测试并保存独立证据文件，不要用一份 P5-test-results/unit.md 笼统覆盖 15 条 BDD。
- **BDD ↔ 测试函数映射**（从 P3-test-cases.md 提取，逐条对应）：
  - BDD-1: `test_bdd_1_compute_sha256_*`（3个，test_agate_common.py）+ `test_bdd_1_pack_offline_imports_compute_sha256_from_agate_common`（test_agate_pack_offline.py）+ `test_bdd_1_verify_checksums_uses_agate_common_compute_sha256`（test_install_offline.py）
  - BDD-2: `test_bdd_2_pack_install_uninstall_roundtrip_no_behavior_change`（test_offline_bundle_roundtrip.py）+ R1 两条 `test_r1_ensure_agate_common_*`（test_install_offline.py，机制细节归入 BDD-2）
  - BDD-3: `test_bdd_3_upgrading_doc_states_checksum_trust_boundary` / `test_bdd_3_scripts_readme_states_checksum_trust_boundary`（test_agate_common.py）
  - BDD-4: `test_bdd_4_find_references_and_uninstall_warn_when_scan_limit_hit`（test_agate_install_uninstall.py）
  - BDD-5: `test_bdd_5_find_references_no_warning_within_scan_bounds`（test_agate_install_uninstall.py）
  - BDD-6: 4 个既有用例（`test_p2_6e_*`/`test_p2_52_*`/`test_p2_52b_*`/`test_p2_6f_*`，test_check_pruning.py）
  - BDD-7: `test_bdd_7_debt0007_status_closed_with_closure_fields`（test_debt_registry_closure.py）
  - BDD-8: `test_tag0031_bdd_8_gate_p4_code_map_uses_resolve_workspace`
  - BDD-9: `test_tag0031_bdd_9_gate_p4_non_standard_nesting_resolves_via_agate_env`
  - BDD-10: `test_tag0031_bdd_10_gate_p4_self_referential_prose_not_matched`
  - BDD-11: `test_tag0031_bdd_11_gate_p4_real_heading_trailing_text_satisfied`
  - BDD-12: 4 个子用例 `test_tag0031_bdd_12_gate_p1_read_rules_yaml_missing_fail_closed` / `..._gate_p6_count_pass_fail_missing_fail_closed` / `..._gate_p7_count_markers_missing_fail_closed` / `..._gate_p7_count_code_map_lines_missing_fail_closed`（均 test_check_gate.py，视为一条 BDD 的证据组，4 个断言输出可合并进同一 `bdd-12.log`）
  - BDD-13: `test_tag0031_bdd_13_gate_p6_p7_new_format_unaffected_regression`
  - BDD-14: **非代码断言**——登记动作。验证方式：`grep -A3 '^## DEBT0028$\|^## DEBT0029$' agate-workspace/debt/tech-debt.md` 确认两条新登记条目存在且 `status: open`，证据存为 `bdd-14.log`（grep 输出）
  - BDD-15: `test_tag0031_bdd_15_six_debts_registry_closed`（test_check_gate.py）
- **P0-brief/P1 声明的信任边界文档验收（DEBT0003）**：BDD-3 的证据除测试输出外，建议额外核对 `agate/UPGRADING.md`/`agate/scripts/README.md` 确认文字确实可读、非机械占位。
- **环境隔离**：所有命令不接触生产环境/`~/.agate`，未触发写 `[PROD_NOT_TOUCHED]`。

### 上游关联
- P5 已全量通过：`gate_commands.P5` 四条命令 exit 0/failed=0（`p5_pass_commit: c42f12a`）
- P4 review approved（backend 域），SELF-GATE protocol-alignment-review 全 ALIGNED（1 处已修正的文档引述纠正）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-requirements.md（15 条 BDD 原文，验收依据）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P5-test-results/unit.md（P5 阶段技术验证结果，参考不复制）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P3-test-cases.md（BDD↔测试函数映射来源，本 dispatch-context 已提炼关键部分，供交叉核对）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0031-debt-cleanup（P5 已 commit，HEAD 6c2fc11 系列），Python 3.12.3 + pytest 9.0.3
- 关键标识：`ui_affected: false`，无 vision/截图要求；`change_type` 未声明 refactor，走标准口径
- 查证结果：本节不预查证具体 BDD 结果——由你逐条实跑产出真实证据
</objective_info>
