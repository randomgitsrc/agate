...
### 目标
执行 P2-design.md §3 声明的 `gate_commands.P5` 全部四个 key（`P5`/`P5_consistency`/`P5_shellcheck`/`P5_offline_bundle`），逐条独立记录 exit code + failed 计数，产出 `P5-test-results/unit.md` + `P5-test-results/fail-list.txt`。

### 约束
- **四个 key 各自独立执行，不用 `&&` 串联**（P2 design 已固化此约束，避免一个命令失败掩盖后续命令未执行的事实）：
  ```
  P5:                "python3 -m pytest agate/tests/unit/ agate/tests/regression/ agate/tests/integration/ -n auto -q --tb=no"（timeout 120s，若超时可按实测放宽到 200s，本仓全量约 35s，留足余量即可）
  P5_consistency:     "python3 agate/scripts/check-protocol-consistency.py --strict-errors-only"（timeout 60s；**必须用 worktree 自己的脚本 `agate/scripts/check-protocol-consistency.py`，不是 ~/.agate 稳定版**——HANDOFF-TAG0031.md §2 铁律，用错脚本等于没测到本次改动）
  P5_shellcheck:      "shellcheck -S warning agate/scripts/*.sh"（timeout 60s）
  P5_offline_bundle:  "python3 -m pytest agate/tests/regression/test_offline_bundle_roundtrip.py -q --tb=no"（timeout 90s）
  ```
- **全量测试套件**（本阶段应运行全量，非仅本任务新增测试）：`P5` key 已是全量命令（含全部 unit/regression/integration），据此判定即可，不需要额外单独跑"仅本任务测试"。
- **PROD_TOUCHED 检查**：所有命令均不应触碰 `~/.agate`（生产安装目录）或任何生产环境；执行前后可用 `ls -la ~/.agate` 快照对比确认未被本次验证污染（正常情况下不会，因为 P4 阶段的测试均已 mock/隔离）。
- **无 formatter 声明**：P2-design.md 未声明 `P5_formatter`，退化为 exit-code-only 模式——`P5-test-results/unit.md` 需手工记录 pytest 输出的汇总行（`N passed, M failed, K skipped`）+ 若有 failed 则逐条列出测试 id。
- **fail-list.txt**：若四个 key 全部 0 failed，产出空文件即可（不需要内容）。

### 上游关联
- P4 review approved（backend 域 review 角色），全量测试当时已达 1435 passed/2 skipped/0 failed（P4 阶段实测，非 P5 阶段——P5 需独立重跑确认，不能复用 P4 数字）
- SELF-GATE protocol-alignment-review 已确认 A1/A3-A7 ALIGNED，A2 的 1 处文档引述纠正已修正
- P2-design.md §3 gate_commands 权威声明（本次派发的四条命令逐字来源于此，不得自行更改命令内容）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-design.md（§3 gate_commands 权威声明）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P4-implementation.md（本次改动文件清单，供你核对测试覆盖范围是否合理）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0031-debt-cleanup（P4 已 commit），Python 3.12.3 + pytest 9.0.3，shellcheck 可用，`~/.venvs/agate-dev/bin/ruff` 可用（本次 gate_commands.P5 未声明 ruff key，不强制跑，但若你想额外确认可以顺手跑，不计入门槛）
- 关键标识：worktree 根 /home/kity/oclab/agateon/.worktrees/agate-TAG0031；consistency 必须用 worktree 自己的脚本（非 ~/.agate 稳定版）
- 查证结果：主 Agent 在 P4 阶段实测过这四条命令（1435 passed/2 skipped、0 ERROR、0 输出、90s 内），本轮请你独立重跑确认，不要复用旧数字
</objective_info>
