...
> 本次是第 2 轮复评：上一轮你（或前一位 plan-eng-review）判 rejected（两处阻塞），architect 已修订。本轮只需复核两处改动点是否修正到位，其余已通过部分（候选方案、gate_commands、影响面梳理其余部分、BDD 覆盖）上一轮已核实通过，无需重新展开完整评审。

### 目标
复核 P2-design.md 本轮修订：① R1 是否补齐了 pyyaml pip install 前的 checksum 前置校验，"先执行后校验"的顺序缺口是否已闭环；② `[SCOPE+]` 标记是否已改为行首独立格式、能被 `check-scope-resolved.py` 正则检出。通过则 status: approved；仍有缺口则 rejected 并具体指出。

### 约束
- **agent≠main**，不得直接改设计文件。
- **复核重点**：
  1. 独立读取修订后的 R1 「缓解设计」段，核实 `_ensure_agate_common` 是否真的把内联 checksum 校验放在 `pip install` 之前（时序关系，不是只看有没有提到"校验"两个字）；核实「回归覆盖」段是否补了 checksum 不匹配场景的用例（mock subprocess.run 断言未被调用）。
  2. 独立执行 `python3 -c "import re; text=open('P2-design.md').read(); print(bool(re.search(r'^\s*-?\s*\[SCOPE\+\]', text, re.MULTILINE)))"`（工作目录为 task 目录）核实正则确实能匹配。
  3. 核实修订未意外改动已通过的部分（三簇拆批、候选方案 1/2、gate_commands、files_to_read、env_constraints、minimal_validation、实现完成标志）。
- **禁止行首 PASS/FAIL 格式**：同上一轮。

### 上游关联
- 第 1 轮 review 结论：rejected，两处阻塞：R1 pyyaml checksum 顺序缺口 + `[SCOPE+]` 行首格式未闭环（详见 {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-gate-diagnosis.md）
- architect 第 2 轮修订摘要：「R1 补齐 pyyaml pip install 前内联 checksum 校验（含代价确认/回归覆盖更新）；`[SCOPE+]` 改为行首独立段落，正则已验证匹配」
- 主 Agent 已独立复核修订后文件：正则匹配确认 `[SCOPE+]` 现位于独立段落行首；R1「缓解设计」段确认新流程为①内联 checksum 校验 pyyaml wheel → 不匹配则报错不 pip install → ②校验通过才 pip install → ③import agate_common，顺序已闭环；「代价确认」段说明 verify_checksums 阶段会对 pyyaml 再校验一次（幂等，非重复实现）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-design.md（修订后全文，重点看 §1.3 R1 全段）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-gate-diagnosis.md（第 1 轮诊断，两处问题的原始描述）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P2-review.md（第 1 轮 review 全文，可选参考）
</dispatch_guide>
...
- 关键标识：task 目录 {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/
- 查证结果：见上「上游关联」节主 Agent 已核实的客观事实
</objective_info>
