...
> 本次是第 2 轮复评：上一轮你（或前一位 requirements-review）判 needs-revision，analyst 已按诊断修订。本轮只需复核两处改动点是否修正到位，其余已通过部分（BDD-1~13、P0_STALE、frontmatter、裁剪说明、同类扫描小节 1/2/4/5/6/7）上一轮已核实通过，无需重新展开完整评审。

### 目标
复核 P1-requirements.md 本轮修订：① 同类扫描第 3 小节口径是否与展示的 grep 命令逐字节对应；② 是否为 DEBT0002/0003/0004/0016/0017/0018 六条补齐了登记闭合验收条件。通过则 status: approved；仍有缺口则 needs-revision 并具体指出。

### 约束
- **agent≠main**，不得直接改需求文件。
- **锚点要求不变**：结论需引用具体 BDD 编号（本轮新增 BDD-15，修改了同类扫描第 3 小节与 BDD-14 的 Given 引用）。
- **复核重点**：
  1. 独立执行 `grep -n "dirname(dirname\|dirname(os.path.dirname" agate/scripts/*.py | wc -l`，核对正文声称的"14 行"是否属实，以及类别 A/类别 B 的拆分（4 行 3 实例 vs 10 行）是否与你独立读取上下文的判断一致。
  2. 核对新增 BDD-15 是否确实覆盖 DEBT0002/0003/0004/0016/0017/0018 六个 debt id 的登记闭合（status 改 closed），且与 BDD-7（DEBT0007）合计构成 7/7 覆盖。
  3. 已通过部分（BDD-1~13 除 BDD-14 引用微调外、P0_STALE、frontmatter、裁剪说明、同类扫描 1/2/4/5/6/7）**不要求重新展开逐项复核**——若你抽查确认未被改动，可直接沿用上一轮结论；若发现被意外改动或产生新的不一致，仍需指出。
- **禁止行首 PASS/FAIL 格式**：同上一轮。

### 上游关联
- 第 1 轮 review 结论：needs-revision，两处：同类扫描第 3 小节口径失真（14 vs 3）+ 六条 DEBT 缺登记闭合 BDD（详见 {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-gate-diagnosis.md）
- analyst 第 2 轮修订摘要：「第 3 小节改为宽口径 14 行按 task_dir/`__file__` 两类拆解并说明风险边界；新增 BDD-15 补齐六条 DEBT 登记闭合验收」
- 主 Agent 已独立复核修订后文件：`grep -c '^#### BDD-'` = 15；BDD-15 正文逐条列出六个 debt id 均对应"status 改 closed"验收条件；同类扫描第 3 小节含"14 行按 task_dir/`__file__` 两类拆解"的完整论证（4 行 3 实例 vs 10 行），与展示的 grep 命令命中数一致

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-requirements.md（修订后全文，重点看同类扫描第 3 小节 + BDD-14 + BDD-15）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-gate-diagnosis.md（第 1 轮诊断，两处问题的原始描述）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-review.md（第 1 轮 review 全文，可选参考，若上一轮 review 就是你写的可直接沿用记忆）
- {AGATE_WORKSPACE}/debt/tech-debt.md（登记格式先例核对用）
</dispatch_guide>
...
- 查证结果：主 Agent 实测同类扫描第 3 小节引用的 grep 命令命中 14 行；BDD 总数 15 条；BDD-15 六个 debt id 字符串（DEBT0002/0003/0004/0016/0017/0018）均在正文中出现在"status 改 closed"验收条件内
</objective_info>
