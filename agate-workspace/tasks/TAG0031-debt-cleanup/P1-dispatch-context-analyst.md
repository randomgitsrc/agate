...
> 本次是 P1 第 2 轮（重试）：上一版 P1-requirements.md 已被 requirements-review 判 needs-revision，见下方「上游关联」引用的诊断文件。

### 目标
修改（不是重写）已有的 P1-requirements.md，只修正 review 指出的两处缺口，其余 12 条 BDD（1-13，除 6 的措辞建议外）、P0_STALE 判定、frontmatter 声明、裁剪说明、同类扫描小节 1/2/4/5/6/7 均已通过独立评审，**不要改动**。

### 约束
- **只改两处，不重写全文**：① 同类扫描第 3 小节口径与 BDD-14；② 补齐 DEBT0002/0003/0004/0016/0017/0018 六条的登记闭合验收条件。改动范围外的其余内容保持原样，避免无谓返工引入新的不一致。
- **第①处修正路径二选一**（review 给出）：
  a. 换更精确的 grep（如追加 `task_dir` 关键词过滤）使正文展示的命令与其声称的命中数逐字节对应；或
  b. 保留原始宽口径命令 `grep -n "dirname(dirname\|dirname(os.path.dirname" agate/scripts/*.py`（主 Agent 独立复核实测命中 14 行），但显式写出筛选依据：按推导起点分两类——`task_dir`（workspace 相对路径推导，2 处，构成同类）vs `__file__`/`script_path`（脚本自身/仓库根路径推导，约 9 处，不构成同类），并说明为何后者不具备 DEBT0016 描述的"workspace 布局非标准嵌套时静默产出错误路径"风险。
  任选其一，但必须保证读者复现同一 grep 命令能得到与正文一致的结论。
- **第②处**：为 DEBT0002/0003/0004/0016/0017/0018 六条补齐"登记条目 status 改为 closed"的显式验收条件（可仿 BDD-7 单条覆盖多个 debt id，也可扩展 BDD-7 本身），比照 `debt/tech-debt.md` 中 DEBT0005/DEBT0006 已有 closed 条目的登记格式（status/closed_at/evidence 追加块，BDD-7 已核对过这一格式一致性，直接复用判定即可）。任务标题是"批量关闭 7 条历史遗留 open 技术债"，修正后 7 条 DEBT 都应有对应登记闭合的验收条件，不能只有 DEBT0007 一条。
- **BDD 编号**：修改/新增 BDD 时延续现有编号序列（当前到 BDD-14），若新增编号顺延写 BDD-15+，不要打乱已通过评审的 BDD-1~13 编号。
- **frontmatter/裁剪说明/P0_STALE 节不动**：这些已被 review 核实通过，重新论证是浪费。

### 上游关联
- 第 1 轮 review 结论：needs-revision，完整诊断见 {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-gate-diagnosis.md（主 Agent 已独立复核两处发现均成立：grep 实测 14 行 vs 正文只讨论 3 行；14 条 BDD 中仅 BDD-7 一条含登记闭合验收，另 6 条 DEBT 缺失）
- P1-review.md 本体也可读（{AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-review.md）——但診断文件已提炼出行动项，无需重读全文找问题点

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-requirements.md（待修改的现有产出，改动前请先读一遍避免推翻已通过的部分）
- {AGATE_WORKSPACE}/tasks/TAG0031-debt-cleanup/P1-gate-diagnosis.md（本轮修改的权威诊断，两处问题的具体描述与修正路径）
- {AGATE_WORKSPACE}/debt/tech-debt.md（DEBT0002/3/4/16/17/18 原始条目 + DEBT0005/DEBT0006 已 closed 条目的登记格式先例）
</dispatch_guide>
...
- 关键标识：tech-debt.md 实际路径 {AGATE_WORKSPACE}/debt/tech-debt.md
- 查证结果：主 Agent 实测 `grep -n "dirname(dirname\|dirname(os.path.dirname" agate/scripts/*.py | wc -l` = 14；`grep -c '^#### BDD-' P1-requirements.md` = 14；`grep -n "status.*closed\|closed_at" P1-requirements.md` 仅命中 BDD-7 段落（3 处，均围绕 DEBT0007 单条）
</objective_info>
