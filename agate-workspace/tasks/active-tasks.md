...
| 编号 | 任务名称 | 状态 | 阶段 | 优先级 | 依赖 | 创建日期 | 更新日期 |
|------|----------|------|------|--------|------|----------|----------|
| TAG0029 | gate 命令解析器修复批（RM-AG0056 + DEBT0027 high + DEBT0023）：agate-read-gate-commands.py 值清洗剥离行内注释+引号闭合校验（DEBT0027 假绿灯：测试没跑却被当红灯放行）+ check-tdd-red judge 分支语法错误不得计入红灯证据 + P3* 键收集收紧（DEBT0023）+ 平台假设扫描器 fixture 数据面豁免并纳入 P3/P4 常驻面（RM-AG0056）——三处同源指向同一解析器，强合并单 task | 🔄 | P2 | 高 | RM-AG0056 | 2026-09-03 | 2026-09-04 |
