...
|------|----------|------|------|--------|------|----------|----------|
| TAG0028 | subagent 存活可观测性与受控自主再派发（RM-AG0055）：命令流日志机制（三平台数据源适配器 + 调用/活动冻结 + 无效重复检测 + 截断排除，阈值两级 expected×2 复用 RM-AG0023）——检测定位'证据+触发核查不自动判死'；心跳文件生命周期；受控自主再派发（执行角色权限下放，judge 例外）；设计文档 v5 已闭环（4 轮评审 + 三平台实机验证）| 🔄 | P7 | 高 | RM-AG0055 | 2026-09-03 | 2026-09-03 |
