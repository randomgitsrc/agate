...
| 编号 | 任务名称 | 状态 | 阶段 | 优先级 | 依赖 | 创建日期 | 更新日期 |
|------|----------|------|------|--------|------|----------|----------|
| TAG0027 | 编排语义统一落地（RM-AG0054）：推进侧状态机 CLI（agate next/advance）——推进决策从 orchestrator 临场判断改为查表推进，复用 check-state-transition.py + check-gate.py exit 三态 + phases.yaml 扩展（next/retreat 字段纳入 S-1/S-2）+ agate-retreat-to.py 对接；CLI 为 /loop 档位 C 可观测层（档位 C 推进强制走 agate next）；编排心智统一文档化（五模式锚点 + 实现注记标记）；设计文档 v3b（三轮独立评审闭环 2026-09-02）| ⬜ | P0 | 高 | RM-AG0054 | 2026-09-02 | 2026-09-02 |
