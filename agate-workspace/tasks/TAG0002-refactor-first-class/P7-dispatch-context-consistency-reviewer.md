...
### 目标
产出 docs/tasks/TAG0002-refactor-first-class/P7-consistency.md——对 TAG0002 P1-P6 产出做跨文件一致性审查：DESIGN_GAP 配对、SCOPE+ 闭环、跨文件一致性、未决项清零。你是独立视角（agent 必须是 consistency-reviewer）。

### 约束
- **只审不写**：不修改任何产出文件，只产出 P7-consistency.md。
- 本任务是 **agate 协议自身改造**（dogfooding）：审查对象是 worktree `agate/`（已含 TAG0003 工作区架构 + TAG0002 refactor 机制）；`~/.agate` 是稳定版 v0.40.2 开发工具（禁止改动）。
- **检查清单**（P7 卡片）：
  1. **DESIGN_GAP 配对**：P4-implementation.md / -review-fix.md / -docfix.md 中的 [DESIGN_GAP] 声明 → 必须在 P7-consistency.md 中逐条转抄 + 配 [DESIGN_GAP_REVIEWED] 标记。未配对 → gate 不通过。
  2. **SCOPE+ 闭环**：P1-requirements.md frontmatter 有 scope_resolved（1 项：ci-gate-backstop refactor 感知）——确认已纳入 P2/P4 实现。
  3. **跨文件一致性**：
     - P2 packages=[agate] 与 P8 release 的 bump 范围一致？
     - P1 的 8 条 BDD 与 P6 的 8 条验收结果数量匹配 + 内容对应？
     - P4 实现（check-gate.sh / agate-md-field-get.py / agate-frontmatter-check.py / ci-gate-backstop.py / P1/P3/P6 卡片 / verifier/test-designer 角色 / WORKFLOW/state-machine/dispatch-protocol）与 P2 方案（§1.1 改动清单）吻合？
     - P4-implementation.md 各文件声明与 git 实际改动一致？
...
### 上游关联
- P4 实现无 DESIGN_GAP 声明（P4-implementation.md §3："无 DESIGN_GAP（P2 方案在实现中未发现歧义/缺口）"）；有 1 项 SCOPE_GAP（测试计划归档，合理）。
- P1 frontmatter scope_resolved 1 项（ci-gate-backstop refactor 感知）。
- P6 验收 8/8 PASS（P6-acceptance.md，provenance 审计 0）。
- P5 全量验证绿（bats 654/0 + consistency 0 ERROR + shellcheck 0 + count 648）。

### 输入文件
- docs/tasks/TAG0002-refactor-first-class/P1-requirements.md（需求基线 + BDD + scope_resolved——必读）
- docs/tasks/TAG0002-refactor-first-class/P2-design.md（方案设计 + packages/domains——必读）
- docs/tasks/TAG0002-refactor-first-class/P4-implementation.md / -review-fix.md / -docfix.md（实现记录——必读）
- docs/tasks/TAG0002-refactor-first-class/P4-review.md（实现评审——必读）
- docs/tasks/TAG0002-refactor-first-class/P5-test-results/unit.md（验证结果——必读）
- docs/tasks/TAG0002-refactor-first-class/P6-acceptance.md（验收结果——必读）
- docs/tasks/TAG0002-refactor-first-class/P0-brief.md（任务简报——必读）
- AGENTS.md（项目约定——必读）
- git log（实际改动核对——按需）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=TAG0002-P6 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 任务进度：P1 8 BDD → P2 方案 A → P3 19 回归测试 → P4 实现（review 三审 approved，R1 BLOCKER + R2 文档残留闭环）→ P5 验证绿 → P6 验收 8/8 PASS。
- 已核实查证：P4-implementation.md §3 声明"无 DESIGN_GAP"；P4 实现改动文件与 P2 §1.1 清单一致（主 Agent 已核对）；P1 scope_resolved 1 项。
</objective_info>
