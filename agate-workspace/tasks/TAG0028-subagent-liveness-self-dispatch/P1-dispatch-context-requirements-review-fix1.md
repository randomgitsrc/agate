...
- 上轮评审产出：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-review.md`（status=needs-revision，subagent `de346328`）
- 上轮 dispatch-context：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-dispatch-context-requirements-review.md`（**复用其全部目标/约束/评审清单**，本文件只写本轮增量）
- 修复轮产出：analyst fix1 已按评审结论修改 `P1-requirements.md`（BDD 25 → 33 条），修复动作见文件「## 11. 修复轮记录（fix1）」

### 重审目标

对修复后的 `P1-requirements.md` 做**复审**：逐条确认评审结论项 1-6 是否真正修复（不是只改措辞）：

1. **红线 1（BDD-4 混实现细节）**：确认 BDD-4 Then/When 不再点名任何实现 API（zlib.zstdDecompress 等）；行为化表述（解压隔离在适配器内部 / 产出统一 IR / 不依赖 python zstandard 二进制）。capability_requirements 中的环境事实声明（Node zlib 可用）可保留——那是能力声明不是 BDD 实现细节，不算违规。
2. **红线 2（I-4 无锚点）**：确认新增 BDD（BDD-5 子 agent 会话定位）锚定 sidecar / delegationDepth，可二值判定，非实现细节（list_sessions/read_commands 属设计 §3.4.4 契约接口名可接受）。
3. **红线 3（多场景未拆号）**：确认 BDD-9/10（调用冻结兜底 alert/suspect）、BDD-11/12（活动冻结 alert/suspect）、BDD-19/20/21（配置覆盖/缺失/损坏）各自只有一条 Given-When-Then。
4. **次要 4a**：BDD-15（结果签名变化不误报）独立编号。
5. **次要 4b**：BDD-16 覆盖 REPEAT_UNIQUE_MIN=3 信息级提示（锚定验证脚本 line 44/148-151）。
6. **次要 5**：BDD-24 覆盖检测/派发输出平台无关、由平台食谱消费。
7. **编号完整性**：BDD-1~33 全局连续无跳号；每条恰一条 Given-When-Then；可 PASS/FAIL 二值判定。
8. **修复轮记录**：§11 存在且逐条对应评审结论项 1-6（含编号重排说明与总数变化 25→33 的净增核算）。

### 约束（延续上轮，重点重申）

- 评审结论必须引用具体锚点（BDD 编号 + 覆盖维度清单）
- 若发现修复不彻底或引入新问题 → 结论 needs-revision/rejected 并列出具体 BDD 编号
- 全部通过 → status: approved
- 只审不写：不修改 P1-requirements.md
- 评审正文禁止行首 `- PASS` / `- FAIL` 格式
- 核实现有 BDD 语义未被修复过程改变（BDD-7 expected×2 下限 30s / BDD-13 轮询 / BDD-17/18 心跳命名 / BDD-22/23 子派发边界 / BDD-25 judge 例外 / BDD-26 gate 返回约定等仍与设计文档数值一致）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-requirements.md`（修复后评审对象）
2. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-review.md`（上轮评审结论，逐项对照）
3. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P0-brief.md`（范围锁定）
...
5. `agate/assets/review-roles/requirements-review.md`（角色定义，评审清单权威）
6. `AGENTS.md`（项目约定，worktree 根）

### 产出文件字段

产出 `P1-review.md`（**覆盖上轮文件**，同一路径）到任务目录；frontmatter 用 `agate-md-field-set` 更新：
phase=P1 / task_id=TAG0028 / trace_id=TAG0028-P1-20260903 / agent=requirements-review /
status=approved（全部通过）| needs-revision | rejected。
覆盖写入时 status 从 needs-revision 更新为最终结论。
</dispatch_guide>
...
<objective_info>
### A. 修复后 BDD 结构（33 条，编号连续）
- Phase 1：BDD-1（IR 字段）/ BDD-2（claude-code JSONL）/ BDD-3（opencode SQLite）/ BDD-4（dsh zstd，行为化）/ BDD-5（子 agent 会话定位，新增）/ BDD-6（新增平台零改动引擎）/ BDD-7（fixture 脱敏）
- Phase 2：BDD-8（expected×2 主信号）/ BDD-9、10（兜底 alert/suspect，拆）/ BDD-11、12（活动冻结 alert/suspect，拆）/ BDD-13（长时间思考不误杀）/ BDD-14（无效重复 SPIN）/ BDD-15（签名变化不误报，拆）/ BDD-16（REPEAT_UNIQUE_MIN=3，新增）/ BDD-17（截断排除）/ BDD-18（轮询误报标注）/ BDD-19、20、21（阈值配置覆盖/缺失/损坏，拆）/ BDD-22（verify 9 场景全 PASS）/ BDD-23（证据+核查不判死）/ BDD-24（平台无关输出，新增）
- Phase 3：BDD-25（心跳父子命名）/ BDD-26（审计豁免）/ BDD-27（清理+兜底）/ BDD-28（两套信号分工）
- Phase 4：BDD-29（执行角色不写 .state.yaml）/ BDD-30（写权限子集）/ BDD-31（judge 例外）/ BDD-32（产出收敛不触发 gate）/ BDD-33（gate 返回约定）
（以上结构为核验参考，以文件实际编号为准）

### B. 阈值与机制锚点（复审数值一致性）
- 调用冻结：expected×2（RM-AG0023 timeout_seconds，下限 30s）/ 兜底 alert=300 / suspect=900
- 活动冻结：alert=60 / suspect=300
- 无效重复：窗口 10 同 (命令, exit, 输出哈希) ≥5 → SPIN；REPEAT_UNIQUE_MIN=3（唯一命令数 <3 → 信息级）
- 截断排除：truncated 不参与哈希比对
- 心跳命名：`${TASK_DIR}/.heartbeat` / `.heartbeat.child-{n}`
- 子派发两边界 + judge 例外（不开放 Agent/subagent_fork）

### C. retries 状态
- `.state.yaml` 已记录 retries[P1] round 1（quality）——本轮重审通过与否决定是否追加
</objective_info>
