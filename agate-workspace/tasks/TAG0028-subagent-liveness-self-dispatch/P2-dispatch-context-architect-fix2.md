...
- 上轮 dispatch-context：`P2-dispatch-context-architect.md` + `P2-dispatch-context-architect-fix1.md`（**均继续有效**，本文件只写本轮增量）
- 触发：主 Agent 在 P3 环境基线捕获时发现 `agate-read-gate-commands.py` 解析出的 `P3`/`P5` 命令值带行内注释尾巴 + 残留未闭合引号（实测：`'python3 -m pytest agate/tests/ -q --tb=short"        # P3 TDD 红灯读取测试运行器（fix1：...）'`），经 shell 最小验证（`bash -c`）确认 unterminated quote 语法错误（exit 2）——check-tdd-red（P3 红灯判定）与 P5 验证执行该命令时必失败，且 judge 分支可能误判为红灯可推进（假绿灯）

### 修复目标（唯一一项：gate_commands 值必须为纯命令，无行内注释）

把 P2-design.md `gate_commands:` 块中 **P3 / P5 两行（及任何其他含行内注释的 key）** 的行内注释移除：
...
- 目标形态（值 = 纯命令字符串，注释移到块外或独立注释行）：
  ...
  ```
- 必须保证：`python3 agate/scripts/agate-read-gate-commands.py`（env GATE_FILE 指向 P2-design.md）输出的每条命令值 == 引号内纯命令、无 `"` 残留、无 `#` 注释尾巴、无行首尾空白

### 约束（延续上轮，重点重申）

- 本轮**只改 gate_commands 块内的注释形态**（行内注释 → 独立注释行），命令字符串本身一字不改（P3/P5 命令内容与 timeout_seconds 均不动）；不得改动方案选型、影响面梳理、files_to_read、env_constraints、minimal_validation、§10 修复轮记录
- 修正后自检命令（必须执行）：
  ```
  cd /home/kity/oclab/agateon/.worktrees/agate-TAG0028
  GATE_FILE=agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md \
    timeout 30s python3 agate/scripts/agate-read-gate-commands.py
  ```
  确认每条 cmd 均为干净命令字符串（无 `"` 残留、无 `#`）
- 在 §10 修复轮记录追加「### 10.5 fix2：gate_commands 行内注释清理」小节：问题现象（解析残留引号+注释、bash -c 语法错误实测）、根因（agate-read-gate-commands 的 strip(chr(34)) 只剥首尾引号、值内注释不属于值）、修法（注释移独立行）、验证（read-gate-commands 输出干净 + bash -c 语法 OK）
- `check-frontmatter.py` 跑一遍确认 exit 0
- 若发现必须改命令内容本身 → 停下报告主 Agent（gate_commands 已固化，内容改动需 P2 重审）

### 输入文件（按需）

1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md`（修改对象，§4 gate_commands 块 + §10 修复轮记录）
2. `AGENTS.md`（项目约定）

### 产出文件字段

沿用已有 frontmatter（不改动任何字段值）。
</dispatch_guide>
...
<objective_info>
### A. 实测证据（2026-09-03，主 Agent 复核）
- `agate-read-gate-commands.py`（env GATE_FILE=P2-design.md）输出 P3 cmd：
  `'python3 -m pytest agate/tests/ -q --tb=short"        # P3 TDD 红灯读取测试运行器（fix1：裸 pytest → python3 -m pytest）'`
- `bash -c 'echo python3 -m pytest ... --tb=short"        # 注释'` → `bash: -c: 行 1: 寻找匹配的 `"' 时遇到了未预期的 EOF`（exit 2）
- 干净命令对照 → exit 0
- 解析根因：`agate-read-gate-commands.py` 的 `val = raw.strip().strip(chr(34)).strip(chr(39))`——值内行注释使字符串不以 `"` 结尾，strip(chr(34)) 只剥开头引号，残留结尾 `"` + 注释

### B. 影响面
- check-tdd-red.py `run_test_with_formatter` 用 `subprocess.run(cmd, shell=True, executable="bash")` 执行 → 语法错误（exit 2）+ 输出含 "未预期的 EOF"；judge_result 无 SyntaxError/ImportError 关键词命中、exit 2 不满足 A 类分支（<120 且 !=1）→ 落入 "unexpected test failure" → exit 0 = **假红灯可推进**，P5 同命令再炸
- P5 主 Agent 从 P2-design.md 读 gate_commands 执行同样受影响
</objective_info>
