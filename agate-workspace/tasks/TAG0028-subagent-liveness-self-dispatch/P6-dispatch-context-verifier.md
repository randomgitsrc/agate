...
产出 `P6-acceptance.md` + `P6-evidence/`：把 P1 的 33 条 BDD 逐条对照实跑验收（PASS/FAIL 二值），
每条 PASS 必须有证据文件引用。本任务 `ui_affected: false`（P2 声明），无 UI/视觉维度，证据形式 =
断言日志 / 测试输出 / 脚本实跑记录。验收对象：命令流三脚本（IR/三平台适配器/检测引擎）+
心跳生命周期 + 再派发边界 + 协议改写（P4 实现，P5 已验证 1434 passed 全绿）。

### 约束

1. **BDD 逐条对照，二值判定**：33 条 BDD（P1 基线，编号 `#### BDD-NN:`），每条只允许 PASS 或 FAIL；
   PASS 行格式 `- PASS BDD-NN: {描述} ({证据路径})`，证据路径真实存在（相对 P6-evidence/ 或引用
   P5-test-results/）。**P6 验收记录验收时的事实**——FAIL 就写 FAIL，不写"修复后 PASS"。
2. **证据非空实质**：P6-evidence/ 必须非空，每个文件含实质内容（断言文件含实际输出，不接受 1 行
   文本充数，T046 教训）。
3. **引用 P5 证据**：本任务 .state.yaml 已写 `p5_pass_commit`（34366ab），且 P5→P6 间无非产出文件
   改动（P6 阶段只产出验收文件）——可复用 P5-test-results/ 作为全量回归证据（check-p6-provenance
   审计 7 判定 reuse_allowed 时允许），不必重跑全量回归；但 **BDD 逐条验收的实跑证据仍须产出**
   （检测引擎判定、适配器解析、心跳生命周期、再派发边界的验证脚本/命令输出落盘 P6-evidence/）。
4. **验收动作可重复**：对每条 BDD 设计可重复的验证命令（pytest 单测 / 脚本 CLI 实跑 / 文档断言
   grep），执行输出落盘 P6-evidence/（如 `{bdd-id}.log`），PASS 行引用之。禁止"看起来对"式结论。
5. **frontmatter 汇总**：`pass:`/`fail:`/`ui_affected: false` 写在文件头 frontmatter
   （`---` 分隔块），pass+fail 必须 = 33。
6. **自查≠gate**：你产出 P6-acceptance.md + P6-evidence/，P6 gate（check-gate/check-p6-evidence/
   check-p6-provenance + P6.5 judge）由主 Agent 亲自跑。不要在返回中声称"验收已通过"。
7. **环境隔离**：[PROD_NOT_TOUCHED]；只读协议文档与任务目录；不读其他用户 DSH 会话（fixture 已脱敏）。
8. **超时纪律**：所有 bash 命令 shell 层 timeout（按预期 ×1.5）。

### 上游关联

- P1-requirements.md（33 BDD，验收锚）
- P5-test-results/unit.md（全量回归证据，可复用）
- P2-design.md / P4-implementation.md（验收对象声明）
- verify_cmdstream_detection.py（BDD-22 锚）
- 三平台 fixture（BDD-2/3/4/5/6/7 解析验收输入）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-requirements.md`（33 BDD + 隐含需求 + 同类扫描）
2. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P5-test-results/unit.md`（全量回归证据）
3. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P4-implementation.md`（实现声明）
4. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md`（方案 + gate_commands）
5. `agate/assets/execution-roles/verifier.md`（角色定义，P6 模式）
6. `AGENTS.md`（项目约定）

### 产出文件字段

产出 `P6-acceptance.md` 到任务目录，frontmatter 用 `agate-md-field-set` 填写：
phase=P6 / task_id=TAG0028 / type=acceptance / parent=P5-verification.md /
trace_id=TAG0028-P6-20260903 / status=draft / created=2026-09-03 / agent=verifier /
pass=N / fail=0 / ui_affected=false（pass+fail=33）。
证据目录：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P6-evidence/`（非空，
每条 PASS 引用真实文件）。
</dispatch_guide>
...
- 协议本体（验收对象）= worktree 的 `agate/`（主 checkout 禁止改动）

### B. BDD 结构速查（33 条，P1 基线）
- Phase 1 解析层（BDD-1~7）：IR 十字段契约 / claude JSONL / opencode SQLite / dsh zstd /
  子 agent 会话定位 / 新增平台零改动引擎 / fixture 脱敏
- Phase 2 检测引擎（BDD-8~24）：调用冻结 expected×2 / 兜底 300/900 / 活动冻结 60/300 /
  长时间思考不误杀 / 无效重复 SPIN / 签名变化不误报 / REPEAT_UNIQUE_MIN=3 / 截断排除 /
  轮询标注 / 阈值配置覆盖缺失损坏 / verify 9 场景锚 / 证据不判死 / 平台无关输出
- Phase 3 心跳生命周期（BDD-25~28）：父子命名 / 审计豁免 / 清理+兜底 / 两套信号分工
- Phase 4 再派发边界（BDD-29~33）：不写 .state.yaml / 写权限子集 / judge 例外 /
  产出收敛不触发 gate / gate 返回约定

### C. 关键验证命令（验收实跑参考，输出落盘 P6-evidence/）
- 检测引擎：`timeout 180s python3 -m pytest agate/tests/unit/test_agate_cmdstream_detect.py -q --tb=short`
  （BDD-8~24 承载）+ verify 脚本（BDD-22 锚）
- 适配器：`timeout 180s python3 -m pytest agate/tests/unit/test_agate_cmdstream_adapters.py -q --tb=short`
  （BDD-2~7）
- IR：`timeout 120s python3 -m pytest agate/tests/unit/test_agate_cmdstream_ir.py -q --tb=short`（BDD-1）
- 心跳/派发：`test_agate_cmdstream_heartbeat.py` + `test_agate_cmdstream_dispatch.py`
- 全量回归（可复用 P5 证据，不重跑）：P5-test-results/unit.md（1434 passed / 0 failed）
- 文档断言类 BDD（BDD-20/25/26/27/28/29/30/31/32/33 含协议文档语义）：可用 grep 断言 +
  测试承载，证据 = 测试输出或文档断言记录

### D. p5_pass_commit 与证据复用
- `.state.yaml` p5_pass_commit = 34366ab（fix3 后全绿）；P5→P6 无非产出文件改动 →
  审计 7 应判 reuse_allowed，可复用 P5-test-results/ 作全量回归证据
- 各 BDD 逐条实跑证据仍须独立产出（每条 PASS 行引用 P6-evidence/ 下真实文件）
</objective_info>
