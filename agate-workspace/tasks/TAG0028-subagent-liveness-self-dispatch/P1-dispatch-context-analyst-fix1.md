...
- 上轮产出路径：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-requirements.md`
- 上轮 dispatch-context：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-dispatch-context-analyst.md`（**复用其全部目标/约束/上游关联/输入文件/产出规格**，本文件只写本轮增量）
- 评审结论：`P1-review.md` status=needs-revision（独立评审 subagent `de346328` 产出）

### 修复目标（评审打回的 3 项红线，全部必须修复）

1. **BDD-4 混入实现细节**（P1 纯净性红线）：BDD-4 的 Then 写明「解压经由 Node 24 原生 zlib.zstdDecompress 在适配器内部完成」——点名具体实现 API。修复：P1 只定义行为——"解压隔离在 dsh 适配器内部完成、产出统一 CommandRecord IR、不依赖 python zstandard/zstd 二进制"；具体解压 API（Node zlib / spawn node / 其他）留给 P2 设计决策。注意 capability_requirements 中"Node 24 原生 zlib.zstdDecompress 可用"作为环境事实可保留（那是能力声明不是 BDD 实现细节），但 BDD-4 正文不得点名实现 API。

2. **I-4 子 agent 会话定位无 BDD 锚点**（隐含需求覆盖缺口）：设计文档 §3.4.4 与验证记录均要求——Claude Code sidecar `subagents/agent-*.jsonl` / DSH 独立 session 文件（delegationDepth 区分层级）的会话定位行为。修复：新增一条 BDD（编号顺延，如 BDD-6 插入后重排或追加在 Phase 1 节内合适位置，**编号必须全局连续**），覆盖"适配器 list_sessions/read_commands 能按平台规则定位子 agent 会话（sidecar / delegationDepth）而非只读主会话"。

3. **BDD-8/9/14 多场景未拆号**（红线"每条 BDD 只有一条 Given-When-Then"）：
   - BDD-8 拆两条：调用冻结兜底·alert（300s）与调用冻结兜底·suspect（900s）——每条一条 Given/When/Then
   - BDD-9 拆两条：活动冻结·alert（60s）与活动冻结·suspect（300s）
   - BDD-14 拆三条：阈值显式覆盖生效 / 配置缺失兜底默认值 / 配置损坏兜底默认值
   - 拆分后编号连续重排，总数变化（预计 +4 或按实际拆分方案），`#### BDD-NN:` 全局连续无跳号

### 次要建议项（随本轮一并修订，评审明确"不单独打回"）

4. **BDD-11 对照场景未拆号**：对照场景（命令名重复但 exit/输出哈希变化 → NORMAL）嵌入 Then 属第二场景——拆为独立 BDD（如"无效重复·结果签名变化不误报"）。
5. **REPEAT_UNIQUE_MIN=3 未体现**：窗口内唯一命令数 <3 但结果签名在变化的合法迭代特征（验证脚本场景 H_正常_长时间思考不误杀/合法迭代）——新增或并入 BDD 覆盖"唯一命令数 <3 时不以重复判定空转（信息级提示）"。
6. **I-15 检测输出平台无关仅部分覆盖**：BDD-16 覆盖了"不自动判死"，但未显式锚定"检测/派发输出平台无关、由平台食谱消费"——补一条 BDD 或扩展 BDD-16 的 Then 覆盖平台无关输出形态。

### 约束（延续上轮，重点重申）

- 修复只动 `P1-requirements.md`，不修改 P1-review.md（评审意见由主 Agent 留存）
- BDD 全局编号连续不跳号；每条 BDD 只有一条 Given-When-Then；可 PASS/FAIL 二值判定
- 阈值数值与设计文档一致（300/900/60/300/10/5 + expected×2 下限 30s + REPEAT_UNIQUE_MIN=3）
- 修复完成后跑 `python3 agate/scripts/check-frontmatter.py <P1-requirements.md>` 确认 exit 0
- 若修复过程中发现需改动超出 P0-brief 锁定范围 → 停下报告主 Agent
- 修复后在 P1-requirements.md 底部追加一节「## 11. 修复轮记录（fix1）」，逐条列出修复动作（对应评审结论项 1-6），供重审与 P7 追溯

### 产出文件字段

沿用上轮（frontmatter 已存在，仅 status 保持 draft；若 agate-md-field-set 校验报错照提示修正，不手写）。
</dispatch_guide>
...
<objective_info>
### A. 评审结论引用（P1-review.md，修复目标锚点）
- 结论项 1：BDD-4 Then 混入 zlib.zstdDecompress（P1 纯净性红线）
- 结论项 2：I-4 子 agent 会话定位无 BDD 锚点（sidecar / delegationDepth）
- 结论项 3：BDD-8/9/14 多场景未拆号（Then 两级阈值分支 / When ①②③ 三场景）
- 次要 4a：BDD-11 对照场景嵌入 Then
- 次要 4b：REPEAT_UNIQUE_MIN=3 未体现（验证脚本 line 44/149-151：窗口唯一命令数 <3 → 重复可疑·信息级）
- 次要 5：I-15 平台无关输出仅由 BDD-16 部分覆盖

### B. 设计文档/验证记录锚点（供修复引用）
- 设计 §3.4.4 line 268：Claude Code sidecar `subagents/agent-*.jsonl` / DSH 独立 session 文件（delegationDepth）——适配器各自实现 list_sessions 时按平台规则处理层级
- 验证记录 line 9/49/90：三平台子 agent 会话定位差异标注
- verify_cmdstream_detection.py 场景 H（正常_长时间思考不误杀）与合法迭代场景：唯一命令数 <3 但结果签名在变化 → 不判空转（REPEAT_UNIQUE_MIN=3 语义）

### C. 编号重排注意事项
- BDD 拆分/新增后全局编号必须连续（BDD-1, BDD-2, ... 不跳号）
- 拆分产生的 BDD 语义 = 原 BDD 的单场景子集，P6 验收按拆分后的总数对照（当前 25 条 → 拆分后总数在 §11 修复轮记录中声明）
</objective_info>
