...
独立评审 `P2-design.md`（TAG0028 subagent 存活可观测性与受控自主再派发，RM-AG0055），
产出 `P2-review.md`。按 plan-eng-review 角色定义评审重点逐项审查：数据流清晰度、状态机完整性、
接口契约明确性、错误边界、测试策略、技术债（若有"后续应重构"须标准 DEBT 格式）、
多方案探索（≥2 候选 + 权衡 + 选择理由）、实现就绪度（files_to_read 覆盖实现所需全部上下文）、
P2 最小验证（minimal_validation 字段存在且合格）。**只审不写**——不直接改 P2-design.md。

### 约束

1. **评审结论必须引用具体锚点**：架构问题须给「具体问题 + 文件/函数/节 + 建议」；approved 须能
   从评审正文看到锁定决策与关键核对结果。
2. **候选方案核查**：P2-design.md 含 3 候选方案（A 平铺脚本族+显式注册表 / B 单脚本聚合 /
   C 子目录包+目录扫描）+ 权衡表 + 选择理由（选 A）。核查 A 的选择理由是否自洽、B/C 是否
   构成真正的替代方案（非稻草人）、权衡表各维度是否客观。
3. **影响面梳理核查**（P2 卡片强制节）：正文含 改什么 M1-M10（逐文件/小节 + 关联 BDD 编号）/
   不改什么 N1-N10（含 check-gate.py/check-state-transition.py 返回约定、verify 脚本 9 场景锚、
   agate-archive-stale-outputs.py）/ 风险在哪 R1-R10（每条配缓解）。核查三部分齐全且动作有客观
   证据（grep 命中 / 消费方代码引用）。
4. **核心约束逐条核对**（延续 P1，重点）：
   - 不破坏 check-gate.py / check-state-transition.py 返回约定（两套独立信号）——N1 核对
   - 阈值保守（宁可多提示、绝不误杀）+ 检测定位"证据+触发核查不自动判死"——BDD-23 核对
   - 适配器模式（新增平台只写适配器、检测引擎零改动）——BDD-6 核对
   - judge 类角色例外（不开放子派发权限）——BDD-31 核对
   - DSH zstd 解压不硬依赖 python zstandard（隔离在 dsh 适配器内部）——BDD-4 核对
   - verify_cmdstream_detection.py 9 场景锚不改（N7）——BDD-22 核对
5. **gate_commands 核查**：逐 key 独立（不拼接 `&&`）；P5_consistency 用 worktree 自己的
   check-protocol-consistency.py（双工作区纪律）；P5_cmdstream_verify 指向验证脚本；
   timeout_seconds 按三档基准声明。
6. **R10 特别注意**：architect 判定 OpenCode 数据源以验证记录为准（SQLite opencode.db），
   与 P1 BDD-3 一致。核查该决策是否有依据且标注了差异来源（防 P4 踩坑）。
7. **minimal_validation 核查**：字段存在且合格——方案依赖外部行为（DSH zstd 解压）时须含
   验证结果或验证方法；纯代码逻辑须显式声明。
8. **SELF-GATE 面核对**：本任务改 agate/scripts/* + agate/*.md（dispatch-protocol.md /
   role-system.md / 模板 / check-p6-provenance.py 注释登记）→ commit 须含 self-gate-review:；
   P5_consistency 0 ERROR。核查设计是否覆盖此约束。
9. **评审产出 Header**：P2-review.md frontmatter 含 phase=P2 / task_id=TAG0028 /
   trace_id=TAG0028-P2-20260903 / agent=plan-eng-review / status（draft → approved / rejected）。
10. **无行首 PASS/FAIL 预判**：评审正文禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。
11. **范围锁定核对**：设计方案未超出 P0-brief/设计文档锁定范围（out-of-scope 六项不变）——
   若发现超范围（如必须动 check-gate.py 返回约定、必须实现 DSH 心跳钩子机制），在评审中标注。

### 上游关联

- P2-design.md 是唯一评审对象（上游输入：P1-requirements.md 33 BDD + P0-brief + 设计文档 v5）
- 设计文档 v5 与验证记录供核对语义（评审时按需对照，不重新全读）
- P2 gate 规则：P2-review.md 存在且 status: approved（agent≠main）→ 候选方案 ≥2 +
  四字段齐全 + 权衡/选择理由

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md`（评审对象）
2. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-requirements.md`（33 条 BDD 验收锚，核对设计覆盖）
3. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P0-brief.md`（scope/known_risks/out-of-scope）
4. `docs/design-notes/260903-design-subagent-liveness-and-self-dispatch/design-subagent-liveness-and-self-dispatch-v5-with-cmdstream-validation.md`（按需对照 §3/§4）
5. `agate/assets/review-roles/plan-eng-review.md`（角色定义，评审重点权威）
6. `AGENTS.md`（项目约定，worktree 根）

### 产出文件字段

产出 `P2-review.md` 到任务目录，frontmatter 用 `agate-md-field-set` 填写：
phase=P2 / task_id=TAG0028 / trace_id=TAG0028-P2-20260903 / agent=plan-eng-review /
...
### B. P2-design 核心决策（评审锚点）
- 方案 A：新增 agate-cmdstream-ir.py / agate-cmdstream-adapters.py / agate-cmdstream-detect.py 三脚本 + 显式注册表 ADAPTERS
- 影响面：M1-M10（改）/ N1-N10（不改）/ R1-R10（风险）
- OpenCode 数据源以验证记录为准（SQLite opencode.db，R10 标注差异来源）
- gate_commands 11 项：P3/pytest、P5/pytest -q、P5_consistency、P5_cmdstream_verify、P5_shellcheck、P5_count_tests + 5 个 timeout_seconds
- minimal_validation：DSH zstd 解压引用验证记录已实机验证结论（confirmed）

### C. 机制锚点（核对设计数值一致性）
- 调用冻结：expected×2（下限 30s）/ 兜底 alert=300 / suspect=900
- 活动冻结：alert=60 / suspect=300；三类活动（思考/输出/工具）
- 无效重复：窗口 10 ≥5 → SPIN；REPEAT_UNIQUE_MIN=3 信息级；truncated 不参与哈希比对
