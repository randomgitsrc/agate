...
- 上轮产出路径：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md`
- 上轮 dispatch-context：`agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-dispatch-context-architect.md`（**复用其全部目标/约束/上游关联/输入文件/产出规格**，本文件只写本轮增量）
- 评审结论：`P2-review.md` status=approved（阻塞问题 0，4 条非阻塞建议）——方案本身已通过，本轮只做 gate 命令修正

### 修复目标（唯一一项：gate_commands 命令可解析性，T075 类）

P2 gate 预跑发现 T075 WARNING：`gate_commands.P3: "pytest"` 与 `gate_commands.P5: "pytest -q --tb=no"` 的裸 `pytest` 命令**在当前环境不可解析**（`command -v pytest` → exit 1，PATH 中无 pytest 可执行文件）；实际可用的是 `python3 -m pytest`（`python3 -m pytest --version` → pytest 9.0.3 正常）。

**影响**：gate_commands 在 P2 固化后 P4-P6 不得修改（P2 卡片「下游影响」：gate_commands 在 P2 固化后 P4-P6 不能改——设计阶段是声明验证契约的唯一窗口）。若保留裸 `pytest`，P3 check-tdd-red 读取测试运行器与 P5 全量验证都会 exit 127 红灯（T075 教训：python 不存在导致 P3 gate exit 127）。

**修复动作**：把 `gate_commands.P3` / `gate_commands.P5`（及任何其他含裸 `pytest` 的 key）改为 `python3 -m pytest` 形态。参照同仓既有任务 TAG0027 的 P2-design gate_commands 惯例：
...
本任务测试面 = 交接单 §4 关键验证命令（三片 + 检测引擎验证脚本）：
...
- P5 全量 = 三片合跑（可按需以一条 P5 命令表达或以多独立 key 表达，逐 key 独立不拼接 `&&`）

同时对照 TAG0027 惯例检查 P5_timeout_seconds 档位是否仍合理（P5 全量三片 + `-n auto` 并行，300s 是否够；TAG0027 取 600s 含 `--reruns 1 -n auto`——本任务按实际测试量自定，宁高勿低，TPV0093 教训）。

### 约束（延续上轮，重点重申）

- 本轮**只改 gate_commands 声明**（及必要的 timeout_seconds 档位调整），不得改动方案选型（候选方案 A 不动）、影响面梳理、files_to_read、env_constraints、minimal_validation 等已评审通过内容；若发现必须改这些 → 停下报告主 Agent
- 修正后跑 `python3 agate/scripts/check-frontmatter.py <P2-design.md>` 确认 exit 0
- P2 gate 预跑（主 Agent 执行）确认 T075 WARNING 消失、exit 2 保留（P2 gate 正常形态即 exit 2，主 Agent 自判）
- 在 P2-design.md 末尾追加「## 10. 修复轮记录（fix1）」：记录 gate_commands 修正（裸 pytest → python3 -m pytest）+ 原因（T075 WARNING，环境无裸 pytest 可执行文件）+ timeout 档位调整（如有）
- 若涉及 test-designer 角色（P3）依赖的命令路径，在修复轮记录中注明

### 输入文件（按需）

1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-design.md`（修改对象）
2. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P2-review.md`（approved，非阻塞建议 4 条——本轮不处理，P3/P4 阶段消化）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（gate_commands 惯例参照，451-460 行附近）
4. `AGENTS.md`（项目约定）

### 产出文件字段

沿用上轮 frontmatter（status 保持 draft 或按 check-frontmatter 要求；若 agate-md-field-set 校验报错照提示修正，不手写）。
</dispatch_guide>
...
<objective_info>
### A. 环境事实（2026-09-03 本会话实测）
- `command -v pytest` → exit 1（PATH 中无 pytest 可执行文件）
- `python3 -m pytest --version` → pytest 9.0.3（可用）
- `python3 --version` → Python 3.12.3
- ruff：`~/.venvs/agate-dev/bin/ruff`（0.16.4）

### B. TAG0027 惯例参照（gate_commands 形态）
```
...
### C. 本任务测试面（交接单 §4）
- 三片：`python3 -m pytest agate/tests/unit/ -n auto` / `agate/tests/regression/` / `agate/tests/integration/`
- 检测引擎验证：`python3 docs/design-notes/260903-.../verify-heartbeat-cmdstream/verify_cmdstream_detection.py`（P5_cmdstream_verify 已有）
- consistency：`python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`（P5_consistency 已有，正确形态）
- count-tests：`bash agate/tests/scripts/count-tests.sh`（P5_count_tests 已有）
- shellcheck：3 个 hook 薄壳 `-S warning`（P5_shellcheck 已有）
</objective_info>
