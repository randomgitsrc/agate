...
独立评审 `P1-requirements.md`（TAG0028 subagent 存活可观测性与受控自主再派发，RM-AG0055），
产出 `P1-review.md`。评审重点是 analyst 的作者盲区：BDD 可二值判定性、隐含需求覆盖、
BDD 跨条一致性、裁剪合理性、审声明（risk_level/ceremony/phases vs 实际改动）、P1 纯净性
（不掺方案设计）。**只审不写**——不直接改 P1-requirements.md，评审意见由主 Agent 回派 analyst 修改。

### 约束

1. **评审结论必须引用具体锚点**（requirements-review.md「实质锚点要求」）：approved 须引用
   每条 BDD 编号 + 覆盖维度清单；隐含需求覆盖须列出覆盖条目编号；裁剪合理须逐个跳过阶段 +
   理由；审声明须引 diff 证据。
2. **BDD 二值判定是红线**：每条 BDD 的 Given/When/Then 必须可 PASS/FAIL 二值判定；
   `#### BDD-NN:` 编号连续不跳号；每条 BDD 只有一条 Given-When-Then。
3. **核心约束逐条核对**（对照 dispatch-context-analyst 的约束 12 条，尤其）：
   - 阈值保守（宁可多提示、绝不误杀）——BDD 不得隐含"超时即自动终止"
   - 冻结判据覆盖三类活动（思考/输出/执行 tool），不能只看最后一条命令
   - 适配器模式（新增平台只写适配器、检测引擎零改动）
   - judge 类角色例外（不得开放子派发权限）
   - 不破坏 check-gate.py / check-state-transition.py 返回约定（两套独立信号）
   - DSH zstd 解压不可硬依赖 python zstandard
4. **同类扫描必查**：P1-requirements.md 正文须含同类扫描结论（S-1~S-8 命中清单 + 逐条判定），
   缺失 → 打回（needs-revision）。
5. **P0-brief 时效性质疑必查**：正文须含"已核对 P0-brief 时效性，无漂移"或
   `[P0_STALE: 具体漂移点]` 行——空白不算做过。
6. **审声明 vs diff 证据**：analyst 声明 risk_level=medium / ceremony=standard /
   phases 全量（P1-P8）。核对本任务暂存区实际改动面（协议机制升级：新增脚本 + 协议文档改写，
   跨 4 phase，SELF-GATE 触发）与声明是否匹配。medium + standard + 全量 phases 是合理保守档位；
   若发现声明与实际不符，结论必须为 needs-revision 或 rejected。
7. **P1 纯净性**：P1-requirements.md 不得掺入解决方案设计（如"用哪个函数实现"）；P1 只定义
   "做什么"与"做完什么样算对"。发现混入实现细节 → 打回。
8. **评审产出 Header**：P1-review.md frontmatter 含 phase=P1 / task_id=TAG0028 /
   trace_id=TAG0028-P1-20260903 / agent=requirements-review / status（初始 draft，
   评审完成后改 approved / rejected / needs-revision——gate 脚本读的就是这个字段）。
9. **无 PASS/FAIL 行首预判**：评审正文禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。

### 上游关联

- `P1-requirements.md` 是唯一评审对象（上游输入：P0-brief.md + 设计文档 v5 + 验证记录）
...
1. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P1-requirements.md`（评审对象）
2. `agate-workspace/tasks/TAG0028-subagent-liveness-self-dispatch/P0-brief.md`（范围锁定来源）
3. `docs/design-notes/260903-design-subagent-liveness-and-self-dispatch/design-subagent-liveness-and-self-dispatch-v5-with-cmdstream-validation.md`（核对机制语义，按需对照 §3/§4）
4. `agate/assets/review-roles/requirements-review.md`（角色定义，评审清单权威）
5. `AGENTS.md`（项目约定，worktree 根）

### 产出文件字段

产出 `P1-review.md` 到任务目录，frontmatter 用 `agate-md-field-set` 填写：
phase=P1 / task_id=TAG0028 / trace_id=TAG0028-P1-20260903 / agent=requirements-review /
status=（draft → 评审结论时改 approved|rejected|needs-revision）。
</dispatch_guide>
...
### B. TAG0028 任务本质（供审声明核对）
- 协议机制升级：命令流日志机制（三平台适配器 + 调用/活动冻结 + 无效重复 + 截断排除）+ 心跳文件
  生命周期 + 受控自主再派发（执行角色权限下放、judge 例外）
- 改动面：新增检测/解析脚本 + 改写 dispatch-protocol.md「Subagent 安全 → 存活检查」节 +
  role-system.md 权限边界 + dispatch-context 模板补节 + 心跳生命周期规范 → 触发 SELF-GATE
- 设计文档 v5 已 4 轮独立评审闭环（2026-09-02/03），三平台数据源实机验证已闭环
- 计划 BDD ≥16 条（analyst 产出 25 条）

### C. 阈值与机制锚点（评审时核对 BDD 数值一致性）
- 调用冻结：未结束 call vs expected×2（复用 RM-AG0023 timeout_seconds，下限 30s）/ 兜底
  alert=300s / suspect=900s
- 活动冻结：最后活动事件距今 alert=60s / suspect=300s
- 无效重复：窗口 10 内同 (命令, exit, 输出哈希) ≥5 → SPIN；REPEAT_UNIQUE_MIN=3
...
### D. judge 启用
- `.state.yaml` 已写 `judge.enabled: true`（RM-AG0039），P1 frontmatter 不重复声明
</objective_info>
