...
### 目标
执行 TAG0001 P4 实现的技术验证（P5 模式）：从 P2-design.md 读取 gate_commands.P5 全部命令并执行，产出 P5-test-results/（unit.md + fail-list.txt + 各补充命令结果）。你**执行命令并记录结果**，不修改任何代码。

### 约束
- **只读验证**：不修改 agate/ 任何文件、不修改测试、不修复 bug。发现失败只记录，不回改。
- 本任务是 **agate 协议自身改造**（dogfooding）：验证对象是 worktree `agate/`（已含 TAG0003 工作区架构 + TAG0002 refactor 机制 + TAG0001 技术债闭环）；**禁止触碰 `~/.agate`**（稳定版 v0.40.2 开发工具）。跑测试用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
- **必须执行 P2 gate_commands.P5 全部命令**（不是子集——T060 教训）：
  ```yaml
  gate_commands:
  ...
  ```
- 工作目录 = worktree 根（/home/kity/oclab/agate/.worktrees/agate-dev）。**注意：从根跑全量 bats（`bats agate/tests/...`），不要 cd 进 agate/tests（check-protocol-consistency.py 的 --root 默认当前目录，cwd 错会误报）**。
- **输出要求**：
  - `P5-test-results/unit.md`：全量 bats 结果，标注 failed 计数 + 测试输出签名（`ok N` / `not ok N` 行）
  - `P5-test-results/fail-list.txt`：failed 测试 id 逐行列出（无失败时为空文件）
  - 补充命令结果写入 unit.md 各节：consistency（0 ERROR 达标）/ shellcheck（0 error）/ count-tests（用例数基线）
  - 日志文件末行须有 `EXIT_CODE: <n>` 格式（M1.3a）
- **预存失败处理**：若发现与本次改动无关的预存失败（如 ARCH.4 全量偶发 flaky），在 unit.md 标注"预存失败：X（与本次改动无关）"并记录，不视为本任务失败。
- **自查≠gate**：你执行记录，最终 gate 由主 Agent 亲自跑。不要声称"P5 已过"。
- 禁止行首 `- PASS` / `- FAIL` 格式（provenance 审计拦截）。

### 上游关联
- P4 已 commit：技术债闭环实现（agate-debt-check.py + check-debt.sh + tech-debt-template.md + check-gate.sh P8 debt_check + agate-retreat-to.sh + 12 文档同步 + debt/ 归类修正），review approved。
- P3 测试：agate-debt-check.bats（20 用例）+ check-gate.bats（G8.9/10 新增）。
- 测试基线：全量 bats 676 用例（TAG0003 631 + TAG0002 23 + TAG0001 新增 22）；consistency 0 ERROR；shellcheck 0。

### 输入文件
- docs/tasks/TAG0001-tech-debt-closure/P2-design.md（gate_commands §4——必读，命令权威来源）
- docs/tasks/TAG0001-tech-debt-closure/P0-brief.md（环境约束——必读）
- AGENTS.md（测试约定/脚本约定——必读）
- docs/tasks/TAG0001-tech-debt-closure/P4-review.md（实现评审记录——选读）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=006c3e6=TAG0001-P4 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 版本隔离三条铁律：跑 gate/读卡片用 `~/.agate`（原版规则）；跑测试用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
- 测试框架：bats 1.10.0；helpers：load.bash / fixtures.bash / git-helper.bash。
- 已核实查证：全量 bats 676 用例（从根跑）；count-tests.sh 基线（TAG0001 新增 22 后应增长）。
- 已知 flaky：ARCH.4（agate-archive-stale-outputs.bats）全量偶发失败（sleep 1 时间戳相关），单独跑绿——若全量重现按预存失败处理。
</objective_info>
