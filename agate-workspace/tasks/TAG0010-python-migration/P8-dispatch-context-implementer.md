...
> 你是 TAG0010（agate 产品逻辑 Python 化）的 P8 releaser（implementer P8 模式）。**只做发布准备产出 P8-release.md，不执行 git commit/tag（主 Agent 亲自执行）。**
...
产出 P8-release.md（发布准备交接文件）。TAG0010 已完成 P1-P7（30 个 .sh → py + hook 薄壳化 + 删档 + 文档同步），准备发布 v0.46.0。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/execution-roles/implementer.md`
3. 设计 packages：`agate-workspace/tasks/TAG0010-python-migration/P2-design.md` §3.2（6 包：agate-scripts / agate-hooks / agate-consistency / agate-tests / agate-protocol-docs / agate-ci）
4. 当前 CHANGELOG：`CHANGELOG.md`（[0.45.0] 已发布，[Unreleased] 待填）
5. 当前版本引用：README.md L6 version badge v0.45.0

### 任务

1. **版本决策**：bump_type = minor（v0.45.0 → v0.46.0，新增 py 化能力 + 破坏性变更：30 脚本改名/删档）
2. **CHANGELOG [Unreleased] → v0.46.0 草稿**：写入 P8-release.md 的 CHANGELOG 更新确认节（主 Agent 落地）：
   - 新增（TAG0010 产品逻辑 Python 化）：30 个 sh → py（3 hook 保留薄壳）、agate_common.py 公共库、install-hook.py、pyproject.toml ruff 规则集
   - 破坏性变更：30 脚本改名/删档、pyyaml 强制依赖、hook 薄壳 fail-closed、shellcheck→ruff
   - 用 `git log v0.45.0..HEAD --oneline` 核对无遗漏
3. **债务清单确认**：读 `agate-workspace/debt/tech-debt.md`（若存在），P8-release.md 写 `debt_check:` 字段
4. **临时资源清单**：检查是否有本任务启动的临时服务/进程/数据（本任务无 debug server、无端口占用——确认并写入"无"）
5. 产出 `agate-workspace/tasks/TAG0010-python-migration/P8-release.md`（含 bump_type / debt_check / 版本号确认 / CHANGELOG 更新确认 / 临时资源清单）

### 约束

- 只产出 P8-release.md；不执行 git commit/tag/version bump（主 Agent 亲自执行）
- 不修改 CHANGELOG.md / README.md / 其他文件
- 返回路径 + 摘要
