...
> 你是 TAG0010（agate 产品逻辑 Python 化）的 P5 技术验证 verifier。**只做验证，不修改任何代码/测试文件。**
...
执行 P2-design.md §4 声明的 gate_commands.P5 全量验证命令，产出 P5-test-results/（含 unit.md + fail-list.txt），判定是否通过。

### 背景

TAG0010 已把 agate/scripts/ 下 30 个 .sh 全部 py 化（3 hook 保留 sh 薄壳），P4 全量验证已绿（756 bats）。你独立重跑验证确认。

### 执行的命令（P2 §4 gate_commands，逐一跑）

```bash
# P5（全量 bats，注意这是阶段一口径：sanity+unit+regression+integration）
bats agate/tests/sanity.bats agate/tests/unit/ agate/tests/regression/ agate/tests/integration/ 2>&1 | tail -40

# P5_consistency
python3 agate/scripts/check-protocol-consistency.py --strict 2>&1 | tail -20

# P5_ruff（本机 ruff 在 ~/.venvs/agate-dev/bin/ruff，不在 PATH 时用绝对路径）
~/.venvs/agate-dev/bin/ruff check agate/scripts/ 2>&1 | tail -20

# P5_scan
python3 agate/scripts/check-platform-assumptions.py 2>&1 | tail -20

# P5_ci
python3 agate/scripts/ci-gate-backstop.py 2>&1 | tail -20
```
...
在 `agate-workspace/tasks/TAG0010-python-migration/P5-test-results/` 下：
- `unit.md`：每命令结果（命令 + exit code + 通过/失败 + failed 数），标注失败用例
- `fail-list.txt`：失败测试 id 逐行（无失败则为空文件）
- 若发现预存失败（与 TAG0010 无关的既有失败）：标注"预存失败：X（与本次改动无关）"，并登记 known-failures.md（从 `{agate_root}/assets/templates/known-failures-template.md` 拷贝）

### 判定

- 全部 exit 0 + failed=0 → 通过
- 有失败 → 区分真 bug / 环境问题 / flaky，逐条记录（不修复，只记录）

### 约束

- 只读验证：不修改任何代码/测试/文档文件（P5-test-results/ 与 known-failures.md 除外）
- 不 commit（主 Agent 负责）
- 用 timeout 包装长命令（如 `timeout 900 bats ...`），单条命令超时按失败记录

### 返回给我

- 每条命令的 exit code + 结果（通过/失败）
- failed 数汇总
- P5-test-results/ 路径 + fail-list.txt 内容
- 一句话结论（P5 通过/未通过）
