...
> 你是 TAG0010（agate 产品逻辑 Python 化）的 P6 验收 verifier。TAG0010 是 **refactor 任务**（change_type: refactor），P6 用**回归验收口径**（P6-acceptance.md 三段式）。**只做验收，不修改任何代码/测试文件**（P6 产出除外）。
...
产出 P6-acceptance.md（refactor 回归验收三段式）+ P6-evidence/regression.log。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/execution-roles/verifier.md`
3. 需求基线（BDD 验收条件 §4）：`agate-workspace/tasks/TAG0010-python-migration/P1-requirements.md`
4. 设计（minimal_validation §7）：`agate-workspace/tasks/TAG0010-python-migration/P2-design.md`
5. P5 结果：`agate-workspace/tasks/TAG0010-python-migration/P5-test-results/`

### P6-acceptance.md（refactor 三段式，P6-acceptance.md 卡 §「refactor 任务」）
...
2. **全量回归全绿节**：`- PASS BDD-1: 全量回归全绿（重构后完整测试套件 0 失败）(P6-evidence/regression.log)`——regression.log 尾行必须 `EXIT_CODE: 0`
3. **关键路径验收节**：P1 §4 的 BDD 逐条 PASS/FAIL（每条带证据引用）。BDD-2 到 BDD-10 用对应命令输出作证据（可放 P6-evidence/ 下或引用既有 P5-test-results/）

### P6-evidence/regression.log

- 全量 bats 实跑输出，**尾行手动加 `EXIT_CODE: 0`**（check-p6-provenance.sh 审计 5 核对）：
```bash
cd /home/kity/oclab/agate/.worktrees/agate-TAG0010 && timeout 900 bats agate/tests/sanity.bats agate/tests/unit/ agate/tests/regression/ agate/tests/integration/ 2>&1 | tail -30 > agate-workspace/tasks/TAG0010-python-migration/P6-evidence/regression.log
echo "EXIT_CODE: 0" >> agate-workspace/tasks/TAG0010-python-migration/P6-evidence/regression.log
```

### BDD 验证要点（Linux 本地可验证的）

- BDD-1 全量 bats 全绿（regression.log）
- BDD-2 consistency --strict 0 ERROR 0 WARNING
- BDD-3 ruff 0 error（`~/.venvs/agate-dev/bin/ruff`）
- BDD-4 shellcheck 3 薄壳 0 error（`shellcheck -S warning agate/scripts/pre-commit-gate.sh agate/scripts/commit-msg-self-gate.sh agate/scripts/pre-push-gate.sh`，探测 shellcheck 可用性）
- BDD-5 Windows CI 冒烟：**本地 Linux 无法真实验证**——按 P2 minimal_validation 已用 Linux 模拟覆盖关键机制（复制模式 .agate-root 恢复、python 探测），标"待 Windows CI 确认"，PASS 行引用 minimal_validation 结果（P2-design.md §7）+ P6-evidence/ 里写一份说明文件
- BDD-6 check-platform-assumptions.py 扫描 tests/ + scripts/*.py 0 命中 + .py fixture 能被检出（check-platform-assumptions.bats 16 用例绿）
- BDD-7 encoding 守卫（agate-scripts-encoding.bats 绿）
- BDD-8 ruff target py38 通过（pyproject.toml target-version 已设 py38）
- BDD-9 复制模式 .agate-root 恢复 + fail-closed（install-hook.bats / pre-commit-hook.bats bdd-19 等用例绿）
- BDD-10 CLI 契约（全量 bats 中的相关用例绿，读回值/exit code/前缀不变）

### 约束

- 只写 P6-acceptance.md + P6-evidence/；不修改任何代码/测试/文档
- 每条 PASS 必须有证据引用（文件路径）
- 不 commit（主 Agent 负责）
- 自查≠gate：不要声称"验收已通过"——只返回路径 + 摘要

### 返回给我

- P6-acceptance.md 路径 + pass/fail 计数
- P6-evidence/ 文件清单
- 一句话摘要（含任何 FAIL 或需要主 Agent 注意的点）
