...
issues:
  ...
  - "硬约束：git hook 入口必须保留 sh 薄壳（shebang 软链机制 + Windows 无 env/python3 可靠解析 + 复制模式 .agate-root 恢复）——约 15 行/个，逻辑全移到 py"
  - "阶段一范围：30 个 sh → py（优先 check-gate.sh 488 行 + pre-commit-gate.sh 404 行两个最重）；gate-result.sh + agate-workspace-resolve.sh → agate_common.py；协议文档引用同步；CI 保持 Linux 全量 + Windows 冒烟"
  - "明确不做：测试框架迁移（阶段二另立）；协议文档不做全量重写，但必要引用同步（dispatch/hook/git-integration/platform-notes Windows 章节/UPGRADING）计入阶段一"
