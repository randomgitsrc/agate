...
验收 TAG0014「agate 派发编排机制」：对照 P1-requirements.md 的 22 条 BDD 逐条验收（PASS/FAIL），产出 P6-acceptance.md + P6-evidence/ 证据目录。这是 self-authored gate——你的产出文件是主 Agent 跑 gate 脚本的唯一判定对象，必须基于真实执行证据。

### 约束

- **BDD 全量对照**：P1 有 22 条 BDD（BDD-1~22），P6 必须逐条验收，PASS 或 FAIL 二值，不允许中间态。验收结果行必须用行首 `- PASS` 或 `- FAIL` 格式（check-gate 用 `grep -cE '^\s*- (PASS|FAIL)'` 匹配）。
- **每条 PASS 必须有证据文件引用**：`- PASS BDD-NN: {描述} ({证据路径})`，证据存入 P6-evidence/，相对路径引用。无证据的 PASS = gate 不通过。
- **证据类型**（本任务非 UI，ui_affected: false）：
  - `P6-evidence/test-output.log` — 验证命令执行日志（必须含 EXIT_CODE: 0 尾行）
  - `P6-evidence/{bdd-n}.log` / `{bdd-n}.txt` — 各 BDD 的针对性验证输出（grep 锚点 / 命令输出）
  - 不接受 1 行文本文件充数——每个证据文件含实质内容（实际命令输出 / 断言结果）
- **BDD 验收方法提示**（协议文档类 BDD 用 grep 锚点判定，脚本类 BDD 用命令输出判定）：
  - BDD-1/7（op 契约）：跑 `python3 agate/scripts/agate-md-field-get.py dispatch_plan` 对含 flow YAML 的临时文件，断言输出合法 JSON 含 mode；无字段/坏 YAML → 空输出 exit 0
  - BDD-2~6（P2 gate 校验）：构造临时 P2-design.md（含 dispatch_plan 各种非法/合法值）+ P2-review.md（status: approved），跑 check-gate.py P2，断言 exit code 与 stderr 内容
  - BDD-8~12（权威节）：grep dispatch-protocol.md 五小节锚点（五维评级表 / 五模式 / 模式 4 三步 / 并行规则三要素 / 全阶段适用表）
  - BDD-13~16（阶段卡）：逐卡片 grep 引用锚点（P3-P6 卡含"派发编排机制" + 阶段特定约束关键词；P7 含"模式 1 单发"；P8 含"多包发布"）
  - BDD-17~18（模板）：grep architect.md 批次设计节 + dispatch-prompt.md 粒度兜底 + 协议内联节
  - BDD-19~21（测试/一致性）：pytest 全量（含 10 条新增用例）+ consistency 0 ERROR + count-tests ≥ 基线
  - BDD-22（self-gate）：git log 检查涉及 agate/*.md + scripts/*.py 的 commit message 含 `self-gate-review:` 路径 + protocol-alignment-review 派发记录（P7 将产出审查报告，验收时检查 commit message 标记已存在）
- **环境**：pytest 9.0.3；gate 工具 ~/.agate 稳定版；验证命令在 worktree 根目录跑
- **只读验收**：P6 不修改任何代码/文档。若发现 BDD FAIL，如实记录，由主 Agent 退回上游阶段。
- **输出路径硬约束**：
  - P6-acceptance.md → {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P6-acceptance.md
  - P6-evidence/ → {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P6-evidence/

### 上游关联

- P1-requirements.md：22 条 BDD 基线（[NO_NEED_CONFIRM]）
- P5-test-results/unit.md：技术验证已通过（pytest 780 passed / consistency 0 ERROR / count 782）
- P4-implementation.md：实现记录（含修复轮最终状态）
- P2-design.md：方案设计 + BDD 映射表（§4）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P1-requirements.md（BDD 验收条件——主要输入）
- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P2-design.md（§4 BDD 映射 + §3 方案落点）
- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P5-test-results/unit.md（技术验证结果）
- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P4-implementation.md（实现记录）
- {project_root}/agate/assets/execution-roles/verifier.md（角色定义）
- {project_root}/AGENTS.md（测试约定）
</dispatch_guide>
...
<objective_info>
- 基线：P1 22 条 BDD；P5 已验证 pytest 780 passed / consistency 0 ERROR / count 782
- 本任务非 UI（ui_affected: false），无需 vision-analyst，证据用日志/断言文件
- BDD-22 验收依据：P4 commit（772bbc2）message 已含 self-gate-review: docs/reviews/agate-alignment-review-TAG0014.md 标记；protocol-alignment-review 报告将在 P7 产出
- 环境：pytest 9.0.3；consistency --strict 时 279 WARNING 基线（既有叙事文件引用）会阻断 exit 2——验收 BDD-21 用"0 ERROR"判定，不用 exit code 判定
</objective_info>
