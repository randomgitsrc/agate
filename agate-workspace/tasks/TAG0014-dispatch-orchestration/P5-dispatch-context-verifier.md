...
执行 P2-design.md gate_commands.P5 声明的全部验证命令（P5 主命令 + P5_consistency + P5_count），产出 P5-test-results/（unit.md + fail-list.txt），供主 Agent 验 gate。这是外部产出 gate——你的产出文件是主 Agent 判定通过的唯一依据。

### 约束

- **命令从 P2-design.md 读取，不得自行修改**：
  - P5: `python3 -m pytest agate/tests/ -q --tb=no`（全量测试）
  - P5_consistency: `python3 agate/scripts/check-protocol-consistency.py --strict`（consistency 用 worktree 自己的脚本；注意 --strict 会让 WARNING 也阻断——本任务当前 279 WARNING 均为既有叙事文件引用，若 --strict 报 WARNING 阻断需在产出中区分 ERROR 与 WARNING 报告）
  - P5_count: `bash agate/tests/scripts/count-tests.sh`（用例计数）
- **产出规格**：
  - P5-test-results/unit.md：标注 failed 数量 + test runner 输出签名（`passed`/`failed` 行）
  - P5-test-results/fail-list.txt：failed 测试 id 逐行（`FAILED ` 前缀），可为空文件（无失败时）
- **紧凑输出模式**：命令用 --tb=no 等汇总模式，只保留通过/失败汇总 + 失败清单
- **环境说明**：pytest 9.0.3；worktree 为改造对象；consistency --strict 若因既有 WARNING 阻断，如实记录（当前基线 279 WARNING 0 ERROR）
- **只读验证**：P5 是只读验证，不修改任何代码/文档。若发现 bug，记录到 P5-test-results/ 由主 Agent 回 P4。
- **预存失败**：若发现改动前就存在的失败（非本任务引入），在 unit.md 标注"预存失败：X（与本次改动无关）"，由主 Agent 登记 known-failures.md。
- **输出路径硬约束**：
  - P5-test-results/unit.md → {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P5-test-results/unit.md
  - P5-test-results/fail-list.txt → {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P5-test-results/fail-list.txt

### 上游关联

- P4 实现已 commit：dispatch_plan op + gate 校验 + 文档改造，P4 review approved
- P4 修复轮后状态：全量 780 passed + 2 skipped；consistency 0 ERROR（279 WARNING 既有）；count-tests 782（10 条新增后）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P2-design.md（gate_commands 来源）
- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P4-implementation.md（实现记录，含修复轮最终状态）
- {project_root}/AGENTS.md（测试约定）
- {AGATE_WORKSPACE}/tasks/TAG0014-dispatch-orchestration/P0-brief.md（env_constraints.test_cmd 参考）
</dispatch_guide>
...
<objective_info>
- 环境：pytest 9.0.3；系统 python3 + pyyaml；consistency 脚本在 worktree（agate/scripts/）
- 已知基线：全量 780 passed + 2 skipped（0 failed）；consistency 0 ERROR / 279 WARNING（--strict 时 WARNING 是否阻断以实际输出为准）；count-tests 782（含新增 10+2 条）
- README badge 已还原 v0.48.0（与 tag 同步）；CHANGELOG/UPGRADING 有 [0.49.0] 章节
</objective_info>
