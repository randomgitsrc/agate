...
**复审轮**：上一轮判定 needs-revision（F1/F2/F3），analyst 已修订并新增 TPV0090-M4 三条 BDD（BDD-35/36/37）。重新评审 `P1-requirements.md`，产出修订后的 `P1-review.md`——判定 status: approved / rejected / needs-revision。

### 约束

- **复审重点**：核验 F1/F2/F3 是否已正确修订 + M4 新增 BDD（35/36/37）是否合格 + 无新引入问题。
  - **F1**：BDD-12 是否改为小写 `- fail：3` Given（覆盖 line 69 残留路径）？若仍未覆盖 line 69，判定 needs-revision。
  - **F2**：BDD-11/12/13 是否显式声明 `LC_ALL=C`（POSIX locale）执行前置？
  - **F3**：BDD-9 括号宽度歧义是否已澄清（Given 用 ASCII 括号 + 字符类加宽声明，或全角纳入声明）？BDD-10 是否补了"括号内无文件名仍被拦"边界用例？
  - **M4**：BDD-35/36/37 是否可二值判定、与 BDD-30/31（RM-AG0002）边界互斥且自洽（NameError → B 类 / 真实 bug → A 类 / globals().get() 向后兼容）？
- **只审不写**：不修改 P1-requirements.md。
- **结论必须引用实质锚点**：approved 必须逐条列出 BDD 编号 + 覆盖维度；不引用 BDD 编号的裸 "approved" 是假完成。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`（被 provenance 预判检测匹配）。改用"通过/失败"或加引号。

### 上游关联

- 上轮评审判定 needs-revision（F1：BDD-12 Given 大写无区分力；F2：BDD-11/12/13 缺 POSIX locale 前置；F3：BDD-9 括号宽度歧义）。
- analyst 修订轮已处理：BDD 总数 34 → 37（新增 TPV0090-M4 三条），编号连续。
- 用户 2026-08-13 明确加入 TPV0090 复盘归入项（M4）到任务范围（P0-brief 已更新）。

### 输入文件

- `agate-workspace/tasks/TAG0004-env-adaptation/P1-requirements.md`（复审对象，修订版）
- `agate-workspace/tasks/TAG0004-env-adaptation/P1-review.md`（上轮评审结论——本轮重写覆盖）
- `agate-workspace/tasks/TAG0004-env-adaptation/P0-brief.md`（缺陷清单与风险声明，已含 M4）
- `HANDOFF-TAG0004.md`（worktree 根：任务范围、TPV0090 归入段）
- 按需：`agate/scripts/check-tdd-red.sh`（M4 代码位置 L70/87-102/104-107 核验）
</dispatch_guide>
...
- 关键路径：复审对象 `agate-workspace/tasks/TAG0004-env-adaptation/P1-requirements.md`（37 BDD）；产出 `P1-review.md`
- 查证结果：修订版 BDD-9 已改 ASCII 括号、BDD-12 已改小写、BDD-11/12/13 已加 LC_ALL=C；新增 BDD-35/36/37
</objective_info>
