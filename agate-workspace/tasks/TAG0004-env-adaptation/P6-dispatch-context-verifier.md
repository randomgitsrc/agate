...
产出 `P6-acceptance.md` + `P6-evidence/`——TAG0004（agate 脚本健壮性 + 环境适配）的 P6 验收。逐条对照 P1 的 37 条 BDD（BDD-1..37）判定 PASS/FAIL，每条 PASS 带证据文件引用（存 P6-evidence/）。

### 约束

- **在 worktree 验证**：`/home/kity/oclab/agate/.worktrees/agate-TAG0004`（workdir）。**禁止改主 checkout `/home/kity/oclab/agate` 和 `~/.agate`**。consistency 检查**必须用 worktree 自己的脚本**。
- **BDD 二值规则**：每条 BDD 结果只允许 PASS 或 FAIL，不允许"调整/跳过/覆盖"等中间态。任何 BDD 标 FAIL → gate 不通过。
- **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`（行首 `- PASS` 或 `- FAIL`，证据路径相对 P6-evidence/ 目录）。不要用表格/emoji/其他格式。
- **覆盖完整性**：P1 有 37 条 BDD → P6 必须有 37 条验收结果（PASS 或 FAIL），全量对照，不能挑验。
- **证据要求**：每条 PASS 必须有对应证据文件，存入 `P6-evidence/`。证据类型：
  - 脚本执行日志/断言输出（本任务无 UI，主要用此类型）——如 `p6-bdd-1.log`、`assert-*.txt`
  - 每个证据文件必须有实质内容（非 1 行 txt 充数）——本任务是脚本修复，证据应为**实际命令执行输出**（bats 单测结果、grep 断言、脚本运行日志）
  - 无证据的 PASS 标记 = gate 不通过
- **证据日志格式约定**：日志文件末行必须是可解析的退出码声明 `EXIT_CODE: <n>`（n 为整数，0 表示成功）。
- **本任务特殊验证方式**（Windows 类 BDD 无法本地实测，P1 capability_requirements windows-runtime=supplementable）：
  - **Linux 可实测的 BDD**（BDD-1..32、34）：实际执行验证，证据为命令输出/日志
  - **Windows 特有 BDD**（BDD-21 盘符/反斜杠、BDD-33 CI windows matrix）：用 Linux fixture 模拟（如 Windows 风格路径参数传给脚本）或静态断言（CI 配置含 windows-latest），证据为模拟/断言输出。**不得声明"已实测 Windows"**——BDD-33 的 CI 通过与否依赖外部 CI 结果，验收以"protocol-tests.yml 含 windows-latest matrix"为证据（P8/PR 阶段回看 CI 实际结果）
- **具体验收命令参考**（P5 gate_commands.P5 已全绿，P6 是 BDD 级验收）：
  - 逐 BDD 跑对应验证：如 BDD-1 用 `bats agate/tests/integration/pre-commit-hook.bats` 空格路径用例、BDD-5 用 `bats agate/tests/unit/agate-scripts-encoding.bats`、BDD-35 用 `bats agate/tests/unit/check-tdd-red.bats` 等（测试已在 P3 写好，P4 实现后全绿——P6 证据 = 这些测试的实跑输出）
  - 全局类 BDD（BDD-32 全量 bats、BDD-34 shellcheck）：复用 P5-test-results/ 或重跑
- **frontmatter 机器汇总**：`pass:` / `fail:` / `ui_affected: false` 写在文件头（pass 应为 37，fail 应为 0）。
- **PROD_TOUCHED 标记**：未触达生产环境写 `[PROD_NOT_TOUCHED]`。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL` 用于总结行。改用"通过/失败"或加引号。

### 上游关联

- P1-requirements.md：37 条 BDD（S1/S2/S3/M4/M5/M6/M9 + Q1/Q2/Q5 + RM-AG0001/AG0002 + TPV0090-M4 + 全局）。
- P5-test-results/：全量 714 bats 绿 + consistency 0 ERROR + shellcheck 0（failed=0）。
- P4 实现已 commit（5a60ca1）：37 BDD 全部修复。

### 输入文件

- `agate-workspace/tasks/TAG0004-env-adaptation/P1-requirements.md`（37 BDD + capability_requirements）
- `agate-workspace/tasks/TAG0004-env-adaptation/P3-test-cases.md`（测试契约，验证命令参考）
- `agate-workspace/tasks/TAG0004-env-adaptation/P5-test-results/`（P5 验证结果）
- `HANDOFF-TAG0004.md`（验收约束：不得宣称已实测 Windows）
- `AGENTS.md`（测试约定）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0004`；协议 v0.43.0；P5 已 commit（a4381d3）
- 关键路径：产出 `agate-workspace/tasks/TAG0004-env-adaptation/P6-acceptance.md` + `P6-evidence/`
- 查证结果：P5 全绿（714 bats + consistency 0 + shellcheck 0）
</objective_info>
