...
执行 `gate_commands.P5`（P2-design.md 声明）全部命令，产出 `P5-test-results/`（unit.md + fail-list.txt）——验证 TAG0004 P4 实现的正确性（37 BDD 修复 + Linux 基线不回归）。

### 约束

- **在 worktree 跑命令**：`/home/kity/oclab/agate/.worktrees/agate-TAG0004`（workdir）。**禁止改主 checkout `/home/kity/oclab/agate` 和 `~/.agate`**。consistency 检查**必须用 worktree 自己的脚本**（检查对象是 worktree 里的协议文件）。
- **gate_commands.P5（从 P2-design.md 读取，逐条执行）**：
  ```
  bats agate/tests/sanity.bats agate/tests/unit/ agate/tests/regression/ agate/tests/integration/ && python3 agate/scripts/check-protocol-consistency.py --strict && shellcheck -S warning agate/scripts/*.sh
  ```
  - **注意命令较长，分步执行**（每步短、大 timeout）：先 bats 全量、再 consistency、再 shellcheck。
  - bats 输出用紧凑模式（tail 汇总即可，不逐条 traceback）。
- **产出**：
  - `P5-test-results/unit.md`：标注 failed 数量（0 = 全绿），含测试 runner 输出签名（行首 `ok`/`not ok`/`passed`/`failed` 计数）
  - `P5-test-results/fail-list.txt`：无失败时为空文件（touch 创建）
  - 若发现预存失败（改动前就存在）：登记到 `known-failures.md`（模板 `~/.agate/assets/templates/known-failures-template.md`），并在 unit.md 标注"预存失败：X（与本次改动无关）"
- **全量测试**：必须跑全量 bats（sanity + unit + regression + integration）——P5 判定含非本任务测试回归。
- **PROD_TOUCHED 标记**：未触达生产环境写 `[PROD_NOT_TOUCHED]`。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`。改用"通过/失败"或加引号。

### 上游关联

- P4 实现已 commit（5a60ca1）：37 BDD 全部修复 + review+cso approved。自查（P4 阶段）已全绿，P5 是独立验证。
- P3 测试 37 条（bdd-1..37）+ 既有 676 基线。
- P2-design.md gate_commands.P5 已固化（P4-P6 不得修改）。

### 输入文件

- `agate-workspace/tasks/TAG0004-env-adaptation/P2-design.md`（gate_commands.P5）
- `agate-workspace/tasks/TAG0004-env-adaptation/P3-test-cases.md`（测试契约）
- `HANDOFF-TAG0004.md`（验证命令参考）
- `AGENTS.md`（测试约定）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0004`；协议 v0.43.0；P4 已 commit
- 关键路径：产出 `agate-workspace/tasks/TAG0004-env-adaptation/P5-test-results/`（unit.md + fail-list.txt）
- 查证结果：P4 自查全绿（bats 691 + consistency 0 ERROR + shellcheck 0）
</objective_info>
