...
> 本文件是**批次 4**（TAG0011 上下文/归档/回退工具批）。**只做本批 4 个 pytest 文件。可以自跑 pytest 自查，不跑全量、不跑 bats、不 commit。**
...
把 4 个"上下文/归档/回退工具" bats 文件迁移为 pytest。
...
1. 本派发指引 + AGATE_CARD
2. P3-test-cases.md 批次 4 表
3. P2-design.md §3.2 语义映射表
4. 4 个 bats 源（全文读）+ conftest.py（task_dir / git_repo / run_cli）

### 关键约束

- **ARCH.4 flaky（时间戳）**：归档两次用时间戳区分——pytest 迁移保留隔离语义（可用 `time.sleep` 或更精确的时间戳区分，确保单跑必过）
- git_repo fixture 用于 migrate-workspace / retreat-to
- 合并流 `.output`；行为等价；不删 bats；只做本批

### 验证（自跑 pytest 自查）

```bash
cd /home/kity/oclab/agate/.worktrees/agate-TAG0010 && python3 -m pytest agate/tests/unit/test_agate_extract_context.py agate/tests/unit/test_agate_archive_stale_outputs.py agate/tests/unit/test_agate_migrate_workspace.py agate/tests/unit/test_agate_retreat_to.py -q
```
