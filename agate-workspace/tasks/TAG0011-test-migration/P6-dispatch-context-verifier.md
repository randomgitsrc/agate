...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P6 验收 verifier。TAG0011 是 **refactor 任务**（change_type: refactor），P6 用**回归验收口径**（三段式）。**只做验收，不修改任何代码/测试文件**（P6 产出除外）。
...
3. 需求 BDD（§4，12 条）：`agate-workspace/tasks/TAG0011-test-migration/P1-requirements.md`
4. P5 结果：`agate-workspace/tasks/TAG0011-test-migration/P5-test-results/`

### P6-acceptance.md（refactor 三段式）
...
**三段式正文**：
1. 行为不变声明节
2. 全量回归全绿节：`- PASS BDD-1: 全量回归全绿（pytest 750 收集 0 失败）(P6-evidence/regression.log)`——regression.log 尾行 `EXIT_CODE: 0`
3. 关键路径验收节：P1 §4 的 BDD-2 到 BDD-12 逐条 PASS/FAIL（证据引用）

### P6-evidence/regression.log

```bash
cd /home/kity/oclab/agate/.worktrees/agate-TAG0010 && timeout 600 python3 -m pytest agate/tests/ -q --tb=no 2>&1 | tail -15 > agate-workspace/tasks/TAG0011-test-migration/P6-evidence/regression.log
echo "EXIT_CODE: 0" >> agate-workspace/tasks/TAG0011-test-migration/P6-evidence/regression.log
```

### BDD 验证要点（Linux 本地可验证）

- BDD-1 全量 pytest 全绿（regression.log + collect-only ≥ 749）
- BDD-2 consistency --strict 0 ERROR 0 WARNING
- BDD-3 ruff 0 error（~/.venvs/agate-dev/bin/ruff check agate/）
- BDD-4 count-tests.sh 改写 pytest 收集（750）
- BDD-5 Windows CI 冒烟：本地无法真机验证——P1 minimal_validation + marker 机制（78 用例），标"待 Windows CI 确认"
- BDD-6 helpers fixture 等价（conftest 体系——test_sanity.py 等绿）
- BDD-7 encoding 守卫（test_agate_scripts_encoding.py 绿）
- BDD-8 py38 兼容（ruff target py38）
- BDD-9 CLI 契约（相关 test_*.py 绿）
- BDD-10 pytest 全平台可跑（Windows marker 78 用例收集）
- BDD-11 文档/CI 同步（test_env_adapt_docs.py 绿）
- BDD-12 冒烟机制落地（marker 78 用例收集，check-windows-smoke 退役）

### 约束

- 只写 P6-acceptance.md + P6-evidence/；不修改代码/测试/文档；不 commit
- 每条 PASS 有证据引用；自查≠gate

### 返回给我

- P6-acceptance.md 路径 + pass/fail 计数
- P6-evidence/ 文件清单
- 一句话摘要（含任何 FAIL 或需注意点）
