...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P2 方案工程评审。**只评审，不修改任何文件。**
...
评审 P2-design.md，产出 P2-review.md（status: approved / rejected）。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/review-roles/plan-eng-review.md`
3. 评审对象：`agate-workspace/tasks/TAG0011-test-migration/P2-design.md`
4. 需求基线：`agate-workspace/tasks/TAG0011-test-migration/P1-requirements.md`

### 评审关注点（工程维度）

1. **候选方案权衡**：9 候选（4 组决策点）是否充分权衡；选择理由是否成立
2. **批次迁移设计可行性**：17 批 + 子批是否足够小、每批验证命令是否明确可执行（用户明确"任务过重→卡死"，批次粒度是核心）
3. **bats → pytest 语义映射**：setup/teardown/run/$output/@test → fixture/assert/mark 的映射是否完整正确
4. **fixture 策略**：conftest.py 组织是否合理（526 行 helpers 迁移）
5. **gate_commands 可执行性**：P3/P5 用 pytest 命令是否可行（pytest 9.0.3 环境）
6. **Windows 冒烟 marker 方案**：@pytest.mark.windows_smoke 选取逻辑是否正确
7. **files_to_read 精度**：是否控制好 P4 implementer 上下文（防爆炸）
8. **风险缓解**：subagent 卡死风险 + 回归风险是否覆盖

### 产出

`agate-workspace/tasks/TAG0011-test-migration/P2-review.md`：
- frontmatter：phase: P2, task_id, agent（非 main）, status
- 结论 approved / rejected + BLOCKER 列表 + 意见

### 约束

- 只评审不改；不 commit；诚实评审（发现问题写 rejected）

### 返回给我

- P2-review.md 路径 + status
- BLOCKER 数 + 意见摘要
