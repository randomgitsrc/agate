...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P5 技术验证 verifier。**只做验证，不修改任何代码/测试文件。**
...
执行 P2-design.md §4 声明的 gate_commands.P5 全量验证命令，产出 P5-test-results/（unit.md + fail-list.txt）。

### 背景

TAG0011 已把 60 个 bats（749 用例）迁移为 pytest（pytest 750 collected），bats 全部退役删除。P4 已验证 pytest 全绿 + consistency + ruff。

### 执行的命令（P2 §4 gate_commands，逐一跑，工作目录 worktree 根）

```bash
# P5 主命令（pytest 全量）
timeout 600 python3 -m pytest agate/tests/ -q --tb=no 2>&1 | tail -20

# P5_consistency
timeout 120 python3 agate/scripts/check-protocol-consistency.py --strict 2>&1 | tail -10

# P5_ruff（本机 ruff 在 ~/.venvs/agate-dev/bin/ruff）
timeout 120 ~/.venvs/agate-dev/bin/ruff check agate/ 2>&1 | tail -5

# P5_scan
timeout 120 python3 agate/scripts/check-platform-assumptions.py 2>&1 | tail -5

# P5_ci
timeout 120 python3 agate/scripts/ci-gate-backstop.py 2>&1 | tail -10
```

### 产出

在 `agate-workspace/tasks/TAG0011-test-migration/P5-test-results/` 下：
- `unit.md`：每命令结果（命令 + exit code + 通过/失败 + failed 数）
- `fail-list.txt`：失败测试 id 逐行（无失败则空）
- 预存失败登记 known-failures.md（如与 TAG0011 无关的既有失败）

### 判定

- 全部 exit 0 + failed=0 → 通过
- 有失败 → 区分真 bug / 环境 / flaky，逐条记录

### 约束

- 只读验证；不修改任何代码/测试/文档（P5-test-results/ 除外）；不 commit
- 单条超时按失败记录
