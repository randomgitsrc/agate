...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P1 需求评审。**只评审，不修改任何文件。**
...
评审 P1-requirements.md，产出 P1-review.md（status: approved / rejected）。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/review-roles/requirements-review.md`
3. 评审对象：`agate-workspace/tasks/TAG0011-test-migration/P1-requirements.md`
4. 上游：P0-brief.md（同目录）

### 评审关注点

1. **BDD 验收条件**：≥1 条、Given/When/Then 格式、可二值判定（PASS/FAIL）、编号连续（#### BDD-NN:）
2. **frontmatter 完整性**：risk_level / phases / packages / domains / change_type 声明合理
3. **批次规划可执行性**：18 批是否足够小（用户明确"任务过重→卡死"，批次粒度必须 1 轮可完成）；每批验证口径是否明确
4. **范围合理性**：58/61 个 bats 差异是否说清；helpers 迁移方案是否可行；文档/CI 清单是否完整
5. **capability_requirements**：三态（available/supplementable/GAP）是否合理，GAP 是否阻断
6. **无未决 [NEED_CONFIRM]**：确认 0 个或已处理
7. **refactor 口径**：change_type: refactor 是否合理（测试框架迁移 = 行为不变）

### 产出

`agate-workspace/tasks/TAG0011-test-migration/P1-review.md`：
- frontmatter：phase: P1, task_id, agent（非 main）, status
- 结论 approved / rejected + 理由
- BLOCKER 列表（rejected 必填）+ 非阻塞意见

### 约束

- 只评审不改；不 commit；不声称"P1 已过"
- 诚实评审：发现问题写 rejected，不为通过而通过

### 返回给我

- P1-review.md 路径 + status
- BLOCKER 数 + 意见摘要
