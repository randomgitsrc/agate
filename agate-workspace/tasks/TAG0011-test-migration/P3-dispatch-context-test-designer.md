...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P3 测试设计师。**只产出 P3-test-cases.md（回归口径声明 + 用例覆盖映射），不写实现代码。**
...
产出 P3-test-cases.md。TAG0011 是 refactor 任务（change_type: refactor），P3 用**回归测试口径**（P3 卡 §refactor 任务）：复用/保留既有测试用例，标注每条回归用例覆盖的迁移路径；不新增功能行为断言。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/execution-roles/test-designer.md`
3. P1 需求基线：`agate-workspace/tasks/TAG0011-test-migration/P1-requirements.md`（12 BDD + 17 批规划）
4. P2 设计：`agate-workspace/tasks/TAG0011-test-migration/P2-design.md`（bats→pytest 语义映射表 + 批次迁移设计 + files_to_read）
5. 既有 bats 测试结构（自己看）：`agate/tests/` 下目录结构

### 产出要求

**P3-test-cases.md 必须包含**：
1. **回归口径声明**：本次重构（bats → pytest）不改外部行为，测试设计 = 复用既有 749 用例 + 标注迁移覆盖；不新增功能行为断言
2. **test_code_dir 声明**：`test_code_dir: agate/tests/`（pytest 同目录替换 test_*.py）
3. **BDD → 测试映射**：12 条 BDD 各对应哪些 pytest 用例/迁移批次（1:1 或 1:N 明确）
4. **用例覆盖映射**：60 个 bats 文件（749 @test）→ pytest 文件（test_*.py）的映射表（按 P2 的 17 批 + 子批），标注每条回归用例覆盖的迁移路径（哪个 bats → 哪个 pytest）
5. **pytest 结构设计**：conftest.py fixture 清单（从 helpers 迁移）、marker（windows_smoke）、run_cli 工具（P2 §3.1 的 CommandResult 合并流）
6. **每批验证命令**：17 批各批的 pytest 运行命令（`python3 -m pytest agate/tests/... -k ...` 或文件路径）

### 约束

- 只产出 P3-test-cases.md；不写 pytest 实现代码（P4 分批写）
- 不 commit；不声称"P3 已过"
- refactor 口径：不新增功能行为断言

### 返回给我

- P3-test-cases.md 路径
- BDD 映射摘要
- 用例覆盖映射规模（60 bats → 多少 pytest 文件）
- 一句话摘要
