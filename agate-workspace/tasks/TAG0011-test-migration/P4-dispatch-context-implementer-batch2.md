...
> 本文件是**批次 2**（TAG0011 共享工具批）。**只做本批 6 个 pytest 文件。可以自跑 pytest 自查，不跑全量、不跑 bats、不 commit。**
...
1. 本派发指引 + AGATE_CARD
2. P3-test-cases.md 批次 2 表（合并流分布：JGET.7、STGET 2 处、RETREAT-STATE 1、SYAML 1、R5P 2）
3. P2-design.md §3.2 语义映射表
4. 6 个 bats 源（全文读）+ conftest.py（run_cli / task_dir / load_fixture 复用）

### 关键约束（合并流语义）

- 空/非空断言用 `result.output`（stdout+stderr 合并流）
- stdin 管道用例用 `run_cli(input=...)`
- env 传参（PROJECT_MODULE 等）用 run_cli 的 env 参数
- 行为等价；不删 bats；只做本批

### 验证（自跑 pytest 自查）

```bash
cd /home/kity/oclab/agate/.worktrees/agate-TAG0010 && python3 -m pytest agate/tests/unit/test_agate_json_get.py agate/tests/unit/test_agate_md_field_get.py agate/tests/unit/test_agate_state_get.py agate/tests/unit/test_agate_retreat_state.py agate/tests/unit/test_agate_state_yaml_check.py agate/tests/unit/test_agate_read_p5_commands.py -q
```
