...
> 本文件是**批次 7**（TAG0011 check 基础 gate 批）。**只做本批 5 个 pytest 文件。可以自跑 pytest 自查，不跑全量、不跑 bats、不 commit。**
...
把 5 个"check 基础 gate" bats 文件迁移为 pytest。

### 范围（只做这 5 个 pytest 文件）
...
1. 本派发指引 + AGATE_CARD
2. P3-test-cases.md 批次 7 表（合并流分布：frontmatter 2、retrospective 3）
3. P2-design.md §3.2 语义映射表
4. 5 个 bats 源（全文读）+ conftest.py（task_dir / run_cli）

### 关键约束

- 合并流 `.output`（5 处 [ -z "$output" ]）
- GATE 前缀断言（stderr）→ 合并流
- 行为等价；不删 bats；只做本批

### 验证（自跑 pytest 自查）

```bash
cd /home/kity/oclab/agate/.worktrees/agate-TAG0010 && python3 -m pytest agate/tests/unit/test_check_changelog.py agate/tests/unit/test_check_frontmatter.py agate/tests/unit/test_check_state_yaml.py agate/tests/unit/test_check_retrospective.py agate/tests/unit/test_check_p6_format.py -q
```
