...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P8 releaser（implementer P8 模式）。**只做发布准备产出 P8-release.md，不执行 git commit/tag。**
...
产出 P8-release.md（发布准备交接文件）。TAG0011 已完成 P1-P7（60 bats → pytest + 退役删档 + 文档重写），准备发布 v0.47.0。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/execution-roles/implementer.md`
3. 设计 packages：`agate-workspace/tasks/TAG0011-test-migration/P2-design.md`（5 包）
4. 当前 CHANGELOG：`CHANGELOG.md`（[0.46.0] 已发布，本任务版本 v0.47.0）
5. 版本引用：README.md badge（v0.46.0 → v0.47.0）

### 任务

1. **版本决策**：bump_type = minor（v0.46.0 → v0.47.0）
2. **CHANGELOG 草稿**：写入 P8-release.md（主 Agent 落地）：
   - 破坏性变更：测试运行器 bats → pytest（60 文件迁移）、check-windows-smoke 退役（marker 承接）、count-tests.sh 改写、helpers/ 退役、bats 全部删档
   - 新增：conftest.py fixture 体系、windows_smoke marker（78 用例）、pyproject pytest 配置
   - 用 `git log v0.46.0..HEAD --oneline --grep="TAG0011"` 核对无遗漏
3. **债务清单确认**：读 `agate-workspace/debt/tech-debt.md`（若存在），写 `debt_check:` 字段
4. **临时资源清单**：确认无临时服务/进程（本任务无）
5. 产出 `agate-workspace/tasks/TAG0011-test-migration/P8-release.md`

### 约束

- 只产出 P8-release.md；不执行 git commit/tag/bump；不修改 CHANGELOG/README
