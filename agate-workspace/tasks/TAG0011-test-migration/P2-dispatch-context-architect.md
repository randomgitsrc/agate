...
> 你是 TAG0011（agate 测试框架迁移阶段二）的 P2 方案设计师。**只产出 P2-design.md，不修改代码/测试。**
...
产出 P2-design.md（方案设计）。TAG0011 是大型测试框架迁移：60 个 .bats（749 @test）→ pytest + helpers 迁移 + 文档重写 + CI 同步。

### 必读

1. 本派发指引（全文）
2. 角色定义：`agate/assets/execution-roles/architect.md`
3. P1 需求基线：`agate-workspace/tasks/TAG0011-test-migration/P1-requirements.md`（17 批规划 + 12 BDD）
4. P0-brief：同目录 P0-brief.md
5. TAG0010 的 P2 参照：`agate-workspace/tasks/TAG0010-python-migration/P2-design.md`（了解仓库设计文档结构）

### 产出要求（P2 卡 §产出规格）

**frontmatter**：candidate_count / packages / domains / ui_affected

**正文必须包含**：
1. **候选方案 ≥2** + 权衡 + 选择理由。核心设计决策点：
   - pytest 目录结构（同目录替换 test_*.py vs 独立 tests_py/ 目录 vs 双轨并行）
   - fixture 迁移策略（conftest.py 组织 vs 每模块独立 fixture）
   - bats 语义映射表（setup/teardown/run/$output/@test 名 → pytest fixture/assert/mark）
   - Windows 冒烟 marker 方案（@pytest.mark.windows_smoke 选取逻辑）
   - count-tests.sh 改写 vs 退役
2. **四字段**：packages / domains / ui_affected / gate_commands（P3/P5 用 pytest 命令，P5_consistency / P5_ruff / P5_scan / P5_ci）
3. **files_to_read**：P4 implementer 需要的文件清单（控制上下文——TAG0010 的教训：subagent 上下文爆炸会卡死，files_to_read 必须精）
4. **env_constraints**：确认/细化（pytest 版本、Windows CI、依赖）
5. **minimal_validation**：关键假设验证（pytest 收集数、marker 选取、fixture 可运行性——跑最小 pytest 样例验证）
6. **批次迁移设计**：对 P1 的 17 批逐批给出迁移要点（每批的 pytest 文件组织、fixture 依赖、验证命令）——**批次粒度必须小**（用户明确"任务过重→subagent 卡死"，P4 每批要 1 轮可完成）
7. **风险与缓解**：重点列 subagent 卡死风险（TAG0010 实战教训）+ 回归风险

### 约束

- 只产出 P2-design.md；不修改其他文件；不 commit
- 参考 TAG0010 P2 的格式
- 自查≠gate：不声称"P2 已过"
