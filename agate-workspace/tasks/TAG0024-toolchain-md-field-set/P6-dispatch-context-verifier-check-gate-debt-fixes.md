...
### 目标
P6 证据并行批次之一：为 BDD-20~24（DEBT0019/20）+ BDD-30（`[SCOPE+ from P4]`，check-pruning.py 隔离修复）逐条产出验收证据。本批次**不写最终 P6-acceptance.md**，只写 `P6-evidence/check-gate-debt-fixes/results.md` + 证据文件。

### 约束
- 同其他批次：无 UI，证据为断言输出/日志文件；验收报告记录验收时实际执行的事实
- **证据来源**：
  - BDD-20~24：重跑 `agate/tests/unit/test_check_gate.py` 对应测试函数（`test_bdd_20_*`~`test_bdd_24_*`），保存单独输出
  - BDD-30：重跑 `agate/tests/unit/test_check_pruning.py` 的 `test_p2_6f_staged_source_count_uses_task_repo_not_outer_cwd_repo_exit_0`（以及验证第 2 轮修复的 3 个既有用例 `test_p2_6e_*`/`test_p2_52_*`/`test_p2_52b_*` 在 `--basetemp=.pytest-tmp` 下确实转绿）
- 每条 BDD 只允许 PASS/FAIL，PASS 必须有证据路径引用
- 总结行不用 `- PASS`/`- FAIL` 开头

### 上游关联
- P1-requirements.md BDD-20~24、BDD-30（`[SCOPE+ from P4]` 增补条目）
- P3-test-cases-check-gate-debt-fixes.md：BDD→测试函数映射
- P4-implementation-check-pruning-isolation-fix.md：BDD-30 两轮修复的完整说明（cwd=task_dir 根因修复 + GIT_CEILING_DIRECTORIES 兼容本仓库 basetemp 场景）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（BDD-20~24, BDD-30）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P3-test-cases-check-gate-debt-fixes.md
- agate/tests/unit/test_check_gate.py（BDD-20~24 测试代码）
- agate/tests/unit/test_check_pruning.py（BDD-30 测试代码，含新增回归测试）
</dispatch_guide>
...
<objective_info>
- 环境状态：P5 阶段已确认 `test_check_gate.py` 182 passed/0 failed；`test_check_pruning.py` 30 passed/0 failed（含 `--basetemp=.pytest-tmp` 场景）
- 查证结果：测试运行需 `--basetemp=.pytest-tmp -p no:cacheprovider`
</objective_info>
