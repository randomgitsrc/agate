...
### 目标
执行 P2-design.md 固化的 `gate_commands.P5` 全部 key，产出 P5-test-results/unit.md + fail-list.txt，供主 Agent 验 external-output-gate。

### 约束
- **必须逐 key 独立执行**（P2 已声明为独立 key，禁止用 `&&` 拼接跳过）：`P5`（全量 pytest）、`P5_consistency`（协议一致性）、`P5_shellcheck`（shell lint）、`P5_count`（测试计数自检）、`P5_ruff`（Python lint）
- **测试命令固定**：`python3 -m pytest agate/tests/ --basetemp=.pytest-tmp -p no:cacheprovider -q --tb=no`（P0-brief 已声明 `/tmp` 只读，必须用此 basetemp 参数；不要用 `--tb=no` 之外的详细模式，紧凑输出）
- **P3_timeout_seconds`/`P5_timeout_seconds` 均为 250**（P2 已声明），执行命令前按此设置 shell 层超时（`timeout 250s <命令>`，全量测试类命令建议留够冗余用 `timeout 300s`）
- **本任务已在 P4 阶段发生过 SCOPE+ 增补**（BDD-30，check-pruning.py 测试隔离修复）——你验证时看到的 29+1=30 条 BDD 均为当前有效基线，不是遗漏
- **不要只跑修复过的部分**：P4 阶段已产出多批代码改动（RM-AG0048/DEBT0019/DEBT0020/RM-AG0049/RM-AG0050 + BDD-30），P5 必须跑全量套件，不能只验证某一批
- 这是 external-output-gate：你的产出必须是可核验的原始命令输出摘要（含 test runner 输出签名，如 `passed`/`failed` 字样），不是你自己的文字总结替代实际执行

### 上游关联
- P4-implementation.md：三批次 + SELF-GATE 修复的完整清单，主 Agent 已在 P4 阶段多次独立执行过全量验证（1285 passed/0 failed，ruff/shellcheck/consistency 均 0 问题）——但那是主 Agent 自己跑的（self-authored 性质），P5 要求由独立 verifier subagent 重新执行产出可核验记录，不能直接照抄 P4 的数字
- P2-design.md §7：gate_commands 完整声明

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-design.md（§7 gate_commands 声明，逐字对照执行）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P0-brief.md（env_constraints，/tmp 只读等环境约束）
</dispatch_guide>
...
<objective_info>
- 环境状态：主 Agent 在 P4 阶段最近一次独立执行（commit e2357fc 之前）确认：全仓 1285 passed/2 skipped/0 failed；ruff 全仓 0 errors；shellcheck 0；consistency --strict-errors-only exit 0；count-tests.sh 显示 1287 个测试用例——你的独立重跑预期应得到一致或非常接近的结果（commit 后代码未再变动）
</objective_info>
