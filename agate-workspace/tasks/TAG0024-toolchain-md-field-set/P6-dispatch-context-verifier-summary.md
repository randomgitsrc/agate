...
### 目标
汇总 verifier：整合三个证据并行批次（`md-field-set-tool`/`check-gate-debt-fixes`/`phases-yaml-consistency`）的 `results.md`，产出唯一的 `P6-acceptance.md`。**不重新执行测试**，只做转抄整合 + 交叉核对（主 Agent 已对三批次逐条独立复核，本步骤是格式整合，非重新验证）。

### 约束
- **不发表新意见，只汇总转抄**：三批次的 PASS/FAIL 结论已经过独立验证（主 Agent 已逐条抽查确认），你只需把 30 条 BDD 结果转抄进统一格式，证据路径改为相对 `P6-evidence/` 的完整路径（如 `md-field-set-tool/bdd-1.log`）
- **交叉核对（必须做，写入 P6-acceptance.md）**：确认三批次 BDD 编号合集恰好等于 P1-requirements.md 全部 30 条 BDD（BDD-1~30），无重复、无遗漏
- **frontmatter 必须含**：`pass: 30`、`fail: 0`、`ui_affected: false`
- **PASS 行格式**：`- PASS BDD-N: {描述} ({证据路径})`，证据路径必须是相对 P6-acceptance.md 所在目录的路径（即 `P6-evidence/{batch}/{文件名}`）
- **总结行不用 `- PASS`/`- FAIL` 开头**
- 本任务无 UI（`ui_affected: false`），不涉及 vision/截图相关规范

### 上游关联
- `P6-evidence/md-field-set-tool/results.md`：BDD-1~19，19 PASS / 0 FAIL
- `P6-evidence/check-gate-debt-fixes/results.md`：BDD-20~24, BDD-30，6 PASS / 0 FAIL
- `P6-evidence/phases-yaml-consistency/results.md`：BDD-25~29，5 PASS / 0 FAIL
- 三批次合计：30 PASS / 0 FAIL，与 P1-requirements.md 的 BDD-1~30（含 BDD-30 `[SCOPE+ from P4]`）总数精确对应

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P6-evidence/md-field-set-tool/results.md
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P6-evidence/check-gate-debt-fixes/results.md
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P6-evidence/phases-yaml-consistency/results.md
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（核对 BDD 总数 = 30）
</dispatch_guide>
...
<objective_info>
- 查证结果：P1-requirements.md 含 BDD-1~30 共 30 条（`grep -c "^#### BDD-"` = 30，主 Agent 已核实）；三批次 results.md 合计 19+6+5=30 条，PASS 30/FAIL 0，编号连续无缺口
</objective_info>
