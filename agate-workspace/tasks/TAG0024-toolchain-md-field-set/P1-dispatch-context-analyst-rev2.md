...
### 目标
本次是 P1 需求基线的**修复轮**（第 2 轮），不是重写。在已有的 P1-requirements.md 基础上做两处增量修改，其余内容（已通过评审的部分）不动。

### 约束
- **只做以下两处修改，不重写其余内容**：
  1. 新增一条 BDD，与 BDD-21（DEBT0019 的"既有合法场景判定结果不变"回归锚点）对称，覆盖 DEBT0020：Given 当前工作目录是仓库根（既有正常调用路径，含 TAG0023 P8 回写校验既有用例）；When 修复后的 `gate_p8()` / `_check_roadmap_done()` 解析 roadmap.md；Then 判定结果（阻断行为/rm_id/status）与修复前完全一致。插入位置：BDD-22/BDD-23 之后（DEBT0020 小节末尾），编号可用 BDD-23a 或整体顺延重排（顺延时后续所有 BDD 编号+1，需同步检查全文交叉引用无遗漏）
  2. 顺带精确化 BDD-28 的 Then 排除措辞：当前写的是"DEBT0019/20 明确修复的 `_check_roadmap_done()` 相关行"，但 DEBT0020 的实际改动点在其调用方 `gate_p8()` 内的 `roadmap_path` 构造行，不在 `_check_roadmap_done()` 函数体本身。改为"`_check_roadmap_done()` 及其调用点 `gate_p8()` 中 `roadmap_path` 定位相关行"这类精确表述，避免 P7 一致性核对阶段产生"是否算相关行"的解释分歧
- 修改后重新跑 `check-frontmatter.py` 自检（若做了编号顺延，需重新核对 BDD 计数与 frontmatter 声明是否仍一致）
- 不得借本轮修复顺便扩大范围或改动已通过评审的其余 27 条 BDD 的实质内容

### 上游关联
- 上轮产出：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（已产出，28 条 BDD，本轮在此基础上增量修改）
- 上轮 dispatch-context：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-dispatch-context-analyst.md（约束仍然有效，本文件只追加本轮增量指令）
- 评审意见：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-review.md（status: needs-revision，阻塞原因见「同源铁律核验」节 DEBT0020 部分；记录项见「记录项」节 BDD-28 措辞建议）——评审的其余核验维度（BDD 可判定性/隐含需求覆盖/裁剪/范围/P0-brief 时效性）均已通过，不需要动

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（待修改对象）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-review.md（评审意见，重点读「同源铁律核验」与「记录项」两节）
</dispatch_guide>
...
<objective_info>
- 查证结果：review 已实地读取 `agate/scripts/check-gate.py:1205-1230` 确认 DEBT0020 改动点在 `gate_p8()` 内 `roadmap_path = os.path.join(...)` 一行，非 `_check_roadmap_done()` 函数体
- 查证结果：BDD-21（DEBT0019 对称锚点）的现有写法可直接作为本次新增 BDD 的结构模板，仅替换 issue 归属与函数名引用
</objective_info>
