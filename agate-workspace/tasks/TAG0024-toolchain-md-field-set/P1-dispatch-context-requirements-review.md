...
### 目标
独立评审 P1-requirements.md（agent≠main，fresh context），产出 P1-review.md，判定 BDD 基线是否可支撑 P2 设计与后续验收。

### 约束
- 你是独立评审者，不是 P1-requirements.md 的作者——不预设其正确，逐条核验
- 必须核验的硬性门槛（check-gate.py P1 会机械校验，评审需先行确认，避免带着不达标的需求进 P2）：
  - BDD 标题严格匹配 `#### BDD-NN:` 格式，编号全局唯一无重复
  - frontmatter 含 `domains` / `packages` / `risk_level` / `phases` 声明，且非空占位符
  - 无未处理的 `[NEED_CONFIRM]`（`[SUGGEST]` 不阻塞，但需确认其未被误用为逃避真正需要人定夺的问题）
  - 正文含「同类扫描」结论节，且三条扫描线索（get 白名单对齐 / roadmap 表格解析消费点 / P6.5 定位消费点）逐条给出命中数量+文件清单+处理判定，不是空话或"待办"
  - `capability_requirements` 三态声明（本任务预期 available，需核实是否合理，非"想当然"）
- **范围核验**：P0-brief.md 锁定 5 项 issue（RM-AG0048 一期 / DEBT0019 / DEBT0020 / RM-AG0049 / RM-AG0050），核对 P1-requirements.md 的 28 条 BDD 是否精确覆盖这 5 项、有无遗漏或范围蔓延
- **同源铁律核验**（本任务的核心风险点，P0-brief known_risks 已强调）：BDD 是否明确声明 RM-AG0048 的 value 校验须与 check-gate.py 同源（同一份 schema），以及 DEBT0019/20 修复不得改变既有合法 roadmap.md 的判定结果——若 BDD 未覆盖这层"不破坏现有判定"的约束，视为需求基线不完整，打回
- **裁剪核验**：`phases` 声明为全阶段不裁剪（`[P1..P8, P6.5]`），核对「裁剪说明」节的理由是否站得住（尤其 P3 未裁的理由——本任务改动核心 gate 判定函数，是否需要 TDD 红灯）

### 上游关联
P1-requirements.md（analyst 产出）已完成自检：`check-frontmatter.py` exit 0；28 条 BDD 覆盖 5 项 issue；同类扫描三条线索均给出真实 grep 命中结果（线索1：38 个 get op 逐组判定；线索2：确认仅 check-gate.py 一处消费按列索引解析 roadmap.md；线索3：18 个文件命中 P6.5 关键字，逐类判定脚本层语义自洽、仅 phases.yaml 结构声明层措辞需统一）；1 条 `[SUGGEST]`（转入 BDD-17，建议 set 白名单定义为机械并集规则而非抄列表）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（待评审对象，全文）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P0-brief.md（核对范围是否精确对齐锁定的 5 项 issue）
...
<objective_info>
- 环境状态：P1-requirements.md 已存在（287 行，24423 字节），`check-frontmatter.py` 已跑过 exit 0
- 查证结果：BDD 计数 `grep -c "^#### BDD-" P1-requirements.md` = 28；`[NEED_CONFIRM]` 关键字仅出现在正文声明为 `[NO_NEED_CONFIRM]` 的两处，无未处理阻塞项；`[SUGGEST]` 出现 1 处（关于 set 白名单定义方式，已转入 BDD-17，非阻塞）
</objective_info>
