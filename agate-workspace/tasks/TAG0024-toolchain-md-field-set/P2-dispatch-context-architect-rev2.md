...
### 目标
本次是 P2 方案设计的**修复轮**（第 2 轮），不是重写。P2-review.md 判定 needs-revision，唯一阻塞点：`dispatch_plan` 声明的三批次（`md-field-set-tool` / `check-gate-debt-fixes` / `phases-yaml-consistency`）宣称"彼此文件不重叠、可并行派发 P4"，但 §1.1「改什么」表格中 `agate/tests/unit/test_check_gate.py` 追加用例一行同时归属 `check-gate-debt-fixes`（DEBT0019/20 用例）与 `phases-yaml-consistency`（RM-AG0049 用例，BDD-25/26），实际存在文件级交叉，与"可并行"的声明矛盾。

### 约束
- **只做以下修改，不重写整份 P2-design.md**：
  1. 解决 dispatch_plan 批次文件重叠问题，二选一（review 已给出）：
     - **方案①（推荐，更彻底）**：把 RM-AG0049 相关的 `test_check_gate.py` 用例改为只在 `test_check_structure_consistency.py` 落地（BDD-26 已有独立覆盖，删除后不影响 BDD 覆盖完整性），使三批次真正零文件交叉，`dispatch_plan` 的"可并行"断言保持成立
     - **方案②**：若确需保留 `test_check_gate.py` 侧的 RM-AG0049 用例，则在 `dispatch_plan` 中显式声明该文件为跨批次共享文件，并加一条串行依赖或合并步骤说明，撤回"可并行"的笼统表述，改为准确描述实际编排方式
     - 选定后需同步更新 §1.1 改什么表格与开篇"范围重述"段落，确保两处表述与 `dispatch_plan` 实际声明一致（不能表格改了、开篇断言没改）
  2. 顺带修正两处非阻塞记录（review 已确认不影响本次修订范围，但建议一并改）：
     - §1.3 风险5 与 §3.8 两处 `grep -c "P4-review" agate/phase-cards/P4-implementation.md` 引用次数从"7 次"改为实测的"10 次"（不影响该项验证的技术结论，只是数值笔误）
     - `gate_commands.P3_timeout_seconds`/`P5_timeout_seconds` 从 240 上调至 ≥250（与设计自身引用的 165.7×1.5=248.55 公式对齐，遵循"宁高勿低"取值原则）
- 其余内容（候选方案 A/B、影响面梳理的改什么/不改什么/风险在哪其余条目、详细设计 §3、files_to_read、env_constraints、minimal_validation 其余 4 项、gate_commands 其余 key）均已通过评审，不要无关改动
- 修改后重新跑 `check-frontmatter.py` 自检

### 上游关联
- 上轮产出：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-design.md（已产出，503 行）
- 上轮 dispatch-context：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-dispatch-context-architect.md（约束仍然有效）
- 评审意见：{AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-review.md（status: needs-revision，阻塞点见「dispatch_plan 声明核验」节；非阻塞记录见「架构问题（非阻塞）」节）——其余 5 个核验维度（同源铁律/候选真实性/不改什么/minimal_validation 方法/gate_commands 拆分/files_to_read 精准度）均已通过，不需要动

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-design.md（待修改对象）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-review.md（评审意见，重点读「dispatch_plan 声明核验」与「架构问题」两节）
</dispatch_guide>
...
<objective_info>
- 查证结果：review 已实测 `agate/tests/unit/test_check_gate.py` 既有 roadmap 相关测试位于第 1531-1568 行区域，gate_p4 相关测试位于第 2552-2673 行区域——两批次若各自追加新用例大概率落在不同行区段，物理 git 冲突概率不高，但"文件级零重叠"这一硬性声明本身不成立，需要修正声明方式而非否认事实
- 查证结果：`grep -c "P4-review" agate/phase-cards/P4-implementation.md` 实测为 10（命中第 90/91/92/93/94/95/97/107/110/153 行）
</objective_info>
