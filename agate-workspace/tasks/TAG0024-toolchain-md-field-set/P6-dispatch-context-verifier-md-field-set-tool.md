...
### 目标
P6 证据并行批次之一：为 BDD-1~19（RM-AG0048 一期，agate-md-field-set 工具）逐条产出验收证据。本批次**不写最终 P6-acceptance.md**，只写 `P6-evidence/md-field-set-tool/results.md`（PASS/FAIL 行 + 证据引用）+ 证据文件，供后续汇总 verifier 整合。

### 约束
- **本任务无 UI**（`ui_affected: false`），证据形式为断言输出/日志文件，不需要截图/vision
- **验收报告记录的是验收时的事实**：必须实际执行相关测试/命令获取真实输出作为证据，不能凭代码审查"应该能过"就写 PASS
- **每条 BDD 只允许 PASS 或 FAIL**，不允许"调整/跳过/覆盖"
- **PASS 必须有文件引用**：`- PASS BDD-N: {描述} ({证据路径})`，证据路径相对 `P6-evidence/md-field-set-tool/` 目录
- **证据来源**：主要复用/重跑 `agate/tests/unit/test_agate_md_field_set.py` 对应测试函数的输出（该测试文件已在 P4 全部转绿），把每条 BDD 对应测试函数的 `pytest -v` 单独输出保存为证据文件（不是笼统一份全量日志，每条 BDD 引用能定位到该条的具体输出片段）
- **总结行格式**：`results.md` 中 `- PASS`/`- FAIL` 只用于 BDD 条目，总结行用其他格式（如 `**Summary**: 19/19 PASS, 0 FAIL`）

### 上游关联
- P1-requirements.md BDD-1~19（Given/When/Then + 断言点）
- P3-test-cases-md-field-set-tool.md：BDD→测试函数映射表（1:1），直接按此表逐条取证
- P4-implementation-md-field-set-tool.md：实现细节（供理解 FAIL 时的诊断方向，本批次预期全部 PASS）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（BDD-1~19）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P3-test-cases-md-field-set-tool.md（BDD→测试函数映射）
- agate/tests/unit/test_agate_md_field_set.py（测试代码，逐条重跑取证据）
</dispatch_guide>
...
<objective_info>
- 环境状态：P5 阶段已确认 `test_agate_md_field_set.py` 全部 35 个测试项（对应 19 条 BDD）转绿，全量 pytest 1285 passed/0 failed
- 查证结果：测试运行需 `--basetemp=.pytest-tmp -p no:cacheprovider`（/tmp 只读约束）
</objective_info>
