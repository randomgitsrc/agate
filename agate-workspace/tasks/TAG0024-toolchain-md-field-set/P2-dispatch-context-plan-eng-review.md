...
### 目标
独立评审 P2-design.md（单一评审角色，C8 映射命中：domains=backend + risk_level=medium → plan-eng-review；risk_level 非 high/full，不触发 cso/plan-ceo-review，无需组长汇总，产出直接写 P2-review.md）。

### 约束
- 你是独立评审者，不预设方案正确，核验重点：
  1. **同源铁律是否真落地**：候选方案 A（importlib 动态加载复用 check-gate.py/agate-frontmatter-check.py/agate-md-field-get.py/check-judge-verdict.py 的 SCHEMAS/_check()/字段分类/_VALID_STATUS）是否真的做到"物理上只有一份校验逻辑"，而不是看似复用实则另建一套判断
  2. **候选方案是否为真实候选**（非稻草人）：候选 B（下沉到 agate_common.py）是否有实质性权衡对比，而非为了凑数而写的明显更差方案
  3. **影响面梳理「不改什么」是否站得住**：尤其"不改 agate-frontmatter-check.py/agate-md-field-get.py/check-judge-verdict.py"三处零改动声明，是否会在后续 P4 实现时被迫打破（如动态加载方式是否真的不需要这三个文件配合改任何一行）
  4. **minimal_validation 是否为真实验证**：architect 声称已用 bash/python3 实测验证 5 项假设（importlib 加载可行性/DEBT0020 worktree 场景/DEBT0019 列数基准/RM-AG0049 影响面），需核实这些验证方法本身是否站得住（不要求你重新跑一遍，但要判断验证方法是否能真正证明其声称的结论，是否有遗漏的验证维度）
  5. **gate_commands 声明**：P3/P5 是否已按「不用 `&&` 短路链」的强制要求拆成独立 key；timeout_seconds 取值依据是否合理
  6. **files_to_read 是否精准**（不过多不过少）：P4 implementer 按此清单读代码，清单过长会导致上下文爆炸
- **范围核验**：P2 设计是否严格落在 P1-requirements.md 锁定的 29 条 BDD 范围内，有无隐性扩大范围（如候选方案 B 的讨论本身提到的"6 类改动"是否被正确排除在候选 A 的实际实现范围外）
- **dispatch_plan 声明核验**：`dispatch_plan: {mode: static-batch, ...}` 的三个批次划分是否真的互不重叠（可并行），核对影响面梳理「改什么」表格中各文件是否精确分配到某一批次，无交叉

### 上游关联
- P1-requirements.md（approved）：29 条 BDD，risk_level=medium，domains=[backend]，packages=[agate-scripts, agate-rules, agate-docs, agate-tests]
- P2-design.md（architect 产出）：candidate_count=2；候选方案 A（importlib 动态复用，选定）vs 候选方案 B（下沉 agate_common.py，陪衬）；影响面梳理含改什么/不改什么/风险在哪三部分

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-design.md（待评审对象，全文）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（核对范围是否精确对齐 29 条 BDD）
- AGENTS.md（项目约定，评审口径参考）
</dispatch_guide>
...
<objective_info>
- 环境状态：P2-design.md 已存在（503 行），`check-frontmatter.py` 已跑过 exit 0
- 查证结果：主 Agent 已抽查 minimal_validation 声称的 2 项（DEBT0020 worktree 场景 `git rev-parse --show-toplevel` 返回本 worktree 根路径；DEBT0019 roadmap.md 真实表头行 `line.split("|")` 长度确实为 9）均属实，未发现编造
</objective_info>
