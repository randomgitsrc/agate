...
### 目标
产出 P8-release.md（发布准备说明）。**你不执行 git commit/tag/bump-version**，这些由主 Agent 在你产出的基础上亲自执行。

### 约束
- **版本号判定**：`README.md` 第 5 行当前 badge 为 `v0.62.0`；`CHANGELOG.md` 最新条目为 `[0.62.0] - 2026-08-25`（TAG0023）。本任务新增 `agate-md-field-set.py`/`agate-md-field-set-gate-commands.py`（全新 CLI 能力，非破坏性）+ 4 处 bug 修复（DEBT0019/20）+ 2 处文档自洽修复（RM-AG0049/50）+ 1 处 SCOPE+ 修复（BDD-30）。按版本判定规则（"加功能/内部重构改 API（向后兼容）→ minor"，新增能力优先级高于单纯 bug fix 的 patch 判定），你需要给出 `bump_type` 建议（预期为 `minor`，即下一版本 `v0.63.0`），并说明理由
- **CHANGELOG 撰写**：新增 `## [0.63.0] - 2026-08-25` 条目（若你判断 bump_type 应为其他值，用对应版本号），按既有条目风格（参照 `[0.62.0]`/`[0.61.0]` 的"新增（TAGxxxx：...，RM-AGxxxx~RM-AGxxxx）"格式）撰写本次变更摘要，须覆盖：RM-AG0048 一期（agate-md-field-set 工具）、DEBT0019/DEBT0020（check-gate.py roadmap-done 健壮性）、RM-AG0049/RM-AG0050（phases.yaml 文档自洽）、BDD-30 SCOPE+（check-pruning.py 测试隔离修复）+ ADR-011 新增。**只在 P8-release.md 里给出建议的 CHANGELOG 文本内容，不要直接编辑 CHANGELOG.md 文件本身**（那是主 Agent 亲自执行的步骤）
- **roadmap.md 回写**：`agate-workspace/roadmap/roadmap.md` 有 3 条关联本任务的条目（RM-AG0048/RM-AG0049/RM-AG0050，当前状态均为 `scheduled`）——check-gate.py P8 会检查这些条目状态是否已回写为 `done`，否则阻断。**你在 P8-release.md 中给出这 3 条应回写为 done 的建议**，实际回写由主 Agent 执行（roadmap.md 是主 Agent 维护的看板类文件）
- **DEBT0019/DEBT0020 状态**：`agate-workspace/debt/tech-debt.md` 中这两条当前 `status: open`。核对 closure_criteria 是否已全部满足（列数校验+回归用例、repo-root 定位+回归用例、全量测试通过），若已满足，在 P8-release.md 的 `debt_check:` 字段给出建议（`reviewed`，并列出这两条 DEBT id 及"已解决"结论），实际状态字段回写由主 Agent 执行
- **临时资源清单**：本任务全程未启动任何调试服务/数据库/临时端口（纯脚本/文档改动，无需运行时环境），如实声明"无临时资源"

### 上游关联
- P2-design.md packages: [agate-scripts, agate-rules, agate-docs, agate-tests]（单一逻辑发布单元，非多包拆分）
- P7-consistency.md：BLOCKER=0，一致性检查已通过
- P5-test-results/unit.md：1285 passed/0 failed（基线证据，供你判断是否需要重新验证或说明"引用 P5 证据"）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P2-design.md（packages 声明）
- README.md（第 1-10 行，版本 badge）
- CHANGELOG.md（前 60 行，最新版本条目格式参照）
- agate-workspace/roadmap/roadmap.md（RM-AG0048/49/50 条目现状）
- agate-workspace/debt/tech-debt.md（DEBT0019/DEBT0020 条目现状，约第 684-730 行）
</dispatch_guide>
...
<objective_info>
- 环境状态：README.md badge 现为 `v0.62.0`；CHANGELOG.md 最新条目 `[0.62.0] - 2026-08-25`
- 查证结果：roadmap.md 第 46-48 行 RM-AG0048/49/50 均关联 `task_id=TAG0024`，状态均为 `scheduled`；tech-debt.md 第 684-730 行 DEBT0019/DEBT0020 均 `status: open`，`scheduled_task: TAG0024`
</objective_info>
