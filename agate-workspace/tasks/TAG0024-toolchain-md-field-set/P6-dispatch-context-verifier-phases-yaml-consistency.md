...
### 目标
P6 证据并行批次之一：为 BDD-25~29（RM-AG0049/50 + 跨 issue 约束 BDD-29）逐条产出验收证据。本批次**不写最终 P6-acceptance.md**，只写 `P6-evidence/phases-yaml-consistency/results.md` + 证据文件。

### 约束
- 同其他批次：无 UI，证据为断言输出/日志文件
- **证据来源**：
  - BDD-25~28：重跑 `agate/tests/unit/test_check_structure_consistency.py` 对应测试函数（`test_bdd_25_*`~`test_bdd_28_*`）
  - BDD-29（RM-AG0048 不改变 check-gate.py/check-events.py 判定逻辑）：本条 P3 阶段已标注"无自动化测试函数，以 P7 一致性检查阶段 diff 逐行核对方式验收"——本批次**不需要**为 BDD-29 造一个假测试，而是执行实际的 diff 核对：`git show e2357fc -- agate/scripts/check-gate.py`（P4 commit）确认改动只在 `_check_roadmap_done()`/`gate_p8()` 的 `roadmap_path` 定位相关行，`git diff` 确认 `check-events.py` 全程零改动，把这次 diff 核对的命令与结论写入 results.md 作为 BDD-29 的证据（证据文件可以是 diff 输出本身保存成文件）
- 每条 BDD 只允许 PASS/FAIL，PASS 必须有证据路径引用
- 总结行不用 `- PASS`/`- FAIL` 开头

### 上游关联
- P1-requirements.md BDD-25~29
- P3-test-cases-phases-yaml-consistency.md：BDD→测试函数映射（含 BDD-29 的非自动化处理说明）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（BDD-25~29）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P3-test-cases-phases-yaml-consistency.md
- agate/tests/unit/test_check_structure_consistency.py（BDD-25~28 测试代码）
</dispatch_guide>
...
<objective_info>
- 环境状态：P5 阶段已确认 `test_check_structure_consistency.py` 17 passed/0 failed
- 查证结果：P4 commit 哈希为 `e2357fc`，可用 `git show e2357fc -- agate/scripts/check-gate.py` 查看该次改动的完整 diff 供 BDD-29 核对
</objective_info>
