...
### 目标
产出 P2-design.md：为 P1-requirements.md 的 29 条 BDD（覆盖 RM-AG0048 一期 / DEBT0019 / DEBT0020 / RM-AG0049 / RM-AG0050 共 5 项 issue）设计实现方案，含影响面梳理（改什么/不改什么/风险在哪）+ 候选方案 + gate_commands 固化。

### 约束
- **范围锁定**：不得超出 P1-requirements.md 已锁定的 29 条 BDD 范围；P1 同类扫描中标注"本次不处理"的项（如 need_confirm_resolved/suggest_resolved/scope_resolved/mechanism_issues/execution_issues/dispatch_plan 三组字段、check-retrospective.py/check-protocol-consistency.py 两个非消费点）不在本次设计范围内
- **同源铁律是设计核心**：RM-AG0048 的 agate-md-field-set.py 的 value 校验必须直接复用（import，不允许复制粘贴另一份）check-gate.py / agate_common.py 已有的 schema 定义与校验逻辑，避免"set 说通过、gate 说不通过"的新漂移（P1 BDD-15 已锁定该约束，P2 需给出具体的复用路径：哪个模块、哪些函数被 import）
- **不改变既有判定语义**（P1 BDD-21/24/28/29 已锁定）：DEBT0019/20 的修复方案必须能同时通过"新场景正确" + "既有合法场景（含仓库根 CWD 正常调用路径）判定结果不变"两组验收，候选方案设计时需说明如何保证向后兼容
- **P4 outputs 声明补全（RM-AG0049）**：phases.yaml P4 outputs 需补 `{file: P4-review.md, required: true, status_field: status}`，方案需说明如何核对 check-structure-consistency.py 的 S-1/S-2 双向一致性不因此产生新的不一致报错（P1 已确认 S-1~S-6 reconcile 在 P1 commit 时 0 mismatch，作为修改前基线）
- **P6.5 口径统一（RM-AG0050）**：以 state-machine.md 的表述为准（"挂载于 P6→P7 转移的强门槛子阶段，非独立 phase 值"），修改 phases.yaml 的结构声明措辞，但不能改变 P1 同类扫描已确认"脚本层实现本身语义自洽"的现状（即不改 check-gate.py/check-judge-verdict.py/agate_common.py 的判定逻辑，只改文档/YAML 措辞层）
- **本任务改动会触发 SELF-GATE**（P0-brief known_risks 已声明）：改动面含 agate/scripts/*.py（新建 + check-gate.py 修改）+ agate/rules/phases.yaml + agate/state-machine.md（措辞）+ dispatch-context/dispatch-prompt 模板，方案需在 gate_commands 中含 `check-protocol-consistency.py` 校验项
- **TDD 强制**：risk_level=medium 且改动含核心 gate 判定函数，P3 不可裁，gate_commands.P3 需声明测试运行器

### 上游关联
- P1-requirements.md frontmatter：`risk_level: medium` / `domains: [backend]` / `packages: [agate-scripts, agate-rules, agate-docs, agate-tests]` / `phases:` 含全部 P1-P8（无裁剪）
- P1 同类扫描三条线索的结论（供影响面梳理直接复用，不需重新 grep）：
  1. get 工具（`agate-md-field-get.py`）共 38 个 op，分 9 组——set 白名单方案需按"task-files.md 通用 Header ∪ phases.yaml task_fields 完整并集"的机械规则设计（P1 BDD-17 已锁定，避免抄 design note 的举例子集导致遗漏）
  2. roadmap.md 表格解析仅 `check-gate.py._check_roadmap_done()` 一处按列索引解析，`check-retrospective.py` 用正则匹配不受影响，`check-protocol-consistency.py` 只是文档一致性锚点引用——DEBT0019 修复不需要同步到其他文件
  3. P6.5 关键字命中 18 个文件，脚本层（8 个）语义已自洽，仅 `phases.yaml`（结构声明层）与 `state-machine.md`（文字定位）表述冲突——RM-AG0050 只需改 `phases.yaml` 措辞层
- P1 评审（P1-review.md，approved）核实通过：同源铁律覆盖完整（BDD-15/21/24/27/28/29）、范围精确对齐 5 项 issue、裁剪合理（全阶段不裁）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P1-requirements.md（需求基线全文，29 条 BDD）
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P0-brief.md（env_constraints 环境约束）
- docs/design-notes/design-md-field-set.md（RM-AG0048 参考输入，非强制照搬——P1 已发现其白名单定义举例不全，BDD-17 已给出修正方向，本阶段设计以 BDD-17 的机械并集规则为准，不是照抄 design note 的字段列表）
</dispatch_guide>
...
<objective_info>
- 环境状态：P1 commit（8755654）已落库，S1-S6 reconcile 0 mismatch（作为本轮修改前的一致性基线）；worktree 基线 pytest 9.0.3 / ruff 0.16.4 可用
- 查证结果：`agate/scripts/agate-md-field-get.py` 已存在（269 行，get 工具，KNOWN_OPS 38 个字段），`agate/scripts/agate-md-field-set.py` 尚不存在（待新建）
- 查证结果：`agate/scripts/check-gate.py` 的 `_check_roadmap_done()` 位于第 1181-1199 行，调用点在 `gate_p8()` 第 1218-1219 行（`roadmap_path = os.path.join("agate-workspace", "roadmap", "roadmap.md")`，CWD 相对路径）
- 查证结果：`agate/rules/phases.yaml` 第 57-66 行 `id: P4` 的 `outputs` 仅含 `P4-implementation.md`（第 61 行），第 88-97 行 `id: P6.5` 与 P4-P8 平级声明；`check-gate.py` 的 `gate_p4()` 第 872-890 行要求 `P4-review.md` 存在
</objective_info>
