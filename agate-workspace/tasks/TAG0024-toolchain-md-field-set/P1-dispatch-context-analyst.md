...
### 目标
产出 P1-requirements.md：把 P0-brief.md 合并的 5 项来源（RM-AG0048 一期 / DEBT0019 / DEBT0020 / RM-AG0049 / RM-AG0050）转成统一的 BDD 验收条件基线（全局唯一编号，不分包各自从 1 编号），含「同类扫描」结论。

### 约束
- **范围锁定**：仅处理 P0-brief.md 「issues（合并来源）」节列出的 5 项，不得擅自扩大范围；若分析中发现需求/设计层面需要超出锁定范围的改动，必须先停下写 `[NEED_CONFIRM]`，不得自行拍板扩大
- **不改变既有判定语义**：
  - RM-AG0048（agate-md-field-set 工具）不得改动 check-gate.py / check-events.py 的判定逻辑——新工具只写 gate 本就要校验的字段，value 校验须与 check-gate 同源（同一份 schema，不允许工具自己另起一套校验规则）
  - DEBT0019/20 只修 `_check_roadmap_done()` 的健壮性（列数校验 + 路径定位），不得改变该函数对既有合法 roadmap.md 的判定结果（回归覆盖 TAG0023 的 P8 roadmap 回写校验 BDD）
- **design note 仅供参考**：`docs/design-notes/design-md-field-set.md` 是 RM-AG0048 的参考输入，不是强制照搬的交付物；发现其中方案有弊端/缺陷，可在需求里调整方向或转登记 DEBT，不必照搬
- **judge 强制声明**：本任务 P1 created 日期晚于 `agate/rules/dispatch.yaml` 声明的 `judge_required_since`（机制后新任务），.state.yaml 已写 `judge.enabled: true`——P1 frontmatter 不需要重复声明，但需求正文不能与此口径冲突
- **同类扫描是本任务的重点交付项**（不是可选项）：P0-brief 已给出三条待做的扫描线索（见下方「上游关联」），必须在 P1 正文用 grep/rg 逐条做实，写出命中数量 + 文件清单 + 逐条"本次处理/本次不处理+理由"

### 上游关联
P0-brief.md 已核实的 5 项 issue 与已知风险（见 known_risks 全文），其中三条待做实的同类扫描线索：
1. grep `agate-md-field-get.py` 的全部 op（get 工具已存在，见客观查证信息），确认 RM-AG0048 的 set 白名单覆盖是否对齐 get 的读取面，避免 get/set 读写不对称
2. grep roadmap.md 表格解析的全部消费点（不止 check-gate.py，还包括 check-retrospective.py 等是否也做类似的 split("|") 解析），确认 DEBT0019 的列数校验修复是否需要同步到其他消费点
3. grep P6.5 定位的全部消费点（check-gate.py / check-judge-verdict.py / state-machine.md / phases.yaml），确认 RM-AG0050 统一口径后哪些消费端需要同步核对（不需要真的改代码，P1 只需列出消费点清单供 P2 设计影响面梳理承接）

P0-brief 时效性已核对：立项日期与本次启动同为 2026-08-25，无跨会话间隔，判定"已核对，无漂移"，P1 正文需写入这一行结论（不要求 `[P0_STALE]` 标注）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0024-toolchain-md-field-set/P0-brief.md（任务简报，5 项 issue 的证据来源与风险声明）
- AGENTS.md（项目开发约定，仓库根）
- docs/design-notes/design-md-field-set.md（RM-AG0048 参考输入，非强制照搬，见上方约束）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 基线已验证全绿（全量 pytest + consistency 0 ERROR，见 HANDOFF-TAG0024.md）；本会话已确认 pytest 9.0.3 / ruff 0.16.4 可用
- DEBT0019 定位：`agate/scripts/check-gate.py` 的 `_check_roadmap_done()`（第 1181-1199 行），按 `line.split("|")` 分列后仅校验 `len(cols) < 8` 才跳过，未校验列数是否精确匹配表头列数——描述列含字面 `|` 时列会整体错位但可能仍满足 `len>=8`，导致 rm_id/status/related_task 取到错误列
- DEBT0020 定位：同函数调用点在 `gate_p8()`（第 1218-1219 行）：`roadmap_path = os.path.join("agate-workspace", "roadmap", "roadmap.md")`，是相对当前工作目录的硬编码拼接，非仓库根锚定——CWD 不在仓库根时该路径不存在，`_check_roadmap_done` 提前返回 `None`（静默跳过检查，不报错）
- RM-AG0049 定位：`agate/rules/phases.yaml` 第 57-66 行 `id: P4` 的 `outputs:` 只列 `{file: P4-implementation.md, required: true}`（第 61 行），未列 P4-review.md；但 `agate/scripts/check-gate.py` 第 872-890 行 `gate_p4()` 实际要求 `P4-review.md` 存在且 `status: approved` 且 `agent≠main`——YAML 声明与脚本实际要求不对称
- RM-AG0050 定位：`agate/rules/phases.yaml` 第 88-97 行把 `id: P6.5` 列为与 P4/P5/P6/P7/P8 平级的独立阶段条目；而 `agate/state-machine.md` 第 74 行、第 152 行明确写"P6.5 是挂载于 P6→P7 转移的**强门槛子阶段**，不是独立 phase 值"——两处对 P6.5 性质的定位表述不一致
- 查证结果：`agate/scripts/agate-md-field-get.py` 已存在（get 工具），`agate/scripts/agate-md-field-set.py` 尚不存在（RM-AG0048 待新建）
</objective_info>
