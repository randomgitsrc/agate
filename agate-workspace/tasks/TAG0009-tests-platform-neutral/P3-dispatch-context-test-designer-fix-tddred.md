...
### 目标（拆分任务 B1：只改 check-tdd-red.bats 一个文件）

修改 `agate/tests/unit/check-tdd-red.bats` 的断言（PATH 平台无关构造 + 样例文本标记），**只此一个文件**，其余文件不碰。

### 约束

- **改动点**（按 P2-design.md §2.4）：
  1. **TD.1b（L48-51）/ TDD.F8（L380-383）**：`env -i PATH="/usr/bin:/bin"` → **`env -u PATH`**（断言 exit 3 不变；TD.1b 保持 `-eq 3 || -eq 1`，TDD.F8 保持 `-eq 3`）
  2. **移除 15 处 `PATH="/usr/bin:/bin"` 覆盖**：TDD.G/F 系列用例（L164/227/245/260/279/296/315/333/351/367/404/435/451 附近）的 `env -u TEST_RUNNER PATH="/usr/bin:/bin" TASK_DIR=...` → 删掉 `PATH="/usr/bin:/bin"`，保留 `env -u TEST_RUNNER TASK_DIR=...`（fake runner 经 TASK_DIR/gate_commands.P3 绝对路径指定，不依赖 PATH 解析；`-u TEST_RUNNER` 保留）
  3. **L139/148 样例文本**：4 处 `/tmp` mock 输出字符串行尾加 `# scan-exempt: mock 输出样例文本（非路径假设）` 标记（**内容原样不动**，只加行尾注释）
  4. **新增 BDD-11 断言用例**（可选）：TD.1b/TDD.F8 修复后断言 exit 仍为 3（保持既有断言即可，不新增冗余）
- **只改这个文件**：不要动 fixtures.bash / 其他 .bats / 被测脚本 check-tdd-red.sh（P4 改）。
- **TDD 语义**：TD.1b/TDD.F8 改 `env -u PATH` 后当前即绿（exit 3 已成立）——属"改写法消除硬编码 PATH"，如实记录；15 处 PATH 覆盖移除后对应用例当前仍绿（行为不变）；样例文本标记后当前仍绿（内容原样）。**本文件改动多为"改写法"非"新行为"**，自跑确认全绿即可，不伪造红。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`。

### 上游关联

- `P2-design.md` §2.4（PATH 修复设计）
- `P3-test-cases.md`（任务 A 产出，BDD-10/11/12 映射）

### 输入文件

- `agate/tests/unit/check-tdd-red.bats`（唯一修改对象）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P2-design.md`（§2.4）
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0005-0009`；任务 A 已完成
- P2-review 已实测：`env -u PATH`（无 TASK_DIR）→ exit 3，与 `env -i PATH="/usr/bin:/bin"` 等价
- 原则：P3 只改测试断言，被测脚本 P4 改
</objective_info>
