...
执行 P2-design.md 声明的 gate_commands.P5 全部命令，验证 TAG0009 测试平台无关化后全量测试全绿，产出 P5-test-results/。

### 约束

- **P5 命令集（P2-design.md §5 固化，全部执行）**：
  ```yaml
  gate_commands:
  ...
  ```
  全部在工作目录 = worktree 根执行。
- **4 条子命令都要独立验证**（P2 §5 分解表）：
  1. `bats agate/tests/sanity.bats agate/tests/unit/ agate/tests/regression/ agate/tests/integration/` → 全量回归（BDD-28）
  2. `python3 agate/scripts/check-protocol-consistency.py --strict` → 0 ERROR（BDD-28）
  3. `shellcheck -S warning agate/scripts/*.sh` → 0 error（BDD-28）
  4. `bash agate/scripts/check-platform-assumptions.sh` → 零命中 exit 0（BDD-8 闭环）
- **P5_consistency 用 worktree 自己的脚本**（I13 双工作区纪律）：`python3 agate/scripts/...`（以 worktree 为 cwd）。
- **Windows 分支模拟验证**（本任务特殊性，BDD-15/17/19/22/23/24/25/26 已由测试用例覆盖，P5 确认这些用例在 Linux 上通过即覆盖 Windows 分支模拟）：确认 helpers-python.bats（BDD-15/17）、install-hook.bats 复制模式（BDD-19）、cp1252 用例（BDD-23）、CRLF 归一化（BDD-22）、无 bc 求和（BDD-24）、shellcheck 探测（BDD-25）在 P5 全量中均绿。
- **compact 输出**：bats 默认紧凑；consistency `--strict` 已紧凑；shellcheck 默认紧凑；扫描器零命中无输出。
- **判定**：四条命令全部 exit 0 才算通过。
- **产出**：
  - `P5-test-results/unit.md`（含测试运行器输出签名 ok/not ok + 四条命令各自 exit code）
  - `P5-test-results/fail-list.txt`（无失败时为空）
  - 命令执行日志落盘（如 bats.log / consistency.log / shellcheck.log / scan.log），末行 `EXIT_CODE: <n>`
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`。

### 上游关联

- `P2-design.md` §5 gate_commands（P5 命令集）
- `P4-implementation.md` / `P4-review.md`（实现说明 + approved）

### 输入文件

- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P2-design.md`（§5 gate_commands）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P4-implementation.md`（实现说明）
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0005-0009`；TAG0009 P1-P4 已 commit
- P4 自查：733 bats 全绿 + consistency 0 ERROR + shellcheck 0 error + 扫描器零命中——P5 需独立重跑确认（external-output-gate，不信 P4 自查）
- P5 命令是 1 主 + 3 辅（P5 / consistency / shellcheck / scan）——check-gate.sh P5 会输出主/辅 WARNING，属 RM-AG0011 新文案 in-situ 验证
</objective_info>
