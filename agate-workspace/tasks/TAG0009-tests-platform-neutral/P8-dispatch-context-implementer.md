...
产出 `P8-release.md`（发布准备记录），为 TAG0005 + TAG0009 合并发布做准备。

### 约束

- **合并发布模式（用户已确认 + HANDOFF §8b）**：本 worktree 承载 TAG0005 + TAG0009 两个任务，**合并为一次发布**（bump 一次版本 v0.45.0，一个 tag，一个 PR）。TAG0005 已 READY（P8-release.md 已产出），TAG0009 现在到 READY。主 Agent 在两者都 READY 后统一执行 bump + CHANGELOG + tag + PR。
- **本次 P8-release.md 是 TAG0009 的发布计划文档**：声明 bump_type / 计划版本号 / debt_check / 临时资源清单 / **TAG0009 变更摘要**（供合并发布写入 CHANGELOG，与 TAG0005 摘要并列）。
- **bump_type 判定**：TAG0009 为测试基建 + 新增扫描器 gate + CI 变更（行为变化：CI 新增 windows bats + 阻断扫描）——判 **minor**（与 TAG0005 合并后 v0.44.0 → v0.45.0）。
- **debt_check**：读 `agate-workspace/debt/tech-debt.md`，确认本任务是否产生/关闭债务，写 `debt_check: none` 或 `reviewed` + 条目 id。
- **CHANGELOG 草稿**：写 TAG0009 变更条目草稿（release note 文本），不实际改 CHANGELOG.md。
- **临时资源清单**：列出本任务执行期间启动的临时服务/进程/临时数据/开发安装。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`。

### 上游关联

- `P2-design.md`（packages: agate-tests / agate-scripts / ci-workflow）
- `P6-acceptance.md`（29/29 PASS）
- `P7-consistency.md`（BLOCKER=0）
- `TAG0005` 任务的 P8-release.md（合并发布对照）

### 输入文件

- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P2-design.md`（packages）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P6-acceptance.md`（验收结果，CHANGELOG 素材）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P7-consistency.md`（一致性结论）
- `agate-workspace/tasks/TAG0005-mechanism-fixes/P8-release.md`（合并对照）
- `agate-workspace/debt/tech-debt.md`（债务清单）
- 参考：`CHANGELOG.md`、`README.md`（version badge）、`agate/UPGRADING.md`
- `{agate_root}/assets/execution-roles/implementer.md`（P8 模式角色定义）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0005-0009`；TAG0005 已 READY；TAG0009 到 READY（P8 产出后）；当前版本 v0.44.0
- 合并发布模式：两任务合并 bump v0.45.0 + tag + PR，主 Agent 统一执行
- TAG0009 关键变更：新增 check-platform-assumptions.sh 扫描器 gate + PYTHON/shim helper + bc→awk + 24 测试文件平台无关化 + CI windows matrix（733 bats 全绿）
</objective_info>
