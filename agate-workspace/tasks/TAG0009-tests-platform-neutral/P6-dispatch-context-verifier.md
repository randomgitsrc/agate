...
产出 `P6-acceptance.md` + `P6-evidence/`，逐条验收 P1 的 29 条 BDD（PASS/FAIL 二值），证据落盘。

### 约束

- **验收口径**：功能任务标准口径。29 条 BDD 逐条实际验证（非文本声称），每条 PASS 必须有对应证据文件引用。
- **证据策略**（本任务为测试基建 + 扫描器 + CI，无 UI）：
  - 脚本行为类 BDD（BDD-1~9 扫描器、BDD-13/15/16/17 helper/shim、BDD-24 bc→awk）：跑实际命令/测试，输出落盘为证据
  - 文档断言类 BDD（BDD-7/10/14/20/27/28 等 grep 断言）：grep/rg 断言输出落盘为证据
  - 测试用例类 BDD（BDD-9/15/17/18/19/22/23/24/25/26 等）：对应 bats 用例运行输出落盘为证据
  - 每条证据文件含实际命令输出，末行 `EXIT_CODE: <n>`
- **BDD 全覆盖**：P1 有 29 条 BDD（BDD-1..29）→ P6 必须有 29 条验收结果（PASS 或 FAIL），编号连续，不许挑验。
- **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`，路径相对 P6-evidence/ 目录。
- **总结行**：行首 `- PASS`/`- FAIL` 只用于 BDD 条目；总结行用 `**Summary**: ...`。
- **自查≠gate**：写完自跑确认脚本可执行，但自查≠P6 gate。不要声称"验收已通过"。
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`。

### 上游关联

- `P1-requirements.md`（29 BDD 验收条件）
- `P2-design.md`（§3 BDD 覆盖映射 + §4 完成标准）
- `P5-test-results/`（P5 全绿证据，可引用）

### 输入文件

- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P1-requirements.md`（BDD，验收主要依据）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P2-design.md`（完成标准 §4）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P5-test-results/`（P5 证据）
- `agate-workspace/tasks/TAG0009-tests-platform-neutral/P3-test-cases.md`（测试契约）
- 按需核验的实际对象：
  - `agate/scripts/check-platform-assumptions.sh` + `agate/tests/scripts/check-platform-assumptions.bats`（BDD-1~9）
  - `agate/tests/helpers/fixtures.bash` + `agate/tests/unit/helpers-python.bats`（BDD-13/15/17）
  - `agate/tests/unit/check-tdd-red.bats`（BDD-10/11/12）
  - `agate/tests/unit/install-hook.bats` / `agate/tests/integration/pre-push-hook.bats`（BDD-18/19）
  - `agate/tests/unit/agate-next-card.bats` / `check-scope-resolved.bats`（BDD-20/21）
  - `agate/tests/unit/ci-gate-backstop.bats`（BDD-22/23）
  - `agate/scripts/agate-extract-context.sh` + `agate/tests/unit/agate-extract-context.bats`（BDD-24）
  - `agate/tests/unit/env-adapt-docs.bats`（BDD-25）
  - `.github/workflows/protocol-tests.yml`（BDD-7/27）
- `{agate_root}/assets/execution-roles/verifier.md`（角色定义）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0005-0009`；TAG0009 P1-P5 已 commit（P5 全绿 733 bats）
- P1 BDD 清单：BDD-1..29（见 P1-requirements.md §3），分组：扫描器(1-9) / PATH(10-12) / PYTHON helper(13-17) / symlink(18-19) / /tmp(20-21) / 编码工具(22-25) / 模拟覆盖(26) / CI 回归(27-29)
- 证据目录：`agate-workspace/tasks/TAG0009-tests-platform-neutral/P6-evidence/`（须创建，非空，每条 PASS 引用其中文件）
</objective_info>
