...
### 目标
批次 **install**（P2-design dispatch_plan 批次 2/3）：产出 agate-install（安装/卸载/环境探测）的测试用例与测试代码。产出 P3-test-cases-install.md + 测试代码。

### 约束
1. **本批范围（BDD 归属）**：BDD-1~8（agate-install：无参装 latest / 指定版本 / 幂等 / current↔latest / 卸载+引用保护 / --check 环境探测）。**只写本批测试**，不碰 resolve-chain（BDD-9~21/30/31）与 offline（BDD-22~29）批次的测试文件。
2. **1:1 BDD 映射**：每条 `#### BDD-NN` 对应一个测试用例，测试名引用 BDD 编号（如 `test_bdd_1_latest_pointer`）。
3. **测试代码落盘位置**：新建 `agate/tests/unit/test_agate_version_install.py`（与既有测试同目录）。fixture 复用 conftest（agate_root / task_dir / git_repo / run_cli / tmp_path / py_path）。git worktree 相关测试用 `git_repo` fixture 或 tmp_path 建临时 repo。
...
5. **红灯确认**：写完后自跑 `python3 -m pytest agate/tests/unit/test_agate_version_install.py -q --tb=no`，确认**全部红灯**且失败原因是"被测模块未实现"（import 失败 / 模块不存在 / 函数未定义 = B 类），不是测试代码自身 bug（A 类）。自跑结果记入 P3-progress.md。
6. **test_code_dir 声明**：P3-test-cases-install.md 须含 `test_code_dir: agate/tests/unit/`。
7. **双工作区纪律**：只写测试代码 + P3-test-cases-install.md + progress；不修改任何功能代码（P4 才实现）；`~/.agate` / 主 checkout 禁止改动。
8. **不跑全量测试**：只跑本批测试文件。

### 上游关联
- P2-design.md §4.5（agate-install：repo 单克隆 + worktree add / 幂等预判 / uninstall 引用保护 + 指针清理 / --check 分平台指引）
- P1-requirements.md BDD-1~8 + I-11/12/13/16
- 参照：agate/tests/conftest.py fixtures；TAG0004 平台无关测试先例

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P1-requirements.md（BDD-1~8）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P2-design.md（§4.5 方案）
...
- 查证结果：被测模块 agate-install.py **尚未实现**；git worktree add 已最小验证可行（重复 add exit 128 → 幂等须程序预判）
</objective_info>
