...
### 目标
P5 技术验证：执行 P2-design.md 声明的 gate_commands.P5 全部命令，产出 P5-test-results/（unit.md + fail-list.txt），如实记录 failed 计数与失败项。

### 约束
1. **执行 P2 gate_commands.P5（固化，不可修改）**：
   - P5: `python3 -m pytest -q --tb=no`（在 worktree 根跑，全量）
   - P5_unit: `python3 -m pytest -q --tb=no agate/tests/unit/test_agate_version_install.py agate/tests/unit/test_agate_version_resolve.py agate/tests/unit/test_agate_summary.py agate/tests/unit/test_install_hook.py`（新增单测）
   - P5_consistency: `python3 agate/scripts/check-protocol-consistency.py`（**必须用 worktree 自己的脚本**，检查对象是 worktree 协议文件；--strict 看 0 ERROR）
   - P5_count: `bash agate/tests/scripts/count-tests.sh`（验证用例数未漂移）
2. **全量测试**：P5 跑全量 pytest（agate/tests/），记录 failed 数量。发现失败时：判断是否本任务引入 vs 预存失败（基线 780 全绿 + 本任务已跑 823 passed）。
3. **产出**：
   - P5-test-results/unit.md：标注 failed 数量 + 每条命令输出摘要（用 grep 提取 passed/failed 汇总行，不贴全量 traceback）
   - P5-test-results/fail-list.txt：failed 测试 id 逐行列出（无失败时可为空文件）
   - 每文件末行可加 `EXIT_CODE: <n>`（0 表示该命令成功）
4. **签名校验**：unit.md 须含 test runner 输出签名（`grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)'` 计数 >0）——主 Agent 会据此判断产出有效性。
5. **双工作区纪律**：只读验证，不修改任何代码/文档（除 P5-test-results/ 产出）。`~/.agate` / 主 checkout 禁止改动。
6. **不跑 E2E**：ui_affected=false，无 Playwright/E2E 需求。
7. **预存失败处理**：若发现改动前就存在的失败，在 unit.md 标注"预存失败：X（与本次改动无关）"，不擅自标通过。
8. **不可逆操作**：本任务验证无数据删除/迁移等不可逆操作 → `[NO_NEED_CONFIRM]`。

### 上游关联
- P4 实现完成：3 批（resolve-chain / install / offline）已 commit；全量 823 passed（P4 自查）
- P2-design.md gate_commands.P5 固化（见上）
- 评审：P4 review + cso 均 approved（3 CRITICAL 已修复）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P2-design.md（gate_commands.P5 读取）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P0-brief.md（env_constraints）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/AGENTS.md（开发命令：pytest / consistency / count-tests）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0008-version-management；P4 已 commit（3 批实现 + 评审 approved）
- 关键路径：worktree=/home/kity/oclab/agate/.worktrees/agate-TAG0008（命令在 worktree 根跑）；AGATE_WORKSPACE 任务目录 agate-workspace/tasks/TAG0008-version-management/
- 查证结果：P4 自查全量 823 passed 2 skipped；consistency 基线 0 ERROR；count-tests 基线 818
</objective_info>
