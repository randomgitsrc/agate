...
### 目标
独立评审 {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P1-requirements.md，产出 P1-review.md（agent≠main，含 BDD 编号引用 + 覆盖维度标注），status 取 approved / needs-revision / rejected。

### 约束
1. **只审不写**：不修改 P1-requirements.md，评审意见写入 P1-review.md。
2. **实质锚点要求**：结论必须引用具体 BDD 编号 + 覆盖维度清单（数据/前端/多端/边界/兼容逐项标注），不得写裸 "approved"。
3. 检查清单（按角色定义全过一遍）：
   - BDD 可二值判定（无中间态）、编号 `#### BDD-NN:` 连续不跳号、每条单一 GWT
   - 隐含需求覆盖五维度（数据/前端/多端/边界/兼容）
   - BDD 跨条一致性（同场景多条 BDD Then 是否矛盾、保护优先级）
   - 裁剪合理性（risk_level=high 全阶段、phases 声明与正文一致）
   - P1 纯净性（无掺方案/实现细节）
4. 本任务影响面表（P1-requirements.md §2）是重点审查对象：确认"~/.agate 消费点"清单是否覆盖脚本层/文档层/测试层，有无遗漏联动点——这是 P0-brief known_risks 强制要求（用户明确：不愿意一轮一轮来回改）。
5. 评审结论与 Header 的 `status:` 字段必须一致。
6. **双工作区纪律**：只读评审，不修改 worktree 内任何文件（除 P1-review.md 产出）。

### 上游关联
- analyst 摘要：P1 需求基线完成，影响面表 + 29 条 BDD，[NO_NEED_CONFIRM]。
- P0-brief 范围锁定：6 组件（agate-install / agate-resolve / hook 解析入口 / summary 集成 / pack-offline / install-offline）+ 环境探测 + 向后兼容红线。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P1-requirements.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P0-brief.md（范围来源，核对 BDD 是否覆盖锁定范围）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/AGENTS.md（项目约定，测试平台无关等约定）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0008-version-management；P1-requirements.md 已产出（356 行，29 BDD，risk_level=high）
- 关键路径：AGATE_WORKSPACE=/home/kity/oclab/agate/.worktrees/agate-TAG0008/agate-workspace
- 查证结果：P1-requirements.md frontmatter 含 risk_level/phases/packages/domains/capability_requirements，[NO_NEED_CONFIRM]，无 GAP
</objective_info>
