...
### 目标
独立评审 {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P2-design.md，产出 P2-review.md（agent=plan-eng-review，≠main），status 取 approved / rejected。

### 约束
1. **只审不写**：不修改 P2-design.md / P1-requirements.md，评审意见写入 P2-review.md。
2. 评审重点（按角色定义全过一遍）：
   - 数据流清晰（resolve 四层解析：env → 项目声明 → current → legacy 兜底，每步异常路径）
   - 状态机完整（版本目录/指针/卸载/引用保护的状态转换）
   - 接口契约明确（resolve-entry ↔ 3 hook 薄壳 ↔ agate_common ↔ agate-summary 的接口）
   - 错误边界（resolve 失败回退 current 不静默禁用 gate 的 fail-closed 语义）
   - 测试策略（P3/P5 gate_commands 是否覆盖 31 BDD 关键验收）
   - 多方案探索（candidate_count=2 + 权衡 + 选择理由是否自洽；B 是否"明显更差的陪衬"）
   - 实现就绪度（files_to_read 是否覆盖影响面表全部联动点；P4 implementer 无步骤计划可否自主实现）
   - P2 最小验证（minimal_validation 是否含验证结果——本任务有 3 项已通过）
   - dispatch_plan（static-batch 3 批是否符合 high 复杂度硬规则 + 共享文件 agate_common 后处理声明）
3. **实质锚点要求**：结论必须引用具体设计锚点（§4.1~4.7 方案节 / files_to_read / minimal_validation / dispatch_plan），不得写裸 "approved"。
4. **关键技术决策审查**（本任务高风险点，重点核）：
   - §4.4 3 个内联 _agate_root 脚本归口 agate_common（引入 pyyaml 依赖，fail-closed 可接受？还是应保留内联？）
   - §4.3 resolve-entry 设计（install-hook 装固定入口、运行时读 .agate-version、Windows 复制模式 .agate-root 恢复）
   - §4.5 uninstall 引用保护扫描（扫 $HOME 下 .agate-version 的深度/mtime 限流合理性）
   - BDD-30 legacy 布局兜底（无 current/latest 时软链目标 = AGATE_ROOT）是否消除 P4 歧义
   - dispatch_plan 三批依赖声明（install/offline 依赖 resolve-chain 的 agate_common）
5. 若提出"后续应重构/存在架构债" → 用标准 DEBT 条目格式（模板 assets/templates/tech-debt-template.md，evidence 必填）。
6. 评审结论与 Header status 字段一致。
7. **双工作区纪律**：只读评审，不修改 worktree 内任何文件（除 P2-review.md 产出）。

### 上游关联
- architect 产出：P2-design.md（2 候选方案选 resolve-entry，static-batch 3 批，31 BDD 全覆盖）
- P1 基线：31 BDD + 影响面表（脚本层/文档层/测试层）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P2-design.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P1-requirements.md（需求基线 + BDD + 影响面表，核对设计覆盖）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P0-brief.md（范围锁定 + known_risks）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/AGENTS.md（项目约定，测试平台无关等）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0008-version-management；P2-design.md 已产出（308 行，candidate_count=2）
- 关键路径：AGATE_WORKSPACE=/home/kity/oclab/agate/.worktrees/agate-TAG0008/agate-workspace
- 查证结果：P2-design.md frontmatter 含 candidate_count=2 / packages=[agate] / domains=[backend, cli, security] / ui_affected=false / dispatch_plan（static-batch, 3 批）；minimal_validation result=confirmed（3 项实测通过）
</objective_info>
