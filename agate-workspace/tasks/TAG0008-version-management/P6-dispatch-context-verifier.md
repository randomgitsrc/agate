...
### 目标
P6 验收：逐条实跑 P1-requirements.md 的 BDD-1~31 验收条件（用户视角行为验证），产出 P6-acceptance.md + P6-evidence/（每条 PASS 有证据引用）。

### 约束
1. **验收依据**：P1-requirements.md BDD-1~31（31 条，全部实跑验证，不允许"调整/跳过/覆盖"中间态）。
2. **实跑原则**：每条 BDD 必须实际执行验证命令/脚本得到客观结果（不"看代码推断应该满足"）。可用 P5 测试结果作为佐证，但 P6 每条 BDD 要有独立验收动作（构造 Given 场景 → 跑 When → 检查 Then）。
3. **本任务 BDD 特点**：全部是 CLI/文件系统/git 行为（无 UI，ui_affected=false）——验收动作是：
   - 在 tmp_path / 假 HOME 构造场景（如 `~/.agate` 布局、`.agate-version` 文件、版本目录）
   - 运行 agate-install / agate-resolve / agate-summary / hook 相关命令（python_exe 探测 python3|python）
   - 检查退出码 + 输出字段 + 文件/软链存在性 + git worktree list
   - 已有 P3 测试（test_agate_version_*.py 等）覆盖大部分 BDD——可直接跑这些测试作为验收证据，但要补独立实跑（如真实 CLI 调用）增强可信度
4. **证据要求**：
   - P6-evidence/ 必须非空，每条 PASS 引证据文件（test 输出日志 / CLI 实跑日志 / 文件断言输出）
   - 证据文件含实质内容（命令输出、exit code、文件状态），不接受 1 行充数
   - 查询类 BDD（断言值是唯一证据）可不截图
   - 证据日志末行可加 `EXIT_CODE: <n>`（check-p6-provenance 审计 5）
5. **行格式**：P6-acceptance.md 每条 BDD 结果行首 `- PASS BDD-NN: 描述 (证据路径)` 或 `- FAIL BDD-NN: 描述`；总结行用 `**Summary**: ...`（禁止行首 `- PASS/FAIL`）。
6. **frontmatter**：pass / fail / ui_affected（false）必填。
7. **诚实边界**：无法验证的 BDD 标 FAIL 不标 PASS。"应该能过"不是证据。
8. **双工作区纪律**：只读验证 + 产出 P6 文件；不修改任何代码/文档（P6 阶段禁止代码变更——pre-commit 会拦截 phase=P6 暂存非证据文件）。`~/.agate` / 主 checkout 禁止改动。
9. **验证环境隔离**：所有验收动作在 tmp_path / 假 HOME（HOME+USERPROFILE env）内完成，不碰真实 ~/.agate。
10. **P5 复用**：可复用 P5-test-results/ 的测试结果作为部分 BDD 的证据（如直接跑对应测试文件输出落盘 P6-evidence/）。

### 上游关联
- P1-requirements.md BDD-1~31（验收依据，31 条）
- P5-test-results/（技术验证已过：823 passed + 单测 29 + consistency 0 ERROR）
- P4 实现（3 批代码已 commit）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P1-requirements.md（BDD 验收条件）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P5-test-results/（复用证据）
- {AGATE_WORKSPACE}/tasks/TAG0008-version-management/P2-design.md（方案 + gate_commands 参照）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/AGENTS.md（项目约定）
- 只读代码：agate/scripts/agate-resolve.py / agate-install.py / agate-pack-offline.py / install-offline.py / resolve-entry.py / agate_common.py（验证时了解 CLI 契约）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0008-version-management；P5 已过（failed=0）
- 关键路径：worktree=/home/kity/oclab/agate/.worktrees/agate-TAG0008；AGATE_WORKSPACE 任务目录 agate-workspace/tasks/TAG0008-version-management/
- 查证结果：BDD-1~31 均已在 P3 测试文件覆盖（test_agate_version_install / resolve / summary / hook_resolve_entry / pack_offline / install_offline）；P5 全量 823 passed
</objective_info>
