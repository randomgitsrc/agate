...
### 目标
修复 PR #148 Windows 冒烟失败 1（`test_csg_1_readme_triggers_warning`）。**根因已收敛到 git→hook 边界**，本任务做根因实证 + 修复 + 验证。这是两个失败中较复杂的一个，专注此任务即可（失败 2 由另一并行任务负责）。

### 约束
1. **已确认事实（直接采信，不要重做）**：
   - CI 实证：returncode==0（commit 成功）+ output 无 WARNING 无 GATE ERROR（仅 `[master d5620ee] update readme`）。
   - 薄壳 fail-closed 分支（GATE ERROR + exit 1）会阻断 commit（returncode!=0）→ 与 CI 矛盾 → **排除薄壳 fail-closed 触发**。
...
2. **git-for-windows 机制（已分析）**：`find_hook` 用 access(X_OK)，mingw_access 剥掉 X_OK 仅判文件存在 → copy 模式 hook 也会被 git 执行；`parse_interpreter` 读 shebang `#!/usr/bin/env bash` → interpreter 取 basename `env`（**丢掉 `bash` 参数！**）→ path_lookup("env") → 用 interpreter 重新构造 argv 执行。
3. **候选修复方向（验证后选一落地）**：
   - **方向 a（shebang 解析）**：Windows 上 git 用 `env` 解释器执行 hook（`bash` 参数被 mingw 剥掉）→ hook 实际以 `env <hook> args` 方式运行 → PATH 无 bash 或 env 不传 bash 参数 → hook 不执行/执行失败但 git 吞掉 → 无 WARNING + returncode 0。**验证**：Linux 上模拟（如创建无 shebang 或 env 前缀的 hook，用 `git commit` 触发看行为）。
   - **方向 b（测试适配）**：若确认 git→hook shebang 是 Windows 固有限制，改测试 `_setup_hook` 在 win32 用别的方式（如 `bash <hook>` 包装 / 软链）确保 hook 执行。**注意**：测试必须走 git 调用才有意义（验证 self-gate 触发），适配不能破坏意图。
   - **其他**：以你的实证为准，若发现更准确的根因（如 `_SELF_GATE_RE` 在 Windows 路径格式不匹配 staged 文件名），修实现或测试均可，但必须保持测试验证 self-gate 的意图。
4. **修复纪律**：
   - 改实现脚本（薄壳 / resolve-entry.py / commit-msg-self-gate.py）时按 TDD：先加/改测试确认红（Linux 模拟），再改实现确认绿
   - 改测试适配时须确认不破坏 self-gate 验证意图
...
6. **边界**：只修失败 1 相关文件。若发现需要超出此范围的大改，在 progress 标注停下，主 Agent 与主 checkout 确认。
7. **双工作区纪律**：worktree 内改代码；`~/.agate` / 主 checkout 禁止改动。

### 上游关联
- FIX-WINDOWS-TAG0008.md 失败 1（交接单）
- 上轮 implementer 诊断（P5-progress.md「implementer fix」节，git-for-windows 机制分析完整）

### 输入文件
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/tests/integration/test_commit_msg_self_gate_integration.py（失败 1 测试）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/scripts/commit-msg-self-gate.sh（薄壳）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/scripts/commit-msg-self-gate.py（self-gate 主程序）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/scripts/resolve-entry.py（解析入口）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate-workspace/tasks/TAG0008-version-management/P5-progress.md（上轮诊断）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/tests/conftest.py（run_cli / git_repo fixtures）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 分支 feat/TAG0008-version-management；PR #148 Windows 冒烟 2 failed
- 关键路径：worktree=/home/kity/oclab/agate/.worktrees/agate-TAG0008
- 查证结果：失败 1 根因收敛到 git→hook 边界（shebang 解析），候选方向 a/b；unit test_cmsg_1 Windows 冒烟通过证明薄壳链可用
</objective_info>
