...
### 目标
修复 PR #148 Windows 冒烟失败 2（`test_bdd_1_latest_pointer_after_noarg_install`）。**根因已实证**，本任务只做一处小修复 + 验证。

### 约束
1. **根因（已实证，直接采信）**：`_run_install` helper（test_agate_version_install.py:21-27）只设 `env={"HOME": str(home)}`，未设 `USERPROFILE`。Windows 上 `os.path.expanduser("~")` 优先用 USERPROFILE（ntpath 不认 HOME）→ `_agate_home()`（agate-install.py:66-68）解析到**真实用户目录** → `latest`/`current` 写到真实 `~/.agate` → 测试断言 `home/.agate/latest` 不存在。对照同批 `test_agate_version_resolve.py:17-19` 的 `_resolve_env` **正确设置了 HOME+USERPROFILE**——install 测试 helper 是遗漏。
2. **修复**：`test_agate_version_install.py` 的 `_run_install` helper 补 `env["USERPROFILE"] = str(home)`（与 resolve 测试一致）。这是**测试缺陷修复**（helper 遗漏），不是改测试迁就实现。
3. **验证**：
   - `python3 -m pytest agate/tests/unit/test_agate_version_install.py -q`（全绿）
   - `python3 -m pytest agate/tests/ -q`（全量无回归，基线 823 passed）
4. **边界**：只改 test_agate_version_install.py 的 `_run_install` helper，不碰其他文件。不扩大范围。
5. **双工作区纪律**：worktree 内改代码；`~/.agate` / 主 checkout 禁止改动。

### 上游关联
- FIX-WINDOWS-TAG0008.md 失败 2（交接单）
- 对照正确实现：test_agate_version_resolve.py:17-19 `_resolve_env`

### 输入文件
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/tests/unit/test_agate_version_install.py（修复对象）
- /home/kity/oclab/agate/.worktrees/agate-TAG0008/agate/tests/unit/test_agate_version_resolve.py（对照）
</dispatch_guide>
...
- 查证结果：失败 2 根因已实证（_run_install 漏 USERPROFILE，CI 路径 C:\Users\runneradmin\.agate）
</objective_info>
