...
### 目标
独立评审 P2-design.md 的前端/UI 设计维度（本任务方案定义的是"业务任务 ui_affected: true 时"的协议机制，不是本任务自身 UI），产出 P2-review-design.md，status ∈ {approved, rejected, needs-revision}。

### 约束
1. **只审不写**：不直接改 P2-design.md，产出评审意见由主 Agent 回派 architect 修改。
2. **评审聚焦**：本方案是协议机制增强（前端维度 = UI/UX 验收机制的协议设计）——重点评审：
   - BDD-4/5：ui_affected: true 任务 P2-design.md 必须含"UI 设计"节（布局/交互/视觉三类 checklist）的机制设计是否完整可行；architect 兼任产出（不新增 designer）的边界是否清晰
   - BDD-6：plan-design-review.md 维度表扩容（五维→七维：视觉设计 + 交互设计细节）的设计是否合理可判定
   - BDD-9/10/13：P6 双证据分档（available/supplementable → vision YAML；GAP → 像素检测+人工复核）、视觉质量 checklist、输入态人工复核的协议设计是否完整
   - BDD-1/2：UX 类别 BDD 基线要求是否可二值判定
3. **场景特殊性**：本任务自身 ui_affected: false（无 UI 产物，不动 UI 代码）；评审对象是"机制"——协议条文 + gate 逻辑 + 单测三件套覆盖是否足够，不是真实 UI 效果。
4. **不写死视觉工具**：机制设计必须保持视觉能力三态运行时探测，不得绑定具体工具名。
5. **评分维度表**：本角色现有五维（交互状态覆盖率/AI Slop 风险/移动端/可访问性/组件完整性），方案提议扩容——评审时对扩容设计本身给出意见（维度定义是否可 0-10 判定、是否遗漏关键 UX 维度）。
6. **锚点要求**：结论必须引用 BDD 编号 + 具体设计节（§2.x / §4 / §6），不裸 approved。
7. **返回**：`File: <路径>` + `Status: <approved|rejected|needs-revision>`。

### 上游关联
- architect 已产出 P2-design.md：candidate_count=4 候选方案（A 三态双门禁选定 / B 纯文档否决 / C 灰度否决 / D 平台无关对照）、BDD-1~15 逐条落实三件套、Windows GUI 评估"保持现状"、影响面核对清单（45 文件）、gate_commands 固化、dispatch_plan: single。
- 方案核心：P1 必须声明 vision 三态 → P2 ui_affected 必须含 UI 设计节 → P6 按三态分档消费视觉证据（GAP 走降级链）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（评审对象——主输入）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P1-requirements.md（需求基线：BDD 分组 + 影响面清单 + 能力声明）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P0-brief.md（任务简报：三个 RM + 修复方向）
- {project_root}/agate/assets/review-roles/plan-design-review.md（你的角色定义）
</dispatch_guide>
...
- 关键设计点：UI 设计节结构规格（§2.4 布局/交互/视觉 checklist 模板）、plan-design-review 扩容（§2.5 七维 + 0-10 评分项）、P6 双证据分档（§2.8 GAP 分支）、输入态判定标准（§2.12）、Windows 评估（§4 保持现状）。
- 现状核验：plan-design-review.md 现五维（交互状态覆盖率/AI Slop/移动端/可访问性/组件完整性），无视觉/交互设计维度——方案扩容方向正确。
</objective_info>
