...
### 目标
更新统一 P2-review.md（专家组组长汇总）：反映 P2 经 SCOPE+ 增补 + 修复轮后的最终状态——双专家均 approved，组长汇总判定 approved。

### 约束
1. **只汇总不新评审**：汇总已 approved 的双专家结论。
2. **组长规则（P2 卡片）**：任何专家 BLOCKER → rejected；分歧 → 交人工；全票无 BLOCKER → approved。
3. **本轮事实**：plan-design-review（第 4 轮修复复审）approved——B1/B2/M1/M2/M3 全闭合、BLOCKER 清零；plan-eng-review（SCOPE+ 增补复审）approved——0 阻塞，4 条非阻塞 NOTE 留痕 P3/P4。无 BLOCKER、无分歧 → 组长 approved。
4. **Header**：agent 必须是组长角色名（review），不能是 main。
5. **保留上轮汇总内容**：P2-review.md 上轮已 approved（原 15 BDD 设计）；本轮追加"SCOPE+ 组长汇总更新（2026-08-17）"节反映增补后的双 approved。
6. **返回**：`File: <路径>` + `Status: <approved>`。

### 上游关联
- plan-design-review（P2-review-design.md）：第 4 轮修复复审 approved（B1/B2/M1/M2/M3 闭合，BLOCKER 清零）——词汇表+规范值比对、-tN 分组豁免、颜色/光照/材质锚点、启发非绑定注、正例单测全部落实。
- plan-eng-review（P2-review-eng.md）：SCOPE+ 增补复审 approved（0 阻塞）——载体选择合理、825 基线不误伤实证成立、证据形式实现路径可测、4 条非阻塞 NOTE 留痕 P3/P4。
- 两专家均 approves，无 BLOCKER、无分歧 → 组长 approved。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review-design.md（专家 1，第 4 轮 approved）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review-eng.md（专家 2，SCOPE+ 复审 approved）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review.md（现有组长汇总，追加更新节）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（被评审对象最终版）
</dispatch_guide>
...
<objective_info>
- 双专家最终均 approved：
  - P2-review-design.md：status approved（第 4 轮修复复审，BLOCKER 清零）
  - P2-review-eng.md：status approved（SCOPE+ 增补复审，0 阻塞）
- 组长判定：approved（全票无 BLOCKER、无分歧）。
</objective_info>
