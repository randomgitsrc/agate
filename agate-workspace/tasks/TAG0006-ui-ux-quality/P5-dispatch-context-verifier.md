...
### 目标
执行 gate_commands.P5（全量 pytest）验证 agate UI/UX 验收机制实现，产出 P5-test-results/（unit.md + fail-list.txt），确认 exit 0 + failed=0。

### 约束
1. **本任务是 agate 协议本体增强**（dogfooding）：验证对象是 worktree 里 `agate/` 的脚本 + 测试。在 worktree（/home/kity/oclab/agate/.worktrees/agate-TAG0006/）跑，不碰主 checkout / ~/.agate。
2. **执行 gate_commands.P5**（P2-design.md 固化）：`python3 -m pytest -q --tb=no agate/tests/`。紧凑输出模式（`--tb=no`）。
3. **本任务 ui_affected: false**（协议机制增强，无 UI 产物），无 P5_e2e（P2 未声明）——不跑 Playwright。
4. **全量测试**：P5 应运行全量测试套件（agate/tests/ 全部）。发现预存失败在 unit.md 标注"预存失败：X（与本次改动无关）"，不阻断。
5. **判定规则**：
   - exit 0 + failed=0 → 通过
   - exit ≠0 或 failed>0 → 在 fail-list.txt 列出失败 + unit.md 记录，返回主 Agent 判定（真 bug 回 P4 / 环境问题修环境）
   - 无 [PROD_TOUCHED]（触发写 `[PROD_TOUCHED]`，未触发写 `[PROD_NOT_TOUCHED]`）
6. **产出规格**：
   - P5-test-results/unit.md：标注 failed 数量 + 通过/失败汇总 + 测试输出签名（含 PASSED/FAILED/passed/failed 行）
   - P5-test-results/fail-list.txt：failed 测试 id 逐行（`FAILED ` 前缀），无失败可空文件
   - 两者用 Write 工具写入约定路径
7. **自查≠gate**：verifier 产出 P5-test-results/，主 Agent 验 gate + 签名校验；不要声称"已通过"，只返回路径 + failed 计数。

### 上游关联
- P4 实现已 commit：7 gate 脚本 + 18 协议文档 + 测试；全量 881 passed + 2 skipped（P4 自查）。
- gate_commands.P5 = `python3 -m pytest -q --tb=no agate/tests/`（P2 §3 固化）。
- P2-design.md §3：P5 无 formatter 声明时就 exit-code-only；本任务有 P5_formatter: pytest.sh。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（gate_commands.P5 读取）
...
<objective_info>
- 测试环境：worktree 全量 pytest 可跑（P4 自查 881 passed + 2 skipped）。
- gate_commands.P5 = `python3 -m pytest -q --tb=no agate/tests/`。
- 本任务 ui_affected: false，无 P5_e2e。
- 基线：881 pytest + consistency 0 ERROR + count-tests。
</objective_info>
