...
### 目标
作为 P2 专家组组长，汇总 plan-design-review（前端/UI 维度）与 plan-eng-review（工程/架构维度）两份评审，产出统一的 P2-review.md（status: approved）。

### 约束
1. **只汇总不新评审**：不发表新意见，不重新评估方案；组长职责是把两个专家的结论汇总成统一 status。
2. **组长规则（P2 卡片）**：
   - 任何专家标 BLOCKER → status: rejected
   - 多位专家分歧 → 标「专家组分歧」交人工
   - 全票无 BLOCKER → status: approved
3. **本轮评审事实**：plan-design-review 与 plan-eng-review 均已 `approved`（第二轮复审后），无 BLOCKER、无分歧 → 组长判定 approved。
4. **锚点要求**：P2-review.md 须引用两份专家评审文件路径 + 各自 status + 覆盖面（BDD 编号），不裸 approved。
5. **Header**：agent 字段必须是组长角色名（review），**不能是 main**——check-gate.py P2 硬拦截 agent=main 的 approved。
6. **返回**：`File: <路径>` + `Status: <approved|rejected>`。

### 上游关联
- plan-design-review（P2-review-design.md）：approved——B1（BDD-4 触发条件矛盾）+ M1/M2/B4 全部解决，方案 A 未推翻，兼容声明独立核实成立。
- plan-eng-review（P2-review-eng.md）：approved——0 阻塞；B2（GAP 默认语义）/B3（vision-blocked fixture）均实证核验闭合，N1/N2 落实，DEBT0005 已登记。
- 两专家无分歧、无 BLOCKER → 组长 approved。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review-design.md（专家 1：plan-design-review，已 approved）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review-eng.md（专家 2：plan-eng-review，已 approved）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（被评审对象）
</dispatch_guide>
...
<objective_info>
- 双专家均 approved（2026-08-17 第二轮复审后）：
  - P2-review-design.md：status approved（B1/M1/M2/B4 解决，评分视觉 8/交互 7 等）
  - P2-review-eng.md：status approved（0 阻塞，B2/B3 实证闭合，DEBT0005 已登记）
- 无 BLOCKER、无分歧 → 组长判定 approved。
</objective_info>
