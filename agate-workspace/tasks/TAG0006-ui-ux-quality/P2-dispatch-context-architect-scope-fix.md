...
### 目标
修复 P2-design.md 的 SCOPE+ 增补未闭合点（plan-design-review 第三轮复审发现的 2 个 BLOCKER + 3 条建议），使方案自洽后重新提交复审。

### 约束
1. **增量修改**：只修 P2-design.md，不碰 P1 产出与评审文件。修改点集中在 §2.15.1（词汇表）/ §2.16（-tN 豁免）/ §2.3（匹配语义）/ §2.4（checklist 明示）/ §2.13（豁免分组）等节。
2. **BLOCKER 级必修（2 项）**：
   - **B1（plan-design-review BLOCKER-1）**：P1-P2 形态值词汇表与匹配语义未定义。修正：在 §2.15.1 定义**规范形态值词汇表**（如 `layout`/`render_component`/`temporal_effects` 为规范值）与「P1 值 ↔ P2 声明标签」同义映射表（layout↔布局型、render_component↔渲染组件型、temporal_effects↔时序特效型），明确 P2 声明行复用规范值并附中文注释；gate 匹配语义改为"规范化值比对"（P2 声明行解析出规范 token 与 P1 字段值比对）——杜绝 ASCII 与中文永不匹配的问题。
   - **B2（plan-design-review BLOCKER-2）**：时序截图 `-tN` 系列雷同分组豁免未显式扩展。修正：§2.16 显式声明「同 BDD 的 `-tN` 时序截图系列与 frames/ 帧序列同权获取分组豁免」（分组键 = bdd-id 前缀，`{bdd-id}-t1..tN` 与 `{bdd-id}-NN` 统一按同 bdd 组豁免相邻样本）；补单测（同 bdd 的 t1/t2 视觉相近 → 不触发雷同降级 exit 0）。
3. **MAJOR 级落实（3 条建议）**：
   - **M1（建议 1）**：§2.4 渲染正确性 checklist 补「颜色/光照/材质归入参考对比锚点」明示。
   - **M2（建议 2）**：§2.3 step4 关键词列表补「启发非绑定」注——「画布/图表/模型/特效/地图/数字地球」为产品域启发词不构成技术栈要求，形态判定以 ui_render_shape/维度选择为准。
   - **M3（建议 3）**：补充词汇表匹配正例单测（P1 值与 P2 声明为规范值 → 一致 → exit 2）。
4. **不得推翻已 approved 内容**：方案 A、§2.15/§2.16 增补主体、33 处 BASELINE_CHANGE 标注、gate_commands（§3）与 dispatch_plan（§8 single）全部保留；只修未闭合点。
5. **不写死技术栈**：词汇表是协议内部规范化值（layout/render_component/temporal_effects），与"不绑定 WebGL 等技术栈"不冲突——技术栈中立不变。
6. **兼容性**：词汇表定义后，既有 task 无形态声明仍走布局型默认（presence 语义），基线不红。

### 上游关联
- plan-design-review 第三轮复审（SCOPE+）：needs-revision。BLOCKER-1（P1-P2 词汇表未定义，I14 匹配语义落空）+ BLOCKER-2（-tN 时序截图雷同豁免未扩展）+ 建议 1/2/3。
- plan-eng-review 第三轮复审（SCOPE+）：approved（0 阻塞，4 条非阻塞 NOTE 留痕 P3/P4 落地——不需本修复轮处理，P3 时纳入 dispatch-context）。
- P1 已增补 + 复审 approved（17 BDD）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（修复对象——主输入）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-review-design.md（评审意见：B1/B2 + 建议 1/2/3，见 §3/§4）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-dispatch-context-architect-scope.md（增补轮派发约束）
...
<objective_info>
- 修复对象：P2-design.md（740 行，SCOPE+ 增补已完成，33 处 BASELINE_CHANGE）。
- 评审状态：plan-design-review needs-revision（B1 词汇表/B2 -tN 豁免）；plan-eng-review approved。
- 关键现状：P1 frontmatter 字段值示例为 ASCII（layout/render_component/temporal_effects），P2 声明行用中文标签（布局型/渲染组件型/时序特效型）——二者需词汇表映射闭合。
- 时序截图 `{bdd-id}-tN.png` 放 screenshots/ 目录，会被既有整目录 md5/ahash 去重扫到 → 需按同 BDD 组豁免。
</objective_info>
