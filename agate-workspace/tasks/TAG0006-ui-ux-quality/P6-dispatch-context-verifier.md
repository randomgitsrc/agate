...
### 目标
产出 P6-acceptance.md + P6-evidence/：逐条对照 P1 的 17 条 BDD 验收 agate UI/UX 验收机制实现，每条 PASS/FAIL + 证据引用。全部 PASS（FAILD=0）才允许推进。

### 约束
1. **本任务是 agate 协议本体增强**（dogfooding）：验收对象是 worktree 里 `agate/` 的 gate 脚本 + 协议文档 + 测试。在 worktree（/home/kity/oclab/agate/.worktrees/agate-TAG0006/）跑，不碰主 checkout / ~/.agate。
2. **本任务 ui_affected: false**（协议机制增强，无真实 UI 产物）——P6 验收以"脚本单测 + 文档条文内容"为客观证据（P1 §3 验收方式约定），不做真实 UI 视觉验收。**不派 vision-analyst**（本任务无 UI 产物）。
3. **BDD 逐条对照**（P1 的 17 条）：BDD-1~17。每条只允许 PASS/FAIL，不允许中间态。所有 PASS 必须引用 P6-evidence/ 下的证据文件（test 输出 / 文档 grep 结果）。
4. **验收标准**（P1 §3 验收方式）：客观证据 = ① 对应协议文档/角色文件是否含该行为要求（read/grep）；② 对应 gate 脚本单测（pytest）是否覆盖并断言该行为。二者兼备才允许 PASS。
5. **证据文件**：P6-evidence/ 至少含两类——测试输出日志（test-output.log，全量或针对性 pytest 实跑输出）+ 能力覆盖证明（跑对应的单测模块确认 BDD 对应测试通过，输出落盘）。每条 PASS 引用具体证据路径。
6. **PASS 行格式**（行首 `- PASS BDD-NN: 描述 (证据路径)`）；FAIL 同理。frontmatter 声明 pass:/fail:/ui_affected:。**大小写敏感（PASS/FAIL）**。
7. **BDD 覆盖完整性**：P1 有 17 条 BDD，P6 必须有 ≥17 条验收结果（可 SCOPE+ 增补），不能挑验。
8. **无 [PROD_TOUCHED]**（触发写 `[PROD_TOUCHED]`，未触发写 `[PROD_NOT_TOUCHED]`）。
9. **自查≠gate**：verifier 产出 P6-acceptance.md + P6-evidence/，主 Agent 验 gate（check-gate.py P6 + check-p6-evidence.py + check-p6-provenance.py）；不要声称"验收已通过"，只返回路径 + 摘要。

### 上游关联
- P1-requirements.md：**17 条 BDD**（BDD-1~15 + SCOPE+ BDD-16/17），每条有"验收方式"声明。
- P4 已实现 + P5 验证通过（881 passed/0 failed）。
- P2-design.md：§2.1-2.16 各 BDD 三件套（协议条文落点 + gate 逻辑 + 单测规格）。
- 本任务能力三态：pytest-test-runner available、pyyaml available、visual-analysis supplementable（本任务无 UI 产物不依赖真实视觉）、gui-e2e-framework-win supplementable。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P1-requirements.md（17 BDD 验收条件——主输入）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P2-design.md（方案设计，§2 BDD 三件套）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P5-test-results/（P5 验证产出）
- {project_root}/agate/assets/execution-roles/verifier.md（你的角色定义）
- {project_root}/agate/scripts/*.py、agate/*.md（验收对象）
- {project_root}/agate/tests/unit/test_check_gate.py、test_check_p6_evidence.py、test_check_p6_provenance.py、test_review_role_docs.py（BDD 对应单测）
</dispatch_guide>
...
<objective_info>
- 验收对象：17 BDD（P1 组 BDD-1/2/3；P2 组 BDD-4/5/6/7/8；P6 组 BDD-9~14；兼容 BDD-15；SCOPE+ BDD-16/17）。
- 证据基础：P5 已全量 881 passed/0 failed；BDD 对应单测模块存在（test_check_gate 20 / test_check_p6_evidence / test_check_p6_provenance / test_review_role_docs 14 等）。
- 本任务 ui_affected: false，无 P5_e2e，不派 vision-analyst。
- 验收方式：读文档 + 跑单测，二者兼备才 PASS。
</objective_info>
