...
### 目标
修复 P1-requirements.md 的评审发现（GAP 降级链语义冲突），使需求基线满足 requirements-review 全部判定后重新提交复审。

### 约束
1. **增量修改**：只改两处——评审发现的 BDD-9 语义冲突 + BDD-15 表述精确性（视你判断 BDD-15 是否值得一并修，最小动作为必须）。
2. **不得改动已 approved 的 14 条 BDD 语义**：BDD-1~8、BDD-10~14 已通过逐条评审，不得重写或改编号；BDD-15 仅改表述（不改变语义与编号）。
3. **编号连续性**：若采纳"新增 BDD-16"方案，编号保持 1-16 连续不跳号；若采纳"修改 BDD-9"方案，编号维持 15 条不变。
4. **修复必须二值可判**：GAP 场景的 P6 验收路径必须可明确判定（如"vision=GAP 时以像素检测 + 人工复核记录为视觉证据，不要求 vision YAML"）。
5. **不写死视觉能力工具**：GAP 分支描述不得绑定具体工具名。
6. **Header 保持**：phase/task_id/type/parent/trace_id/status/created/agent 不变；status 保持 draft（评审产出才改 approved/rejected/needs-revision）。
7. **只改 P1-requirements.md**：不碰 P1-review.md（评审记录冻结，主 Agent 保存）。

### 上游关联
- requirements-review 已评审 P1-requirements.md（status: needs-revision）。核心发现（发现 1）：
  BDD-9 Then"每条 UI 类 PASS 必须同时含运行时证据 + 视觉证据（vision YAML 引用）"未限定 capability 状态；而 RM-AG0004① 三态中 GAP → 降级为"双证据截图+行为日志 + 像素检测 + 人工复核"——GAP 任务无法产出 vision YAML 引用，BDD-9 对其不可满足。诊断：需求基线描述机制对任意任务的行为，GAP 分支必须有定义。
- 发现 2（minor）：BDD-15 "既有 823 用例（含新增用例）全绿"表述易误读为"总数必须仍是 823"，建议改"既有 823 基线用例全绿 + 新增用例全绿"。
- 评审其余部分全部通过：15 条 BDD 二值可判、编号连续、无工具名绑定、零裁剪理由充分、能力三态正确、影响面清单详实。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P1-requirements.md（修复对象——主输入）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P1-review.md（评审意见——发现 1/2 的具体建议见 §发现节）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P1-dispatch-context-analyst.md（原始派发约束，修复不得违反）
- {AGATE_WORKSPACE}/tasks/TAG0006-ui-ux-quality/P0-brief.md（需求来源）
</dispatch_guide>
...
<objective_info>
- 评审状态：needs-revision（阻塞点 1 项：BDD-9 与 GAP 降级链语义冲突）。
- P1-review.md 建议方案：① 修改 BDD-9 限定视觉证据来源（available/supplementable → vision YAML；GAP → 像素检测+人工复核记录）；或 ② 新增 BDD-16 定义 GAP P6 验收路径。任选其一。
- 本任务自身声明 visual-analysis 为 supplementable（不暴露 GAP 坑），但需求基线须描述机制对任意任务的行为。
</objective_info>
