...
<dispatch_guide>
> ⚠️ 复核轮派发（P1 第 1 次重试的 review 复核，共享 retry 预算 MAX=3）。增量模式：只复核打回项
> 修复是否闭环，其余沿用首轮评审结论（BDD-1~21 逐条成立已确认，不重审）。

### 复核目标

对修复后 `P1-requirements.md` 做增量复核，产出更新后的 `P1-review.md`：

1. 核对打回项 1 已修复：frontmatter `phases` 含 P2（期望 `[P1..P8]` 全 8 项），与 §3"P2 不可裁 /
   全覆盖、无裁剪"一致。主 Agent 已预验证 grep 含 P2 + check-frontmatter exit 0——reviewer 须独立复核一次。
2. 核对打回项 2 已修复：BDD-6 Then 含"dispatch-context 模板含 BDD-5 载体锚词（模板锚词同锁）"，
   载体系补模板路径 + 清理/残留/环境还原锚词 grep 断言。
3. 确认打回项 3 未动（信息项）：BDD-16 落点"或"表述保留。
4. 其余不变项抽查：BDD 仍 21 条连续（`^#### BDD-` 计数 21）；正文无行首 `- PASS` / `- FAIL`；
   无 `[NEED_CONFIRM]` / `[CAPABILITY_GAP]`。
5. 全部通过 → `status: approved`（agent=requirements-review，含 BDD 编号锚点）；
   仍有问题 → `needs-revision` 并列出剩余项（主 Agent 继续回派 analyst）。

### 上轮产出路径

- 上轮 review：`P1-review.md`（status: needs-revision，本轮直接改此文件：保留首轮评审正文，
  追加「复核结论」节并更新 status）
- 上轮 analyst 修复：`P1-requirements.md`（已改：phases 补 P2 + BDD-6 补模板锚词锁）
- 上轮 review 约束：`P1-dispatch-context-requirements-review.md`（继续有效）

### 约束

1. 只审不写：不直接改 P1-requirements.md。
2. status 字段与结论一致（gate 读的就是这个字段）。
3. 复核正文禁止行首 `- PASS` / `- FAIL`。
4. 返回前重跑 check-frontmatter.py 确认 P1-review.md 自身合法。

### 产出文件字段

直接改 `P1-review.md`（status 用 agate-md-field-set 更新为 approved 或 needs-revision）。
</dispatch_guide>
...
<objective_info>
- 修复依据：analyst fix1 已完成（主 Agent 亲自验证：phases 含 P2 + BDD-6 含模板锚词锁 + BDD 21 条连续 + frontmatter exit 0）
- Card 块由 agate-inject-card.py 注入
</objective_info>
