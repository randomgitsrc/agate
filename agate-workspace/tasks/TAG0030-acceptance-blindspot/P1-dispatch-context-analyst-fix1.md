...
<dispatch_guide>
> ⚠️ 修复轮派发（P1 第 1 次重试，共享 retry 预算 MAX=3）。增量模式：复用上轮约束，只做下述修复目标。

### 修复目标（来自 P1-review.md「打回项」）

1. （硬阻断）frontmatter `phases` 补 P2 → `[P1, P2, P3, P4, P5, P6, P7, P8]`，
   与 §3"P2 不可裁 / 全覆盖、无裁剪"对齐；否则 check-pruning "P2 不可裁剪"硬闸 P2 门直接 exit 1。
   用 agate-md-field-set 改 `phases` 字段，不要手写 frontmatter。
2. （次要）BDD-6 载体补 dispatch-context 模板锚词锁定（否则 BDD-5 条目被删不断言转红），
   或在 BDD-6 Then 加"模板锚词同锁"一句。
3. （信息项，无需改）BDD-16 落点"或"表述 P1 保留，由 P2 影响面梳理 pin 定落点文件（P7 核对）——
   确认 P1 不做改动即可。

### 上轮产出路径

- `agate-workspace/tasks/TAG0030-acceptance-blindspot/P1-requirements.md`（本轮直接改此文件）
- 上轮 dispatch-context：`P1-dispatch-context-analyst.md`（约束 1-11 继续有效，本轮修复不得违反，
  尤其约束 1 范围锁定 + 约束 8 BDD 二值性）

### 约束

1. 只改打回项 1-2，§0-§9 其余内容不动（review 已逐条确认成立，打回项外不重写）。
2. 改完后重跑 `python3 agate/scripts/check-frontmatter.py <文件>` 确认 exit 0（worktree 自己的脚本）。
3. grep 确认 `phases` 已含 P2 且 BDD 仍为 21 条连续。
4. 正文无行首 `- PASS` / `- FAIL`。

### 产出文件字段

直接改 `P1-requirements.md`（frontmatter 用 agate-md-field-set 改 `phases`）。
`need_confirm_resolved/suggest_resolved/scope_resolved` 保持现状（首次产出留空，无需动）。
</dispatch_guide>
...
<objective_info>
- 修复依据：`P1-review.md`（status: needs-revision）「打回项」1-3
- Card 块由 agate-inject-card.py 注入（手工路径唯一合法方式）
</objective_info>
