...
P5 技术验证（verifier subagent 模式）：执行 P2-design §5 gate_commands.P5 的全部命令，
产出 `P5-test-results/`（unit.md + fail-list.txt），逐条判定通过/失败。本任务是**纯协议文档面
改造**（14 个协议文件条文新增 + 断言审计单测），验证 = 全量测试回归 + 一致性 + shellcheck +
用例计数。

### gate_commands.P5（P2 固化，逐 key 独立执行，禁 && 链路）
...
P5_consistency: "python3 agate/scripts/check-protocol-consistency.py --strict-errors-only"  # 120s
...
P5_count: "bash agate/tests/scripts/count-tests.sh"  # 120s
```

### 约束

1. **只读验证**：P5 是 external-output-gate，只跑命令 + 写 P5-test-results/，**不改任何代码/文档**
   （发现需修复 → 报告主 Agent 回 P4，不在 P5 顺手改）。标记 `[PROD_NOT_TOUCHED]`。
2. **命令超时兜底**：每条命令外层 timeout（值 = 声明 timeout_seconds ×1.5）：
   unit/regression `timeout 450s`、integration `timeout 900s`、consistency `timeout 180s`、
   shellcheck `timeout 90s`、count `timeout 180s`。分片命令已含 `-n auto` 并行（套件按隔离设计
   可安全并行，CI 同口径）。
3. **全量回归**：三片全部运行（unit/regression/integration），不得只跑改动相关片
   （T027 教训：修复可能引入回归）。每片汇总输出写 P5-test-results/{unit,regression,integration}.md，
   标注 failed 数量。
4. **fail-list.txt**：failed 测试 id 逐行列出（`FAILED ` 前缀），无失败时为空文件。若全部通过，
   空文件即可（P5 gate 检测缺失时优雅降级 WARNING，不新增拦截）。
5. **预存失败处理**：若发现改动前就存在的失败，在对应 md 标注"预存失败：X（与本次改动无关）"，
   不阻断推进；主 Agent 按修复成本判断。
6. **输出签名**：unit.md 末行含可解析退出码声明 `EXIT_CODE: <n>`（M1.3a 约定）。
7. **无行首预判格式**：P5-test-results/*.md 正文允许 `- PASS`/`- FAIL` 以外的格式（本阶段产出
   是测试汇总，不含验收判定）；但不要写 `- PASS BDD-NN` 类行（那是 P6 的格式）。
8. **consistency 必须用 worktree 自己的脚本**（`python3 agate/scripts/check-protocol-consistency.py`，
   不读 ~/.agate 的——检查对象是 worktree 里的协议文件）；shellcheck 用系统二进制。
9. **环境基线**：P3/P4 已跑过 capture-env-baseline（无 formatter 放弃捕获属正常）；P5 跑全量
   即验证。若某片超时/挂起，停止该片、记录、报告主 Agent（不换命令不深挖）。
10. **自查≠gate**：你产出 P5-test-results/ 后由主 Agent 验 gate（检查目录存在 + failed 计数 +
    签名），不自称"P5 已过"。

### 上游关联

- P2-design.md §5 gate_commands（唯一命令源）+ §9 完成标志（consistency 0 ERROR + 全量零失败
  + 用例数较基线 +N）
- P3 commit 167a044（断言审计测试 21 用例，P4 后应全绿）
- P4 commit e39c897（14 协议文件落笔）+ SELF-GATE 3c2d647（aligned）
- 既有测试基线：count-tests 1436 用例（P1 时点）；本任务新增 21 审计用例 → 期望 1457±

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P2-design.md`（§5 gate_commands + §9 完成标志）
2. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P1-requirements.md`（BDD 背景，按需）
3. `agate/assets/execution-roles/verifier.md`（角色定义）
4. `AGENTS.md`（worktree 根，测试约定 + 工具纪律）

### 产出文件字段

产出 `P5-test-results/` 目录（unit.md + regression.md + integration.md + fail-list.txt +
consistency.md + shellcheck.md + count.md），写在任务目录下
`agate-workspace/tasks/TAG0030-acceptance-blindspot/P5-test-results/`。
每份 md 用 agate-md-field-set 填写 frontmatter（phase=P5, task_id=TAG0030,
trace_id=TAG0030-P5-20260904, agent=verifier, status=draft；无 schema 约束的按惯例手写）。
</dispatch_guide>
...
- P5-test-results 产出目录 = 任务目录下（`.gitignore` 可能忽略，若忽略主 Agent 用 git add -f）
- 双工作区：跑 gate 用 ~/.agate 稳定版；**consistency 用 worktree 自己的**；改代码在 worktree

### B. 已知验证预期（非结论，供核对）
- 断言审计测试 `agate/tests/unit/test_tag0030_assertions.py`：P4 落笔后应 21/21 全绿（P3 红灯 → P4 转绿）
- 双保险既有测试：test_review_role_docs.py（14 用例）+ test_protocol_mechanism_anchors.py（28 用例）
  应保持全绿（CHECK11 三锚词 + 既有机制锚词未被误删）
- consistency：P4 后 0 ERROR（329 WARNING 为存量陈旧引用，非本任务引入）
- shellcheck：三个 hook 薄壳 0 error（本任务未改脚本，应保持）
- count-tests：较 1436 基线 +21（新增审计用例）
</objective_info>
