...
独立评审 `P2-design.md`（TAG0030 验收盲区机制批：RM-AG0057 四类 + DEBT0024/25/26，纯协议文档面
改造），产出 `P2-review.md`。评审重点是 architect 的作者盲区：候选方案 ≥2 + 权衡（v0.6 nudge）、
实现就绪度（files_to_read 覆盖）、影响面梳理完整性、gate_commands 固化正确性、范围锁定、
门槛契约保持（0-10/status / CHECK11 三锚词 / 形态机制不重构）、最小验证。

**只审不写**——不直接改 P2-design.md，评审意见由主 Agent 回派 architect 修改。

### 约束

1. **评审结论引用具体锚点**（plan-eng-review 角色「输出结构」+ P2 卡「实质锚点要求」）：
   结论须引用 P2-design.md 具体节/行（影响面三栏 / §1 候选 / §2 phase 详述 / §3 BDD-16 pin 定 /
   §4 连带同步 / §5 gate_commands / §6 files_to_read / §7 env / §8 minimal_validation），
   不写裸 "approved"。
2. **TAG0030 核心约束核对**（对照 P1 约束 1-11 + P2 dispatch 约束 1-10，尤其）：
   - 范围锁定：不重构形态声明 / 不改 check-gate 判据 / 不实现清理运行器 / TAG0029+TAG0031 out-of-scope
   - plan-design-review 保持 0-10 + status 门槛契约（gate 读 status；无 P1/P2 形态声明回落布局型默认）
   - 视觉契约「可表达子集」边界（仅五类 DOM 度量，不收主观视觉）
   - DEBT0026 边界（只补派发模板拆小默认指导，不重复实现 TAG0028 §4）
   - BDD-16 落点 pin 定（§3：architect.md 视觉 checklist 头部单源）
   - role-system 行 47「七维」同步为形态驱动口径（§4 判 Modify）
   - CHECK11 三锚词保持（§4 禁动三行）
3. **实现就绪度**：files_to_read（13 项带行号/批次）是否覆盖 P4 三包（phase-cards / assets-roles /
   templates-tests-meta）批实现所需全部上下文；dispatch_plan static-batch 三批是否合理。
4. **gate_commands 固化核对**：P3 断言审计命令 + P5 回归命令（consistency/shellcheck/全量 pytest）
   独立 key、无 && 短路（DEBT0012）；per-key timeout_seconds 声明合理。
5. **P2 最小验证**：§8 minimal_validation 含"纯代码逻辑，无外部系统依赖"声明 + 理由 + 已做的最小
   验证动作（grep 拟新增锚词 × CHECK14/15 零冲突 + CHECK11 锚点保持可行）。
6. **技术债**：若提出"后续应重构/架构债"→ 必须用标准 DEBT 条目格式（模板
   `agate/assets/templates/tech-debt-template.md`，evidence 必填），登记 `{AGATE_WORKSPACE}/debt/tech-debt.md`；
   不提则不产 DEBT。
7. **评审产出 Header**：P2-review.md frontmatter 用 agate-md-field-set 填写：phase=P2 /
   task_id=TAG0030 / trace_id=TAG0030-P2-20260904 / agent=plan-eng-review /
   status（初始 draft，评审完成后改 approved|rejected|needs-revision——gate 读的就是这个字段）。
8. **无行首预判格式**：评审正文禁止行首 `- PASS` / `- FAIL`（provenance 审计拦截）。

### 上游关联

- `P2-design.md` 是唯一评审对象（上游输入：P1-requirements.md 21 BDD + P0-brief）
- C8 映射：domains=[backend] + risk_level=high + ceremony=standard + 无 NEED_CONFIRM → 仅触发
  plan-eng-review（单评审角色，产出直接写 P2-review.md，无需组长汇总）
- P2 gate 规则：P2-review.md 存在 + status:approved + agent≠main → exit 2；缺失/agent=main → exit 1

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P2-design.md`（评审对象，241 行）
2. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P1-requirements.md`（BDD-1~21，核对方案→BDD 映射）
3. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P0-brief.md`（范围/约束）
4. `agate/assets/review-roles/plan-eng-review.md`（角色定义，评审清单权威）
5. `agate/assets/review-roles/plan-design-review.md`（Phase 3 改造对象，核对门槛契约保持方案）
6. `agate/scripts/check-protocol-consistency.py`（CHECK11 行 ~895-943 三锚词口径，核对 §4 表述）
7. `AGENTS.md`（worktree 根，双工作区纪律 + DEBT0025 载体）

### 产出文件字段

用 `FILE={AGATE_WORKSPACE}/tasks/TAG0030-acceptance-blindspot/P2-review.md agate-md-field-set --list`
查看本阶段应填字段；逐个写入；写入失败照提示修正，不要手写 frontmatter。

frontmatter 必填：phase=P2, task_id=TAG0030, trace_id=TAG0030-P2-20260904,
agent=plan-eng-review, status=（draft → 评审结论 approved|rejected|needs-revision）。
</dispatch_guide>
...
- 改造对象 = worktree 的 `agate/`；主 checkout 禁止改动；consistency 用 worktree 自己的脚本

### B. P2-design.md 事实摘要（评审核对用，非结论）
- frontmatter：candidate_count=2 / packages=[agate-phase-cards, agate-assets-roles,
  agate-assets-templates] / domains=[backend] / ui_affected=false /
  dispatch_plan={mode: static-batch, parallel_limit: 3, batches: [{phase-cards, medium},
  {assets-roles, medium}, {templates-tests-meta, low}]}
- §0 影响面三栏（Modify 9 行文件落点表 + Not Modify + Risk 3 条）
- §1 候选方案 A（单源提及式，推荐）/ B（分散定义式，备选）+ 选择理由 + 稻草人自检
- §2 四 phase 改动详述（Phase1 BDD-1~6 / Phase2 BDD-7~9 / Phase3 BDD-10~15 / Phase4 BDD-16~21）
- §3 BDD-16 落点 pin 定：architect.md 视觉 checklist 头部（BDD-16 定义 + BDD-17 提及同文件单源）
- §4 连带同步：role-system 行 47 判 Modify（同步形态驱动口径）；CHECK11 三锚词禁动
- §5 gate_commands：P3 断言审计命令 + P5 多 key（consistency/shellcheck/全量 pytest）独立无 &&
- §6 files_to_read 13 项带行号 + 批次 + why
- §8 minimal_validation：「纯代码逻辑，无外部系统依赖」+ grep 验证输出（21 锚词 × CHECK14/15 零冲突）
