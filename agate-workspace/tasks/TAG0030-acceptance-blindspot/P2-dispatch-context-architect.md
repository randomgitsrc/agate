...
产出 `P2-design.md`：把 P1-requirements.md（BDD-1~21）转为可执行的协议文档改造方案，供 P3 断言审计
测试设计与 P4 批量落笔使用。本任务是**纯协议文档面改造**（改 worktree 的 `agate/`：phase-cards +
assets/roles + assets/templates + tests/README + UPGRADING + 断言审计测试），不新增脚本运行时行为。

范围严格锁定 P1 四 phase（BDD 分组即 phase 分组：Phase1→BDD-1~6 / Phase2→BDD-7~9 /
Phase3→BDD-10~15 / Phase4→BDD-16~21），候选方案 ≥2 + 权衡 + 选择理由。

### 约束

1. **范围锁定**（P0-brief + P1 §2 范围复核）：不重构形态声明机制；不改 check-gate.py 既有判据
   （含 `_gate_p1_ui_shape`）；不实现清理钩子运行器；gate 解析器归 TAG0029、check-gate 健壮性
   归 TAG0031。超出即停报主 Agent。
2. **plan-design-review 保持门槛契约**：0-10 评分输出格式 + `status` 字段（门槛读 status）不变，
   只加形态分组内部逻辑。无形态声明时回落布局型默认。
3. **视觉契约「可表达子集」**：只收五类 DOM 度量（宽度/高度/对齐/重叠/溢出），不收主观视觉；
   P2/P6 指南须防"所有视觉都必须断言"误解。
4. **DEBT0026 边界**：只补 dispatch-context 模板「>5 文件/大文档拆小默认指导」，不重复实现
   TAG0028 §4 内部自主拆（已落地：role-system 权限边界 + dispatch-protocol §4.1 + judge 例外）。
5. **断言审计 TDD 落点**：P3 写 grep 断言审计测试（P1 BDD-6 载体），P4 批量改文档后转绿。
   断言审计测试文件落 `agate/tests/`（unit 面，具体文件名/用例组织由 P3 test-designer 定，
   P2 只定"测什么 + 锚词清单"，不限定文件名）。
6. **影响面梳理三栏齐全**（改什么 / 不改什么 / 风险在哪），写在候选方案之前，带 grep 证据。
7. **连带同步点**（P1 §7 #5 / §9）：改 plan-design-review 维度表述须保持 consistency CHECK11
   三锚词（「视觉设计/交互设计/渲染正确性与时序」）仍在 + role-system.md 行 47「七维」描述同步为
   形态驱动口径（P7 核对项，P2 列为 Not Modify 或 Modify 二选一并给理由）。
8. **BDD-16 落点 pin 定**（review 打回项 3 要求）：P1 保留"或"表述，P2 必须 pin 定视觉契约落点文件
   （P2 视觉 checklist 在 architect.md 或 P2 卡；P6/verifier 指南在 verifier.md 或 P6 卡——
   以「提及可表达子集」为度，不新造机制）。
9. **任务粒度**：本阶段产出唯一文件 P2-design.md，不拆批（分析连贯性要求）；P4 批量落笔再按
   packages 三包面（agate-phase-cards / agate-assets-roles / agate-assets-templates）拆批，
   在 `dispatch_plan` 字段声明（mode: static-batch 候选 + 并行上限）。
10. **minimal_validation**：纯文档 + pytest/grep 改造，无浏览器/外部系统依赖——声明口径按 P2 卡，
    但须附"最小验证"动作：用 grep 确认拟新增锚词不与 CHECK14/15 平台词护栏冲突、
    CHECK11 三锚词保持方案可行（把验证输出记入 minimal_validation 字段）。

### 上游关联

- P1-requirements.md（BDD-1~21 + §7 同类扫描 10 行 + §9 下游落笔注意）是唯一需求输入
- P1-review.md 首轮 needs-revision → fix1 → 复核 approved（打回项 1 phases 补 P2 / 2 BDD-6 补模板
  锚词锁 / 3 BDD-16 落点 P2 pin 定——本阶段须落实第 3 项）
- .state.yaml phase 待推进 P2（主 Agent 在 P2 产出 commit 时同 commit 推进）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P1-requirements.md`（需求基线，必读全文）
2. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P0-brief.md`（范围/约束）
3. `agate/assets/execution-roles/architect.md`（角色定义 + UI 设计节规格 + 批次设计节）
4. `agate/assets/review-roles/plan-design-review.md`（Phase 3 改造对象，38 行）
5. `agate/phase-cards/P3-tdd.md` + `agate/phase-cards/P6-acceptance.md`（Phase 1 改造对象）
6. `agate/phase-cards/P1-requirements.md`（Phase 2 改造对象）+ `agate/assets/execution-roles/analyst.md`
   （Phase 2 改造对象，节选人工体验验收节落点周边）
7. `agate/assets/templates/dispatch-context.md`（Phase 1/4 改造对象，61 行）
8. `agate/dispatch-protocol.md`（§4 自主再派发行 ~989-1003 + 派发编排机制行 ~518-573，DEBT0026 边界）
9. `agate/rules/review-mapping.md`（plan-design-review 触发映射面，P1 §7 #5 引用面）
10. `agate/scripts/check-protocol-consistency.py`（CHECK11 行 ~910-911 锚点口径 + CHECK14/15 护栏口径，节选读）
11. `AGENTS.md`（worktree 根：DEBT0025 载体「改脚本的工作流」节 + 双工作区纪律）
12. `agate/tests/README.md`（DEBT0024 载体「何时更新」节）

### 产出文件字段

用 `FILE={AGATE_WORKSPACE}/tasks/TAG0030-acceptance-blindspot/P2-design.md agate-md-field-set --list`
查看本阶段应填字段；逐个写入；失败照提示修正，不要手写 frontmatter。

frontmatter 必填：phase=P2, task_id=TAG0030, type=design, parent=P1-requirements.md,
trace_id=TAG0030-P2-20260904, status=draft, created=2026-09-04, agent=architect,
candidate_count / packages / domains / ui_affected（本任务 ui_affected=false：不产出业务 UI，
只约束下游 frontend 任务；理由写进 env_constraints 或正文）。
正文必填：候选方案 ≥2 + 权衡 + 选择理由；影响面梳理（Modify/Not Modify/Risk）；
gate_commands（含 P3 断言审计测试命令 + P5 全量回归命令，拆独立 key，禁 && 链路）；
files_to_read（只列 P4 必读，控制上下文）；env_constraints；minimal_validation；
dispatch_plan（P4 三包拆批声明候选）。
</dispatch_guide>
...
- 改造对象 = worktree 的 `agate/`；主 checkout 禁止改动；consistency 必须用 worktree 自己的脚本
- gate 跑 `~/.agate` 稳定版 check-gate.py P2

### B. C8 评审映射（供 architect 预判，不替代主 Agent 派发）
- domains=[backend] + risk_level=high + ceremony=standard + 无 NEED_CONFIRM → 仅触发
  **plan-eng-review**（backend 行 + high 硬规则行去重后 1 个）。不触发 plan-design-review /
  cso / plan-ceo-review。单评审 → 主 Agent 直接派发，无需组长汇总。

### C. P1 关键事实（architect 可直接引用，不重复验证）
- BDD-1~6（Phase 1）：P3 卡 + P4 卡 step0 补创建型清理钩子要求；P6 卡 post-test 残留检查；
  dispatch-context 模板约束条目位；BDD-6 断言审计单测回归防线（含模板锚词同锁）
- BDD-7~9（Phase 2）：P1 卡 + analyst.md 补「人工体验验收」节（seed→页面有内容 BDD 强制）
- BDD-10~15（Phase 3）：plan-design-review 形态驱动（布局型三组 / 渲染组件型渲染正确性+动效时序；
  ≥2 候选下沉 UI 层；CHECK11 三锚词保持；缺省回落布局型）
- BDD-16~21（Phase 4）：视觉契约可表达子集（BDD-16 落点本阶段 pin 定）；DEBT0024 tests/README
  真实 gate 语义；DEBT0025 AGENTS.md 新增 CHECK 全量扫描第 0 步；DEBT0026 dispatch-context
  模板拆小默认指导
- 同类扫描锚点计数：capture-env-baseline 12 文件 / candidate_count 23 文件 /
  plan-design-review 11 文件 / ui_render_shape 20 文件（plan-design-review 唯一评审角色未命中）/
  afterEach·清理钩子·seed·人工体验·视觉契约·DOM 度量·拆小默认指导 0 命中

### D. gate_commands 基线（供 architect 固化）
- P3：断言审计测试命令（pytest 指向 agate/tests/ 新增断言审计文件，P3 test-designer 定文件名）
- P5：全量 pytest（分片 + -n auto）/ consistency --strict-errors-only / shellcheck——拆独立 key
- 超时档：单元/断言审计类 120s 档；全量回归 600s 档（按预期耗时声明 per-key timeout_seconds）
- 禁 `&&` 链路（DEBT0012 教训）
</objective_info>
