...
P6 验收（verifier subagent 模式）：逐条验收 P1-requirements.md 的 BDD-1~21（全部 21 条），
产出 `P6-acceptance.md` + `P6-evidence/`。每条 BDD 只允许 PASS 或 FAIL（二值），
PASS 必须有证据文件引用。本任务是**纯协议文档面改造**——BDD 载体 = 协议条文 grep 锚词命中 +
断言审计测试全绿 + consistency 0 ERROR，证据 = 对应命令输出/文件快照。

### BDD 分组与证据形式（P1 §7 同类扫描 + P2 §9 完成标志）

- **Phase1 BDD-1~6**（P3/P4 卡清理钩子条文 + P6 卡残留检查 + dispatch-context 模板条目 +
  断言审计单测）：证据 = `test_tag0030_assertions.py` 对应用例 PASS 输出 + 对应卡文件 grep 命中行
- **Phase2 BDD-7~9**（P1 卡/analyst 人工体验节）：证据 = 对应用例 PASS + 文件 grep 命中
- **Phase3 BDD-10~15**（plan-design-review 形态驱动 + CHECK11 三锚词保持 + 0-10/status 保持）：
  证据 = 对应用例 PASS + consistency 0 ERROR（CHECK11 白名单校验输出）
- **Phase4 BDD-16~21**（视觉契约/DEBT 三连）：证据 = 对应用例 PASS + tests/README/AGENTS/
  dispatch-context 模板 grep 命中 + count-tests 1457（+21）

### 约束

1. **二值判定**：每条 BDD 只允许 PASS/FAIL，行首 `- PASS BDD-NN: {描述} ({证据路径})` 格式
   （check-p6-format.py 精确匹配）；禁止中间态、禁止表格/emoji 格式。
2. **证据必须真实**：P6-evidence/ 非空、每个文件含实质内容（命令输出/文件快照，>1KB 或含实际
   输出）；每条 PASS 行的证据引用路径必须真实存在（provenance 审计 1c 检查）。
3. **全量对照**：P1 有 21 条 BDD → P6 必须 21 条验收结果（PASS/FAIL），挑验 = gate 不通过。
4. **验收记录事实**：P6-acceptance.md 记录验收时的事实（命令实际输出），不写"修复后 PASS"。
5. **自查≠gate**：你产出 P6-acceptance.md + P6-evidence/ 后由主 Agent 跑 check-p6-format.py --fix +
   check-gate P6 + check-p6-evidence + check-p6-provenance；不自称"验收已通过"。
6. **环境隔离**：纯只读验证 + 写 P6 产出，不改任何协议文件，标记 `[PROD_NOT_TOUCHED]`。
7. **命令超时兜底**：所有 bash 命令外层 timeout（断言审计单测 `timeout 240s`；consistency
   `timeout 180s`；grep 验证 `timeout 60s`）。
8. **frontmatter 汇总**：`pass:`/`fail:`/`ui_affected: false` 写文件头 frontmatter（用
   agate-md-field-set 写：phase=P6, task_id=TAG0030, type=acceptance, parent=P5-verification.md,
   trace_id=TAG0030-P6-20260904, status=draft, created=2026-09-04, agent=verifier,
   pass=<N>, fail=0, ui_affected=false）。
9. **无行首预判**：本文件（dispatch-context）与 P6-progress.md 禁止行首 `- PASS`/`- FAIL`
   （provenance 审计拦截）；P6-acceptance.md 是唯一允许 PASS/FAIL 行首的产出。

### 上游关联

- P1-requirements.md（21 条 BDD，验收唯一基准）
- P5-test-results/（技术验证已过：断言审计 21/21、consistency 0 ERROR、count 1457(+21)——
  P6 在此基础上做验收性复核，不重跑全量）
- P2-design.md §9 完成标志（锚词 grep 命中清单）
- P4 commit e39c897（14 协议文件落笔）+ P5 commit 196aca8

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P1-requirements.md`（BDD-1~21，必读全文）
2. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P2-design.md`（§2 锚词 + §9 完成标志）
3. `agate-workspace/tasks/TAG0030-acceptance-blindspot/P5-test-results/unit.md`（断言审计 21/21 证据）
4. `agate/tests/unit/test_tag0030_assertions.py`（断言用例——按 BDD 编号对应验收）
5. `agate/assets/execution-roles/verifier.md`（角色定义）
6. `agate/phase-cards/P6-acceptance.md`（本阶段卡，产出规格权威）
7. `AGENTS.md`（worktree 根，项目约定）

### 产出文件字段

产出 `P6-acceptance.md` + `P6-evidence/`（目录下放各 BDD 证据文件），写在任务目录下
`agate-workspace/tasks/TAG0030-acceptance-blindspot/`。
P6-evidence/ 证据组织建议：`bdd-NN-anchor.txt`（grep 命中行快照）或 `bdd-NN-assert.log`
（断言用例输出），引用路径相对 P6-evidence/。
</dispatch_guide>
...
- P6-evidence 产出目录 = 任务目录下 `P6-evidence/`

### B. 已知技术验证结果（P5 已过，P6 复核引用）
- 断言审计 `test_tag0030_assertions.py` 21/21 全绿（P4 后转绿）
- consistency --strict-errors-only 0 ERROR（329 WARNING 存量）
- count-tests 1457（较基线 1436 +21）
- unit 片 1 预存 flaky 已登记 known-failures.md（TAG0011 竞态，串行全量 1313 全绿）——
  P6 验收不受影响（断言审计三文件 63/63 全绿）

### C. ui_affected=false
- 本任务不产出用户可见页面，无 Playwright/E2E、无 vision-analyst、无截图证据；
  证据 = 条文 grep + 测试输出 + consistency 输出（P2-design §7 已声明）
</objective_info>
