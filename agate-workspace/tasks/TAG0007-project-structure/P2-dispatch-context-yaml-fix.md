---
phase: P3
generated_by: 主 Agent（小型修复，非常规阶段派发，第 2 轮）
task_id: TAG0007
role: test-designer
---

## 【第 2 轮：P3-test-cases.md 文字修正，与 P2-design.md 修复无关的独立小任务】

上一轮修复了 P2-design.md 的 YAML 转义 bug（第 276 行，已确认 CHECK 1 PASS、0 ERROR）。本轮是
另一处**独立的文字修正**：`P3-test-cases.md`「红灯自检记录」节末尾声称
...
失败的真实原因是 P2-design.md 自身第 276 行的 YAML 转义 bug（TAG0007 自己的 P2 产出物缺陷，不是
"改动前已存在"的存量问题），该 bug 已在上一轮修复，修复后这两个测试已恢复 PASS，且全量回归
`python3 -m pytest agate/tests/` 复跑结果为 `11 failed, 1017 passed, 2 skipped`（1011 条既有
用例 + 6 条新增"合规对照"用例 = 1017 全绿，11 条新增红灯用例失败，与 P3-test-cases.md 其余部分
声明的用例清单完全吻合，无任何遗留的意外失败）。

**任务**：只修改 `P3-test-cases.md`「红灯自检记录」节最后一句关于这两个测试的表述，改为准确
反映实际情况（这两个测试当时的失败是 P2-design.md 自身 YAML 转义 bug 导致，已在本任务内修复，
现已恢复 PASS，不是预先存在的基线问题）；同时更新全量回归结果为
`11 failed, 1017 passed, 2 skipped`（0 遗留意外失败）。不改动文件其余任何内容（BDD 用例清单、
测试映射表、frontmatter 等均不动）。

修改文件：{AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P3-test-cases.md（「红灯自检记录」
节，文末最后一段）

<dispatch_guide>
> ⚠️ 派发指引是强制指令。本次是**已 approved 的 P2-design.md 的一处纯格式修复**，不改变任何
设计决策内容。

### 背景
P3 阶段跑 `check-protocol-consistency.py`（本任务自己声明的 gate_commands.P5_consistency）时
发现 CHECK 1「YAML 代码块可解析」ERROR：
`agate-workspace/tasks/TAG0007-project-structure/P2-design.md:267` 的 YAML 代码块无法解析。
根因已定位：第 276 行 `note:` 字段值是一个双引号包裹的 YAML 字符串，但字符串内部又出现了未转义
的 ASCII 双引号（`不存在"意外匹配空值"的边界条件`），导致 YAML 解析器在内部双引号处提前截断
字符串，破坏了整个代码块的语法。

### 目标
只修复这一处 YAML 转义错误，不改变 `minimal_validation` 该条目的语义内容（仍然是同一句话，
同一个 assumption/method/result/note 结构）。

### 修复方式（二选一，你自行判断哪种更符合本文件其余 YAML 字符串的既有风格）
① 把内部双引号转义为 `\"`（YAML 双引号字符串标准转义）
② 把内部双引号改为中文全角引号「」或"…"（本文件其余处已大量使用全角书名号/引号风格，
   与 `不存在“意外匹配空值”的边界条件` 这种表达方式更协调，且天然规避 ASCII 双引号冲突）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P2-design.md（L267-281 一带，`minimal_validation`
  YAML 代码块，问题行是 L276）

### 验证
修复后跑 `python3 agate/scripts/check-protocol-consistency.py`，确认 CHECK 1 变为 PASS，且本次
修复引入的 ERROR 数为 0（跑之前先确认改动前 ERROR 计数含这一条，改动后不再含）。
</dispatch_guide>
...
## 门槛
- P2-design.md 该 YAML 代码块可被标准 yaml 解析器解析（`python3 -c "import yaml,re; ..."` 或
  直接跑 check-protocol-consistency.py 确认 CHECK 1 从 FAIL 变为 PASS）
- 除这一行的引号转义方式外，不改动 P2-design.md 任何其他内容（包括本行其余文字、其他行、
  frontmatter）

## 返回给我
只返回：
  1. 修改的行号 + 采用了哪种转义方式
  2. `check-protocol-consistency.py` 复跑结果（CHECK 1 是否已 PASS）
