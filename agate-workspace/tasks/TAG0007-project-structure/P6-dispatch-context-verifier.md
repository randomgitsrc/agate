...
<dispatch_guide>
> ⚠️ 以下派发指引是本次任务的强制指令。执行优先级：派发指引 > 客观查证信息 > 阶段卡片（参考规范）

### 目标
把 P1-requirements.md 的 11 条 BDD 逐条实跑验收，产出 P6-acceptance.md + P6-evidence/。本任务
不是 refactor 任务（无 `change_type: refactor` 声明），走标准功能验收口径；`ui_affected: false`，
不需要 vision-analyst/截图。

### 约束

1. **BDD→验证动作映射**（P3-test-cases.md 已把每条 BDD 映射到具体 pytest 测试函数，P6 的"实跑"
   就是重新执行这些测试函数并把输出作为证据，不是重新设计验证方式）：

   | BDD | 验证动作 | 证据产出 |
   |-----|---------|---------|
   | BDD-1 | 跑 `test_bdd_1_bootstrap_missing_skeleton_exit_1` + `test_bdd_1_bootstrap_with_skeleton_title_exit_2` -v，确认 exit 1/exit 2 行为符合描述 | test-output.log（本条+BDD-3 共享） |
   | BDD-2 | 跑 `test_skeleton_template_stack_neutral.py` 全部 3 个函数 -v；另外直接 `cat agate/assets/templates/skeleton-template.md` 确认人工可读的参数化表达（不只是测试通过，要让验收报告里能看到实际内容片段） | test-output.log + skeleton-template-content.log |
   | BDD-3 | 跑 `test_bdd_3_field_missing_no_regression_exit_2` + `test_bdd_3_established_explicit_no_regression_exit_2` -v | 同 BDD-1 test-output.log |
   | BDD-4 | 跑 `test_bdd_4_7_gate_p4_warning_when_table_missing` + `test_bdd_4_7_gate_p4_no_warning_when_table_present` -v；另外 `grep -A3 "新增文件核对表" agate/phase-cards/P4-implementation.md` 确认产出规格文字真实存在 | test-output.log（本条+BDD-7 共享）+ p4-checklist-spec.log |
   | BDD-5 | 引用 P5-test-results/unit.md（本次全量回归 1028 passed 含改动前 1011 条不失败的证明） | 引用 `../P5-test-results/unit.md` |
   | BDD-6 | 跑 `test_code_map_template.py` 全部 2 个函数 -v；`cat agate/assets/templates/code-map-template.md` 确认五字段真实存在；`test -f agate-workspace/agents/CODE-MAP.md && grep -c "模块\|层\|依赖方向\|关键文件\|约定" agate-workspace/agents/CODE-MAP.md` 确认 dogfooding 实例也存在（BDD-6 原文验收对象） | test-output.log + code-map-content.log |
   | BDD-7 | 同 BDD-4（同一对测试函数） | 同 BDD-4 |
   | BDD-8 | 跑 `test_bdd_8_9_gate_p7_internal_consistency_mismatch_exit_1` + `test_bdd_8_9_gate_p7_transcription_mismatch_exit_1` + `test_bdd_8_9_gate_p7_paired_matches_exit_0` + `test_bdd_8_9_gate_p7_mechanism_not_adopted_no_check` -v | test-output.log（本条+BDD-9 共享） |
   | BDD-9 | 同 BDD-8（同一组 4 个测试函数） | 同 BDD-8 |
   | BDD-10 | 跑 `test_bdd_10_gate_p4_refactor_not_exempt_warning` + `test_bdd_10_gate_p7_refactor_not_exempt_pairing_check` -v | test-output.log |
   | BDD-11 | 引用 P5-test-results/unit.md（同 BDD-5，含新增用例不破坏基线的证明） | 引用 `../P5-test-results/unit.md` |

2. **证据文件组织**：可以把所有 12 个新增测试函数的 `-v` 输出整合进一份
   `P6-evidence/test-output.log`（一次 pytest 调用跑全部 12 个函数即可，`-k` 表达式或直接列全部
   函数名），PASS 行引用同一份 test-output.log（多条 PASS 共享同一证据文件是允许的，见 verifier.md
   质量门槛）。骨架模板内容/CODE-MAP 内容/P4 核对表规格文字这三份是独立的小文件，分别产出。

3. **BDD-5/BDD-11 证据引用方式**：P5-test-results/unit.md 路径相对 P6-evidence/ 是
   `../P5-test-results/unit.md`（工作区任务目录内的相对路径），PASS 行示例：
   `- PASS BDD-5: 骨架机制实现改动不破坏现有回归基线（全量 1028 passed 含改动前 1011 条基线不
   失败）(../P5-test-results/unit.md)`

4. **查询类证据，非操作类**：本任务全部 BDD 都是"检查代码/文档状态是否符合预期"的查询类验证
   （跑命令看输出/看文件内容），不是 UI 操作类，不涉及截图去重规则，不需要 md5 差异化。

5. **回归引用注意**：`.state.yaml` 已有 `p5_pass_commit` 字段（供审计 7 判定 P5→P6 间是否有
   非产出文件改动）——本任务不是 refactor 任务，不适用"引用 P5 证据、不重跑"那一整套 refactor
   专用机制（那是 §「P6-acceptance.md（引用 P5 证据、不重跑）」节的适用范围，仅限
   `change_type: refactor` 任务的回归口径），但 BDD-5/BDD-11 本身语义就是"不破坏现有回归基线"，
   直接引用 P5 已产出的 unit.md 作为证据是合理且符合"查询类可不重跑"精神的，不需要另外产出
   regression.log。

6. **无 NEED_CONFIRM**：P6 验证不涉及不可逆操作（纯读取文件/跑测试命令），写
   `[NO_NEED_CONFIRM]`。

7. **capability_requirements 检查**：P1 声明 `capability_requirements: []`（无特殊能力需求），
   本阶段前置条件"P1 声明的 capability_requirements 中 ability 为 available"天然满足（无条目即
   无需检查）。

### 上游关联
P5-test-results/unit.md（本任务的 P5 阶段产出，独立实跑确认）：4 条 gate_commands.P5* 命令全部
exit 0，pytest 1028 passed/2 skipped/0 failed，consistency 0 ERROR，count-tests 1030 用例，
shellcheck 0 issue。P4-implementation.md（review approved）记录了实现细节。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P1-requirements.md（11 条 BDD，验收依据）
- {AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P5-test-results/unit.md（P5 证据，BDD-5/11
  直接引用）
- {AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P3-test-cases.md（BDD→测试函数精确映射）
- {AGATE_WORKSPACE}/tasks/TAG0007-project-structure/P4-implementation.md（实现记录）
...
<objective_info>
- P1-requirements.md frontmatter：risk_level=high, domains=[backend], ui_affected 不适用
  （P2 声明 ui_affected: false），capability_requirements=[]
- P5-test-results/unit.md 已确认：4 条 gate_commands.P5* 全部 exit 0，pytest 1028 passed/
  2 skipped/0 failed
- 12 个新增测试函数分布在 `agate/tests/unit/test_check_gate.py`（BDD-1/3/4/7/8/9/10 相关，
  共 10 个函数）+ `test_skeleton_template_stack_neutral.py`（BDD-2，3 个函数）+
  `test_code_map_template.py`（BDD-6，2 个函数）
- `.state.yaml` 含 `p5_pass_commit: f63e207daac99b5812ff1d87b787e3270476a887`（P5 通过时的父
  提交哈希，本任务非 refactor，不使用 P6 的"引用 P5 证据不重跑"专用机制，但 BDD-5/11 直接引用
  P5-test-results/unit.md 是合理证据）
</objective_info>
