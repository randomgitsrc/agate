...
独立评审 `P2-design.md`（方案设计，430 行），产出 `P2-review.md`（status: approved /
rejected）。**只审不写**——不直接改 P2-design.md，评审意见由主 Agent 回派 architect 修改。

### 约束

1. **评审标准 = plan-eng-review 角色文件 + P2 卡片**（已注入下方卡片全文）：数据流清晰 /
   状态机完整 / 接口契约明确 / 错误边界 / 测试策略 / 技术债（提"后续重构/架构债"须用标准
   DEBT 条目格式）/ 多方案探索 ≥2 / 实现就绪度（files_to_read 覆盖）/ minimal_validation。
2. **C8 映射**：TAG0027 = domains [backend,cli,api] + risk_level high → 本角色（plan-eng-review）
   是唯一 P2 评审，直接产出 P2-review.md（无组长汇总环节）。
3. **实质锚点要求**：结论引用具体产物锚点（设计节号 § / BDD 编号 / 文件落点），不得裸
   "approved"。approved 时须给出锁定决策清单；有阻塞问题 → rejected；需修改 → needs-revision。
4. **范围锁定核对**：设计是否守住 P0-brief out-of-scope —— P6.5 judge 机制本身不动 /
   dispatch-protocol 五模式本体不重构 / 平台食谱不产品化 / 不新开独立一致性检查（S-1/S-2
   复用）/ check-gate.py + check-state-transition.py 返回约定不改。越界 → 打回。
5. **BDD 语义权威核对**：设计语义与 P1 25 条 BDD + state-machine.md 转移权威源一致——
   P6.5 非独立 phase（74-78）、P5/P6→P4 回退（132/148）、P6 exit 2→P6.5 特例（139）不落盘
   exit2-resolution、diff≥2→PAUSED、P6.5 needs-revision→P6（151-157）。
6. **8 设计决策面逐项核对**（dispatch-context objective_info B 节转述于下方）——每项是否定案、
   备选否决理由是否自洽。
7. **P1 评审 4 条边界观察处理核对**：P2 §6 是否显式处理（BDD-10 示例用边、task 禁词词边界、
   排查面 vs 扫描面、adr.md 豁免理由修正）。
8. **minimal_validation**：是否声明（纯代码逻辑 + 依赖内部函数清单）——方案依赖外部行为时
   须有验证结果。
9. **审声明**：frontmatter 字段（candidate_count=2 / packages=[agate-protocol] /
   domains=[backend,cli,api] / ui_affected=false / dispatch_plan static-batch 4 批）与设计正文
   一致性。

### 上游关联

- architect 产出 P2-design.md（430 行）：影响面梳理（Modify 15 行/Not Modify 12 项/Risk 13 条）
  → 候选 2（A 薄 CLI 消费方=采纳 / B 中心控制器=否决）→ 8 决策面定案 → gate_commands 9 key →
  files_to_read 14 项 → env_constraints → minimal_validation → BDD 覆盖映射表 → §6 边界观察处理
  → §7 实现完成标志 → §8 dispatch_plan（static-batch 4 批：B1-core-rules-cli high /
  B2-render-audit high / B3a-docs-clean medium / B3b-guardrail-scripts high）
- P1-requirements.md（25 BDD 语义权威）+ P0-brief（范围锁定）
- design v3b `docs/design-notes/design-orchestration-semantics.md`（形态来源）
- P2-progress.md（architect 调查记录：脚本实读结论 + 实证命中，供核实设计落点真实性）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（评审对象，全读）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（BDD 语义对照）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P0-brief.md`（范围/out-of-scope）
4. `docs/design-notes/design-orchestration-semantics.md`（设计 v3b，按需对照）
5. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-progress.md`（architect 调查记录，
   核实设计落点真实性——设计声称读过的代码/实证是否与 progress 一致）
6. `agate/state-machine.md`（74-78/132/139/148/151-157，核对 BDD 语义权威，按需）
7. `agate/rules/phases.yaml` + `agate/rules/schema/phases.schema.json`（设计扩展对象现状，按需）
8. `agate/scripts/check-gate.py` / `check-state-transition.py`（返回约定现状，按需核实 BDD-13）

### 产出文件字段

产出 `P2-review.md`，Header 必须含 `status` 字段（approved / rejected / needs-revision）。
用 `FILE=... agate-md-field-set --list` 看字段清单再逐个 set，不要手写 frontmatter。
</dispatch_guide>
...
- 协议本体（改造对象，按需读）= worktree 的 `agate/`（不是 ~/.agate 稳定版）

### B. 8 设计决策面（P2 定案项，评审逐项核对）
① next/retreat schema 形态：P8 next:null、P6.5 gate_subphase（hosted_on/forward_to/
   needs_revision_to，不写 next/retreat）——核对与 BDD-1/2/3 + state-machine.md:74-78 一致
② S-1/S-2 md 侧扩展：WORKFLOW 总览表加列，_TABLE_ROW_RE 不锚行尾实证兼容——核对 BDD-4/5
③ exit2-resolution：任务目录 {phase}-exit2-resolution.md + judge-verdict 挂载（不新增 events
   类型）——核对 BDD-8/9/12
④ agate next/advance CLI：exit 三态分支、retreat-to 委托、只 add 不 commit（由 hook 校验跳变）、
   P6 exit2 特例——核对 BDD-6/7/8/9/10/11/13 + check-state-transition 语义
⑤ agate dispatch 单命令 Lazy Injection：子进程复用 next-card + CARD-SOURCE 标记——核对
   BDD-18/19/20/23/25
⑥ 审计 2 双锚点剥离：渲染标记优先 + 物理块兜底——核对 BDD-20/21
⑦ 档位 C 文档层 + append_event 可观测证据——核对 BDD-11
⑧ 护栏 1 = check-protocol-consistency 新 CHECK 14（md 段落级判据）/15（数据面词边界 + 豁免
   词典机械生成）——核对 BDD-15/16/22/24 + P1 评审观察②

### C. P1 评审 4 条边界观察（P2 §6 须显式处理）
① BDD-10 Given 示例 P7→P4 用边不当（应 P6→P4）
② BDD-15 禁词 task vs dispatch.yaml law-1 既有命中（词边界判定 + 字段名豁免）
③ BDD-17 排查面（9 文件）vs 机械化扫描面（全协议 md）
④ D-2 adr.md 豁免理由误述（adr.md 在 agate/ 协议区，须改决策叙事豁免理由）
</objective_info>
