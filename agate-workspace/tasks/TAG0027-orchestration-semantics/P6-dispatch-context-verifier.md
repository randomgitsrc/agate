...
产出 `P6-acceptance.md` + `P6-evidence/`：把 P1 的 26 条 BDD（BDD-1~26）逐条对照验收
（PASS/FAIL 二值），每条 PASS 附证据文件引用。功能型任务（非 refactor），ui_affected=false。

### 约束

1. **BDD 全覆盖**：P1 现 26 条 BDD（BDD-1~26，含 exit2fix 回改后的语义 + BDD-26
   gate_pass_exit）。P6 验收必须全量对照（PASS+FAIL ≥ 26），不能挑验。
2. **证据来源**（本任务是 agate 协议脚本改造——验收证据 = 自动化测试实跑 + 行为断言）：
   - BDD 对应测试 = `agate/tests/unit/test_tag0027_b*.py`（48 用例，tag0027 批）——每条 BDD
     至少一个测试锚点（P3-test-cases.md 有 BDD↔测试映射表）。验收时实跑 tag0027 批测试，
     输出落 P6-evidence/。
   - 部分 BDD 需额外行为验证（如 BDD-6 健康任务 exit:2 直推、BDD-12 judge 不误拦）——测试已
     覆盖（exit2fix 补的锚点），以测试实跑输出为证据。
   - 每条 PASS 引用实际证据文件（test output / 断言日志）。
3. **P5 证据复用（审计 7）**：P5（fcf3fd2）→ P6 之间无非产出文件改动 → 判定应
   `reuse_allowed`——全量回归可引用 P5-test-results/unit.md（P6 不需要独立 regression.log，
   但 tag0027 批测试实跑证据须 P6 自行产出落 P6-evidence/）。
4. **P6-acceptance.md 格式**（P6 卡片产出规格）：
   - 每条 BDD 行首 `- PASS BDD-NN: {描述} ({证据路径})` 或 `- FAIL BDD-NN: …`（禁中间态）
   - frontmatter：phase/task_id/type/parent/trace_id/status/agent + pass: N / fail: 0 /
     ui_affected: false
   - 无总结行用 `- PASS/FAIL` 前缀；总结行用 `**Summary**: 26/26 PASS, 0 FAIL` 格式
   - 证据路径相对于 P6-evidence/（如 `(bdd-6-healthy-exit2.log)`）；引用 P5 全量回归可写
     `(../P5-test-results/unit.md)`
5. **验收事实原则**：记录验收时的事实，FAIL 就写 FAIL（不掩盖）；本任务 P4 全量已绿预期全
   PASS。
6. **[PROD_NOT_TOUCHED]** 标注。分阶段落盘 P6-progress.md（勿攒）。

### 上游关联

- P1-requirements.md（26 BDD 语义权威——BDD-6/8/11/12 已回改 exit2fix 语义、BDD-26 新增）
- P3-test-cases.md（BDD↔测试映射表——验收对照）
- P5-test-results/（P5 全量 1381 passed 证据，可复用引用）
- P2-design.md（定案语义——按修正版 §3.1/§3.3/§3.4 验收，非旧版）
- P4-implementation.md（实现汇总——了解 BDD 落点）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（26 BDD 全读）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P3-test-cases.md`（BDD↔测试映射）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（§3.1/§3.3/§3.4 修正语义）
4. `agate/tests/unit/test_tag0027_b*.py`（48 用例——验收实跑对象，按映射挑跑）
5. `agate-workspace/tasks/TAG0027-orchestration-semantics/P5-test-results/unit.md`（复用引用）
6. `AGENTS.md`（项目约定）

> ⚠️ 命令在 worktree 根跑。产出写任务目录 P6-acceptance.md + P6-evidence/。**不 commit**
> （主 Agent commit）。

### 产出文件字段

- `P6-acceptance.md`（frontmatter 用 agate-md-field-set 填 pass/fail/ui_affected）
- `P6-evidence/`（非空，每证据文件含实质内容）
</dispatch_guide>
...
- 协议本体 = worktree 的 `agate/`（验收对象）

### B. 26 BDD 验收锚点速查（测试映射）
- BDD-1~5（Phase 1 转移表/schema/S-1-S-2）→ test_tag0027_b1_phases_transfer_fields + b3b_structure
- BDD-6~13（Phase 2 CLI/judge）→ test_tag0027_b1_agate_next_cli（含健康 exit:2 直推）+
  agate_advance_cli + judge_exit2_review
- BDD-14~17（Phase 3 心智/注记）→ test_tag0027_b3a_platform_name_docs
- BDD-18~25（Phase 4 渲染/审计/护栏）→ test_tag0027_b2_* + b3b_protocol_check14_check15
- BDD-26（gate_pass_exit）→ test_tag0027_b1_phases_transfer_fields（对照 check-gate return）
</objective_info>
