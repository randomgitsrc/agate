...
独立评审 `P1-requirements.md`（需求基线，25 条 BDD），产出 `P1-review.md`。
**只审不写**——不直接改 P1-requirements.md，评审意见由主 Agent 回派 analyst 修改。

### 约束

1. **评审标准 = requirements-review 角色文件检查清单 + P1 阶段卡片**（已注入下方卡片全文）：
   BDD 可二值判定 / BDD 编号 `#### BDD-NN:` 连续不跳号 / 隐含需求五维度覆盖 / BDD 跨条一致性 /
   裁剪合理性 / 审声明（risk_level/ceremony/phases vs 实际改动）/ P1 纯净性（无掺方案、无实现细节）。
2. **本任务无 frontend 域**（domains: [backend, cli, api]）——不套用 UI/UX/vision 评审要点
   （P1 已声明 capability_requirements: [] 且不涉 frontend/mcp/security/UI 视觉维度）。
3. **实质锚点要求**（role 文件强制）：结论必须引用具体产物锚点，不得裸 "approved"。approved
   时须逐条列 BDD 编号 + 覆盖维度（数据/前端/多端/边界/兼容逐项标注）；不引用 BDD 编号的裸
   approved 是假完成。
4. **审声明证据**：本任务尚未 commit（评审在 commit 前），`git diff --cached` 可能为空——
   用 `git status` + 任务目录文件清单作 diff 证据（P1 产出文件 = P1-requirements.md +
   P1-progress.md + P1-dispatch-context-analyst.md + .state.yaml phase=P1 +
   active-tasks.md 状态行）。核对 analyst 声明的 risk_level=high / phases 全量 P1-P8 /
   packages=[agate-protocol] / domains=[backend, cli, api] 与实际改动面是否匹配。
5. **范围锁定核对**：BDD 是否守住 P0-brief out-of-scope（P6.5 judge 机制本身不动、五模式本体
   不重构、平台食谱不产品化、不新开独立一致性检查）。若发现 BDD 越过范围（如写成改造
   check-gate/check-state-transition 返回约定、P6.5 独立 phase 化、新开一致性检查）→ needs-revision。
6. **行为视角 vs 实现细节**：BDD 是"系统行为"（CLI exit code / 文件存在性 / 渲染产物内容 /
   扫描命中数 / schema 校验结果），不绑定具体实现函数名。发现 BDD 写死实现（如"调用 X 函数"）
   → 打回。
7. **P1 基线保护前置**：P1-requirements.md 是活基线，评审只判质量不改内容。

### 上游关联

- analyst 产出 `P1-requirements.md`（307 行）：需求复述（四 phase + 护栏 1 + 设计诚实边界）、
  17 条隐含需求（I-1~I-17）、25 条 BDD（BDD-1~25，Phase 1: 1-5 / Phase 2: 6-13 / Phase 3:
  14-17 / Phase 4: 18-23 / 回归拦截: 24-25）、同类扫描 D-1~D-7 逐条判定、[NO_NEED_CONFIRM]、
  裁剪说明（不裁 P2-P8）、能力声明空
- 设计权威源：`docs/design-notes/design-orchestration-semantics.md`（v3b，2026-09-02 三轮评审
  闭环 PASS）——BDD 全部源自其 §4/§5/§6；评审链 v2/v3 文档采纳语义须在 BDD 中有体现
  （P6.5 非独立 / S-1/S-2 纳入 / 结构性判据 / WORKFLOW 豁免 / exit2-resolution 可复核）
- P0-brief.md：范围锁定 + out-of-scope + known_risks 七条（judge 机制不动、五模式本体不动、
  平台食谱不产品化、不新开一致性检查、审计 2 联动 A1、两路并存、档位 C 行为变更）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（评审对象，全读）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P0-brief.md`（范围/out-of-scope/约束）
3. `docs/design-notes/design-orchestration-semantics.md`（设计 v3b 权威源，按需对照 BDD 语义）
4. `agate/state-machine.md`（74-78 P6.5 口径 / 132-133,148 回退 / 139 P6 exit 2→P6.5，按需核对
   BDD-2/3/6-10 语义与权威源一致）
5. `docs/guides/worktree-dogfooding-guide.md` 或 `AGENTS.md`（双工作区纪律：协议文件读 worktree
   自己的 `agate/`，工具用 ~/.agate 稳定版，按需）
6. `docs/reviews/review-orchestration-semantics-v3-20260902.md`（v3 复审 PASS 语义，按需对照）

### 产出文件字段

产出 `P1-review.md`，Header 必须含 `status` 字段（approved / rejected / needs-revision）——
gate 脚本读的是 Header 的 status，不是返回摘要。用 `FILE=... agate-md-field-set --list` 看字段
清单再逐个 set，不要手写 frontmatter。
</dispatch_guide>
...
<objective_info>
### A. 路径拓扑
- worktree 根 = `/home/kity/oclab/agateon/.worktrees/agate-TAG0027`（project_root，分支
  feat/TAG0027-orchestration-semantics）
- 任务目录 = `agate-workspace/tasks/TAG0027-orchestration-semantics/`
- 协议本体（改造对象，读它）= worktree 的 `agate/`；`~/.agate` = 稳定版主 checkout（勿读勿改）
- 编排/派发工具用 `~/.agate/scripts/` 稳定版（TAG0016 教训）

### B. 评审对象质量信号（analyst 自检已过，供起步）
- check-frontmatter.py exit 0；frontmatter：risk_level=high / phases=[P1..P8 全量] /
  packages=[agate-protocol] / domains=[backend, cli, api] / agent=analyst / status=draft /
  created=2026-09-02 / trace_id=TAG0027-P1-20260902
- 25 条 BDD 编号连续（BDD-1~25），已核对均为 `#### BDD-NN:` 锚点格式
- 正文含：已核对 P0-brief 时效性无漂移、[NO_NEED_CONFIRM]、同类扫描表（D-1~D-7）、
  capability_requirements: []、裁剪说明（不裁剪全量 P2-P8）
- 设计诚实边界节（exit 2 不假装消灭模型自判）已在需求复述中显式标注

### C. 复核线索（评审重点核查项，来自 P0 known_risks + 设计 v3b）
1. BDD-2/3：P6.5 非独立 phase 口径 + retreat 值域与 state-machine.md 一致（P5/P6→P4、
   P6.5→P6、diff≥2→PAUSED、P6 exit 2→P6.5 特例）——对照 state-machine.md:74-78/132-133/139/148
2. BDD-4/5：S-1/S-2 纳入（不新开检查）+ schema 结构兼容既有消费方（agate-next-card.py M3、
   S-3/S-4）——对照 check-structure-consistency.py 现状
3. BDD-6~10：exit 三态分支（0 直推 / 1 回退 retry+1 / 2 暂停转主 Agent + exit2-resolution
   落盘；P6 exit 2→P6.5 不落盘特例）+ 与 agate-retreat-to.py 对接——对照 check-gate.py 头注释
   exit 语义与 state-machine.md
4. BDD-11：档位 C 全程 agate next + 硬中断点 PAUSED 非 retry + 档位 A/B 不受影响——对照
   loop-orchestration.md 档位定义
5. BDD-14~17：五模式唯一锚点 / 数据面禁平台名 / 叙述文档仅限实现注记段落 / 排查可追溯——
   对照 dispatch-protocol.md 五模式 + WORKFLOW.md:141-148 豁免表定位
6. BDD-18~21/25：方案 A 渲染时注入 + 手工兜底两路并存 + 审计 2 A1（渲染产物标记来源，排除
   逻辑不变）+ 文件版兜底——对照 check-p6-provenance.py 审计 2 现状（~318-355 行）
7. BDD-22/24：护栏 1 机械化结构性判据（非文件名单）+ 豁免清单（platform-notes.md / SETUP.md /
   WORKFLOW 已知适用环境表）——检查豁免清单是否与 BDD-16/17 自洽
8. 同类扫描（强制节）：D-1~D-7 逐条有判定（本次处理/不处理+理由）+ 回归拦截结论——缺任一
   条 = needs-revision
</objective_info>
