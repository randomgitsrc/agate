...
|------|------|------|------|
| `rules/phases.yaml` | 9 个主线条目（P0-P8）| 每条目新增 `next:` / `retreat:` / `gate_pass_exit:` 键（值域见 §3.1）| BDD-1/3/5/13 |
| `rules/phases.yaml` | P6.5 条目 | 新增 `gate_subphase:` 结构（不写 next/retreat）+ `gate_pass_exit: 0`（judge 通过码）| BDD-2/12 |
| `rules/schema/phases.schema.json` | items.properties（14-63 行）| 声明 `next` / `retreat` / `gate_subphase` / `gate_pass_exit` 四个新属性（additionalProperties:false 现状必须同步，否则 S-5 ERROR；`gate_pass_exit` 全局 required，规避子集不支持 if/then 的条件必填）| BDD-1/2/5/13 |
...
| `scripts/agate-next.py`（新增）| 推进 CLI（pass_set 三态判定，§3.4）| §3.4 契约 | BDD-6/7/8/9/11 |
...
| `scripts/check-judge-verdict.py` | P6.5 校验逻辑 | 复核谓词改 Fix C（只校验已存在的 resolution 文件格式/完整性 + 与账本对应，§3.3）+ `_strip_card`（98-111 行）双锚点剥离同步（CARD-SOURCE 行起物理块优先，A2 消费方同步面 §3.6）| BDD-12/20 |
...
| `agate/tests/` | 新增 pytest 文件（按 §8 dispatch_plan 分 4 批）| 覆盖 25 BDD | BDD-1~25 |
...
| R5 | P6 条件式推进特例被误泛化到其它 phase，或 exit 2 通过码被误当暂停 | 特例判定硬编码 `phase == "P6"`：exit 2 ∈ pass_set（gate_pass_exit）→ 直推候选，但推进前置 gate_p65 裁决（§3.1/§3.4）；其余 phase 的 exit ∈ gate_pass_exit 一律直推（不落盘 resolution）；真暂停（∉ pass_set 且 ≠ 1，协议实际极少）才落盘；回归测试锁 P6 条件式 / 普通 pass 直推 / 真暂停三分支（BDD-8/9/6）|
...
| R12 | 档位 C 硬中断点被 retry 语义覆盖 / exit 2 通过码被误停 | loop-orchestration.md 硬中断点（50-63 行，真暂停/需人决策场景）语义保持：不自动 retry、转主 Agent + exit2-resolution；但 **exit ∈ gate_pass_exit（正常通过码，多数 phase = 2）→ 直推不停**——只有真暂停（∉ pass_set 且 ≠ 1）才转主 Agent 落盘（§3.4/§3.7 修正语义）|
...
- exit code 消费：按 phases.yaml `gate_pass_exit`（每 phase 声明「检查通过」出口码）判定——exit ∈ pass_set → 直推（多数 phase 正常通过码就是 exit 2，非暂停）；exit 1 → 按 retreat 表值委托回退；真暂停（exit ∉ pass_set 且 ≠ 1，协议实际极少）→ 任务目录落盘 `{phase}-exit2-resolution.md`；复核挂载 = check-judge-verdict.py 既有校验扩展（不新增机制，Fix C：只校验已存在的 resolution 文件，§3.3/§3.4）。
...
**定案（形态 D1-A）**：主线 P0-P8 每条目新增三键（`next` / `retreat` / `gate_pass_exit`）；P6.5 条目新增 `gate_subphase`（不写 next/retreat）+ `gate_pass_exit: 0`（judge 通过码——所有条目都声明通过出口码，无条件性）：
...
- id: P5
  ...
  gate_pass_exit: 2  # P5 的「检查通过」出口码 = 2（check-gate.py gate_p5 L1048 return 2 实证）
# P8 条目（无后继例外）：
  next: null         # P8 无自动后继：gate_pass_exit（2）通过后转 READY 由人/发布流程处理（值域含 null）
  retreat: null      # P8 失败无跨阶回退：exit 1 → 重试 P8（retry+1 本阶段）
  gate_pass_exit: 2  # gate_p8 L1376 return 2 实证

# P6.5 条目（新增 gate_subphase 结构，非独立转移边）：
- id: P6.5
  name: 独立 Judge 复核
  exec_role: judge
  ...
  gate_pass_exit: 0        # judge verdict 通过 = gate_p65 return 0（L1110/1120 实证）
```

**gate_pass_exit 字段语义（CRITICAL-1 Fix A 定案，exit 三态与真实语义对齐）**：
- **每个 phase 条目声明本 phase 的「检查通过」出口码**（`gate_pass_exit: [0|2]`）。P4 review 实证：
  check-gate 的 exit 0 只在 P4（暂存含代码）/ P7（干净产物）/ P6.5（judge verdict 通过）可达——
  **P0/P1/P2/P3/P5/P6/P8 的正常通过码就是 exit 2**（gate_p0 L577 恒 2、p1 L698、p2 L883、p3 L892、
  p5 L1048、p6 L1093、p8 L1376 均 `return 2` 实证）；P6 含 P6.5 门槛（gate_p65 exit 0）前置后推进的
  特例保持。pre-commit 对每次成功 commit 记 `gate_run exit:2`——exit 2 是**账本常态**，不是暂停。
- 逐 phase 出口码表（agate next 消费基准，§3.4）：

| phase | gate_pass_exit | 实证依据（check-gate.py）| 推进语义 |
|-------|---------------|------------------------|---------|
| P0/P1/P2/P3 | 2 | gate_p0 L577 / p1 L698 / p2 L883 / p3 L892 return 2 | exit 2 → 直推 next（正常通过，主 Agent 判下一步）|
| P4 | 0 | gate_p4 L990 return 0（暂存含代码文件时）| exit 0 → 直推 next |
| P5 | 2 | gate_p5 L1048 return 2 | exit 2 → 直推 next |
| P6 | 2 | gate_p6 L1093 return 2（FAIL=0/证据非空）| exit 2 → 条件式推进（gate_p65 exit 0 前置，见下）|
| P6.5 | 0 | gate_p65 L1110/L1120 return 0 | judge 通过 → forward_to P7 |
| P7 | 0 | gate_p7 L1241 return 0（干净产物）| exit 0 → 直推 next |
| P8 | 2 | gate_p8 L1376 return 2 | exit 2 → 转 READY/发布流程（next: null）|

- **"exit 2 = 需主 Agent 自判"是给主 Agent 的信号语义**（检查过了、主 Agent 判下一步），**不是暂停等
  解决**。真暂停（需落盘 exit2-resolution）在协议里极少（∉ pass_set 且 ≠ 1 的异常分支，如回退抵达
  main L1396 sys.exit(2) 场景下主 Agent 决策）。P2 原设计把 exit 2 放大成通用暂停分支是错误前提，
  本修正收窄（§3.4 判定重写）。
- **Fix B（把 P0-P3/P5/P8 成功码统一改 exit 0）否决**：涉及 check-gate.py 核心返回语义变更（BDD-13
  禁止面扩大 + pre-commit gate_run 账本历史不可迁移），Fix A 只改消费方声明不改 gate 语义 → 采纳。

**P6 主线 next 条件式裁决（A1 定案）**：P6 条目 `next: P7`——值域合法（P6 的转移目标就是 P7；
P6.5 非独立 phase 不入 next 值域，schema 不含 P6.5）。P6 的 `gate_pass_exit: 2`（gate_p6 L1093
恒 return 2，「通过」= FAIL=0/证据非空 + check-p6-provenance exit 0）→ P6→P7 **不是简单 exit 直推
可达，而是条件式推进边**：`agate next` 在 `.state.yaml phase == P6` 且 gate exit ∈ pass_set(2) 时按
以下裁决消费 `next: P7`——
...
exit ∈ pass_set 的 P6 条件式分支）是 `gate_subphase.forward_to` 的机械消费方；schema 侧无需
表达（next: P7 值域合法），裁决逻辑在 CLI 分支。

schema 声明（phases.schema.json items.properties 增四键）：
- `next`: `{"oneOf": [{"$ref": "#/definitions/phaseId"}, {"type": "null"}]}`（phaseId = `enum [P0..P8]`，**不含 P6.5**——P6.5 不是主线 next 目标，保证不出现 `next: P6.5` 的独立边写法）。required 条件（id ∈ P0-P8 → next/retreat 必填）经**应用层 S-1/数据面断言承载**——check-yaml-schema.py 子集不支持 if/then（DEVIATION-1 定案，§3.9），用 phase 存在性/缺键断言兜底；schema 层以 `next`/`retreat` 属性声明 + 值域枚举锁形态。
- `retreat`: 同 phaseId 联合 null。
- `gate_subphase`: object，必填 hosted_on/forward_to/needs_revision_to，值域 phaseId；required 条件：id = P6.5 → 必填且不得有 next/retreat——同受子集限制，改由**应用层断言**（test 断言 P6.5 条目含 gate_subphase 三键且无 next/retreat 键）+ S-1 P6.5 负面形态检查承载（DEVIATION-1）。
- `gate_pass_exit`: `{"type": "integer", "enum": [0, 2]}`，**顶层全局 required**（所有条目含此键——规避子集不支持条件 required，纯 `required` 关键词子集支持）。
- **P6 条目 `next: P7` 合法但推进为条件式**：P6 通过码 = exit 2（非 exit 0 直推），`next: P7` 由 §3.1「P6 主线 next 条件式裁决」+ §3.4 P6 分支消费（gate_p65 exit 0 前置）——schema 只管值域（P7 ∈ phaseId），不管裁决。
...
**备选形态（否决）**：a) P8 next 写 `READY`——READY 非 phase id、值域引入状态混入，且 S-1 表行无 READY next 概念 → 用 `null` 更纯；b) 自由字符串值域（不枚举）——S-1/S-2 无物可比、BDD-3 无法机械核对 → 值域枚举 + null 双形态；c) P6.5 也写 `next: P7`——违反 state-machine.md:74-78 非独立口径（BDD-2 直接 FAIL）→ 否决；d) exit 2 用全局统一语义（不声明 gate_pass_exit）——多数 phase 通过码是 2、P4/P7/P6.5 是 0，无逐 phase 声明则消费方无法区分"通过 exit 2"与"真暂停 exit 2"（CRITICAL-1 根因）→ 逐条目声明 gate_pass_exit。
...
> **gate_pass_exit 不加列（机器声明，注释说明）**：gate_pass_exit 是 agate-next 消费的机器字段
> （exit 通过码，值域 {0,2}，出口码速查表在 §3.1），不是转移边——S-1/S-2 比对面（next/retreat）
> 无需 md 侧锚点；为避免"YAML 有 md 无"的漂移误读，总览表头注释注明 gate_pass_exit 属机器声明
> 字段（§3.1 表为准），不进 S-1 比对。P6.5 行注释已表达 judge 通过/needs-revision 方向，无需再加。
...
**定案（形态 D3-A）**：
- **触发面（CRITICAL-1/2 修正收窄）**：exit2-resolution 只在**真暂停**时落盘——gate exit ∉
  gate_pass_exit 且 ≠ 1（∉ pass_set，协议实际极少：如 P6 特例条件不满足的异常停留、回退抵达
  main L1396 sys.exit(2) 场景下主 Agent 决策、非通过码的 exit 2 事件）。**exit ∈ gate_pass_exit
  （多数 phase 正常通过码 = exit 2）不落盘**——那是正常通过，直接推进（§3.4），不是暂停。
...
- **复核挂载（Fix C 定案，CRITICAL-2 修正）**：`check-judge-verdict.py` P6.5 复核谓词从"凡账本
  exit:2 事件都要求 resolution 文件"改为**只校验经 agate next 真暂停分支落盘的 resolution 文件**：
  1. 谓词按 `{phase}-exit2-resolution.md` **文件存在性**驱动——任务目录存在该文件（= agate next
     真暂停分支落过盘）→ 校验其 frontmatter/必填节完整（phase/task_id/type=exit2-resolution/
     parent/created/agent + 触发/客观证据/解决 三节，与 §3.3 模板一致）且 phase 与账本事件对应；
  2. 文件不存在（任务从未真暂停过，账本里 P0-P3/P5/P8 的 exit:2 全是正常通过码）→ **不要求文件**，
     judge 复核通过（不误拦健康任务）；
  3. **P6 自身 exit 2 条件式推进不落盘**（BDD-9：FAIL=0/证据非空 + provenance exit 0 → 经
     gate_p65 前置裁决直通，该 gate_run 的 exit:2 属 pass_set 正常通过，不带 resolution 要求）；
  4. 账本可追溯性：resolution 文件 frontmatter `phase` 须命中账本中存在对应 phase 的真暂停
     gate_run（文件 phase ∉ 账本任何事件 phase → WARNING 级提示，不阻断——健康任务账本可能
     不含真暂停事件）。
  语义对照：Fix B（pass-set 过滤：账本 exit:2 事件 phase ∈ pass_set → 视为 pass 不要求文件）与
  Fix C 等价，但 Fix C 由"唯一落盘场景"驱动判定，不依赖 pass_set 表反查账本事件分类（实现面
  更小、与 agate-next 落盘契约单一来源）→ **采纳 Fix C**；BDD-12 验收语义随此调整（Given 改为
  "任务在运行中**经真暂停分支**产生过 exit2-resolution 产物"）。
...
2. 读 phases.yaml 当前 phase 条目的 `gate_pass_exit`（pass_set）+ `next`/`retreat`。
3. 子进程跑 `check-gate.py {phase} {TASK_DIR}`，读 exit code，按 **pass_set 三态判定**（CRITICAL-1
   Fix A 重写——不再把 exit 2 一律当"需自判暂停"）：
   - **exit ∈ gate_pass_exit（通过，直推候选）**：
     - **P6 条件式推进分支**（`phase == P6`，gate_p6 exit 2 ∈ pass_set）：① 查 .state.yaml
       `judge.enabled`；未启用（历史任务）→ gate_p65 早退 exit 0（judge 跳过）→ 裁决成立；② 启用 →
       子进程跑 `check-gate.py P6.5 {TASK_DIR}`（= P6.5-judge-verdict.md 存在 + check-judge-verdict.py
       + check-events.py 双 exit 0，即 gate_subphase.forward_to 门槛）：exit 0 → **推进裁决成立** →
       消费 `next: P7`；exit 1 → 裁决不成立 → 停留 P6，输出「judge 复核未过（缺 verdict 或 verdict
       校验失败/账本审计未过；judge.rounds ≤2，超限人工接管）」指引，**不推进、不落盘**（P6 推进
       裁决前置 gate_p65 是唯一"条件式"点，BDD-9）。
     - **普通 phase 直推**（其余 phase，含 P0-P5/P7/P8）：exit ∈ pass_set（如 P0-P3/P5/P8 的 exit 2、
       P4/P7 的 exit 0）→ **正常通过**，查 `next`：`next: Pn+1` → 更新 .state.yaml phase 为 Pn+1
       （保留 judge/retries 块，append_event `state_transition`）+ `git add .state.yaml` + 提示 commit
       （**不自行 commit**——让 pre-commit hook 对暂存 diff 跑 check-state-transition.py 校验跳变
       合法性，与手动推进同一机械路径，R3 缓解）；`next: null`（P8）→ 提示转 READY/发布流程，不
       推进。**此分支不落盘 exit2-resolution、不停等**——exit 2 通过码推进是常态（P0/P1/P2/P3/P5/P8
       全程 agate next 可达成，BDD-11/6）。
   ...
   - **真暂停/异常（exit ∉ gate_pass_exit 且 ≠ 1）**：协议实际极少——如 P6 特例条件不满足的
     异常停留（provenance exit 1 / 证据空且 gate exit 2 但判定面异常）、回退抵达 main L1396
     sys.exit(2)、非通过码的 exit 2 事件。此时：**不自动推进**，落盘 `{phase}-exit2-resolution.md`
     （§3.3 模板，若已存在则提示更新），输出暂停转主 Agent 提示，exit 0（CLI 成功完成"暂停动作"，
     由主 Agent 决策后续，真暂停才落盘 resolution——BDD-8 触发面收窄到此处）。
4. 每步 append_event（`gate_run` 已由 check-gate 自身/或 CLI 记录 + `state_transition`），作为档位 C 可观测证据（BDD-11）。
...
**备选形态（否决）**：a) `agate next` 自行 commit——commit message 语义（self-gate-review 标注等）与 hook 链由主 Agent commit 统一管理，CLI 自 commit 会绕过 commit-msg hook 约定 → CLI 只 add 不 commit；b) advance 内联多阶回退循环（逐阶自写归档/retry）——与 retreat-to 双实现漂移 → 委托；c) exit 分支仍按"exit 2 一律需自判暂停"判定——多数 phase 正常通过码即 exit 2，会把健康任务正常推进误判为暂停并落盘假 resolution 死锁主线（CRITICAL-1 实证）→ 按 gate_pass_exit pass_set 判定重写；d) 推进前对每个 phase 预跑 judge/人工复核门槛——把 P6.5 条件式特例泛化到全部 phase，过度设计且与 judge 机制 scope 冲突 → 只 P6 条件式。
...
- `loop-orchestration.md` 档位 C 定义（46-74 行）与 gate 处理流程（229-243 行）更新：pre-commit 通过后的"自动进入下一 phase"动作 = 运行 `agate next {TASK_DIR}`（而非主 Agent 临场改 .state.yaml）；**exit ∈ gate_pass_exit（正常通过码，多数 phase = exit 2）→ agate next 直推 next phase**（档位 C 全程用 agate next 推进，BDD-11 可达成）；exit 1 → `agate next` 自动走 retreat 分支（与档位 A/B 手动路径不冲突，只替换自动判定点）；真暂停（exit ∉ pass_set 且 ≠ 1）→ 停 PAUSED 转主 Agent + 落盘 resolution（硬中断点 50-63 行语义保持，不 retry）。
- **可观测证据（BDD-11）**：`agate next` 每次推进 append_event（`state_transition`，字段 from/to/ts）+ 对应 .state.yaml commit（git log 可查）→ BDD-11 的"When 检查推进记录"落在 gate-events.jsonl / git log，可二值判定"推进均经 agate next 判定"（事件含 cmd 溯源；`gate_run` exit ∈ pass_set 后跟 `state_transition` 记录即证据——健康任务 P0-P3/P5/P8 的 gate_run exit:2 直推全程可达成）。测试用临时任务目录跑两次推进断言事件流。
- 硬中断点仍必停 PAUSED 而非 retry：CLI 真暂停分支不产生 retry 记录（§3.4），文档同步声明。
...
### 3.9 P4 review DEVIATION-1/2 范围决策记录（exit2 修正轮）

**DEVIATION-1（phases.schema.json if/then 条件约束未落地）——范围决策：不扩展 schema 强制，改数据面/应用层承载 + DESIGN_GAP 立项**：

- P4 review 实证：phases.schema.json 无 `if/then/not`（`"if" in raw == False`）；check-yaml-schema.py
  手写校验器只支持 type/required/enum/properties/items/additionalProperties/minItems 子集（L10-11、
  `_validate_value` 实读确认），无参数化固定校验。
- **定案**（采纳 P4 review 建议第一分支）：P2 §3.1/§7 原定的 schema if/then 反例验收（"P6.5 写
  next: P7 被 schema 拒"、"主线 P1 缺 next/retreat 被拒"）**改为数据真值断言承载，不扩展校验器子集**：
  - "P6.5 不得有 next/retreat 独立边" → 应用层 pytest 断言 phases.yaml P6.5 条目**不含** next/retreat
    键 + 含 gate_subphase 三键（§3.1 schema 声明段已改此口径）+ S-1 P6.5 负面形态检查（md 表
    P6.5 行不出现指向独立后继 phase 的 plain `P7` 值，§3.2 既有设计）；
  - "主线条目必含 next/retreat/gate_pass_exit" → `gate_pass_exit` 进 schema **全局 required**（纯
    required 子集支持）+ pytest 断言 9 主线条目三键齐全；
  - 记录 `[DESIGN_GAP: schema 层不强制 P6.5 无 next/retreat 的条件约束（校验器子集不支持 if/then，
    2026-09-03 定案由数据面断言 + S-1 负面形态承载，不扩展子集）]`——未来若给 P6.5 加 next: P7，
    S-1 负面形态检查 + 数据面断言能拦（数据真值而非 schema 强制），与 P4 review DEVIATION-1 表述
    一致。schema 反例类完成标志（§7 原"反例被拒"）相应改为数据面断言拒绝。
- **不扩展 check-yaml-schema.py 支持 if/then**：评估过回归面（子集实现膨胀 + 其它 rules schema 消费
  面），超出本任务护栏 scope；DESIGN_GAP 记录留待后续 schema 层专项。

**DEVIATION-2（CHECK 14 扫描面只扫 agate/ 顶层 *.md，assets/ 非豁免适配段无机械拦截）——范围决策：记录 assets 清理为一次性，不扩 CHECK 14 扫描面**：

- P4 review 实证：`check_md_platform_paragraphs` 只遍历 `root/"agate"/*.md`（L1233-1237 glob
  实证）；§3.8/§6③ 原排查面叙述含"assets/ 适配说明命中段"，B3a 已对 architect.md:229 /
  custom-role.md:49-56 挂注记（存量已清）。
- **定案**（采纳 P4 review 建议倾向分支：记录一次性决策，不动 B3b 已落地的扫描实现）：
  - **assets/ 角色/模板 md 属协议区但非叙述面**——语义叙述面 = `agate/*.md` 顶层协议文档（CHECK 14
    现状扫描面不变）；assets/ 其余执行角色/评审角色/模板 md 的**存量**平台适配说明命中段（architect
    .md:229、custom-role.md:49-56）由 B3a 一次性挂注记清理，**不进 CHECK 14 扫描面**；
  - **记录为一次性范围决策**（范围决策记录 = 本段 + §6③ 修订）：后续 assets/ 新增平台名段落无
    CHECK 14 机械拦截——接受该面（assets/ 角色/模板 md 由角色文件评审流程覆盖，非 CI 扫描面）；
    不扩 `check_md_platform_paragraphs` 扫描到 assets/（避免误伤 assets/templates/dsh/ 平台食谱
    整目录等豁免结构 + B3b 已落地实现返工）。
- 两决策均**不动已 approved 部分**：DEVIATION-1 不改 schema 属性声明（next/retreat/gate_subphase
  值域枚举保留，仅条件 required 改应用层），DEVIATION-2 不改 B3b CHECK 14/15 扫描实现与
  S-1/S-2 扩展。
...
files_to_read:
  - path: agate/rules/phases.yaml
    why: 加 next/retreat/gate_subphase/gate_pass_exit 字段的落点（9 主线 + P6.5 条目；gate_pass_exit 逐 phase 出口码，§3.1 表）
  - path: agate/rules/schema/phases.schema.json
    why: additionalProperties:false 现状，新增键必先声明否则 S-5 ERROR；gate_pass_exit 全局 required（纯子集支持）
  ...
  - path: agate/scripts/check-gate.py:1379-1414
    why: main() exit 分发 + 各 gate_p* return 码（gate_pass_exit 实证对照；新 CLI 消费方只需知道怎么调）
  ...
  - path: agate/scripts/check-judge-verdict.py:99-130
    why: _strip_card 同款剥离 + P6.5 复核谓词 Fix C 改动点（只校验已存在 resolution 文件，§3.3）
...
minimal_validation:
  ...
  method: "依赖的内部函数/数据转换清单（P4 实现时按此自检）：① check-gate.py exit 通过码子进程消费（读 phases.yaml gate_pass_exit pass_set，非假设 exit 2 一律暂停——CRITICAL-1 修正）；② check-state-transition.py P2.3-P2.5 由 pre-commit hook 在 commit 时对 git diff --cached 校验（新 CLI 只 add 不 commit，跳变合法性依赖此链）；③ agate_common.MAX_RETRY_MAP（P6.5 无键 → 读 phases.yaml retry_cap）；④ check-p6-provenance.py 审计 2 剥离逻辑（AGATE_CARD_START/END + frontmatter 剥除）→ 双锚点扩展（CARD-SOURCE 行起物理块）；⑤ agate-next-card.py M3 渲染（_render_sections 读 outputs/gates/retry_cap，新字段不破坏其读取——S-3 回归）；⑥ _TABLE_ROW_RE 加列兼容性（已实证：不锚行尾、前 3 列不变）；⑦ schema 新键声明（gate_pass_exit 全局 required 纯子集支持；next/retreat/gate_subphase 值域枚举——if/then 反例约束由数据面断言承载，§3.9 DEVIATION-1）；⑧ P6 judge 后推进裁决链（gate_p65 exit 0 → 消费 P6.next:P7，§3.1/§3.4）+ 普通 phase exit 2 ∈ pass_set 直推（CRITICAL-1）；⑨ CARD-SOURCE 块外标记下渲染产物过 pre-commit 2p hash（§3.5/§3.6，A2 机制）"
  result: "not_needed"
  note: "本任务为纯代码逻辑（无浏览器/外部系统/网络依赖），但代码逻辑正确性假设**不豁免失败测试首写**：设计经实读已定位 4 条 P4 实现时必先写失败测试确认红的主线——②'CLI 不 commit、由 hook 校验'、⑥'加列兼容'、⑧'P6 judge 后推进（A1 裁决）+ 普通 phase exit 2 通过直推（CRITICAL-1）'、⑨'CARD-SOURCE 产物过 2p hash（A2 机制）'——先红后绿，避免把实读推断当实现依据（A1/A2/CRITICAL 教训：实读可发现却未定案的矛盾，须测试锁死；CRITICAL-1/2 正是测试契约与真实语义脱节的盲区，P4 修复须先补健康任务全历史 exit:2 推进/复核场景测试）"
```
...
| BDD | 设计落点 | 验证手段 |
|-----|---------|---------|
| BDD-1 | §3.1 next/retreat/gate_pass_exit + schema 声明；9 主线条目均含三键；P8 `next: null` | pytest: schema 校验 exit 0 + 9 条目键齐全（gate_pass_exit ∈ {0,2}）（P5_schema）|
| BDD-2 | §3.1 P6.5 gate_subphase（hosted_on/forward_to/needs_revision_to）+ gate_pass_exit: 0，不写 next/retreat；反例由**数据面断言**（§3.9 DEVIATION-1 承载，非 schema if/then）拦 `next: P6.5` 独立边 | pytest: 数据面断言 P6.5 条目含 gate_subphase 三键 + 无 next/retreat 键 → 违规即失败；S-1 P6.5 负面形态检查 |
...
| BDD-6 | §3.4 agate-next exit ∈ gate_pass_exit → 按 next 更新 .state.yaml phase + add（check-state-transition 由 hook 校验）；exit 0 与 exit 2 通过码均直推 | pytest: 临时任务 .state.yaml P5→P6（next: P6，exit ∈ pass_set 直推路径——P5 gate exit 2 mock 直推）；**P0-P3/P5/P8 exit 2 通过直推锚点（CRITICAL-1）**：健康任务正常 exit 2 不落盘 resolution、phase 正常推进；**P6 条件式推进锚点（A1）**：judge.enabled 未启用（历史任务）→ P6 场景 exit 2 + provenance exit 0 → 直推 P7 |
...
| BDD-8 | §3.4 **真暂停**（exit ∉ gate_pass_exit 且 ≠ 1，非 P6）→ 不推进 + 落盘 {phase}-exit2-resolution.md；exit ∈ pass_set 的 exit 2 正常通过**不落盘** | pytest: 真暂停场景 mock（exit 2 且 phase ∉ pass_set，如异常停留）→ 文件存在 + frontmatter 字段；正常通过 exit 2（P5）→ 无 resolution 文件生成（健康任务反向断言，CRITICAL-1 盲区补测）|
| BDD-9 | §3.4 P6 exit 2（∈ pass_set）条件式推进特例（FAIL=0/证据非空 + provenance exit 0）→ 经 gate_p65 前置裁决推进 P7，不落盘不停等 | pytest: P6 分支无 exit2-resolution 文件生成；**P6 judge 后推进路径锚点（A1）**：judge.enabled=true + P6.5-judge-verdict.md 存在 + gate_p65 exit 0（check-judge-verdict/check-events 双 0）→ phase P6→P7 + state_transition 事件；gate_p65 exit 1（缺 verdict）→ 停留 P6 不推进 |
| BDD-10 | §3.4 advance --to + retreat-to 委托；示例用 P6→P4（§6①）| pytest: diff≥2 人工直跳 --to 提示 PAUSED 拦截（retreat-to 逐阶路径不触发拦截，单测断言逐阶调用）|
| BDD-11 | §3.7 档位 C 文档更新 + agate next 事件证据（exit ∈ pass_set 直推可全程达成）| pytest: 健康任务两次推进（P0-P3/P5/P8 exit 2 直推）后 gate-events.jsonl 含 state_transition 记录且无 resolution 落盘 |
| BDD-12 | §3.3 check-judge-verdict 复核 exit2-resolution（Fix C：只校验**已存在** resolution 文件）| pytest: 正向——存在合规 resolution 文件 → verdict 通过；文件格式非法 → 校验失败；**健康任务反向（CRITICAL-2 盲区补测）**：账本含 P0-P3/P5/P8 exit:2 正常通过事件但无 resolution 文件 → verdict **通过**（不误拦）|
| BDD-13 | Not Modify（check-gate/check-state-transition 返回约定不变；exit 0/1/2 语义原样——gate_pass_exit 只声明不改变量）| pytest: 两脚本头注释 + exit code 行为回归 |
...
| ③ | BDD-17 排查面（9 文件）窄于机械化扫描面（未来扫全部协议 md + assets/）| 排查/扫描/豁免三面并陈（A3 定案① + §3.9 DEVIATION-2 决策）：**排查面** = P1 D-2 9 文件 + **assets/ 平台适配说明命中段**（execution-roles/architect.md:229「无 prompt 派发场景」举例、templates/custom-role.md:49-56 平台适配说明 → 挂 `> 实现注记：`，Phase 3 完成，BDD-17 逐文件判定可追溯）；**扫描面** = CHECK 14 上线后 `agate/*.md` 顶层协议文档（**assets/ 不进扫描面**——assets 角色/模板 md 清理为**一次性**范围决策（§3.9 DEVIATION-2），后续 assets 新增平台名段由角色文件评审流程覆盖，无 CI 机械拦截为接受面）；**结构豁免面** = platform-notes.md/SETUP.md 整文件 + **assets/templates/dsh/ 平台食谱目录**（SKILL.md 等，A3）+ WORKFLOW.md「已知适用环境」表行。排查面是存量一次性清理（含 assets 命中段），扫描面是持续机械拦截（顶层 md）——清理（排查面）+ CHECK 上线（扫描面）同批（B3a/B3b），豁免面先行声明，上线首跑 0 命中为基线 |
...
- phases.yaml 9 主线条目含 next/retreat/gate_pass_exit（P8 next 为 null；P0-P3/P5/P6/P8 gate_pass_exit=2、P4/P7=0）+ P6.5 条目含 gate_subphase + gate_pass_exit: 0；schema 校验通过且反例由数据面断言拒绝（P6.5 条目含 next/retreat 键 → pytest 失败，§3.9 DEVIATION-1——schema 层不含 if/then，反例验收以数据面断言为准）。
...
- exit code 消费（CRITICAL-1 修正闭环）：`agate next` 按 gate_pass_exit pass_set 判定——健康任务 P0-P3/P5/P8 正常通过（exit 2 ∈ pass_set）经 agate next 直推全程可达成（不落盘假 resolution、不停等，BDD-11/6）；真暂停（exit ∉ pass_set 且 ≠ 1）才落盘 exit2-resolution.md（BDD-8 收窄触发面）；P6 条件式推进保持（gate_p65 exit 0 前置，BDD-9）。check-judge-verdict P6.5 复核（Fix C，CRITICAL-2 修正）：只校验**已存在**的 resolution 文件格式/完整性 + 与账本对应；健康任务（账本含 P0-P3/P5/P8 exit:2 正常通过、无 resolution 文件）judge 复核**通过**不误拦（BDD-12 反向）。**P6 推进裁决闭环（A1）**：judge.enabled 任务经 gate_p65 exit 0 → agate next 把 phase P6 推 P7（非人工改 .state.yaml）；gate_p65 exit 1 → 停留 P6 有指引。
...
- 护栏 1：CHECK 14/15 在 worktree check-protocol-consistency.py 生效；**存量清理（9 顶层 md + assets/ 适配说明命中段注记——assets 为一次性清理，不进 CHECK 14 扫描面，§3.9 DEVIATION-2 决策）后首跑 0 ERROR（--strict-errors-only），assets/templates/dsh/ 平台食谱目录结构豁免（A3）**；插入裸平台名 → exit 1，补注记 → exit 0。
...
- **B1 core-rules-cli（high）**：phases.yaml + phases.schema.json（§3.1，含 gate_pass_exit）+ agate-next.py/agate-advance.py（§3.4 pass_set 判定）+ check-judge-verdict.py exit2-resolution 复核谓词 Fix C（§3.3）+ loop-orchestration.md 档位 C（§3.7）+ 对应 pytest。独占文件：phases.yaml、schema、两新 CLI、check-judge-verdict.py、loop-orchestration.md。产出 ≤3（rules 数据面 / CLI 脚本组 / 测试组）。
...
## 9. P1 需求基线回改清单 + 新 BDD 草案（[BASELINE_CHANGE] 授权对象）

> exit2 修正轮牵动 P1 需求基线（exit 2 双义语义前提修正 + gate_pass_exit 新字段 + judge 谓词
> 语义调整）。以下清单供主 Agent 走 [BASELINE_CHANGE] 授权（dispatch-context 约束 5/6）；未授权前
> P1-requirements.md 不改，本清单是 architect 侧的影响声明 + 草案。

### 9.1 P1 需回改条目（现有 BDD 语义修正，8 条）

| # | P1 位置 | 现文（错误前提）| 修正为（与 P2 §3.1/§3.3/§3.4 修正语义一致）| 性质 |
|---|--------|---------------|-----------------------------------------|------|
| R1 | §需求复述 Phase 2 | "check-gate.py exit 三态（0 直推下一 phase / 1 按转移表回退 retry+1 / 2 暂停转主 Agent 并落盘 exit2-resolution）" | "exit ∈ gate_pass_exit（多数 phase 正常通过码 = exit 2）直推 / exit 1 按转移表回退 retry+1 / 真暂停（exit ∉ gate_pass_exit 且 ≠ 1，协议实际极少）暂停转主 Agent 落盘 exit2-resolution——gate_pass_exit 逐 phase 声明见 phases.yaml" | 需求复述语义修正 |
| R2 | §设计诚实边界 | "exit 2 分支的'暂停转主 Agent'不假装消灭模型自判——CLI 在 exit 2 分支暂停转主 Agent 是设计意图" | "真暂停分支（exit ∉ pass_set 且 ≠ 1）转主 Agent 是设计意图；exit ∈ pass_set（正常通过码，多数 = 2）直推不是暂停——模型自判留给主 Agent 判下一步（信号语义）" | 边界表述修正 |
| R3 | BDD-6 Given | "check-gate.py Pn 跑出 exit 0" | "check-gate.py Pn 跑出 exit ∈ gate_pass_exit（如 P5/P8 的 exit 2、P4/P7 的 exit 0）" | BDD-6 Given 语义修正 |
| R4 | BDD-8 Given | "check-gate.py Pn 跑出 exit 2（需主 Agent 自判）" | "check-gate.py Pn 跑出 exit ∉ gate_pass_exit 且 ≠ 1（真暂停/异常，如 P6 异常停留、回退抵达）" | BDD-8 Given 触发面收窄 |
| R5 | BDD-11 Then | "每一处进入下一 phase 的推进均经 agate next 判定" | "每一处进入下一 phase 的推进均经 agate next 判定（正常通过码 exit 2 ∈ pass_set 直推，健康任务 P0-P3/P5/P8 全程可达成）" | BDD-11 Then 澄清可达性 |
| R6 | BDD-12 Given | "任务 T 在运行中产生过 exit2-resolution 产物（exit 2 分支解决留痕）" | "任务 T 在运行中经真暂停分支（exit ∉ gate_pass_exit）产生过 exit2-resolution 产物" | BDD-12 Given 收窄 |
| R7 | BDD-12 Then | "产物缺失或格式不合法时 judge 复核不通过" | "已存在的 resolution 文件格式不合法/不完整时 judge 复核不通过；任务从未真暂停（无 resolution 文件）不要求文件——健康任务（账本含正常通过 exit:2）judge 复核通过（Fix C）" | BDD-12 Then 谓词校准 |
| R8 | BDD-13 Then | "check-gate.py 仍为 exit 0=通过/1=未通过/2=需主 Agent 自判" | "check-gate.py 仍为 exit 0/1/2 返回约定不变（本任务只新增 gate_pass_exit 消费声明，不改 gate 返回语义）" | BDD-13 表述修正（exit 2 注释语义以 gate_pass_exit 为准）|

### 9.2 新增 BDD 草案（gate_pass_exit 机器字段，1 条）

> 由主 Agent 评估并入 P1 基线（编号接续 25 条后 = BDD-26）或由 BDD-1/5 覆盖（gate 字段齐全 + 兼容消费方）。草案语义：

```markdown
#### BDD-26（草案）: phases.yaml 每条目声明 gate_pass_exit 且 agate next 按 pass_set 判定通过
- Given `agate/rules/phases.yaml` 全部阶段条目（P0-P8 主线 + P6.5）均含 `gate_pass_exit` 键
  （值 ∈ {0, 2}），与 check-gate.py 各 phase 真实通过出口码一致（P0-P3/P5/P6/P8=2、P4/P7/P6.5=0）
- When 运行 `agate next`（或直接断言 phases.yaml 各条目 gate_pass_exit 与 check-gate.py gate_p* return 值一致）
- Then 每 phase 的 gate_pass_exit 与其 gate 函数真实 return 码一致（pytest 断言对照），
  且 `agate next` 在 gate exit ∈ pass_set 时直推、exit ∉ pass_set 且 ≠ 1 时才落盘 exit2-resolution
```

### 9.3 下游改动面联动

- **schema（phases.schema.json）**：新增 `gate_pass_exit` 属性（enum [0,2]，全局 required）——B1 落地。
- **WORKFLOW.md 总览表**：gate_pass_exit **不加列**（机器声明字段，S-1 比对面只含 next/retreat；
  表头注释注明机器声明字段归属，§3.2）。
- **check-judge-verdict.py**：复核谓词改 Fix C（§3.3/§5 BDD-12）——B1 落地。
- **S-1/S-2 比对范围**：不变（next/retreat 4/5 列；gate_pass_exit 不进 md 比对，防漂移）。
- **check-gate.py 头注释**：exit 2 语义注释补充"多数 phase 为正常通过码，pass 判定以
  gate_pass_exit 为准"（文档级，不改返回）。
- **P3 测试设计**：BDD-6/8/11/12 测试点按 §5 修正语义设计（健康任务 exit:2 推进 + judge 反向
  不误拦为必补锚点）。
