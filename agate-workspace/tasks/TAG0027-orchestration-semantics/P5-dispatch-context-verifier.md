...
执行 P2-design.md §4.1 gate_commands.P5 全部命令（P5 技术验证），产出
`P5-test-results/`（unit.md + fail-list.txt），如实记录通过/失败。TAG0027 = 单包
（agate-protocol）任务，串行执行。

### 约束

1. **执行命令**（P2-design.md §4.1 gate_commands，全部在 worktree 根跑）：
   - `P5`: `python3 -m pytest agate/tests/ -q --tb=short -n auto`（注：P2 声明 `--reruns 1 -n
     auto`，但本环境无 pytest-rerunfailures 插件——**用 `-n auto` 并行 + 若遇存量偶发 sha256
     漂移（test_agate_next_card）改串行确认**）
   - `P5_consistency`: `python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`
     （worktree 自己的脚本）
   - `P5_structure`: `python3 agate/scripts/check-structure-consistency.py`（worktree 脚本）
   ...
   - `P5_selfgate`: `python3 ~/.agate/scripts/check-protocol-consistency.py --strict-errors-only`
     （稳定版——验证稳定版不被本任务破坏）
2. **判定**：每条命令 exit 0 + failed=0 → 通过；exit ≠0 或 failed>0 → 如实记录（区分真失败/
   环境问题/存量偶发）。**全量测试必须跑**（P5 卡片：未跑全量须标注"未运行全量测试"——本任务
   必须跑全量）。
3. **预存失败**：若发现与本次改动无关的预存失败，记录"预存失败：X（与本次改动无关）"并登记
   known-failures（本任务基线 1381 passed + 2 skipped，应无新增失败）。
4. **产出**：
   - `P5-test-results/unit.md`：各命令结果 + failed 计数 + test runner 输出签名（P5 卡片 N5
     校验 grep 计数 >0）
   - `P5-test-results/fail-list.txt`：failed 测试 id 逐行（可为空文件）
   - 无 UI（ui_affected: false）→ 无 e2e.md
5. **PROD_TOUCHED 标注**：未触发写 `[PROD_NOT_TOUCHED]`。
6. **分阶段落盘**：每条命令执行前后追加 P5-progress.md（要跑什么/结果），不要攒。

### 上游关联

- P2-design.md §4.1 gate_commands（命令权威源——P5 后不得改）
- P4 实现已 commit（fcf3fd2）：四批 + exit2fix，全量 1381 passed 基线
- P1-requirements.md（26 BDD，P6 验收对照）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（§4.1 gate_commands
   精确命令）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P0-brief.md`（env_constraints）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P4-implementation.md`（实现汇总，
   了解改动面）
4. `AGENTS.md`（项目约定——gate 脚本分层/测试约定）

> ⚠️ 命令在 worktree 根跑（cd worktree 根）。读 P2 §4.1 后执行。产出写任务目录
> P5-test-results/。**不 commit**（主 Agent commit）。

### 产出文件字段

- `P5-test-results/unit.md` + `P5-test-results/fail-list.txt`（目录不存在则创建）。
</dispatch_guide>
...
- 协议本体（验证对象）= worktree 的 `agate/`
- 测试基线：全量 1381 passed + 2 skipped（P4 exit2fix 后实测）

### B. 命令预期时长（设 timeout）
- P5 全量 pytest：~90s（-n auto）→ timeout 600s
- consistency/structure/schema/shellcheck：各 <60s → timeout 120s
- counttests：~90s → timeout 180s
- selfgate：<60s → timeout 120s
</objective_info>
