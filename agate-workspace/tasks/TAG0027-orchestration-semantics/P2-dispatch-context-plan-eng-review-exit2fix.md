...
**P2 复审轮（exit2 语义修正）**：P2-design.md 经 exit2fix 修正轮（641 行，architect
dd858932）+ P1-requirements.md 已按 [BASELINE_CHANGE] 授权回改（R1-R8 + 新增 BDD-26，现 26
条 BDD）。复审判定修正后设计是否消除 P4 review 的 2 CRITICAL 语义错误前提、DEVIATION 是否
记录决策、与 P1 回改是否一致，产出 `P2-review.md`（status: approved / rejected /
needs-revision）。

### 约束

1. **复审范围 = exit2 修正闭合核对**：逐项核对 P4 review CRITICAL-1/2 Fix 方向是否在
   P2-design 修正版落实 + DEVIATION-1/2 范围决策是否记录 + P1 回改（R1-R8/BDD-26）是否与
   P2 修正一致。首轮 approved 的架构方向（候选 A + 8 面形态）不重新全量评审，只核修正波及节。
2. **评审标准 = plan-eng-review 角色 + P2 卡片**（已注入下方）。实质锚点要求不变。
3. **核对要点**：
   - **CRITICAL-1 Fix**：§3.1 gate_pass_exit 逐 phase 声明（P0-P3/P5/P6/P8=2、P4/P7/P6.5=0，
     实证 return 行标注）+ §3.4 exit 三态判定重写（∈ pass_set → 直推 / =1 → retreat / 真暂停
     ∉ pass_set 且 ≠1 → 落盘 resolution）——健康任务 P0-P3/P5/P8 exit 2 正常推进闭环是否成立
   - **CRITICAL-2 Fix**：§3.3 judge 复核谓词 Fix C（只校验已存在 resolution 文件；健康任务账本
     exit:2 正常通过无文件不误拦）——BDD-12 语义调整是否自洽
   - **DEVIATION-1**：schema if/then 不落地 → gate_pass_exit 全局 required + 数据面断言 +
     S-1 负面形态承载 + DESIGN_GAP 立项记录
   - **DEVIATION-2**：assets 清理 = 一次性（不进 CHECK 14 扫描面）范围决策记录
   - **P1 回改一致性**：R1-R8 + BDD-26 与 P2 修正版 §3.1/§3.3/§3.4/§5/§7 语义一致
   - **不推翻已 approved 部分**：B3a/B3b/B2/总览加列保持
4. **若修正引入新问题** → 如实标 needs-revision/rejected；全部闭合 → approved。

### 上游关联

- P4-review.md（rejected：CRITICAL-1/2 实证 + Fix A/B/C + DEVIATION-1/2——本轮修正的触发源）
- P2-design.md（修正版 641 行——评审对象）+ P2-progress.md（exit2fix 轮记录）
- P1-requirements.md（已回改：R1-R8 [BASELINE_CHANGE] + BDD-26，26 条）
- P2-dispatch-context-architect-exit2fix.md（architect 修正轮 dispatch——修正目标清单）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P4-review.md`（CRITICAL Fix 方向基准）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（修正版，全读重点
   §3.1/§3.3/§3.4/§5/§7/§9）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（回改后 26 BDD，
   核对 R1-R8 + BDD-26）
4. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-progress.md`（exit2fix 轮，按需）
5. `agate/scripts/check-gate.py`（exit 语义实证源，按需核实 gate_p* return）
6. `agate/rules/phases.yaml`（B1 已落地 next/retreat/gate_subphase——gate_pass_exit 加入点
   现状，P4 未落地时此处无该键，核对 P2 修正版声明的落地计划一致性）

### 产出文件字段

产出 `P2-review.md`（覆盖前版——终局复审结论）。Header status 用 agate-md-field-set 写。
</dispatch_guide>
...
- 协议本体（按需读）= worktree 的 `agate/`

### B. exit 真实语义速查（P4 review 实证，修正基准）
- exit 2 = P0/P1/P2/P3/P5/P6/P8 正常通过码（gate_p0 恒 2 / p1 L698 / p2 L883 / p3 L892 /
  p5 L1048 / p8 L1376 return 2）
- exit 0 = P4（暂存含代码）/ P7（干净产物）/ P6.5（judge verdict 通过）
- 真暂停（∉ pass_set 且 ≠ 1）= 协议实际极少（回退抵达/异常）
</objective_info>
