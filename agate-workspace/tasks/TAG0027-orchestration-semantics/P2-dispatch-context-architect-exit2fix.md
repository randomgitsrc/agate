...
**P2 设计修正轮（P4 review rejected 触发，CRITICAL 根因修复）**：P4 review 实证发现 P2-design
§3.1/§3.4 对 check-gate exit code 语义的错误前提——**多数 phase（P0/P1/P2/P3/P5/P8）的正常通过
码就是 exit 2**（非"需自判暂停"），exit 0 仅 P4/P7/P6.5 可达。修正 P2-design.md 使 exit 三态
语义与协议真实语义一致，为 P4 实现重做提供正确依据。同时处理 2 DEVIATION。

### 约束

1. **修正对象**：`agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（现文
   430+ 行，定点修订）。增量模式：上轮约束见 P2-dispatch-context-architect.md / -retry1.md
   （沿用），本文件只列修正目标。
2. **实证语义基准（P4 review 已实测确认，勿再怀疑）**：
   - check-gate exit 0 = 仅 P4（暂存含代码文件）/ P7（干净产物）/ P6.5（judge verdict 通过）
     可达——完全机械可判
   - check-gate exit 2 = **P0/P1/P2/P3/P5/P6/P8 的正常通过码**（gate_p0 恒 2、p1 L698、p2 L883、
     p3 L892、p5 L1048、p8 L1376 均 return 2）+ pre-commit 对每次成功 commit 记 gate_run exit:2
   - "exit 2 = 需主 Agent 自判"是给主 Agent 的信号语义（检查过了、主 Agent 判下一步），**不是
     暂停等解决**
   - 真暂停（需落盘 exit2-resolution）在协议里其实极少——P2 原设计把它放大成通用分支是错误
3. **修正内容（CRITICAL-1/2 Fix A 方向，P4-review 七节修复路径）**：
   - **§3.1/§3.4：exit 三态消费重写**——phases.yaml 每条目增 `gate_pass_exit: [0|2]` 字段
     （声明该 phase 的"检查通过"出口码）：P0-P3/P5/P6/P8 = 2（P6 含 P6.5 门槛后推进特例）、
     P4/P7 = 0。`agate next` 判定：gate exit ∈ gate_pass_exit → **直推 next**（推进）；
     gate exit = 1 → retreat（表值存在则委托 retreat-to）；gate exit ∉ pass_set 且非 1
     （真暂停/异常）→ 才落盘 exit2-resolution 转主 Agent。P6 特例（gate_p65 exit 0 前置）保持。
   - **§3.3/BDD-12：judge 复核谓词校准**——check-judge-verdict 的 exit2-resolution 检查不再
     "凡账本 exit:2 事件都要求文件"（那是正常通过码），改为：仅当任务存在 **经 agate next 真
     暂停分支落盘的 {phase}-exit2-resolution.md** 时才校验其格式/完整性 + 与账本对应
     （Fix C 最小语义：judge 复核 = 已存在的 resolution 文件合规，而非强制每个 exit:2 都有）；
     或 Fix B（pass-set 过滤：账本 exit:2 事件 phase ∈ pass_set → 视为 pass 不要求文件）。
     **P2 定案选一个并写明 BDD-12 验收语义调整**。
   - **§7/BDD-8/9/11 语义校准**：BDD-8（exit 2 落盘）触发面收窄到"真暂停 exit 2"（∉ pass_set）；
     BDD-9（P6 特例不落盘）语义不变；BDD-11（档位 C 全程 agate next）在 pass_set 语义下可达成。
   - **DEVIATION-1**：schema if/then 若 validator 子集不支持 → 定案"P6.5 反例约束由 S-1 负面
     形态检查 + 数据面断言承载"，在 P2 记录 DESIGN_GAP 立项（不改 schema 层强制的验收声明）。
   - **DEVIATION-2**：定案 assets/ 非豁免适配段清理 = 一次性（不进 CHECK 14 扫描面，B3a 已清），
     记录为范围决策；或 CHECK 14 扫描面扩 assets（P2 选一，倾向记录一次性决策——不动 B3b 已
     落地的扫描实现，减少返工）。
4. **不动已 approved 部分**：B3a 文档清理、B3b CHECK 14/15 + S-1/S-2 扩展、B2 渲染时注入 +
   审计双锚点、WORKFLOW 总览加列——P4 review 已锁定正确，P2 修正不得推翻。
5. **frontmatter/phases.yaml 改动标注**：新增 gate_pass_exit 字段须同步 P1 需求（[BASELINE_CHANGE]
   授权流程由主 Agent 处理）、schema 声明、WORKFLOW 表（如需加列或注释）、S-1 比对范围——列出
   受影响的 BDD 与下游改动面。
6. **P1 需求基线影响评估**：修正牵动的 BDD（BDD-6/8/9/11/12 及新增 gate_pass_exit 相关）——
   architect 输出"P1 需回改清单 + 新 BDD 草案（若需）"，由主 Agent 走 [BASELINE_CHANGE] 授权。

### 上游关联

- P4-review.md（rejected：CRITICAL-1/2 实证 + Fix 方向 A/B/C + DEVIATION-1/2 + 锁定正确部分）
- P2-design.md（现文，修正对象）+ P2-dispatch-context-architect.md / -retry1.md（上轮约束）
- P1-requirements.md（BDD 语义——修正牵动 BDD-6/8/9/11/12）
- check-gate.py 真实语义（gate_p0 恒 2 / p1 L698 / p2 L883 / p3 L892 / p5 L1048 / p8 L1376
  return 2；P4/P7/P6.5 return 0——实证见 P4-review 二节）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P4-review.md`（全读——CRITICAL Fix
   方向 + DEVIATION 建议）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（修正对象，全读，
   重点 §3.1/§3.3/§3.4/§3.7/§5/§7）
3. `agate/scripts/check-gate.py`（exit 语义实证源，按需核实 return 位置）
4. `agate/rules/phases.yaml`（B1 已加 next/retreat/gate_subphase——gate_pass_exit 加入点）
5. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（BDD-6/8/9/11/12
   现状）
6. `AGENTS.md`（项目约定）

> ⚠️ 读 worktree 自己的 `agate/`。只改任务目录 P2-design.md，不碰协议文件。

### 产出文件字段

产出 = `P2-design.md`（修正版）。frontmatter 用 agate-md-field-set 维护。
</dispatch_guide>
...
### B. exit code 真实语义速查（P4 review 实证）
| phase | 通过出口 | 说明 |
|-------|---------|------|
| P0/P1/P2/P3/P5/P8 | exit 2 | 正常通过码（gate 检查过 → 主 Agent 判下一步）|
| P6 | exit 2 | 正常通过 → P6.5 judge 复核（A1 特例）|
| P4/P7 | exit 0 | 完全机械可判 |
| P6.5 | exit 0 | judge verdict 通过 |

### C. 修正落点速查
- §3.1：phases.yaml 增 gate_pass_exit 字段（P0-P3/P5/P6/P8=2，P4/P7=0）+ schema 声明
- §3.4：agate-next exit 三态判定重写（∈ pass_set → 直推；=1 → retreat；∉ pass_set 且非 1 →
  真暂停落盘 resolution）
- §3.3/BDD-12：judge 复核谓词校准（Fix B pass-set 过滤 或 Fix C 已存在 resolution 才校验）
- §5/§7：BDD 映射表与完成标志更新
- DEVIATION-1/2：P2 记录范围决策（DESIGN_GAP 立项 / 一次性清理决策）
</objective_info>
