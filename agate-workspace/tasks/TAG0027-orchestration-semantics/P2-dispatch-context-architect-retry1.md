...
**修复轮**：按 plan-eng-review 的 rejected 结论（P2-review.md，3 阻塞 A1/A2/A3 + 3 非阻塞
B1/B2/B3）修改 `P2-design.md`，使设计消除内部自相矛盾与既有脚本消费面冲突。修改对象 =
`agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（在现文上修订，不重写）。
修复完成后 P2-review.md 的修正点全部闭合，重审可转 approved。

### 约束

1. **修复 = 评审发现的最小改动集**（P2-review.md「结论」节 + A1/A2/A3 + B1/B2/B3 各节建议），
   不引入评审未要求的新范围。架构方向（候选 A + 8 决策面形态）已被评审锁定，**不推翻**——
   只补 A1/A2/A3 的裁决/一致性 + 修 B1/B2/B3 的表述。
2. **增量模式**：上轮产出 = `P2-design.md`（现文）；上轮 dispatch-context =
   `P2-dispatch-context-architect.md`（复用其全部约束，本次只列修复目标）。上轮调查 =
   `P2-progress.md`（实读结论仍有效，按需补读）。
3. **3 个阻塞问题的修复方向**（来自 P2-review.md，逐条落实进设计对应节）：
   - **A1（P6→P7 judge 后推进裁决缺失）**：设计须显式定义 P6 条目推进裁决——`.state.yaml
     phase==P6 && judge.enabled && P6.5-judge-verdict.md 存在 && gate_p65（check-judge-verdict +
     check-events）exit 0 → agate next 直推 P7。P6 的 next 建模为条件式（P6.5-门槛后→P7）或由
     gate_subphase 的 forward_to 由 CLI 消费。§3.1 主线 P6 条目的 next 取值须定案（不得与 schema
     "next 不含 P6.5"冲突）；§3.4 exit 2 分支的 P6 特例须覆盖 judge verdict passed 后的推进裁决；
     §5 BDD-9/6 补 P6 通过路径测试锚点（gate_p65 exit 0 后 phase P6→P7）。
   - **A2（CARD-SOURCE 标记位置 vs pre-commit 2p hash 冲突 + §3.5/§3.6 两处不一致）**：统一来源
     标记位置与 2p 抽取边界，二选一定案并写清同步面：(a) CARD-SOURCE 注释放 **AGATE_CARD_START
     之前**（不在 _extract_card 抽取区间、不影响 hash），audit 2 剥离锚点改为"从 CARD-SOURCE 行起
     的物理块"，同步 check-p6-provenance.py:323 审计 2 / check-judge-verdict.py _strip_card /
     pre-commit-gate.py _extract_card 三处消费方；或 (b) 纯用 frontmatter generated_by 区分来源
     （audit 2 先剥 frontmatter 再剥块，标记不落卡片块内），同步改 §3.6 剥离逻辑。§3.5/§3.6 必须
     指向同一方案；§5 BDD-25 补"渲染产物过 2p hash 仍通过"的机制改动与测试锚点。
   - **A3（护栏 1 扫描面 assets/ 存量未处置）**：显式处置 assets/ 平台名命中面（实测：
     agate/assets/templates/dsh/SKILL.md 全篇平台名 + custom-role.md:49,54 +
     execution-roles/architect.md:229）——二选一写进 §3.8 + §6③ + §8 B3a 批边界：① assets/ 命中
     纳入清理/挂注记批（SKILL.md 判"平台食谱整文件豁免"或全文挂注记，拆给 subagent）；② CHECK 14
     扫描面限定/豁免区补"assets/templates/dsh/ 平台食谱目录"类结构豁免。完成标志（§7）须可达。
4. **3 个非阻塞问题的修复**：
   - **B1（diff 口径互斥）**：§3.4 exit 1 分支以"retreat 表值存在 → 委托 retreat-to（内部逐阶）"
     为条件，删除 diff=1 前置或改写为"retreat-to 会逐阶处理，CLI 不预判 diff"；§3.1 把 P6→P4 标为
     "表内唯一跨阶（diff=2）但由 retreat-to 逐阶落地"（P5→P4 diff=1、P6→P4 diff=2，对齐
     state-machine.md:132/148）；§6① diff 表述同步修正。
   - **B2（加列位置三处不一致）**：统一为"加在执行角色后（第 4/5 列）"，示例表头同步（评审角色/
     门槛顺延），S1S2-ANCHOR 注释注明 4/5 列语义。
   - **B3（minimal_validation 表述过强）**：A1/A2 定案后，§4.4 补两条"P4 首写失败测试"主线
     （P6 judge 后推进、CARD-SOURCE 产物过 2p hash），与现有 ②⑥ 平行；弱化"关键假设均已实读确认"
     表述。
5. **测试缺口补锚点**（P2-review「测试缺口」节 3 条 → §5 映射表）：P6 judge 后推进路径测试
   （A1 裁决）、CARD-SOURCE 产物过 2p 测试（A2）、assets/ 命中清理或豁免测试（A3）。
6. **P2 产出规格不变**：frontmatter 四字段（candidate_count/packages/domains/ui_affected）保持
   或按需微调；影响面梳理/候选方案/gate_commands/files_to_read/env_constraints/minimal_validation/
   dispatch_plan 均在现文基础上修订。
7. **改动落盘纪律**：修订 P2-design.md 用 edit 工具做定点修改（不整文件重写）；frontmatter 若有
   变更用 agate-md-field-set；每完成一个阻塞项修复追加 P2-progress.md。

### 上游关联

- P2-review.md（rejected，211 行）：3 阻塞（A1 实证 check-gate.py gate_p6 1051-1093 恒 exit 1/2
  无 exit 0 分支 + gate_p65 1096-1120 exit 0 后 phase 保持 P6 无推进裁决；A2 实证 pre-commit-gate.py
  2p 425-448 + _extract_card 171-189 抽 AGATE_CARD_START..END 间全部行；A3 实证 PROTOCOL_DIRS 69 行
  含 agate/assets/ + SKILL.md 命中）+ 3 非阻塞（B1 diff 口径 / B2 加列位置 / B3 minimal_validation）
  + 锁定决策（候选 A + 8 面形态批准，仅 A1/A2/A3/B1/B2/B3 需修正）
- P2-design.md（现文 430 行）+ P2-progress.md（上轮调查）+ P2-dispatch-context-architect.md
  （上轮约束，全部沿用）
- P1-requirements.md（25 BDD 语义权威，不因修复改动 BDD——若修复牵涉 BDD 表述，标
  [BASELINE_CHANGE] 报主 Agent，不擅自改）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-review.md`（修复指令，全读，重点
   A1/A2/A3/B1/B2/B3 各节建议 + 锁定决策）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（修改对象，全读）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-progress.md`（上轮调查，按需）
4. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-dispatch-context-architect.md`
   （上轮约束全沿用，按需）
5. 实证补读（按需，核实 A1/A2/A3 修正的落点）：`agate/scripts/check-gate.py`（gate_p6
   1051-1093 / gate_p65 1096-1120）、`agate/scripts/pre-commit-gate.py`（2p 425-448 /
   _extract_card 171-189）、`agate/scripts/check-p6-provenance.py`（审计 2 318-355）、
   `agate/scripts/check-judge-verdict.py`（_strip_card）、`agate/scripts/check-protocol-consistency.py`
   （PROTOCOL_DIRS 69 行 / CHECK 14 挂载点）、`agate/rules/phases.yaml`（P6 条目）+ 
   `agate/rules/schema/phases.schema.json`（next 值域）、`agate/state-machine.md`（139/148/151-157）、
   `agate/assets/templates/dsh/SKILL.md`（平台食谱命中面）
6. `AGENTS.md`（项目约定，按需）

> ⚠️ 协议文件读 worktree 自己的 `agate/`，不是 ~/.agate 稳定版。

### 产出文件字段

产出仍为 `P2-design.md`（修订版）。frontmatter 用 agate-md-field-set 维护；不要手写 frontmatter。
</dispatch_guide>
...
- 协议本体（按需补读）= worktree 的 `agate/`（不是 ~/.agate）

### B. 评审锁定方向速查（修复须对齐，不推翻）
- 候选 A（数据面权威 + 薄 CLI 消费方）成立
- next/retreat 值域 = 枚举 phaseId（不含 P6.5）+ null；P6.5 gate_subphase 三键不写 next/retreat
- exit2-resolution 落任务目录文件 + judge verdict 复核（check-judge-verdict.py 扩展，非新机制）
- CLI 只 add 不 commit（pre-commit 校验跳变）；手工兜底两路并存
- CHECK 14 段落级判据 + CHECK 15 词边界 + 豁免词典机械生成；豁免结构（整文件/整表/实现注记段）
- 8 面其余形态批准（①③⑤⑦⑧ 除 A1/A2/A3 修正点外不动）
</objective_info>
