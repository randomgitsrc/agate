...
**复审轮**：对 architect 修复后的 `P2-design.md`（498 行）重新评审，判定首轮 rejected 的
3 阻塞（A1/A2/A3）+ 3 非阻塞（B1/B2/B3）是否全部闭合，产出 `P2-review.md`（status:
approved / rejected / needs-revision）。

### 约束

1. **复审范围 = 首轮发现闭合核对**（增量）：逐条核对 A1/A2/A3/B1/B2/B3 在 P2-design.md 的修复
   是否到位、是否引入新矛盾；同时抽查架构方向与范围锁定未因修复被推翻。首轮已 approved 的
   部分（候选 A + 8 面形态锁定）不重新全量评审，只核修复波及节的连贯性。
2. **评审标准不变**（plan-eng-review 角色文件 + P2 卡片，卡片已注入下方）。实质锚点要求不变
   （结论引用设计节 + 实证行号）。
3. **测试缺口 3 条**（首轮「测试缺口」节）须确认已在 §5 映射表补锚点。
4. **若修复引入新问题** → 如实标 needs-revision / rejected（新问题 + 具体位置 + 建议），
   不因"已修两轮"放水；若全部闭合 → approved。

### 上游关联

- 首轮 P2-review.md（rejected，211 行）——**本轮的修复指令基准**（A1/A2/A3/B1/B2/B3 各节
  建议 + 锁定决策 + 测试缺口 + §5 补锚点要求）
- architect 修复记录：`P2-dispatch-context-architect-retry1.md`（修复目标清单）+ P2-progress.md
  （修复轮追加记录，核实修复动作真实性）
- 修复后 P2-design.md（498 行）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-review.md`（首轮 rejected 基准，
   重点 A1/A2/A3/B1/B2/B3 建议原文）
2. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-design.md`（修复后对象，全读，
   重点 §3.1 P6 next 裁决 / §3.4 exit 2 P6 分支 / §3.5-3.6 CARD-SOURCE 统一 / §3.8 + §6③ +
   §8 B3a assets 处置 / §4.4 B3 / §5 测试锚点补全）
3. `agate-workspace/tasks/TAG0027-orchestration-semantics/P2-progress.md`（修复轮记录，按需）
4. `agate-workspace/tasks/TAG0027-orchestration-semantics/P1-requirements.md`（BDD 语义，按需）
5. 实证补读（按需核实修复落点真实性）：`agate/scripts/check-gate.py`（gate_p6 1051-1093 /
   gate_p65 1096-1120）、`agate/scripts/pre-commit-gate.py`（2p 425-448 / _extract_card 171-189）、
   `agate/scripts/check-p6-provenance.py`（审计 2 318-355）、`agate/scripts/check-protocol-consistency.py`
   （PROTOCOL_DIRS 69 行）、`agate/rules/phases.yaml` + schema、`agate/state-machine.md`
   （132/139/148/151-157/647-654）

### 产出文件字段

产出 `P2-review.md`（覆盖首轮文件——复审终局结论）。Header status 用 agate-md-field-set 写。
</dispatch_guide>
...
### B. 首轮 6 问题（复审闭合核对项）
- **A1** P6→P7 judge 后推进裁决缺失 → 修复应见：§3.1「P6 主线 next 条件式裁决」（next: P7 +
  gate_p65 exit 0 前置 + judge.enabled 分支 + gate_subphase forward_to 由 CLI 消费）+ §3.4 exit 2
  P6 特例分支扩展（judge verdict passed → 直推 P7）+ §5 BDD-9/6 P6 通过路径测试锚点
- **A2** CARD-SOURCE 标记位置 vs 2p hash 冲突 + §3.5/§3.6 不一致 → 修复应见：定案 (a)
  CARD-SOURCE 置于 AGATE_CARD_START 之前（块外不进 _extract_card 区间）+ 三处消费方
  （pre-commit-gate.py / check-p6-provenance.py / check-judge-verdict.py）同步面 + §5 BDD-25
  "渲染产物过 2p 仍通过"机制改动与测试锚点
- **A3** 护栏 1 assets/ 存量未处置 → 修复应见：§3.8 豁免结构补 assets/templates/dsh/ 平台食谱
  目录 + §6③ 清理面扩 assets + §8 B3a 批边界含 assets 命中处置 + §7 完成标志可达
- **B1** diff 口径互斥 → 修复应见：P5→P4 diff=1 / P6→P4 diff=2 对齐 state-machine:132/148 +
  retreat-to 逐阶落地不触发 PAUSED + CLI 不预判 diff（retreat 表值存在即委托）
- **B2** 加列位置三处不一致 → 修复应见：统一第 4/5 列 + 示例表头同步 + S1S2-ANCHOR 注明
- **B3** minimal_validation 表述过强 → 修复应见：补"P6 judge 后推进 / CARD-SOURCE 过 2p"两条
  主线 + 弱化"均已实读确认"表述
</objective_info>
