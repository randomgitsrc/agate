# P5-dispatch-context-verifier — TAG0022 技术验证

> 派发对象：verifier（P5 模式）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/`

## 目标

执行 P2-design.md §6 固化的 `gate_commands.P5` 全部命令（各 key 独立跑、独立记录 pass/fail），产出 `P5-test-results/`（unit.md + fail-list.txt），并完成 BDD-2/4/9 的验收锚验证。

## gate_commands（P2 固化，逐条执行）

```yaml
P5: "python3 -m pytest agate/tests/ -q -p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp"      # timeout 600
P5_consistency: "python3 agate/scripts/check-protocol-consistency.py --strict-errors-only"                       # worktree 自己的；timeout 120
P5_structure: "python3 agate/scripts/check-structure-consistency.py"                                             # timeout 120
P5_ruff: "/home/kity/.venvs/agate-dev/bin/ruff check agate/"                                                    # timeout 120
P5_count: "bash agate/tests/scripts/count-tests.sh"                                                              # timeout 120
```

## BDD 验收锚验证（本任务特有，必须全部实测）

1. **BDD-2（ruff 零违规）**：`P5_ruff` 连续跑 **2 次**，均 exit 0（All checks passed）
2. **BDD-4（迁移后全量绿）**：`P5` exit 0 + failed=0；`P5_count` ≥ 1202；`P5_consistency` 0 ERROR；`P5_structure` 0 ERROR
3. **BDD-9（任意 basetemp 0 失败）**——两位置全量 pytest：
   - 位置 1（仓库外）：`P5` 命令（ptmp）
   - 位置 2（仓库内）：在 worktree 内建临时 basetemp（如 `agate/.bt-p5-inrepo/`）跑全量 pytest；**关键**：test_bdd_25 在 basetemp ∈ 仓库根时注入 `AGATE_CONSISTENCY_SKIP_DIRS`（M15 已实现），两位置均应 0 failed；跑完清理临时目录（rm -rf，注意不要误删仓库文件）
   - 判定：两位置均 0 failed → PASS；任一失败 → FAIL（记录失败明细）
4. **BDD-10（平台无关）**：`python3 agate/scripts/check-platform-assumptions.py` exit 0（R1-R5 0 命中）

## 输入文件（读相关节，勿全仓扫描）

1. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P2-design.md`（§6 gate_commands + §3 完成标准）
2. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-requirements.md`（BDD-2/4/9/10 验收锚原文）
3. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P0-brief.md`（env_constraints）
4. `{agate_root}/phase-cards/P5-verification.md` + `{agate_root}/assets/execution-roles/verifier.md`（角色定义）

## 约束（硬约束）

1. **只读验证**：不修改任何代码/协议文件（除 P5-test-results/ 产出与临时 basetemp 目录的创建/清理）；发现 bug → 记录到 fail-list，不自行修复
2. **紧凑输出**：pytest 用 `-q`；每次命令 bash 一律 `timeout` 包裹（P5 600s，其余 120s）
3. **全量测试**：P5 跑全量套件（agate/tests/），不得只跑目标文件；发现预存失败按 P5 卡 known-failures 流程标注
4. **P5 commit 不得混入非产出文件改动**（R9）：若发现顺手修复的必要性，记录后交主 Agent 走回 P4，不自己改
5. **环境**：Linux；/tmp 只读；双工作区纪律（consistency 用 worktree 自己的脚本；稳定版 ~/.agate 只读）；状态标记 `[PROD_TOUCHED]`/`[PROD_NOT_TOUCHED]`
6. **测试签名**：unit.md 必须含 test runner 输出签名（`grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)'` 计数 >0）——主 Agent 会按 P5 卡 N5 校验

## 产出规格

`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P5-test-results/`：
- `unit.md`：每个 gate 命令的输出签名（passed/failed 计数 + exit code）+ BDD-2/4/9/10 判定行（`- PASS` / `- FAIL` 行首格式）
- `fail-list.txt`：failed 测试 id 逐行列出（无失败时为空文件或写 "no failures"）
- 两位置 pytest 的各自输出签名（可合并在 unit.md 或独立文件）

## 分阶段落盘

每跑完一个命令，追加写 `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P5-progress.md`（命令 + exit code + 摘要）。

## 返回给我

只返回两行：① P5-test-results/ 路径；② 一句话摘要（各 gate exit code + BDD-2/4/9/10 判定汇总，≤50 字）。绝不返回文件全文。
<!-- AGATE_CARD_START -->
## 当前阶段卡片：P5
