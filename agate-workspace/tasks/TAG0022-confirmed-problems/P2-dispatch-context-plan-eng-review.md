# P2-dispatch-context-plan-eng-review — TAG0022 方案评审

> 派发对象：plan-eng-review（P2 方案评审，C8 机械映射：domains=[backend] + risk_level=high → 必派；单评审角色，直接产出 P2-review.md）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/`

## 目标

独立评审 `P2-design.md`（只审不写——不直接改设计文件），产出 `P2-review.md`：
- Header status ∈ {approved, rejected}（本角色无 needs-revision；评审意见由主 Agent 回派 architect 修改）
- 结论须引用具体锚点（方案节/文件/函数/BDD 编号），不裸 "approved"

## 评审重点（按 plan-eng-review 角色定义）

1. **数据流/状态机/接口契约**：五子项方案的数据流与异常路径；0039 judge 判据（created ≥ judge_required_since 的 ISO 字典序比较、缺失 created fail-open）边界是否完整
2. **错误边界**：check-gate.py 迁移（0038 A/B/C/D 组）行为逐字节等价声明是否有漏洞；S-3 收紧（S-3a/S-3b 双向）是否会误伤既有卡片条文
3. **测试策略**：0038 静态扫描测试（test_md_parse_scan.py）、0039 judge P1 用例、0041 test_bdd_7/25 改造 + M15 排除钩子用例是否覆盖验收锚；是否缺场景
4. **多方案探索**：candidate_count=2 是否真候选（候选 2 是否"某些维度上优于候选 1"，非稻草人）；选择理由是否自洽
5. **实现就绪度**：files_to_read 是否覆盖实现所需上下文；方案是否清晰到 implementer 无需步骤计划即可自主实现
6. **最小验证**：minimal_validation 字段是否含验证结果（ptmp 写可 confirmed / read_rules_yaml 路径 confirmed / GIT_CEILING_DIRECTORIES confirmed / 两纯代码逻辑 not_needed）
7. **分批纪律（D3）**：dispatch_plan 4 批（A-ruff/C-migration/B-judge/D-env-tests）——C 先 B 后错开 check-gate.py 是否成立（非重叠改动块）；批文件集是否互不交叉；B 依赖 C（created op + 重构后 gate_p1）是否合理
8. **SCOPE+ 判定**：§1.4 的 [SCOPE+]（M15 iter_md_files opt-in 排除钩子）——主 Agent 已采纳；评审核对：该改动是否最小、默认关闭行为不变、是否真为 BDD-9 仓库内位置 0 失败所必需（对照 TAG0020 known-failures.md 条目 2 证据），若备选方案（外部 basetemp 降级）更优可提非阻塞建议
9. **技术债**：若提出"后续应重构/存在架构债"，须用标准 DEBT 条目格式（evidence 必填，登记于 `{AGATE_WORKSPACE}/debt/tech-debt.md`——如判断需登记请产出该文件，或明确说明不涉及）

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P2-design.md`（评审对象）
2. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-requirements.md`（BDD-1..10 + §5 D1-D4）
3. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-review.md`（N1/N2/N3，评审验证 P2 是否闭环）
4. `/home/kity/oclab/dsh-workspace/agate-research/tag0019-21-analysis.md`（证据基准，按需）
5. `{agate_root}/assets/review-roles/plan-eng-review.md`（角色定义）

## 客观查证信息（可复算核对）

- 设计声称的实证（§8 minimal_validation）：ptmp 可写 / read_rules_yaml(phases.yaml) 10 阶段解析 OK / GIT_CEILING_DIRECTORIES rc=128——可自行复核（bash timeout 包裹）
- BDD-6/7 判据：rules/dispatch.yaml 将新增 `judge_required_since: "2026-08-22"`；P1 `created` 字段读自 agate-md-field-get created op
- BDD-3 判定口径：check-gate.py A/B/C/D 组清零、E/F 不计入（D2，P1 §4.2）
- N3 基线：count-tests 实测 1202（可复核）

## 环境约束

Linux；/tmp 只读；bash 一律 `timeout` 包裹；双工作区纪律（只读消费，写操作仅限 P2-review.md 与按需的 tech-debt.md）。状态标记：`[PROD_NOT_TOUCHED]`/`[PROD_TOUCHED]`。

## 分阶段落盘

每读完输入/完成关键核对，追加写 `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P2-progress.md`。

## 产出规格

`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P2-review.md`，Header：
...
status: approved            # 按你结论改写 approved / rejected
created: 2026-08-22
agent: plan-eng-review
---

正文按角色输出结构：架构问题（阻塞级/非阻塞）/ 测试缺口 / 锁定决策 + 逐项评审结论（引用具体锚点）。

## 门槛（什么算完成）

- P2-review.md 存在且非空；Header status ∈ {approved, rejected}；agent=plan-eng-review
- 结论引用具体锚点（方案节/文件/函数/BDD 编号）

## 返回给我

只返回两行：① 产出文件路径；② Status + 阻塞问题数量 + 一句话摘要。绝不返回文件全文。
