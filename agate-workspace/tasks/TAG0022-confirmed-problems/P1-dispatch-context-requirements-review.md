# P1-dispatch-context-requirements-review — TAG0022 需求基线独立评审

> 派发对象：requirements-review（P1 独立评审，不可裁）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/`

## 目标

独立视角审查 `P1-requirements.md`（只审不写——不直接改需求文件），产出 `P1-review.md`：
- Header status 最终为 `approved` / `needs-revision` / `rejected`（gate 脚本读 Header status，与返回摘要必须一致）
- review 结论必须引用具体锚点（每条 BDD 编号 + 覆盖维度清单；审声明核对引用 diff 证据）——裸 "approved" 或 "BLOCKER=0" 会被 gate 判为假完成

## 检查清单（按 requirements-review 角色定义逐项执行）

1. **BDD 条件可二值判定**：每条 Given/When/Then 是否可明确 PASS/FAIL；`#### BDD-NN:` 编号连续；每条单一 GWT；无中间态
2. **隐含需求覆盖**：数据/前端/多端/边界/兼容维度逐项（本任务为纯协议/脚本/CI/测试改造，重点核对协议面隐含依赖是否覆盖：SELF-GATE 触发、双工作区纪律、count-tests 冻结、/tmp 只读、judge 对本任务生效、UPGRADING 章节）
3. **BDD 跨条一致性**：同场景多 BDD 的 Then 是否矛盾；测试数据设计是否考虑环境约束
4. **裁剪合理性**：`phases: [P1..P8]` 全保留理由是否充分；`risk_level: high` 是否与实际匹配（改动面 = CI 配置/check-gate.py/state-machine/P6 卡/P1 卡/测试 + 同簇互扰 + SELF-GATE——high 合理？）
5. **审声明核对（TAG0019）**：analyst 声明的 risk_level/ceremony/phases vs 任务实际改动形态——用 `git status` / `git diff` 核对当前工作区实际改动面（P1 阶段实际只有需求文件+派发上下文+state 更新，代码改动在后续阶段；评审核对的是「需求声明的五子项改动面」（§5 范围表）与 P0-brief/分析文档证据是否一致）
6. **P1 纯净性**：有无掺入解决方案设计（候选方案/机制设计应留 P2）；BDD 是否绑定实现符号或"设 required check 当实现"的越界
7. **同类扫描完整性**：§4 四组扫描（ruff / check-gate.py md 解析 / judge.enabled / ceremony）是否齐备、逐条判定、结论落盘

## 客观查证信息（可复算的核对基准）

- 需求文件：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-requirements.md`
- P0-brief：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P0-brief.md`（5 个 issue 的验收锚原文）
- 分析文档：`/home/kity/oclab/dsh-workspace/agate-research/tag0019-21-analysis.md`（证据基准：check-gate.py 22 处 md 解析；ruff CI job protocol-tests.yml:106；TAG0019/20 违规数；全仓无 ceremony: thin 实战；test_bdd_7/25 复现）
- 交接单：`{project_root}/HANDOFF-TAG0022.md`（范围/纪律/验收锚）
- 工作区当前实际改动面：`git status --short`（应只有 task 目录产出 + 派发上下文；确认 analyst 无越界写协议文件）

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-requirements.md`
2. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P0-brief.md`
3. `/home/kity/oclab/dsh-workspace/agate-research/tag0019-21-analysis.md`（按需）
4. `{project_root}/HANDOFF-TAG0022.md`（按需）
5. `{agate_root}/assets/review-roles/requirements-review.md`（角色定义）

## 环境约束

Linux；/tmp 只读；bash 命令一律 `timeout` 包裹；双工作区纪律（只读消费，写操作仅限产出的 P1-review.md）。状态标记：未触发写 `[PROD_NOT_TOUCHED]`。

## 分阶段落盘

每读完一个输入文件/完成关键核对，追加写入 `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-progress.md`。

## 产出规格

`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-review.md`，Header（直接复制）：
...
status: approved            # 最终由你按评审结果改写：approved / needs-revision / rejected
created: 2026-08-22
agent: requirements-review   # 必须是评审角色名，≠ main
---

正文按角色输出格式：`## BDD 评审`（逐条 BDD-NN + 判定 + 覆盖维度）、`## 隐含需求覆盖`、`## 裁剪评审`、`## 审声明`、评审结论。

## 门槛（什么算完成）

- P1-review.md 存在且非空，Header status ∈ {approved, needs-revision, rejected}，agent=requirements-review
- 结论引用具体 BDD 编号锚点（approved 时逐条 BDD 编号 + 覆盖维度清单）

## 返回给我

只返回两行：① 产出文件路径；② Status + 一句话摘要（如 "approved，BDD-1..10 全过 + 审声明核对通过" / "needs-revision，BDD-X 判定不可行……"）。绝不返回文件全文。
