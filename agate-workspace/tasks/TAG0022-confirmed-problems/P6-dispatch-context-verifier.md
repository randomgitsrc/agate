# P6-dispatch-context-verifier — TAG0022 验收

> 派发对象：verifier（P6 模式）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/`

## 目标

逐条验收 P1 的 BDD-1..10（全部 PASS 或 FAIL，无中间态），产出 `P6-acceptance.md` + `P6-evidence/`（真实证据，不接受 1 行文本充数）。本任务 domains=[backend]，无 UI/视觉验收面（P1 §9 已声明）。

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P1-requirements.md`（BDD-1..10 原文）
2. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P2-design.md`（§4.4 RM-AG0040 实证计划[BDD-8 验收对象] + §3 完成标准 + §6 gate_commands）
3. `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P5-test-results/`（P5 证据，可引用）
4. `{agate_root}/phase-cards/P6-acceptance.md` + `{agate_root}/assets/execution-roles/verifier.md`（角色定义）

## 逐条 BDD 验收口径（证据形式）

| BDD | 验收动作 | 证据 |
|-----|---------|------|
| BDD-1 | 核对 `.github/workflows/protocol-tests.yml` 有稳定 `name: ruff` job（无歧义改名）+ grep 文档（UPGRADING/AGENTS）含「将 ruff job 设为 PR required check（维护者配置）」步骤文本 | workflow diff/文档 grep 输出落 P6-evidence/ |
| BDD-2 | `~/.venvs/agate-dev/bin/ruff check agate/` 两次均 exit 0 | 输出落证据（可引用 P5-test-results/ 的 ruff 记录 + 本次再跑） |
| BDD-3 | `python3 -m pytest agate/tests/unit/test_md_parse_scan.py -q -p no:cacheprovider --basetemp=<ptmp>`（静态扫描 A/B/C/D 命中=0 断言通过） | pytest 输出落证据 |
| BDD-4 | 全量 `python3 -m pytest agate/tests/ -q -p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp`（0 failed）+ `count-tests.sh` ≥1202 + `check-protocol-consistency.py --strict-errors-only` 0 ERROR + `check-structure-consistency.py` 0 ERROR | 各命令输出落证据（全量 pytest 可引用 P5-test-results/pytest-external.log + 本次验证） |
| BDD-5 | `python3 -m pytest agate/tests/unit/test_check_structure_consistency.py -q -p no:cacheprovider --basetemp=<ptmp>`（S-3a/S-3b 漂移用例 + 双侧一致用例通过） | pytest 输出落证据 |
| BDD-6/7 | `python3 -m pytest agate/tests/unit/test_check_gate.py -q -p no:cacheprovider --basetemp=<ptmp>`（judge P1 7 用例 + gate_p65 三态） | pytest 输出落证据 |
| BDD-8 | 核对 P2-design.md §4.4 实证执行计划含 M3 四要素（评审轮数指标/真实发现数指标/TAG0018 基线值/不达标决策规则）+ 触发条件 | 核对记录落证据（引用 P2-design.md §4.4 行号） |
| BDD-9 | 双位置全量 pytest：仓库外（ptmp）0 failed + 仓库内（临时 basetemp，如 agate/.bt-p6-verify，测后清理）0 failed | 两位置 pytest 输出落证据（可引用 P5-test-results/pytest-inrepo.log + 本次复跑） |
| BDD-10 | `python3 agate/scripts/check-platform-assumptions.py` exit 0（R1-R5 0 命中） | 输出落证据 |

## 产出规格

### P6-acceptance.md
- frontmatter：`pass: N` / `fail: 0` / `ui_affected: false`（与 P2 一致）
- 每条 BDD 一行 `- PASS BDD-NN: {描述} ({证据路径})`（行首格式，供 gate grep）；证据路径相对于 P6-evidence/ 目录
- 总结行不用 `- PASS/FAIL` 行首（用 `**Summary**: N/10 PASS`）

### P6-evidence/
- 必须非空；每个文件实质内容（命令输出/断言记录/日志）
- 建议：`bdd-01-workflow-docs.log`、`bdd-02-ruff.log`、`bdd-03-scan.log`、`bdd-04-full-pytest.log`、`bdd-05-s3.log`、`bdd-06-07-judge.log`、`bdd-08-plan-check.md`、`bdd-09-dual-position.log`、`bdd-10-platform.log`、`test-output.log`（通用，主 Agent P6 卡要求）

## 约束（硬约束）

1. **只读验收**：不修改任何代码/协议文件（除 P6-evidence/ 与 P6-acceptance.md）；发现 FAIL → 记录，不修复（修复走回退流程）
2. **功能验证 + gate 格式都满足**：先跑真实验收，再写格式合规的 PASS/FAIL 行
3. **验收记录事实**：PASS/FAIL 基于证据实际输出；证据文件必须真实存在且非空
4. **P5 证据引用**：全量 pytest/ruff 可引用 P5-test-results/ 证据（P5→P6 间仅 P4 回退修复 1 个测试函数 + P5 产出，全量复跑已覆盖——但 BDD-4/9 建议本次再实测一次以保证 P6 证据新鲜；自行决定，注明引用/实跑）
5. 环境：Linux；/tmp 只读（pytest `--basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider`）；bash 一律 timeout（全量 pytest 900s）；双工作区纪律；状态标记 `[PROD_TOUCHED]`/`[PROD_NOT_TOUCHED]`
6. 仓库内 basetemp 跑完必须清理

## 分阶段落盘

每验收完一个 BDD，追加写 `{AGATE_WORKSPACE}/tasks/TAG0022-confirmed-problems/P6-progress.md`。

## 返回给我

只返回两行：① P6-acceptance.md 路径；② 一句话摘要（PASS 数/FAIL 数，≤30 字）。绝不返回文件全文。
<!-- AGATE_CARD_START -->
## 当前阶段卡片：P6
