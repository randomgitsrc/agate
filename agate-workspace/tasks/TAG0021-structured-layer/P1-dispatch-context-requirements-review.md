...
独立评审 TAG0021「协议结构化层（RM-AG0022）」的 P1 需求基线 `P1-requirements.md`，产出 `P1-review.md`（status 终值 approved / rejected / needs-revision 三选一，gate 读的是 Header status 字段）。

### 评审对象

`/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace/tasks/TAG0021-structured-layer/P1-requirements.md`（16 条 BDD，按 M0/M1/M2/M3 分组）

### 输入文件（按序读取）

1. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-requirements.md（评审主对象）
2. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P0-brief.md（需求来源：issues/known_risks/env_constraints）
3. /home/kity/oclab/agate/.worktrees/agate-TAG0021/HANDOFF-TAG0021.md（任务交接：范围/硬约束）
4. /home/kity/oclab/dsh-workspace/agate-research/design-structured-layer.md（P1 分析基础：可判定规则/S-1~S-6/M0-M3）
5. {agate_root}/phase-cards/P1-requirements.md（评审规范锚点，随本上下文已注入卡片全文）
...
### 评审要点（对照 P1 卡片「推进条件」逐项核）

1. **BDD 验收条件**：≥1 条、`#### BDD-NN:` 格式、Given/When/Then 可二值判定、按 M0/M1/M2/M3 分组、编号唯一（16 条）
2. **同类扫描**：三组扫描（脚本 grep md 解析点 / phase-cards 门槛产出派发字段 / CHECK 编号空间）命中数量 + 文件清单 + 逐条「本次处理/本次不处理+理由」；回归拦截声明；「只此一处」也要显式写出
3. **P0-brief 时效性**：已质疑；无漂移记录「已核对」；有漂移含 `[P0_STALE: 具体漂移点]` 且按阻塞/记录二选一处理（本任务 1 处轻微漂移已记录，判定其处理方式合规）
4. **frontmatter 机器字段**：`domains` / `packages` / `risk_level` / `phases` / `ceremony`（standard，fail-closed：不声明或要素不满足按 standard）/ `capability_requirements` 三态（available / supplementable / GAP）
5. **NEED_CONFIRM**：无未决 `[NEED_CONFIRM]`；写了 `[NO_NEED_CONFIRM]`；`[SUGGEST:]` 项不阻塞（主 Agent 可采纳）
6. **裁剪说明**：`phases` 全保留的逐阶段理由成立；无违规裁剪
7. **能力自查**：domains=[backend] 无 frontend → 无视觉能力硬要求；capability_requirements 无 GAP

### 评审产出要求

- Header：`phase: P1` / `task_id: TAG0021-structured-layer` / `type: review` / `parent: P1-requirements.md` / `trace_id: TAG0021-P1-20260822` / `status: <final>` / `created: 2026-08-22` / `agent: requirements-review`（**不得** agent=main）
- 正文：逐条 BDD 编号引用（`BDD-NN`）+ 覆盖维度标注（如「BDD 质量 / 同类扫描 / 时效性 / frontmatter / 裁剪 / 能力声明」维度命中情况），rejected/needs-revision 时给出具体问题定位（BDD 编号或节号）与修改要求
- 评审结论与 Header status 一致

### 客观查证信息（本机实测，硬约束）

- 只读评审：不修改 P1-requirements.md、不写任何任务产出文件；唯一写操作是产出 P1-review.md
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep 工具；单步串行不并行 bash
- 状态标记：`[PROD_NOT_TOUCHED]`
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-review.md`
- 分阶段落盘：评审过程关键发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-review-progress.md`

### 返回

只返回两行：① P1-review.md 路径；② status 终值 + 一句话结论（≤30 字）。
</dispatch_guide>
