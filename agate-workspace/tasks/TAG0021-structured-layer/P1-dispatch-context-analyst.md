...
产出 `P1-requirements.md`（需求基线）：TAG0021「协议结构化层（RM-AG0022）」——把 agent 消费的协议规则从 8000+ 行自由文本 markdown 抽成机器可读的 YAML 权威源（rules/phases.yaml + dispatch.yaml + roles.yaml + JSON Schema），双向一致性 gate（S-1~S-6）防漂移，gate 脚本从 grep markdown 迁移到读 YAML（M0-M3 渐进）。设计文档 `design-structured-layer.md` 含架构/schema 草案/S-1~S-6/M0-M3 路径。

### 交付物（M0-M3 阶段化，需求须按迁移阶段组织 BDD）

1. **M0（数据层就位，只加不改）**：`agate/rules/{phases,dispatch,roles}.yaml` + `agate/rules/schema/*.json` + `check-structure-consistency.py`（S-1~S-6）+ `check-yaml-schema.py`——现有脚本继续 grep md
2. **M1（双跑对账）**：选 3-5 个高频脚本（agate-read-gate-commands / check-pruning / check-gate）读 YAML 与 grep 结果对账，不一致告警不阻断
3. **M2（切换权威源）**：对账稳定后脚本改用 YAML，grep 逻辑删除，一致性 gate 提升为阻断
4. **M3（卡片渲染化）**：phase-cards 从 YAML 渲染（模板 + 数据），agate-inject-card.py 改造
5. 测试：schema 校验 / S-1~S-6 双向一致性 / 对账模式回归；count-tests 只增不减

### 核心约束

- **YAML 只承载可判定规则**；叙事（why/示例/注意）留 markdown（人类叙事层）
- **M0-M3 每阶段独立可回退**（纯增量起步）；TDD 先红后绿
- **工具链自举纪律**：~/.agate 稳定版跑 gate，worktree 改（TAG0016 教训）
- **S-1~S-6 双向一致性 gate**：YAML→md（S-1 阶段总览/S-3 卡片/S-4 脚本字段）、md→YAML（S-2）、schema（S-5）、引用完整性（S-6）；漂移 ERROR 阻断
- 测试平台无关（agate 核心约束）；/tmp 只读（--basetemp）
- 改动面极大（协议文档 + 脚本 + 卡片渲染）→ 全部触发 SELF-GATE

### 上游关联

P0-brief 已锁定（issues/known_risks/env_constraints）；design-structured-layer.md（架构/schema 草案 §3/S-1~S-6 §4/M0-M3 §5/风险 §8）为 P1 分析基础；TAG0014 复盘（agent 读 8000+ 行 md 摩擦）与 DEBT0010 实证（grep 解析缺陷）为立项锚。任务工作区 = `{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/`。

### 强制同类扫描（HANDOFF §5 + P0-brief，扫描结论必须写进 P1-requirements.md 正文）

1. **grep 53 个脚本对 markdown 的解析点**（grep -cE 模式清单，按脚本归类）：统计哪些脚本 grep md、解析什么字段（risk_level/phases/packages/domains/gate_commands/candidate_count 等）——为 M1 选脚本提供依据
2. **grep phase-cards 门槛/产出/派发字段清单**：9 张卡各自的门槛/产出/派发字段（为 M0 phases.yaml + M3 渲染化提供数据面）
3. **grep check-protocol-consistency CHECK 编号空间**：现有 CHECK 1-12 编号（新 S-1~S-6 防冲突；是否并入或独立编号空间——设计文档 §6 建议考虑合并）

每条命中：记录命中数量 + 文件清单，逐条判定"本次处理 / 本次不处理 + 理由"。同类型未来新增 → 声明回归拦截手段并转 BDD。

### 产出规格（P1 卡片为准，关键点摘要）

- BDD 验收条件 ≥1 条（Given/When/Then 格式），编号 `#### BDD-NN:`——**按 M0/M1/M2/M3 分组**（如 BDD-M0-1..、BDD-M1-1..，或注释标注阶段归属）
- frontmatter：`phase: P1` / `task_id: TAG0021-structured-layer` / `type: problems` / `parent: P0-brief.md` / `trace_id: TAG0021-P1-20260822` / `status: draft` / `created: 2026-08-22` / `agent: analyst` / `risk_level:` / `phases:` / `packages:` / `domains:`
- P0-brief 时效性质疑：按 P0 卡判据核对（TAG0020 已合并同步——确认结构化层方案正交不漂移），无漂移写"已核对 P0-brief 时效性，无漂移"
- 无未决 `[NEED_CONFIRM]`；无 `status: GAP`；`capability_requirements:` 三态声明

### 输入文件（按序读取，每读完一个追加 progress）

1. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P0-brief.md
2. /home/kity/oclab/dsh-workspace/agate-research/design-structured-layer.md
3. /home/kity/oclab/agate/.worktrees/agate-TAG0021/HANDOFF-TAG0021.md
4. {agate_root}/agate/scripts/*.py（扫描对象 1：grep md 解析点）
5. {agate_root}/agate/phase-cards/*.md（扫描对象 2：门槛/产出/派发字段）
6. {agate_root}/agate/scripts/check-protocol-consistency.py（扫描对象 3：CHECK 编号空间）
7. {agate_root}/agate/WORKFLOW.md / dispatch-protocol.md / state-machine.md / role-system.md（YAML 权威源的数据面参考）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace`；{agate_root}（稳定版，读卡片/角色/脚本用）= `/home/kity/oclab/agate/agate`（= ~/.agate）；改造对象（要 grep 的仓库本体）= `/home/kity/oclab/agate/.worktrees/agate-TAG0021`（含 TAG0019/TAG0020 合并内容——worktree 的 agate/ 已有 ceremony/judge 机制，扫描以 worktree 为准）。
...
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-requirements.md`
- 分阶段落盘：每读完一个输入文件/关键步骤，把发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-progress.md`
