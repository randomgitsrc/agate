...
产出 `P2-design.md`（方案设计）：TAG0021「协议结构化层（RM-AG0022）」——把 agent 消费的协议规则从自由文本 markdown 抽成机器可读 YAML 权威源 + JSON Schema + S-1~S-6 双向一致 gate + gate 脚本迁移（M0-M3 渐进）。

### 上游输入（按序读取，每读完一个追加 progress）

1. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-requirements.md（需求基线：16 条 BDD 按 M0-M3 分组、扫描 1-3 结论、决策 D1-D3）
2. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-review.md（需求评审 approved——记下评审 7 项核验口径）
3. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P0-brief.md（issues/known_risks/env_constraints）
4. /home/kity/oclab/dsh-workspace/agate-research/design-structured-layer.md（架构/schema 草案 §3、S-1~S-6 §4、M0-M3 §5、风险 §8——P2 候选方案的基础）
5. /home/kity/oclab/agate/.worktrees/agate-TAG0021/HANDOFF-TAG0021.md（任务硬约束）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace`；{agate_root}（读卡片/角色/脚本） = `/home/kity/oclab/agate/agate`（= ~/.agate 稳定版）；改造对象 = worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0021`（协议本体含 TAG0019/TAG0020 合并内容）。

### 产出规格（P2 卡片为准，关键点摘要）

- frontmatter：`phase: P2` / `task_id: TAG0021-structured-layer` / `type: design` / `parent: P1-requirements.md` / `trace_id: TAG0021-P2-20260822` / `status: draft` / `created: 2026-08-22` / `agent: architect` + 机器字段 `candidate_count:`（≥2，本任务候选方案多，勿 trivial 化）/ `packages: [agate]` / `domains: [backend]` / `ui_affected: false`
- 候选方案 ≥2 + 权衡表 + 选择理由
- **影响面梳理节（写在候选方案之前）**：改什么（逐文件/小节 + 关联 BDD 编号）/ 不改什么（显式列出 + 理由）/ 风险在哪（每条配缓解）；三组同类扫描结论（P1 §4.1-4.3）作影响面证据基底
- 四字段正文：`gate_commands:`（P3/P5 等，参考卡片样例，勿用 && 拼接多命令——拆独立 key；本任务 /tmp 只读，P5 命令须 `-p no:cacheprovider --basetemp=<可写目录>`）/ `files_to_read:`（控制 P4 implementer 上下文，只列确实要参考的）/ `env_constraints:`（确认 P0-brief 约束 + BDD-16 平台无关）/ `minimal_validation:`（本任务纯代码/文档逻辑→声明"纯代码逻辑，无外部系统依赖"须写明依赖的内部函数/数据转换；若有可跑的最小验证（如 pyyaml 解析样例）也写结果）
- `dispatch_plan:`（可选）：本任务 M0-M3 分批实现（改动面大），若 architect 设计为分批/并行实现可声明 {mode: static-batch} 等，参照卡片契约
- 设计要点对齐 P1 决策：D1（S-1~S-6 独立脚本 + 独立编号空间，不并入 check-protocol-consistency）、D2（BDD 连续编号已定）、D3（M1 首批三脚本：agate-read-gate-commands / check-pruning / check-gate）
- P2 方案须回答：YAML 权威源的数据边界（哪些规则进 YAML、哪些留 md 叙事层）、schema 设计（枚举约束）、S-1~S-6 六条 gate 的判定口径与触发点（pre-commit/CI）、M0-M3 各里程碑的落地文件清单与回退边界、对账模式（M1）的差异可观测出口（WARNING + 计数，BDD-6）

### 评审映射（P2 卡片 C8 机械映射）

domains=[backend] + risk_level=high → **plan-eng-review 必派**（硬规则）。架构产出后主 Agent 会派 plan-eng-review 独立评审你的方案；你只需把方案写清楚（含权衡与风险缓解），评审意见回来后再按需修改。

### 客观查证信息（本机实测，硬约束）

- /tmp 只读 + ptmp 只读（Errno30）：任何测试/脚本运行须 `-p no:cacheprovider --basetemp=<可写目录>`（可写目录例如 worktree 下 dist/ 或 .worktrees 树内）；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；check-protocol-consistency.py 若跑用 worktree 自己的
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep/glob 工具（不占 bash）；单步串行不并行 bash
- 状态标记：`[PROD_NOT_TOUCHED]`（只读稳定版/主 checkout，写操作全部落 worktree）
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-design.md`
- 分阶段落盘：每读完一个输入文件/关键步骤，发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-progress.md`
