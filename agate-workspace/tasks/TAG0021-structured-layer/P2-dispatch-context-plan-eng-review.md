...
独立评审 TAG0021「协议结构化层（RM-AG0022）」的 P2 方案设计 `P2-design.md`，产出 `P2-review.md`（status 终值 approved / rejected 二选一；gate 读 Header status 字段）。

### 评审对象

`/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace/tasks/TAG0021-structured-layer/P2-design.md`（candidate_count: 3，推荐 C1「内聚最小面」，含影响面梳理 + 四字段 + dispatch_plan serial M0-M3）

### 输入文件（按序读取）

1. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-design.md（评审主对象）
2. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-requirements.md（需求基线与 16 条 BDD、决策 D1-D3、同类扫描结论——方案合规性的上游锚）
3. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-review.md（需求评审 approved）
4. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P0-brief.md（任务边界）
5. /home/kity/oclab/dsh-workspace/agate-research/design-structured-layer.md（设计文档：架构/S-1~S-6/M0-M3 权威方案参考）
6. {agate_root}/phase-cards/P2-design.md（评审规范锚点，随本上下文已注入卡片全文）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace`；{agate_root} = `/home/kity/oclab/agate/agate`（~/.agate 稳定版）。

### 评审要点（对照 P2 卡片「推进条件」+ 工程性专项）

1. **候选方案与权衡**：≥2；权衡表客观（C1/C2/C3 差异真实）；选择理由成立；candidate_count 与正文一致
2. **影响面梳理**：改什么（逐文件/小节 + 关联 BDD）+ 不改什么（显式 + 理由）+ 风险在哪（每条配缓解）；三部分齐全且写在候选方案之前；以 P1 同类扫描为证据基底
3. **四字段**：frontmatter packages=[agate]/domains=[backend]/ui_affected=false；gate_commands 各 key 独立（无 && 拼接）、P3/P5 命令含 --basetemp 适配 /tmp 只读、timeout_seconds 分档；files_to_read 精简可控（P4 上下文）；env_constraints 与 gate_commands 边界清晰（声明 vs 执行）
4. **方案与 P1/设计文档一致**：BDD-1..16 均有设计落点或明确回退；D1（S 独立编号空间）/D3（M1 首批三脚本）落实；design-structured-layer §3-§5 对齐
5. **工程性专项（工程评审职责）**：YAML 权威源数据边界合理（可判定规则进 YAML、叙事留 md）；schema 设计可校验（枚举/类型/required）；S-1~S-6 判定口径无歧义、触发点（pre-commit/CI）完整；M1 对账差异可观测出口满足 BDD-6；M2 切换的破坏性变更被识别（UPGRADING）；M3 渲染化的稳定版隔离满足 BDD-13；dispatch_plan serial 合理性（同文件跨批两轮改造的处理）
6. **SCOPE+ 发现**：2 处已在方案内覆盖（SCOPE+1 命名空间、SCOPE+2 随 md-field-get 生效）、SCOPE+3 客观查证修正——确认无越界扩大也无遗漏

### 评审产出要求

- Header：`phase: P2` / `task_id: TAG0021-structured-layer` / `type: review` / `parent: P2-design.md` / `trace_id: TAG0021-P2-20260822` / `status: <final>` / `created: 2026-08-22` / `agent: plan-eng-review`（不得 agent=main）
- 正文：逐项核验结论 + BDD/节号引用；rejected 时给出具体问题定位与修改要求
- 评审结论与 Header status 一致

### 客观查证信息（本机实测，硬约束）

- 只读评审：不修改 P2-design.md / P1-requirements.md / P1-review.md；唯一写操作是产出 P2-review.md
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep 工具；单步串行
- 状态标记：`[PROD_NOT_TOUCHED]`
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-review.md`
- 分阶段落盘：评审过程关键发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-review-progress.md`

### 返回

只返回两行：① P2-review.md 路径；② status 终值 + 一句话结论（≤30 字）。
</dispatch_guide>
