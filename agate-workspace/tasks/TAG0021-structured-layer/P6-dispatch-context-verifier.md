...
执行 TAG0021「协议结构化层（RM-AG0022）」P6 验收：逐条实跑 P1 的 16 条 BDD（用户/系统行为视角），产出 `P6-acceptance.md` + `P6-evidence/`。本任务 domains=[backend]、ui_affected=false——无 UI/视觉验收，证据以脚本/数据/命令执行输出为主。

### 上游输入（按序读取，每读完一个追加 progress）

1. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P1-requirements.md（16 条 BDD 权威：Given/When/Then 判据 + 按 M0/M1/M2/M3 分组）
2. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P2-design.md（C1 方案 §3.7 完成标志 7 条 + gate_commands——BDD 判据的落点）
3. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P5-test-results/unit.md（P5 验证证据：全量 pytest 1198 过 + 7 命令签名）
4. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P4-implementation.md（实现记录：M0-M3 改动清单 + 环境假象说明）
5. {AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P0-brief.md（约束）
6. {agate_root}/phase-cards/P6-acceptance.md（随本上下文已注入卡片全文）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0021/agate-workspace`；{agate_root} = `/home/kity/oclab/agate/agate`（~/.agate 稳定版）；被验收对象 = worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0021`。

### BDD 判据速查（16 条，验收时逐条实跑/实查）

**M0（BDD-1..5）**：
- BDD-1: rules/ 三 YAML 存在且 check-yaml-schema.py 校验全过 exit 0
- BDD-2: check-structure-consistency.py S-1/S-2，人为制造漂移 → 非 0；无漂移 → 0（含正反用例）
- BDD-3: S-5/S-6 引用缺失/枚举非法 → 非 0；合法 → 0（正反用例）
- BDD-4: M0 后全量 pytest 全绿（income P5 已证）+ count ≥ 基线 + consistency 0 ERROR
- BDD-5: S-3/S-4 三方一致（phases.yaml ↔ P2 卡 ↔ check-gate P2 判定）exit 0

**M1（BDD-6/7）**：对账模式 WARNING + 差异计数 + 退出码原语义；覆盖面 ≥3 脚本 + 3 类解析点

**M2（BDD-8/9/10/11）**：对账清零判据 / 静态零命中（已迁移模式 grep 0 命中）/ 一致性 gate 阻断（pre-commit+CI）/ 迁移后回归全绿

**M3（BDD-12/13/14）**：渲染一致 + 篡改 YAML → S-3 非 0 / inject-card 渲染化 + 稳定版隔离 / 渲染化回归全绿

**跨里程碑（BDD-15/16）**：count-tests 只增不减（1202 ≥ 749）/ 平台无关（check-platform-assumptions 0 新增命中）

### 验收方法论

- **逐条实跑/实查**：每条 BDD 的 When 动作真实执行（运行命令/制造漂移/检查文件），Then 判据从命令输出/文件内容实读，记录 exit code
- **证据落盘**：每条 PASS 的证据文件写入 P6-evidence/（命令输出日志带 EXIT_CODE 尾行 / 断言记录 / 对照表），PASS 行引用 `(P6-evidence/{file})` 格式；不造假、不用 1 行文本充数
- **只读验收**：不修改代码/协议文件（验收是观察事实，不是修复）；制造漂移用例在临时副本上做（如复制 YAML 到 tmp 再改），验收后还原
- P5 已全量验证的基底（pytest/consistency/count）可引用 P5-test-results/ 作佐证，但 BDD-1/2/3/5/6/7/8/9/10/12/13 判据须本阶段实跑
- 注意 M0 环境假象：test_bdd_7/test_bdd_25 为沙箱环境假象（P4 已登记 CAPABILITY_GAP），验收 BDD-4/11/14 时以「全量 pytest 1198 过 + 2 环境假象（隔离复跑转绿或固有）」口径记录，不判 FAIL

### 硬约束

- 只读验收：不修改任何代码/协议/测试文件；唯一写操作是产出 P6-acceptance.md + P6-evidence/ + P6-progress.md
- bash 一律外层 timeout（30-600s）；读文件优先 read/grep 工具；单步串行
- 状态标记：未接触生产环境写 `[PROD_NOT_TOUCHED]`
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P6-acceptance.md` + `P6-evidence/`
- 分阶段落盘：`{AGATE_WORKSPACE}/tasks/TAG0021-structured-layer/P6-progress.md`（每条命令执行前追加一行）

### 产出规格（P6 卡）

- `P6-acceptance.md`：frontmatter（phase: P6 / task_id: TAG0021-structured-layer / type: acceptance / parent: P5-verification.md / trace_id: TAG0021-P6-20260822 / status: draft / created: 2026-08-22 / agent: verifier + 机器汇总 pass: N / fail: 0 / ui_affected: false）
- 逐条 `- PASS BDD-NN: 描述 (P6-evidence/{file})`（或 FAIL，但本任务期望 16/16 PASS 0 FAIL）；总结行用 `**Summary**: ...` 格式（不用 - PASS/- FAIL 前缀）
- `P6-evidence/`：每条 PASS 对应证据文件（命令输出日志尾行 `EXIT_CODE: <n>` / 断言记录文件 / 对照表），非空且实质内容

### 返回

只返回两行：① P6-acceptance.md 路径；② 一句话摘要（≤30 字，含 PASS/FAIL 计数）。
</dispatch_guide>
