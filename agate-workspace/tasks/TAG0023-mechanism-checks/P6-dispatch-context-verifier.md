# P6-dispatch-context-verifier — TAG0023 验收

> 派发对象：verifier（P6 验收）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`
> `ui_affected: false`（P2-design.md frontmatter），不涉及 vision-analyst/UI 证据链。

## 目标

逐条验收 P1-requirements.md 的 13 条 BDD，产出 `P6-acceptance.md` + `P6-evidence/`。**本轮先完成 BDD-1~8 与 BDD-10~13 共 12 条**（均可本地验证）；**BDD-9（连续 5 次 CI 稳定）本轮不产出结论**——主 Agent 正在并行触发真实 GitHub Actions CI 运行（5 次），CI 数据就绪后会用一条追加指令让你补上 BDD-9 行并定稿。**先把 BDD-9 行留空（不写 PASS 也不写 FAIL，正文占位说明"待主 Agent CI 数据补齐"），frontmatter 的 `pass`/`fail` 计数先按 12 条已验收的结果填，不要把 13 条都算进汇总**——本轮产出的文件不会被拿去跑 gate（gate 要求 FAIL=0 且 13 条全部有结论），只是先做好前置证据收集，避免等 CI 期间浪费时间。

## 约束（硬约束，按角色文件「诚实边界」）

1. **每条 PASS 必须有真实证据引用**（P6-evidence/ 下的文件路径，文件必须真实存在且非空）；无法验证的标 FAIL，不要因为"覺得应该没问题"就标 PASS
2. **验收报告记录验收时的事实**：如果验收时发现某条 BDD 实际上没通过，如实写 FAIL，不要为了让本轮看起来顺利而回避
3. **不得凑证据数量**：1 行文本文件充数、空文件引用等会被 provenance 审计拦截，也是诚信问题
4. **本任务 domains: [backend]，无 UI**，不需要视觉证据/vision-analyst

## 逐条验收指引（基于 P2-design.md §4 完成标准表的具体判据，不是重复 BDD 原文）

- **BDD-1**（评审重试 WARNING）：跑 `pytest agate/tests/unit/test_check_state_transition.py -k bdd_1 -v`，确认相关用例（含 2 个负面锚点）全部通过；证据=测试输出日志（`P6-evidence/bdd-1-pytest.log`）
- **BDD-2**（P5→P4 阻断）：同上 `-k bdd_2`，含首次单步回退回归用例
- **BDD-3**（子代理空返回 WARNING）：同上 `-k bdd_3`，含分批命名回归用例
- **BDD-4**（回归防呆）：同上 `-k bdd_4`
- **BDD-5**（P8 roadmap 阻断）：`pytest agate/tests/unit/test_check_gate.py -k bdd_5_p8_roadmap -v`
- **BDD-6**（无关联不误拦）：`-k bdd_6_p8_roadmap`
- **BDD-7**（RM-AG0032 历史补记）：`pytest agate/tests/unit/test_check_gate.py -k bdd_7_roadmap -v` **且**直接 `grep "RM-AG0032" agate-workspace/roadmap/roadmap.md | grep done` 确认非空——两个证据都要留（测试通过 + 数据实际存在）
- **BDD-8**（环境敏感测试四要素齐全）：`pytest agate/tests/unit/test_agate_debt_check.py -k bdd_8 -v`
- **BDD-10**（集中清单）：`pytest agate/tests/unit/test_env_sensitive_tests_registry.py -v`
- **BDD-11**（写时自检小节）：`pytest agate/tests/unit/test_agate_render_dispatch_prompt.py -k bdd_11 -v`
- **BDD-12**（错误消息修复提示）：`pytest agate/tests/unit/test_check_frontmatter.py -k bdd_12 -v`
- **BDD-13**（历史回归 3 类）：`pytest agate/tests/unit/test_check_frontmatter.py agate/tests/unit/test_check_routing.py -k bdd_13 -v`

每条用真实跑出来的 pytest 输出（含 PASSED/FAILED 签名行）作为证据文件内容，不要转述。

## 上游关联

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`（BDD 原文）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（§4 完成标准表，逐条判据）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P5-test-results/`（P5 已通过的全量证据，本轮可引用不必重跑全量）

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`
2. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（§4 完成标准表）
3. `{agate_root}/assets/execution-roles/verifier.md`（角色定义）
4. `{agate_root}/phase-cards/P6-acceptance.md`（本阶段卡片）

## 命令超时兜底

每条 pytest 命令用 `timeout 30s <命令>`（单文件/单关键词子集应该很快）。

## 产出

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P6-acceptance.md`（frontmatter 用 `---` 分隔）：
  ```
  ---
  phase: P6
  ...
  created: 2026-08-25
  agent: verifier
  pass: 12   # 本轮 12 条，BDD-9 待补
  fail: 0
  ui_affected: false
  ---
  ```
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P6-evidence/`：各 BDD 的真实 pytest 输出日志 + roadmap.md grep 结果

## 门槛（本轮，非最终 gate 门槛）

- BDD-1~8、10~13 共 12 条均有 PASS/FAIL 结论 + 真实证据引用
- BDD-9 行占位说明清楚（不误标 PASS/FAIL）
- 每个证据文件真实存在、非空、被引用

## 返回给我

只返回两行：① 产出文件路径；② 一句话摘要（12条结果+BDD-9待补，≤40字）。绝不返回文件全文。
