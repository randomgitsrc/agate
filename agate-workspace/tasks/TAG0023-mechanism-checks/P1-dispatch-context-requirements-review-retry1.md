# P1-dispatch-context-requirements-review-retry1 — TAG0023（P1 复评，第 2 轮）

> 派发对象：requirements-review（P1 需求基线独立评审，复评轮）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`（完整目录名：TAG0023-mechanism-checks）

## 背景

上一轮（第 1 轮）你（或前一位 requirements-review agent）给出 `needs-revision`，核心理由：§4.1 同类扫描 1（retries 消费点）实跑复核为 28 文件而非声明的 25，且 `agate/rules/state-transitions.md` 完全缺席判定表。见 `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-review.md`（第 1 轮评审全文，仍保留在原路径，本轮产出会覆盖同名文件——**先读它了解上一轮要求了什么**）。

analyst 已针对性修补（见 `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md` 当前版本 §4.1/§4.3/BDD-13）。主 Agent 已独立跑 `grep -rl "retries" --include="*.py" --include="*.md" agate/ | grep -v '\.pyc' | sort | wc -l` 复核，实测确为 **28**，与本轮 §4.1 声明一致。

## 本轮任务：复评，只审改动点 + 兜底扫一遍全局是否有新问题

1. **重点复核 §4.1**：判定表是否已覆盖全部 28 个命中文件（不要求你重新逐条过一遍所有文件的判定合理性——第 1 轮已判定其余内容通过——但要确认「命中数量」与「表格行覆盖的文件总数」现在能对上，尤其确认 `agate/rules/state-transitions.md` / `agate/tests/conftest.py` / `agate/phase-cards/P1-requirements.md` 三个上一轮遗漏项现在已有判定）
2. **复核 §4.3 统计行**：确认"测试 4"已改"测试 3"
3. **复核 BDD-13**：确认已按二选一之一处理（本轮 analyst 选择了"保留合并 + 补充说明"，检查该说明是否清楚交代了"有意批量校验，非遗漏拆分"）
4. **不要重新评审**上一轮已通过的其余项（BDD 可判定性/编号格式/隐含需求覆盖/跨条一致性/裁剪合理性/审声明/P0_STALE 处理/D1-D5 留白/P1 纯净性/扫描 2/扫描 3 文件清单）——除非本轮修补动作意外破坏了这些已通过项（如果发现被破坏，才需要指出）
5. 若以上 3 点均已妥善修补且未引入新问题 → `status: approved`；若仍有遗漏或修补引入新问题 → `status: needs-revision` 并给出具体点

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`（本轮评审对象，含 §4.1/§4.3/BDD-13 的本轮修补）
2. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-review.md`（上一轮评审全文，了解修改要求，本轮产出会覆盖此文件）
3. `{agate_root}/assets/review-roles/requirements-review.md`（角色定义）

## 验证命令（供交叉核对，非强制）

```bash
grep -rl "retries" --include="*.py" --include="*.md" agate/ | grep -v '\.pyc' | sort | wc -l
```
...
要求同第 1 轮：逐条 BDD 引用锚点（本轮可延续第 1 轮 BDD-1~12 的判定结论，重点补充 §4.1/§4.3/BDD-13 三处的复核结论）+ Header `agent: requirements-review` + `status` 三选一明确。

## 门槛

- P1-review.md 存在且非空，`status` 明确
- 含对 §4.1/§4.3/BDD-13 三个修补点的显式复核结论
- 若 approved：仍需引用 BDD 编号锚点（不允许裸 approved）
