# P2-dispatch-context-plan-eng-review-retry2 — TAG0023（P2 复评，第 3 轮，MAX=3 最后一次预算）

> 派发对象：plan-eng-review（P2 方案评审，复评轮）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`
> **注意**：P2 阶段 retry 上限 MAX=3，本任务已记录 2 次 P2 rejected（round 1/2）。本轮若再次 rejected，主 Agent 将按 state-transitions.md 的重试超限规则处理（PAUSED，交人工决策），不会有第 4 次自动重试。请认真评审，但不要因为"这是最后一次"就降低标准放水。

## 背景

第1轮 rejected：BDD-1 事件源（扫描 review.md status:rejected）结构性不可观测。
第2轮 rejected：architect 重新设计后（评审角色 retry/rev 文件存在性），正则用"文件名含 review 子串"宽松通配，你（或前一位 reviewer）核实出 2 个真实假阳性（`P4-dispatch-context-implementer-review-fix-retry1.md` role:implementer、`P7-dispatch-context-consistency-reviewer-retry1.md` role:architect）。

architect 本轮（第3次尝试）已修正：
1. 正则改为 C8 角色 token 精确枚举：`^P(\d+)-dispatch-context-(requirements-review|plan-eng-review|plan-design-review|plan-ceo-review|cso|review|design-review|review-eng|review-cso)-(retry|rev)\d+\.md$`
2. D1 校验强度：BDD-1 由阻断下调为高优 WARNING（承认枚举仍有残余风险——未来新角色可能恰好用到枚举内 token 字符串、且 `retries[Pn]` 字段本身当前不可靠，双重不确定性不适合阻断级）
3. 显式承认 `P7-dispatch-context-consistency-reviewer-retry1.md` 这类历史命名与实际角色不一致的残余边界无法被任何正则排除
4. 声称 34 个历史文件中新正则匹配 13/21，两个已知假阳性均被排除

**主 Agent 已独立复核**：用新正则重新扫描全仓（含少量因目录结构重复计数的路径），确认两个已知假阳性文件（implementer-review-fix / consistency-reviewer）确实不再匹配新正则，13 个匹配的文件本身没有再核对是否引入新的假阳性。

## 本轮任务：复评，聚焦本次修正的 4 点 + 兜底扫一遍

1. **核实新正则是否引入新的假阳性/假阴性**：自己独立跑一遍新正则，逐一检查所有匹配文件的真实 role（有 frontmatter 的查 frontmatter，没有的看正文/上下文），确认这 13 个左右的匹配文件确实都是真评审角色的重试/复评文件，没有漏网的假阳性
2. **核实枚举列表本身是否有遗漏或多余**：对照 `agate/rules/review-mapping.md`（C8 映射表）核实枚举的评审角色 token 是否完整覆盖所有已知评审角色，有无遗漏（导致假阴性——某评审角色重试文件因不在枚举里而被漏判）
3. **D1 强度下调（BDD-1 阻断→WARNING）是否合理**：这是一个保守化处理（宁可漏判也不误判），检查这个权衡是否站得住——WARNING 而非阻断意味着 RM-AG0042 原始要修的问题（"退货未记录"）对 BDD-1 场景只能提示不能强制拦截，这是否仍然满足 P1 BDD-1 的验收锚"被拦截"（P1 BDD-1 原文 Then 允许"阻断或高优WARNING两路径均满足"，检查 WARNING 路径是否确实是 P1 已经允许的选项，不是本轮临时松绑验收标准）
4. **残余边界承认是否诚实、兜底是否可行**："等 dispatch-protocol.md 完成命名固化 + 线上验证无新假阳性后可升级"这个后续路径是否讲清楚了谁在什么阶段做什么（不是模糊的"以后再说"）
5. **不要重新评审**上一轮已通过的其余项（D2/D3/D4/D5/BDD-2/BDD-3/BDD-4/其余6个候选方案/SELF-GATE纪律/gate_commands）

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（本轮评审对象，重点 §2.1 D6 专节 + D1 + §4 BDD-1 行）
2. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-review.md`（第2轮评审全文，本轮产出会覆盖此文件）
3. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`（BDD-1 原文验收锚，核对 WARNING 路径是否本就允许）
4. `{agate_root}/assets/review-roles/plan-eng-review.md`
5. `{project_root}/agate/rules/review-mapping.md`（C8 映射表，核实枚举完整性）

## 产出（覆盖同名文件）

`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-review.md`

Header 用 `---` 分隔符，trace_id 用 `TAG0023-P2-review-20260824-r3`。

## 门槛

- P2-review.md 存在且非空，status 明确
- 含对本轮 4 点修正的独立验证结论（自己跑正则/查角色映射，不是照抄 architect 自述）
- 若 approved：引用具体锚点

## 返回给我

只返回两行：① 产出文件路径；② 一句话摘要（结论 + 核心理由，≤40字）。绝不返回文件全文。
