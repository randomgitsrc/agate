# P1-dispatch-context-requirements-review — TAG0023 机制校验补强批

> 派发对象：requirements-review（P1 需求基线独立评审）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`（完整目录名：TAG0023-mechanism-checks）

## 目标

独立评审 `P1-requirements.md`（analyst 产出，13 条 BDD，覆盖 RM-AG0042/0043/0044/0045 + RM-AG0032 历史补记），产出 `P1-review.md`，结论 approved / needs-revision / rejected 三选一。**只审不写**，不直接改 P1-requirements.md。

## 约束（硬约束）

1. 按角色文件「检查清单」逐项审：BDD 可二值判定 + 编号连续格式（`#### BDD-NN:`）/ 隐含需求覆盖 / BDD 跨条一致性 / 裁剪合理性 / 审声明（风险分级/裁剪声明 vs 暂存区 diff 证据）/ P1 纯净性（有无掺入解决方案设计）
2. **本任务 domains: [backend]，不含 frontend** —— UI/UX 机制评审要点（UX 类别 BDD / vision 能力声明）不适用，无需按此项打回
3. **审声明核对**：本任务尚无暂存区实际改动（P1 阶段仅产出需求文档，未动生产代码），`git diff --cached` 证据核对可基于"当前无代码改动，risk_level: high 依据改动面预判（check-gate.py/check-state-transition.py/多张卡片/CI/测试）是否合理"判断，不要求存在 diff
4. **同类扫描核实**：analyst 声明的 3 组扫描结论（retries 消费点 25 文件 / roadmap 消费点 5 文件 / 环境敏感测试清单 9 文件）需核实"命中数量 + 逐条判定"是否落盘完整、判定理由是否站得住，而非只看有没有这一节标题
5. **P0-brief 时效性质疑**核实：analyst 记录"已核对无漂移"+ 一条 `[P0_STALE: 派发指引路径拼写误差]` 记录性标注——检查该标注是否符合 P1 卡「阻塞/记录二选一」规则（本例应属"记录"档，不阻塞）
6. **5 个 [SUGGEST] 决策项（D1-D5）**：检查是否每项都在 P1 阶段留白给 P2（不预设具体实现），且没有过度预设方案导致"P1 纯净性"违规
7. **BDD-8（RM-AG0044 复现定位交付形态）**：检查这条是否算"绕过验收锚硬要求"——P0-brief 验收锚是"test_bdd_14 连续 5 次 CI 稳定"，analyst 把它拆成 BDD-8（复现计划+证据基线，本阶段锚）+ BDD-9（连续 5 次 CI 稳定，最终锚）两条，检查这个拆分是否合理保留了最终验收锚而不是用中间产物顶替最终验收标准

## 上游关联

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`（评审对象）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P0-brief.md`（对照原始 issue/验收锚是否被准确转译）
- `{project_root}/HANDOFF-TAG0023.md`（对照范围/纪律/风险止损是否被吸收）
- `{agate_root}/assets/review-roles/requirements-review.md`（角色定义，评审依据）
- `{agate_root}/phase-cards/P1-requirements.md`（本阶段卡片，含 gate 规则）

## 输入文件（逐一读，每读完一个追加 progress）

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`
2. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P0-brief.md`
3. `{project_root}/HANDOFF-TAG0023.md`
4. `{agate_root}/assets/review-roles/requirements-review.md`
5. `{agate_root}/phase-cards/P1-requirements.md`

## 产出

`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-review.md`

要求（按角色文件「输出格式」+「实质锚点要求」）：
- 逐条 BDD-1..13 判定 + 覆盖维度标注（数据/前端/多端/边界/兼容，本任务多数条目前端维度标 ✗ 属正常，因 domains 不含 frontend）
- 结论必须引用具体锚点（BDD 编号 + 覆盖维度清单），不允许裸 "approved"
- Header `agent: requirements-review`（agent≠main 是 gate 硬性要求）
- 结论三选一：approved / needs-revision / rejected；needs-revision/rejected 须给出具体修改点
...
- 含 BDD 编号引用锚点 + 覆盖维度标注
- status 字段明确三选一之一
- agent 字段为 requirements-review（非 main）

## 返回给我

只返回两行：① 产出文件路径；② 一句话摘要（结论 + 若 needs-revision/rejected 则含核心理由，≤40 字）。绝不返回文件全文。
