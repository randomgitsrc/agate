# P5-dispatch-context-verifier — TAG0023 技术验证

> 派发对象：verifier（P5 技术验证）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`

## 目标

执行 P2-design.md 固化的 `gate_commands.P5`（+ P5_consistency/P5_ruff/P5_count），产出 P5-test-results/，标注通过/失败情况。这是 external-output-gate（主 Agent 验证你的产出，不是自己跑命令），你的产出必须真实可信、含真实测试运行器输出签名。

## 约束（硬约束）

1. **全量测试**：跑 `agate/tests/` 全量套件（不是只跑本任务改动相关的测试文件），P5 阶段要求全量验证含非本任务测试
2. **依次执行 4 条 gate_commands**（P2-design.md §3「四字段」已固化，不可改）：
   ```bash
   ...
   ```
   各自独立跑，不要用 `&&` 拼接（短路反模式）
3. **产出必须含真实 test runner 输出签名**（P5 commit→push 窗口残余风险 N5 缓解要求）：`P5-test-results/unit.md` 里必须能被 `grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)'` 命中 >0 次，不能是摘要转述
4. **预期结果**（供你核对，不代表可以不跑直接抄）：主 Agent 在 P4 阶段已独立跑过一次，得到 `1238 passed, 2 skipped`，`check-protocol-consistency.py --strict-errors-only` exit 0（0 ERROR），`ruff check agate/` All checks passed，`count-tests.sh` 1240（collect-only 口径，含 skipped）——你需要自己重新跑一遍验证这个结果在当前暂存/HEAD 状态下仍然成立，不是抄数字
5. **命令超时兜底**：全量 pytest 预期约 150s，用 `timeout 300s <命令>`；其余三条命令较轻量，`timeout 60s`
6. **环境**：Linux；`/tmp` 只读，pytest 必须用 `--basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider`（已在命令里）
7. **无 PROD_TOUCHED**：全程只读/写 worktree 与指定的 basetemp 目录

## 上游关联

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（`gate_commands` 固化声明）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P4-implementation.md`（本次实现总结）

## 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（§3 四字段，gate_commands 权威声明）
2. `{agate_root}/phase-cards/P5-verification.md`（本阶段卡片，产出规格 + 判定规则）

## 产出

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P5-test-results/unit.md`：标注 4 条命令各自的通过/失败情况 + failed 数量 + 真实 test runner 输出片段（含可被上述 grep 命中的行）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P5-test-results/fail-list.txt`：failed 测试 id 逐行列出，无失败则为空文件

## 门槛（什么算完成）

- 4 条命令全部 exit 0（pytest 0 failed；consistency 0 ERROR；ruff All checks passed；count-tests 不低于 P3/P4 基线）
- P5-test-results/unit.md 含真实签名行（非转述）
- 无 PROD_TOUCHED

## 返回给我

只返回两行：① 产出目录路径；② 一句话摘要（4 条命令结果，≤40字）。绝不返回文件全文。
