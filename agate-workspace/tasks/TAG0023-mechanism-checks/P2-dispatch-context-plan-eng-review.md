# P2-dispatch-context-plan-eng-review — TAG0023 方案评审

> 派发对象：plan-eng-review（P2 方案评审，工程经理视角）。这是本轮的强制指令，不是参考信息。
> 任务目录：`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/`
> 触发依据（C8 机械映射）：`domains: [backend]`（必派）+ `risk_level: high`（硬规则叠加），去重后只需 1 个 plan-eng-review，无需专家组组长汇总。

## 目标

独立评审 `P2-design.md`（architect 产出，8 候选方案覆盖 4 子项 + dispatch_plan 5 批），产出 `P2-review.md`，结论 approved / rejected。**只审不写**。

## 约束（硬约束，按角色文件「评审重点」逐项审）

1. **数据流/状态机/接口契约/错误边界**：逐子项检查方案的输入→处理→输出路径是否清晰，尤其 RM-AG0042 的三类事件源判定（评审 rejected/P5→P4 回退/子代理空返回）是否都有明确的"读什么→怎么判→怎么输出"链路
2. **多方案探索（nudge 强度）**：8 个候选方案（4 组×2）是否每组都有真实权衡（不是稻草人陪衬）+ 理由自洽——你只需要判断"是否探索了+理由自洽"，不需要判断"是否选择了客观最优"
3. **P2 最小验证**：`minimal_validation` 字段是否符合要求——RM-AG0042/0043/0045 声明"纯代码逻辑无外部依赖"是否理由站得住；RM-AG0044 声明的"已做真实验证（CI 日志 forensic 分析）"是否证据链完整可信（不是自我宣称，要看是否真的给出了可核查的证据：CI job id / commit sha / 具体日志摘录）
4. **实现就绪度**：方案是否清晰到 implementer 无需额外步骤计划即可自主实现；`files_to_read` 是否覆盖各批次实现所需全部上下文（分批次列出，检查有无遗漏关键文件）
5. **D1-D5 决策闭环核实**：P1 §5 留给 P2 的 5 个决策点，检查本设计是否都给出了明确结论（不能停留在"留待后续"）：
   - D1（RM-AG0042 校验强度）：§2.1 声明"分层——BDD-1/2 阻断，BDD-3 高优 WARNING"，检查这个分层理由是否站得住
   - D2（RM-AG0043 匹配算法）：§2.2 声明"task_id 精确匹配「关联任务」列，支持一对多"，检查是否覆盖 P1 BDD-5/6 两个边界
   - D3（RM-AG0044 根因交付形态）：P1 说的是"本阶段交付复现计划+证据基线，根因结论留 P2/P4"——本设计声称已经在 P2 阶段就把根因**从候选升级为已确认**（用真实 CI 日志），检查这个升级是否有充分证据支撑，不是自我宣称
   - D4（0042/0043 分批纪律）：本设计声称"两者最终生产代码文件零重叠"（0042 落 check-state-transition.py，0043 落 check-gate.py），推翻了 P1/dispatch-context 预判的"同触碰 check-gate.py"——检查这个推翻是否成立（可用 grep 或读代码验证两个函数落点是否真的不冲突）
   - D5（RM-AG0045 写路径载体）：§2.4 声明"挂载既有 dispatch-prompt.md 返回前自检节，不新造拦截机制"，且诚实标注了"源码数检查仍依赖 git add"的边界——检查这个诚实标注是否掩盖了"实质上没有真正提前"的问题，还是确实构成了有效的"写时提前"（判断标准见 dispatch-context 原文的"折返成本单位"论证是否自洽）
6. **同簇互扰风险重新评估**：H3 原判"0042/0043 同触碰 check-gate.py"，本设计声称已解除——若你核实后发现这个结论不成立（例如某处遗漏），必须标记为阻塞级问题
7. **BDD 完成标准可执行性**：§4「完成标准」表逐条 BDD 给出的判据是否真的可从命令/文件读出明确值（不是又一层模糊描述）
8. **SELF-GATE 处理纪律**：§4 末尾是否已声明后续分批 commit 的 self-gate 逐次声明纪律（不是一次性笼统带过）
9. **gate_commands 反模式检查**：是否有 `&&` 拼接短路问题（本设计沿用 dispatch-context 原值未新增 key，检查沿用是否合理，有无遗漏需要新增的验证命令）

## 上游关联

- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`（评审对象）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`（BDD-1..13 + D1-D5 原始留白）
- `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-dispatch-context-architect.md`（派发指引，含硬约束清单，核对 architect 是否全部响应）
- `{agate_root}/assets/review-roles/plan-eng-review.md`（角色定义）
- `{agate_root}/phase-cards/P2-design.md`（本阶段卡片，含 gate 规则）

## 输入文件（逐一读，每读完追加 progress）

1. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-design.md`
2. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P1-requirements.md`
3. `{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-dispatch-context-architect.md`
4. `{agate_root}/assets/review-roles/plan-eng-review.md`
5. `{agate_root}/phase-cards/P2-design.md`
6. 按需核实：`agate/scripts/check-state-transition.py`（L132-141 现有 diff>=2 阈值）、`agate/scripts/check-gate.py`（L1181 gate_p8）、`agate/scripts/agate-retreat-state.py`（L47 attempts.append）——architect 声称的几个"关键发现"可以用 grep/read 快速核实是否属实，不要只信文字描述

## 产出

`{AGATE_WORKSPACE}/tasks/TAG0023-mechanism-checks/P2-review.md`

Header 用 `---` YAML frontmatter 分隔符（**不要用 markdown 代码围栏 ```，P1 阶段刚吃过这个亏**）：
```
---
phase: P2
...
type: review
parent: P2-design.md
trace_id: TAG0023-P2-review-20260824
...
架构问题（阻塞级）：
  - [具体问题 + 文件/函数 + 建议]（无则写"无"）
架构问题（非阻塞）：
  - [具体问题 + 记录到 TD-xxx]（无则写"无"；若提"后续应重构/存在架构债"必须用标准 DEBT 格式登记到 {AGATE_WORKSPACE}/debt/tech-debt.md，不能只在评审里提一句）
测试缺口：
  - [什么场景没有测试覆盖]（无则写"无"）
锁定决策：
  - [本次评审后确定下来的技术方向]（逐条列出你认可的 D1-D5 决策 + 8 候选方案的选择）
```

## 门槛（什么算完成）

- P2-review.md 存在且非空，`status` 明确 approved/rejected
- 含具体锚点引用（候选方案选择/D1-D5 决策/BDD 完成判据），不允许裸 approved
- agent 字段为 plan-eng-review（非 main）

## 返回给我

只返回两行：① 产出文件路径；② 一句话摘要（结论 + 若 rejected 则含核心理由，≤40字）。绝不返回文件全文。
