...
| `agate/tests/regression/test_repo_url_no_stale_rename.py`（多处，第 6/9/38/92/195/286 行） | **本次不处理**（P6.5 judge 复核发现，见下方第二条 `[BASELINE_CHANGE]`） | P3 新增的本任务权威判定测试文件自身——文件顶部注释/docstring/正则源码出于说明与断言目的引用了字面 `randomgitsrc/agate` 字符串，是该测试"要检测什么"这一逻辑本身结构性必需的自指引用（测试代码必须包含它要搜索的目标字符串），不是需要被替换的活跃品牌层引用，性质与上述 4 处同源。 |
...
[BASELINE_CHANGE: P6.5 judge 第一轮独立复核（fresh context，只信证据与 git log）发现第 5 处边界
案例——P3 新增的 `test_repo_url_no_stale_rename.py` 自身命中扫描，但该测试文件的 `_is_exempt()`
函数早已实现"文件自我豁免"逻辑（P4/P5/P6 阶段 dispatch-context 也一直把这条豁免当"已知盲区"
使用），却从未正式走本节"逐条判定+理由"的基线变更流程。judge 据此判 BDD-10 为 FAIL
（status: needs-revision，P6.5-judge-verdict.md 第一轮，2026-08-26）。主 Agent 已核实并批准，
追加第 5 处边界案例（对应 BDD-10 正文的第 6 类豁免），判定理由与既有 4 处方法论完全同源，
不改变本节判定方法论，只是补齐正式授权。]

上述 5 处"不处理"的判定已固化为 BDD-10 的显式排除清单（见第 4 节），使"全仓无旧 URL 残留"
验收锚可判定，不会因为这 5 处历史/任务性质文档而永远 FAIL。
...
- When 从命中结果中排除以下豁免范围：① `archived/`、`agate-workspace/tasks/**`、
  ...
  既有豁免清单变得完整、使验收锚真正可达成] ⑥ `agate/tests/regression/test_repo_url_no_stale_rename.py`
  （P3 新增的本 BDD 权威判定测试文件自身——文件顶部注释/docstring/正则源码出于说明与断言目的
  引用了字面 `randomgitsrc/agate` 字符串，是该测试"要检测什么"这一逻辑本身结构性必需的自指
  引用，不是需要被替换的活跃品牌层引用，性质与②③④⑤同源）
  [BASELINE_CHANGE: P6.5 judge 独立复核（fresh context，只信证据与 git log）发现：该测试文件
  的 `_is_exempt()` 函数（第 90-99 行）已实现"文件自我豁免"这第 6 类豁免逻辑，且 P4/P5/P6 阶段
  的 dispatch-context 已多次引用这条"已知盲区"结论作为 BDD-10 判定依据，但 P1-requirements.md
  原文豁免清单当时只列了 5 类，从未正式补入这第 6 类、也没有对应的 `[BASELINE_CHANGE]` 标注——
  按 P1 原文字面授权执行残留扫描，仓库当前真实状态下剩余命中数不为 0（命中于该测试文件自身），
  官方 `gate_commands.P5_bdd10_residual_scan` 命令原文实测 exit 1，与本 BDD"排除后剩余命中数为
  0"的验收目标矛盾——judge 据此判 BDD-10 为 FAIL（status: needs-revision，P6.5-judge-verdict.md
  第一轮，2026-08-26）。这是流程合规缺口（豁免逻辑先斩后奏，未经基线变更协议走完主 Agent 核实
  批准），不是功能性错误——该测试文件自 P3 产出以来内容从未变化，"自我豁免"这条逻辑本身的必要性
  与设计 §5.3"保持历史真实/说明性引用不算活跃品牌层"的既有判定方法论完全同源（与②③④⑤四类
  豁免同一判定逻辑，只是这次的"历史性/说明性引用"载体是测试代码而非文档）。主 Agent 已核实并
  批准，补第 6 类豁免，不改变本 BDD 的判定逻辑，只是让豁免清单与测试文件已实现的
  `_is_exempt()` 逻辑及 P4-P6 阶段一直沿用的"已知盲区"判定口径取得正式的基线授权一致性。本次
  修订后需重新走 P6 验收 + P6.5 judge 第二轮复核，不能仅凭本条文本变更就视为已解决]
- Then 排除后剩余命中数为 0
