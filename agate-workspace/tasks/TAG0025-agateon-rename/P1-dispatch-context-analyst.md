...
### 目标
产出 `P1-requirements.md`：把 P0-brief 的一句话任务（Agateon 品牌改名执行 Phase 0-1）展开为
含 BDD 验收条件的需求基线，供 P2 设计与 P6 验收使用。**范围只到 Phase 0（品牌声明）+ Phase 1
（仓库改名 + 硬编码 URL 同批更新 + remote 迁移）**，Phase 2（v1.0 CLI 别名/品牌 prose 统一/
brand-check/CHECK 10 白名单扩展）与 Phase 3（门户）不展开 BDD，只在裁剪说明里写"不在范围，
理由 = 设计文档已定阶段边界"。

### 约束

1. **内部命名空间禁动**：`agate/` 目录 / `agate-workspace/` / `~/.agate` / `AGATE_*` /
   `agate-*.py` / `agate_common` 一律不改。backtick token 一律保留（判定规则见
   `docs/design-notes/design-rename-execution.md` §5.3）。**禁止任何"全局 find-replace
   agate→agateon"式的 BDD 措辞**——每条 BDD 必须点名具体文件+行。
2. **不可逆操作双确认**：GitHub 仓库改名执行前需 ① `gh` repo 管理权限实测 ② 用户在场放行——
   这两条中的权限核实已由主 Agent 完成（见下方客观查证信息 A），**BDD 里不要重复设计成
   "申请权限"步骤**；但需把"改名前必须再次确认用户在场放行"写成需求层面的强制前置条件
   （权限核实不能替代每次操作前的放行确认，二者不是一回事）。
3. **CHANGELOG 纪律**：当前 CHANGELOG.md 无 `[Unreleased]` 段（已核实，见客观查证信息 C）——
   本任务首个 commit 批次需建立该段并含 TAG0025 条目，写成 BDD。
4. **验收锚必须落成可判定 BDD**：设计文档 §7 表格给出的 Phase 0/1 两行验收锚（首页可见品牌
   声明；301 / `git ls-remote` / 无旧 URL 残留 / `in:name` 首屏命中）必须逐条转成
   Given/When/Then，不能只在正文提一句。
5. **P1 只回答"要不要改/改完什么样算对"**，不要在 BDD 里写具体 sed/grep 替换命令（P4 实现
   细节）；"GitHub 仓库改名"具体放在哪个阶段执行、以什么交互形式确认用户放行，由 P2 设计决定，
   P1 只需要把"改名前必须确认用户在场放行"写成强制前置条件。
6. **同类扫描不可省**：下方客观查证信息 D 已给出全仓硬编码 URL 扫描的完整命中列表，analyst
   需要做的是逐条判定"本次处理/本次不处理 + 理由"并写进 P1-requirements.md 正文，不需要重新
   跑扫描。

### 上游关联

- P0-brief.md 已锁定范围与已知风险五条（同类/影响面预判、remote 迁移、内部命名空间禁动、
  不可逆操作、CI 耦合、CHANGELOG 纪律、design note 非强制照搬、用户侧协同项）
- 执行地基 `docs/design-notes/design-rename-execution.md` 三轮评审通过（2026-08-25 入 main），
  §4 影响面盘点表列出的 7 处硬编码 URL 是 Phase 1 范围核心，§7 迁移节奏与验收锚表是本任务
  验收锚的权威来源
- `agate-workspace/tasks/TAG0025-agateon-rename/env-rename-handoff.md` 已给出 remote 迁移的
  完整机制说明（worktree 共享 `.git/config`），P1 涉及"remote 迁移影响面"BDD 时以此 + 下方
  客观查证信息 B 为准，不要重新枚举 `git worktree list`

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P0-brief.md`
2. `docs/design-notes/design-rename-execution.md`（重点 §4/§5.1/§5.3/§7）
3. `agate-workspace/tasks/TAG0025-agateon-rename/env-rename-handoff.md`
4. `docs/design-notes/rename-recommendation.md`（决策记录，回答"为什么改"，按需查阅不必通读）
5. `{agate_root}/WORKFLOW.md`「需求与验收机制」一节
6. `AGENTS.md`（项目约定）

### 产出文件字段
用 `FILE={AGATE_WORKSPACE}/tasks/TAG0025-agateon-rename/P1-requirements.md agate-md-field-set --list`
...
<objective_info>
### A. `gh` 权限（对应 P0-brief known_risks 第 4 条 ①）
```
$ gh auth status
✓ Logged in to github.com account randomgitsrc — token scopes 含 delete_repo/repo/admin:org
$ gh api repos/randomgitsrc/agate --jq '.permissions'
{"admin":true,"maintain":true,"pull":true,"push":true,"triage":true}
```
结论：`gh` 对 `randomgitsrc/agate` 具备 admin 权限，改名操作技术上可执行。**用户在场放行仍需在
实际执行改名那一步单独确认**，权限核实不能替代放行确认。

### B. 本机 worktree 拓扑（对应 P0-brief known_risks 第 2 条）
```
$ git worktree list
/home/kity/oclab/agate                           c1e889f [main]
/home/kity/oclab/agate/.worktrees/agate-TAG0025  5d80d3d [feat/TAG0025-agateon-rename]
$ git remote -v
origin  https://github.com/randomgitsrc/agate.git (fetch)
origin  https://github.com/randomgitsrc/agate.git (push)
```
结论：本机只有 2 个工作树（主 checkout + 本 TAG0025 worktree），共享同一 `.git/config`
（env-rename-handoff.md 已实测确认）——remote 迁移只需在主 checkout 执行一次
`git remote set-url`，无需逐 worktree 操作；迁移后建议每个 worktree 各跑一次 `git fetch`
做验证性抽查。

### C. CHANGELOG 现状（对应约束 3）
```
...
## [0.63.0] - 2026-08-25
```
结论：当前无 `[Unreleased]` 段，最新已发布段是 `[0.63.0]`。

### D. 全仓硬编码仓库 URL 扫描（对应 P0-brief known_risks 第 1 条，P1 补全仓扫描要求）
命令：`grep -rn "randomgitsrc/agate\b" --include=*.md --include=*.py --include=*.sh
--include=*.yml --include=*.yaml . --exclude-dir=.git --exclude-dir=.worktrees`（已排除
`randomgitsrc/agateon` 自身命中）。完整命中分类如下：

**设计 §4 已列的 7 处（本次必须处理，Phase 1 范围内）**：
- `install.sh:24`
- `agate/scripts/agate-changes.py:116`
- `agate/scripts/agate-install.py:55`
- `README.md:5`（badge）、`README.md:29`（安装入口）
- `README.zh-CN.md:5`（badge）、`README.zh-CN.md:29`（安装入口）

**补全扫描新发现，需 analyst 判定的边界案例（均不在设计 §4 清单里，也不在归档豁免层）**：
- `docs/design-notes/agateon-trademark-research.md:3`（研究文档正文引用当时仓库 URL，描述
  "拟更名"背景）
- `docs/superpowers/specs/2026-08-15-docs-suite-review.md:89`（历史评审快照表格里的一行核对
  结果）
- `HANDOFF-TAG0025.md`（本任务自己的交接单，多处引用旧 URL 用于描述验收命令/验收锚——是任务
  执行说明文档，不是面向最终用户的产品文档）

**确认不在处理范围（已有归属，仅供参照，不需要 analyst 重新判定）**：
- `agate-workspace/tasks/**`、`agate-workspace/archived/**`、`archived/**` 下的所有命中——
  均为任务记录/归档文档，设计 §5.3 迁移范围边界表已明确豁免（"保持历史真实"）
</objective_info>
