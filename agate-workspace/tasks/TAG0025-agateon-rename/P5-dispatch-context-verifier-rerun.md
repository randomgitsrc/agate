...
### 目标
首轮 P5 验证发现 1 个真实失败（ruff RUF005），已回 P4 修复（改动
`agate/tests/regression/test_repo_url_no_stale_rename.py` 第 260 行一处，implementer 已自查
3 条命令通过）。本次是**修复后全量重跑**（T027 教训：修复可能引入回归，不能只检查修复的那一项），
重新执行全部 24 个 `gate_commands.P5_*` key，覆盖 P5-test-results/（覆盖首轮产出，写最终结果）。

### 约束

1. **必须全量重跑，不是只跑修复项**：24 条 key 全部重新执行，不要因为"其他 23 条上次都过了"
   就跳过。
2. **P5_bdd10_residual_scan 已知盲区规则同首轮**：该 shell key 对
   `test_repo_url_no_stale_rename.py` 自身文档字符串的盲区依然存在（修复 ruff 违规不影响这个
   问题，两者无关）。判定规则同首轮：失败输出若精确只等于该测试文件自身命中 → 已知盲区非真实
   残留，以 pytest 版本 `test_bdd_10_repo_wide_residual_scan_zero_after_exemptions` 为权威判定。
3. **P5_count_tests 预期**：延续首轮已核实的口径——`count-tests.sh` 按测试函数计数，本任务
   净增 11 个测试函数（不是 1 个文件），预期数字为 **1304**（1293 基线 + 11）。此次修复未增删
   任何测试函数，数字应与首轮一致。
4. **P5_unit 是本次重点**：首轮失败点在这里（`test_bdd_34_shellcheck_three_hook_shells_and_ruff`），
   本次必须确认该测试 PASSED，且不能只孤立验证这一个测试——要跑 `agate/tests/unit/` 全量，
   确认没有因为这次一行修复引入其他新问题（预期没有，因为改动是纯语法等价替换，但仍要实跑确认，
   不能假设）。
5. **不要执行任何写操作**：改名/remote 迁移已完成，本次不重复执行。

### 上游关联

- `P5-test-results/unit.md`（首轮产出）「1. P5_unit」节记录了原始失败详情，可引用背景，但你的
  产出是**覆盖性重写**（不是在首轮基础上打补丁），因为可能涉及的执行时间戳/HEAD commit 等
  上下文都已变化
- `P4-implementation.md`「重试 1：ruff RUF005 修复」节记录了 implementer 的修复与自查结果

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P2-design.md`（gate_commands 全文，权威清单）
2. `agate-workspace/tasks/TAG0025-agateon-rename/P5-test-results/unit.md`（首轮产出，了解背景，
   本次要覆盖重写而非叠加）
3. `agate-workspace/tasks/TAG0025-agateon-rename/P4-implementation.md`（「重试 1」节）

### 产出文件字段
覆盖重写 `agate-workspace/tasks/TAG0025-agateon-rename/P5-test-results/unit.md` +
`fail-list.txt`（用 Write 工具整体重写，不是 Edit 追加）。
</dispatch_guide>
...
<objective_info>
- 修复已实测（implementer 自查）：`ruff check` All checks passed；
  `test_bdd_34_shellcheck_three_hook_shells_and_ruff` PASSED；
  `test_repo_url_no_stale_rename.py` 11 个测试全部仍 PASSED
- 唯一改动：`agate/tests/regression/test_repo_url_no_stale_rename.py:260`
  `CORE_FILES + ["CHANGELOG.md"]` → `[*CORE_FILES, "CHANGELOG.md"]`（语法等价替换）
- 当前工作区：该修复尚未 commit（仍是未暂存的工作区改动），HEAD 仍是 `18a6b7b`（P4 批次 2）
</objective_info>
