...
### 目标
把 P1-requirements.md 的 16 条 BDD（品牌声明 3 + 硬编码 URL 同批更新 7 + 不可逆操作前置条件 1 +
改名验收锚 3 + remote 迁移 2）转成可实现的技术方案，产出 `P2-design.md`。

### 约束

1. **候选方案 ≥2**：本任务虽然目标状态已被设计文档锁定（改成什么样已定），但**怎么编排执行**
   仍有真实的设计空间，不是伪命题——至少要在下面这个关键决策点上给出候选方案权衡：
   **不可逆的 GitHub 仓库改名操作放在哪个阶段/哪一步执行、以什么具体机制获得"用户在场放行确认"**
   （BDD-11 要求）。例如候选 A："改名"作为 P4 implementer 的最后一步，implementer 在执行改名
   调用前停下向主 Agent 报告 `[NEED_USER_CONFIRM: 即将执行不可逆改名]`，主 Agent 转达用户后
   subagent 才继续；候选 B：改名作为独立于 P4 常规实现批次之外的单独确认步骤，由主 Agent 亲自
   执行（不派发给 subagent），implementer 只负责改名前后的文件层改动。**不要把这个决策点想成
   形式化陪衬**——它直接决定 P4 怎么编排、subagent 能不能被信任执行不可逆操作，选错了要么在
   P4 卡住要么造成事故。除此之外，若还有其他真实的设计分歧点（如 7 处 URL 更新是否用脚本化
   批量替换 vs 逐文件手改；是否落地 P1 §3.4 SUGGEST 的回归测试），也可以在候选方案里体现权衡。
2. **不得使用 `design_trivial`/`follows_existing_pattern` 简化为单候选**：本任务含不可逆外部操作
   的编排决策，不满足"trivial"或"已有既定模式可抄"的简化条件。
3. **内部命名空间禁动**：设计方案不得涉及任何触碰 `agate/` 目录本身路径名、`~/.agate`、
   `AGATE_*`、`agate-*.py` 文件名、`agate_common` 的内容——这些只是"被改动的文件"（如
   `agate/scripts/agate-install.py` 的文件内容一行 URL），不是"被重命名的对象"，方案里不要
   混淆两者。
4. **批次原子性（对应 BDD-9）**：方案设计必须回应"Phase 1 核心 7 处 URL 更新如何保证落在同一
   commit"，不能只在文字上承诺"会注意"，要给出具体执行顺序（如：先在同一批次内改完 7 处文件 →
   一次性 `git add` + `git commit`，不允许边改边分批提交）。
5. **gate_commands 设计要覆盖本任务的验证特殊性**：本任务的验收手段大部分不是传统单元测试，而是
   grep 审计 / curl 探测 / git 命令探测。P2 固化的 gate_commands 需要把这些验证手段变成可重跑的
   命令（哪怕是简单的 shell 一行），不能只声明 `pytest` 就完事——BDD-4~10（7 处 URL + 残留扫描）
   与 BDD-12~16（改名验收锚 + remote 迁移）都需要各自可执行的验证命令声明。同时不能漏掉
   HANDOFF-TAG0025.md 已给出的**回归底线**："1293 用例全绿 + consistency 0 ERROR"——这条底线
   与本任务新增的 BDD 验证命令是两回事，都要在 gate_commands 里体现（不要用 `&&` 拼接，参考
   phase 卡片"`--strict` 反模式"一节，每个校验独立一个 key）。
6. **minimal_validation 字段不要套模板句子**：本任务不是"纯代码逻辑"（依赖 GitHub 的 301 重定向
   行为、`gh api` 改名调用的真实效果），但这类外部系统行为的验证有个特殊性——**重定向只有在改名
   真正发生之后才能观察到，无法在方案设计阶段做"预先最小验证"**（不像"验证某 API 返回什么 MIME
   类型"那样可以现在就 curl 一下）。请如实声明这个情况（不可预先验证 + 为什么 + 替代的风险缓解
   方式是什么，例如"改名后立即跑 4 条验收锚，若 301 未按预期出现，属已知风险已在 P0-brief 声明，
   不阻断——GitHub 官方文档承诺仓库改名保留 301"），不要为了凑格式硬写"纯代码逻辑，无外部系统
   依赖"这种不成立的声明。
7. **files_to_read 精简**：只列 P4 implementer 真正需要参考的文件（预计：install.sh、
   agate/scripts/agate-install.py、agate/scripts/agate-changes.py、README.md、
   README.zh-CN.md、CHANGELOG.md），不要把全仓文档都列进去。

### 上游关联

- P1-requirements.md（16 条 BDD，approved）：`risk_level: medium`、`domains: [docs, cli, ops]`、
  `phases:` 全流程不裁（P1-P8）、`packages: [agate-brand-docs, agate-installer-scripts,
  agate-repo-admin]`
- P1-review.md 已 approved，含 1 条非阻塞 SUGGEST（BDD-10 豁免类别①措辞可对齐设计 §5.3 原文
  口径，加宽为 `agate-workspace/**`）——architect 可在方案里顺手采纳（改动极小），非强制
- P1 §3.4 的 `[SUGGEST: 为 Phase 1 核心 7 处文件新增一条轻量回归测试]`——由本阶段 architect
  决定是否采纳，若采纳需在候选方案中体现并落到 gate_commands

### C8 评审映射（主 Agent 已判定，仅告知不需要 architect 重复判断）

P1 声明的 `domains: [docs, cli, ops]` 不是 role-system.md C8 表里的标准枚举值
（backend/frontend/mcp/security），但 C8 表本身声明"表内触发是最低要求，表外应根据实际场景主动
派评审"。本任务改动含真实代码文件（`agate/scripts/*.py`、`install.sh`）与一个不可逆外部操作，
主 Agent 已判定按 `backend` 同等对待，派发 **plan-eng-review**（单一评审角色，无需组长汇总）。
architect 不需要重复这个判断，只需要把方案写清楚供 plan-eng-review 审。

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P1-requirements.md`
2. `agate-workspace/tasks/TAG0025-agateon-rename/P1-review.md`
3. `agate-workspace/tasks/TAG0025-agateon-rename/P0-brief.md`
4. `docs/design-notes/design-rename-execution.md`
5. `agate-workspace/tasks/TAG0025-agateon-rename/env-rename-handoff.md`
6. `HANDOFF-TAG0025.md`（含回归底线基线数字：1293 用例 + consistency 0 ERROR，与验证命令样例）
7. `AGENTS.md`

### 产出文件字段
用 `FILE={AGATE_WORKSPACE}/tasks/TAG0025-agateon-rename/P2-design.md agate-md-field-set --list`
...
<objective_info>
- 回归基线（HANDOFF-TAG0025.md 已实测，2026-08-26）：unit 1160 passed + 2 skipped，其余 131
  passed，consistency 0 ERROR（--strict-errors-only；总 collect 1293）
- 关键验证命令（HANDOFF-TAG0025.md §4，可直接复用/改造进 gate_commands）：
  ```
  ...
  curl -sI https://github.com/randomgitsrc/agate | head -3   # 改名后验收锚
  timeout 30 git ls-remote https://github.com/randomgitsrc/agateon.git HEAD
  grep -rn "randomgitsrc/agate\b" --include="*.md" --include="*.py" --include="*.sh" \
    --include="*.yml" . --exclude-dir=.git --exclude-dir=.worktrees
  ```
- `gh` 权限与 worktree 拓扑已在 P1 阶段实测（见 P1-dispatch-context-analyst.md 客观查证信息
  A/B），本阶段无需重新核实，可直接引用
- 本任务不涉及 `ui_affected`（无 frontend 域），P2-design.md 不需要「UI 设计」节
- `project_phase` 未声明 bootstrap（既有项目），P2-design.md 不需要 `P2-skeleton.md`
</objective_info>
