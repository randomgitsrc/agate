...
### 目标
独立评审 `P1-requirements.md`（analyst 已产出，16 条 BDD），产出 `P1-review.md`：判定
...
1. **只审不写**：不得直接修改 P1-requirements.md；发现问题写进 P1-review.md，交主 Agent 回派
   analyst 修改。
2. **实质锚点要求**：结论必须逐条引用 BDD 编号 + 覆盖维度标注，不接受裸 "approved"。
3. **重点核查项（本任务特有风险点，按角色定义检查清单执行，以下是需要重点验证的地方，不是
   额外清单）**：
   - **内部命名空间禁动是否被误触**：本任务最大反模式是"全局 find-replace agate→agateon"，
     逐条 BDD 检查是否点名具体文件+行，而不是笼统措辞
   - **BDD-10（全仓无旧 URL 残留）的豁免清单是否真的让验收锚可判定**：P1 正文 §3.2 给了 3 处
     边界案例的"不处理"判定（商标调研文档 / 历史评审快照 / HANDOFF-TAG0025.md），BDD-10 是否
     把这 3 处显式写进了排除范围——若没写全，这条验收锚会永远 FAIL，需打回
   - **BDD-11（用户在场放行确认）与"gh 权限已核实"是否被正确区分**：约束里明确权限核实不能
     替代放行确认，检查 BDD-11 的 Given/When/Then 有没有把两者混为一谈
   - **BDD-9（批次原子性）是否真的可二值判定**："同一 commit"是可验证的客观标准，检查有没有
     退化成主观描述
   - **裁剪声明（phases 全流程不裁）是否有充分理由**：正文 §6 给了每阶段保留理由（P3 因
     risk_level=medium 不可裁 / P7 因跨文件一致性风险保留 / P8 因 roadmap 回写强制），逐条核对
     是否站得住，不是抄一遍卡片模板
   - **risk_level: medium 是否与需求范围匹配**：正文 §6 末尾给了 low/high 两侧排除理由，核对
     逻辑是否自洽（本 P1 阶段暂存区只有需求文档本身的改动，尚无实现层 diff，`risk_level` 判断
     依据应该是"计划中的改动范围"而非"当前已存在的 diff 规模"——审声明节按此语境核对，不要
     因为暂存区 diff 很小就判定 risk_level 虚高）

### 上游关联

- P1-requirements.md 由 analyst 产出，基于 P0-brief.md + 执行地基
  `docs/design-notes/design-rename-execution.md`（三轮评审通过）
- P1-progress.md 记录了 analyst 的完整读取与产出轨迹，可用于交叉核对结论出处

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P1-requirements.md`（评审对象）
2. `agate-workspace/tasks/TAG0025-agateon-rename/P0-brief.md`
3. `docs/design-notes/design-rename-execution.md`（重点 §4/§5.3/§7，核对 BDD 是否覆盖设计
   文档给出的验收锚与判定规则）
4. `AGENTS.md`（项目约定）

### 产出文件字段
用 `FILE={AGATE_WORKSPACE}/tasks/TAG0025-agateon-rename/P1-review.md agate-md-field-set --list`
...
<objective_info>
- P1-requirements.md 已通过 `agate-frontmatter-check.py` 校验（exit 0，无 ERROR，analyst 已跑过）
- `.state.yaml` 已确认含 `judge.enabled: true`（P0 阶段写入，本次 P1 未改动）
- P1-requirements.md frontmatter：`risk_level: medium` / `phases: [P1..P8]`（全流程不裁）/
  `packages: [agate-brand-docs, agate-installer-scripts, agate-repo-admin]` /
  `domains: [docs, cli, ops]`（无 frontend，UX 分类框架要求不适用）
</objective_info>
