...
### 目标
独立评审 `P2-design.md`（architect 已产出，2 候选方案 + 影响面梳理 + 26 条 gate_commands），
产出 `P2-review.md`：判定 approved / rejected。单一评审角色，无需组长汇总。

### 约束

1. **重点核查项 1——候选方案的"致命问题"论证是否站得住**：architect 否决候选 A（改名放在 P4
   implementer 内部、subagent 停下等待恢复）的理由是"DSH 平台是否支持暂停/恢复 subagent 无实测
   证据"，选定候选 B（改名抽离出来由主 Agent 本人执行）。请核实这个论证是否只是"为了选 B 而找
   的借口"，还是真实的风险点——检查 P0-brief.md `executor_env.platform: "dsh"` 与
   `has_task_tool: true` 声明，判断"能派发子任务"是否确实不等于"能暂停并恢复同一子任务实例"这个
   区分是否成立、是否被架构师如实呈现（而非夸大）。
2. **重点核查项 2——`[SCOPE+]` 发现是否真实且处理得当**：architect 在本阶段重新实跑了 P1 用过的
   grep 命令，发现 `docs/design-notes/design-rename-execution.md:35` 未被 P1 BDD-10 的 4 类豁免
   覆盖，会导致该 BDD 永久 FAIL；已补第 5 类豁免并写入 `gate_commands.P5_bdd10_residual_scan`。
   请**自己重新跑一遍**同一条 grep 命令，核实这个发现是真的（不是编造的），且 5 类豁免应用后
   命中数确实为 0；同时判断"不改 P1-requirements.md 正文、只让 gate_commands 落地修正后的清单"
   这个处理方式是否合理（P1 基线保护规则：P2-P8 不应直接改 P1，需变更须走
   `[BASELINE_CHANGE: 理由]` 标注——architect 选择不改 P1 文本，只在 P2 与 gate_commands 落地，
   需要判断这样处理是否已经足够，还是必须要求 P1 文本也补一次 `[BASELINE_CHANGE]`）。
3. **重点核查项 3——gate_commands 的 shell 命令本身是否正确可执行**：26 条 key 里含多条复杂
   shell 一行式（如 `P5_bdd9_atomic_commit`、`P5_bdd10_residual_scan`、`P5_bdd4to8_new_url_present`）。
   请挑重点在本地（worktree 内，只读方式，不产生实际改动/不执行 `gh api`/`git remote set-url`）
   验证这些命令语法是否正确、逻辑是否符合它所对应的 BDD 语义（例如 `P5_bdd10_residual_scan` 的
   排除正则是否漏了什么，`P5_bdd9_atomic_commit` 在当前无改动状态下跑会得到什么结果、是否合理）。
   不要求全部 26 条都实跑（部分依赖改名已发生，当前无法跑），但至少验证不依赖改名的那些
   （BDD-1~10 相关的 key）。
4. **重点核查项 4——`minimal_validation` 是否如实**：检查 5 条 `minimal_validation` 记录，尤其
   第一条（BDD-10 豁免验证从 refuted 到 confirmed）是否真的对应上面第 2 点的实测过程，其余几条
   "not_needed"的理由是否是真实的技术限制（重定向只有改名后才能观察）而非偷懒占位。
5. **实质锚点要求**：结论必须引用具体产物锚点（候选方案权衡、影响面三部分、gate_commands 覆盖
   的 BDD 编号），不接受裸 approved。

### 上游关联

- P1-requirements.md（approved，16 条 BDD）+ P1-review.md（approved，含 1 条非阻塞 SUGGEST）
- P2-design.md 由 architect 产出：`candidate_count: 2`、`packages`/`domains` 与 P1 一致、
  `ui_affected: false`

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P2-design.md`（评审对象）
...
4. `docs/design-notes/design-rename-execution.md`（含 §3.1，用于核实重点核查项 2 的行号引用）
5. `agate-workspace/tasks/TAG0025-agateon-rename/env-rename-handoff.md`

### 产出文件字段
用 `FILE={AGATE_WORKSPACE}/tasks/TAG0025-agateon-rename/P2-review.md agate-md-field-set --list`
...
<objective_info>
- P2-design.md 已通过 architect 自检：`agate-frontmatter-check.py`、gate_commands 经
  `agate-md-field-set-gate-commands.py` 写入并回读校验、`check-gate.py P2` 预跑仅剩预期的
  "P2-review.md 不存在"阻塞项
- 本次 P2 属单一评审角色（plan-eng-review），已按 C8 表外场景由主 Agent 判定派发（见
  P2-dispatch-context-architect.md「C8 评审映射」节），不需要组长汇总
</objective_info>
