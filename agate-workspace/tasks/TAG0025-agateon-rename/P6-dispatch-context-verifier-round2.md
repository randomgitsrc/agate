...
### 目标
本次是 P6 验收第 2 轮（第 1 轮被 P6.5 judge 判 `needs-revision`，BDD-10 判 FAIL，其余 15 条
PASS）。根因已修复：P1-requirements.md BDD-10 的豁免清单当时只正式授权 5 类，但判定 BDD-10 时
实际依赖了测试文件 `_is_exempt()` 里一个未经授权的第 6 类"自我豁免"逻辑——现已在
P1-requirements.md 补齐第 6 类豁免的 `[BASELINE_CHANGE]` 正式授权（第 4 节 BDD-10 正文 + 第 3.2
节边界案例表均已更新）。第 1 轮的 `P6-acceptance.md` + `P6-evidence/` 已被
`agate-archive-stale-outputs.py` 归档清空，本轮**从零重新产出**，不是在旧文件基础上修修补补。

### 约束

1. **16 条 BDD 全部重新实跑**：即使第 1 轮 15 条已判 PASS，本轮仍要逐条独立重新执行验证命令、
   重新产出证据文件（不得复用/引用归档目录里的旧证据）。
2. **BDD-10 是本轮重点**：现在 P1-requirements.md BDD-10 的 Given/When 已正式列出 6 类豁免（含
   新增第 6 类：`agate/tests/regression/test_repo_url_no_stale_rename.py` 自身）。请：
   - 读取 P1-requirements.md 最新的 BDD-10 完整 Given/When/Then 文本（6 类豁免的完整清单）
   - 用 P1 原文这 6 类豁免重新执行全仓残留扫描（可以直接跑 pytest 权威判定
     `test_bdd_10_repo_wide_residual_scan_zero_after_exemptions`，该测试的 `_is_exempt()`
     函数逻辑现在与 P1 正式授权的 6 类豁免一致），确认命中数为 0
   - 证据文件里要明确写出"现在 6 类豁免均已在 P1-requirements.md 正式授权"，不要再用"已知盲区"
     这种暗示"未被正式承认"的措辞——这次是正式修复后的状态，不是变通处理
3. **其余 15 条 BDD 沿用第 1 轮判定方向即可，但仍要独立重新执行命令**：第 1 轮 judge 已确认
   BDD-1~9、11~16 的证据支持 PASS，本轮预期结果一致，但你仍需重新跑一遍命令拿到本轮的真实
   证据（不是照抄归档文件）。
4. **BDD-9 证据记得完整**：第 1 轮 judge 指出 BDD-9 的证据文件 `diff --stat` 输出被截断，虽然
   judge 独立用 `git show --stat` 核实后确认无误判为 PASS，但这次请确保证据文件里的
   `diff --stat` 输出完整（不要截断到看不全 5 个核心文件），避免下一轮 judge 又要额外做独立核实。
5. **不执行任何写操作**：改名/remote 迁移已完成，不重复执行。

### 上游关联

- P1-requirements.md（本轮已更新，BDD-10 含 6 类豁免，第二处 `[BASELINE_CHANGE]` 标注）
- P6.5-judge-verdict.md（第 1 轮判定，記录在案供你理解为什么要重跑——但你仍需独立验证，不能
  只是"因为 judge 说过 15 条 PASS 就直接抄"）
- `.archived/{timestamp}-P6/`（第 1 轮归档，仅供你了解结构参考，不得引用其内容作为本轮证据）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0025-agateon-rename/P1-requirements.md`（16 条 BDD 全文，重点
   BDD-10 最新 6 类豁免版本）
2. `agate-workspace/tasks/TAG0025-agateon-rename/P0-brief.md`

### 产出文件字段
覆盖式重新产出 `agate-workspace/tasks/TAG0025-agateon-rename/P6-acceptance.md` +
`agate-workspace/tasks/TAG0025-agateon-rename/P6-evidence/`（16+ 个证据文件）。
</dispatch_guide>
...
<objective_info>
- P1-requirements.md BDD-10 现含 6 类豁免（① archived/等归档层 ② 商标调研文档 ③ 历史评审快照
  ④ HANDOFF-TAG0025.md ⑤ design-rename-execution.md ⑥
  test_repo_url_no_stale_rename.py 自身），均含 `[BASELINE_CHANGE]` 正式授权标注
- 测试文件 `agate/tests/regression/test_repo_url_no_stale_rename.py` 的 `_is_exempt()`
  函数逻辑本身未改动（第 1 轮起就已实现第 6 类豁免逻辑），本次只是 P1 文本补齐了对应授权，
  代码不需要改
- 第 1 轮归档位置：`agate-workspace/tasks/TAG0025-agateon-rename/.archived/`（仅供了解，本轮
  不得引用其内容作为证据）
</objective_info>
