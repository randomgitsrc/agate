...
<dispatch_guide>
> ⚠️ 修复轮（增量模式）：只做下述两项最小改动，其余不动。
> 上轮产出：`agate-workspace/tasks/TAG0029-gate-parser-fix/P4-implementation.md`
> 上轮约束：`agate-workspace/tasks/TAG0029-gate-parser-fix/P4-dispatch-context-implementer.md`（复用其全部约束）
> 评审定论：`agate-workspace/tasks/TAG0029-gate-parser-fix/P4-review.md`（S1 更新 + I1 简化）

### 修复目标（仅两项，不做其他改动）
1. **S1 存量同步**（P4-review §3 S1）：`agate/tests/unit/test_check_tdd_red.py` L588-611 `test_pyx_1_read_gate_commands_p3_html_and_project_module`——将其对 `P3_html` 被收集的断言更新为退役语义：`P3_html` 不再被收集（删 `"cmd": "npx vitest run"` 与 `"formatter": "vitest.sh"` 两行断言，改为断言输出中无 `npx vitest run`）；`P3` 与 `project_module` 断言保留。函数名保留（历史可追溯），可在 docstring 首行加 `[TAG0029: P3_html 已退役]` 注记。
2. **I1 机械简化**（P4-review §2 I1）：`agate/scripts/agate-read-gate-commands.py` L106 `elif key == "P3" and not is_gate_meta_key(key):` 简化为 `elif key == "P3":`（后半恒真，与 P2 §3.2 字面一致）。

### 约束
- 两处最小改动，不碰其他代码；不改 P3 新增测试；不改公共库/YAML。
- 改完自跑：S1 所在测试 + A/B 两批 10 用例，全绿。
- P4-implementation.md 追加一行备注（S1/I1 落地说明），不动已有内容。
- 返回前状态标记写 `[PROD_NOT_TOUCHED]`。
</dispatch_guide>

### 上游关联
- P4-review.md status=approved（CRITICAL 0；S1 更新 + S2/S3 保留；I1 可选简化——本次一并执行）。

### 输入文件
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P4-review.md`（§2 I1 + §3 S1 定论）
- `agate/tests/unit/test_check_tdd_red.py`（L588-611）
- `agate/scripts/agate-read-gate-commands.py`（L106）
...
- S2（P3_js 断言仅 returncode）/ S3（公共判据参数化）保留不动——grep 确认未误伤。
