...
### 目标
逐条实跑 P1 9 条 BDD（BDD-1~9），产出 `P6-acceptance.md`（frontmatter pass/fail/ui_affected + 9 条 `- PASS BDD-NN: 描述 (证据)` 行，二值判定）+ `P6-evidence/`（每条 BDD ≥1 证据文件，非空且有实质内容，尾行 `EXIT_CODE: 0`）。

### 约束
- **逐条实跑，不看代码推断**：每条 BDD 按 Given/When/Then 实际执行（调真实解析器/judge/扫描器/读卡片文本），输出落盘对应证据文件。
  - BDD-1：含行内注释块 → 解析器输出纯命令 + `bash -c` exit ≠ 2（证据：解析输出 + bash 执行日志）
  - BDD-2：未闭合引号块 → exit 非 0 + stderr 解析错误 + 无残渣（证据：退出码 + stderr 日志）
  - BDD-3：exit 2 + 语法文案 + 零统计 → judge exit 1（中/英各一例；证据：判定日志）
  - BDD-4：P3_xxx 块 → commands 无该键（证据：JSON 输出）
  - BDD-5：三键共存 → 含裸 P3 且无元键（证据：JSON 输出）
  - BDD-6：读 worktree `agate/phase-cards/P2-design.md` gate_commands 节 → 含禁令文本 + 原因（证据：节选文本）
  - BDD-7：fixture 数据面 → R2 0 命中 exit 0（证据：扫描日志）
  - BDD-8：目录外裸调用 → R2 命中 exit 1（证据：扫描日志）
  - BDD-9：本任务 P2-design P3/P4 块含 scanner 条目（证据：节选文本）
- **PASS 行格式**：`- PASS BDD-N: 描述 (证据文件名)`（括号引用相对 P6-evidence/ 的真实存在文件）；总结行不用 `- PASS` 行首（provenance 审计要求）。
- **frontmatter**：phase=P6, task_id=TAG0029, type=acceptance, parent=P5-test-results/unit.md（实际为 P5 产出），trace_id=TAG0029-P6-20260904（执行日为准）, agent=verifier, status=draft, pass=9, fail=0, ui_affected=false（与 P2 一致；backend 任务无 vision/UI）。
- **证据要求**：P6-evidence/ 非空；每证据文件含实质内容（命令输出全文，不是 1 行充数）；需执行的证据脚本输出尾行 `EXIT_CODE: 0`。
- 无 UI 面：不派 vision-analyst，不截图，不写 vision 引用。
- 每验完一条追加 `P6-progress.md`。返回前自跑语法确认，不声称"验收已通过"。
</dispatch_guide>

### 上游关联
- P1-requirements.md 9 条 BDD（验收依据）；P5-test-results/unit.md（全量 1444 绿，技术验证已过）；P4-implementation.md（实现落点）。
- `.state.yaml` phase=P5（P6 推进随 P6 产出 commit 一起）；judge.enabled=true（P6 后强制 P6.5）。

### 输入文件
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P1-requirements.md`（BDD-1~9 原文——逐条实跑依据）
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P5-test-results/unit.md`（技术验证结论）
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P2-design.md`（BDD-9 断言对象：P3/P4 scanner 条目）
- `agate/phase-cards/P2-design.md`（BDD-6 断言对象：禁令子节）
- `agate/scripts/agate-read-gate-commands.py` + `agate/scripts/check-tdd-red.py` + `agate/scripts/check-platform-assumptions.py`（实跑对象）
...
- backend 纯脚本任务：无 UI、无 vision、无 E2E；证据全为命令执行日志/输出文件。
- P6.5 judge 复核在 P6 commit 后强制执行（主 Agent 另派，本次派发不含）。
- 注：该文件禁止包含 verdict 预判（provenance 审计要求——本文件自身不得出现行首 - PASS/- FAIL）。
</objective_info>
