...
### 目标
独立评审 `/home/kity/oclab/agateon/.worktrees/agate-TAG0029/agate-workspace/tasks/TAG0029-gate-parser-fix/P1-requirements.md`，产出 `P1-review.md`（Header `status: approved` / `needs-revision` / `rejected` 之一，`agent: requirements-review`）。只审不写——不直接改 P1-requirements.md，评审意见由主 Agent 回派 analyst 修改。

### 约束
- 按角色文件检查清单逐项核对：BDD 可二值判定（编号 `#### BDD-NN:` 连续、每条单 Given-When-Then、无主观词）+ 隐含需求五维度覆盖 + BDD 跨条一致性 + 裁剪合理性（本任务主张全阶段，逐项确认不可裁理由）+ 审声明（risk_level=high / phases 全量 vs 实际改动面匹配）+ P1 纯净性（无解决方案设计混入）。
- 本任务 domains=[backend]，无 frontend——确认无需 UX 类别 BDD 与视觉能力条目；若发现实际存在前端面，打回。
- 重点核对三处易错点：① 同类扫描是否把 `agate-read-p5-commands.py` / `agate-gate-missing-cmds.py` 的"本次不处理"理由坐实（P0-brief scope 锁定 vs 同源扩散的平衡）；② BDD-3 是否真正区分"命令串语法错误 exit 2"与"运行器正常退出报告错误"两种输入（假绿灯根因正在于此）；③ BDD-7/BDD-8 豁免边界是否可二值判定（目录声明绑定 vs 宽匹配，H5 要求）。
- 评审结论必须引用具体锚点：approved 须列每条 BDD 编号 + 覆盖维度清单（数据/前端/多端/边界/兼容逐项标注）+ 隐含需求条目编号 + 审声明 diff 证据。裸 approved 视为假完成。
- 产出文件 Header 字段：phase=P1, task_id=TAG0029, parent=P1-requirements.md, trace_id=TAG0029-P1-20260904（与 P1-requirements.md 同日），agent=requirements-review, status 按结论填写（初值 draft 评审完改）。BDD 编号锚点须出现在正文（P1 gate 检查锚点存在性）。
- 返回前跑 `python3 agate/scripts/check-frontmatter.py` 自检你写的文件（worktree 根执行），非 0 先修正再返回。
</dispatch_guide>

### 上游关联
- P1 analyst 已产出 P1-requirements.md（9 条 BDD：BDD-1~2 解析器值清洗 / BDD-3 judge 侧 / BDD-4~6 P3 键收集 / BDD-7~9 扫描器；risk_level=high；phases 全量；domains=[backend]；`[NO_NEED_CONFIRM]`；无 CAPABILITY_GAP；时效性质疑结论为无漂移）。
- 主 Agent 初检：frontmatter 校验通过；9 条 BDD 编号连续；无真阻塞 `[NEED_CONFIRM]` 标记；无 CAPABILITY_GAP。

### 输入文件
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P1-requirements.md`（评审对象——精读全文）
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P0-brief.md`（范围锁定依据：三缺口 scope + out-of-scope 三条——核对需求是否越界）
- `agate-workspace/tasks/TAG0029-gate-parser-fix/P1-dispatch-context-analyst.md`（上轮约束：BDD 覆盖要求 + frontmatter 要求——核对 analyst 是否偏离派发）
...
- `.state.yaml` 已推进 phase=P1 + judge.enabled=true（TAG0028 惯例格式）。
- 评审对象关键事实（主 Agent 实测，供交叉核对）：值清洗同模式 5 处中 P1 主张处理 2 处（同文件 L57/L66）、不处理 2 文件 3 处（P5 脚本 2 处 + missing-cmds 1 处）；P3 收集仅 L60 一处；R2 正则全仓唯一；judge_result exit 2 落末尾 exit 0。
