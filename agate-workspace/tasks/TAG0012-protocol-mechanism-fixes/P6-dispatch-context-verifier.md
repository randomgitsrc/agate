...
对 P1-requirements.md 的 23 条 BDD（BDD-1~22 + BDD-15b）逐条做**用户视角验收**（这里的"用户"是
未来读这些协议文件的主 Agent/subagent），产出 P6-acceptance.md + P6-evidence/。本任务无 UI
（`ui_affected: false`），不涉及截图/vision，但**不能只做关键词存在性检查**（那是 P5 pytest 已
做过的机械检查）——P6 要验证的是**语义正确性**：内容是否真的满足 BDD 的 Given/When/Then 意图，
不是只看关键词字面出现。

### 约束

1. **P6 ≠ 重跑 P5**：`test_protocol_mechanism_anchors.py` 28/28 通过只证明"关键词存在"，不证明
   "内容语义正确"。你需要对每条 BDD 实际打开对应文件、读实际新增内容，判断它是否真的回答了
   P1 BDD 的 Then 子句要求的问题（例如 BDD-10 不能只看"止损轮次"四个字出现，要确认数值确实是
   2、确实声明了独立计数、确实有 READY 后三条归属判据这三点都写清楚了）。
2. **证据形式**：本任务无截图，用**文本摘录证据**——每条 BDD 的 PASS 行引用一个 `P6-evidence/`
   下的证据文件，文件内容是从对应协议文件摘录的实际新增文本片段（可以是 `git show HEAD -- <file>
   | grep -A N` 或直接摘录段落），让证据文件本身能独立证明"内容确实存在且语义正确"，不是空文件
   或一行话充数。
3. **PASS/FAIL 只能二值**，不接受"部分满足"。若发现某条 BDD 的实际落地与 Then 子句有出入（如
   数值不对、逻辑遗漏），标 FAIL 并说明具体缺口——不要因为"关键词都在"就宽松判 PASS。
4. **frontmatter**：`pass: N` / `fail: N` / `ui_affected: false`，无 `regression_pass`（本任务
   非 refactor 类型，P1 未声明 `change_type: refactor`）。
5. **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`，证据路径用
   `(P6-evidence/{filename})` 格式，多文件用逗号分隔。总结行不能用 `- PASS`/`- FAIL` 开头。
6. **P5 结果可引用但不能替代**：P5-test-results/unit.md 的全量回归结果可以作为"回归未破坏既有
   行为"的旁证，但每条 BDD 仍需要独立的语义验证证据，不能用"P5 全绿"一句话覆盖全部 23 条。
7. **BDD-22 特殊**：验收标准是"P2-design.md §3.7 显式声明了'不做脚本硬校验'的决定与理由 +
   test_protocol_mechanism_anchors.py 测试文件存在且全部用例可运行"——检查这两点即可判 PASS，
   不需要额外的脚本改动证据（因为设计决定就是不改脚本）。
8. **完成后自跑 `check-p6-format.py --fix`**（格式归一化，非 gate，是产出后的自查步骤）：
   ```
   python3 ~/.agate/scripts/check-p6-format.py --fix /home/kity/oclab/agate/.worktrees/agate-TAG0012/agate-workspace/tasks/TAG0012-protocol-mechanism-fixes/P6-acceptance.md
   ```

### 上游关联

- P1-requirements.md（23 条 BDD 原文，Given/When/Then 完整定义，验收对照的唯一权威来源）
- P4-implementation.md（改动记录，含每个文件的改动摘要，可作为定位起点，但不能只信任它的自述，
  仍需自己打开实际文件核实）
- P5-test-results/unit.md（回归证据，28 条锚点 + 909 全量测试全绿，可引用不可替代语义验证）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0012-protocol-mechanism-fixes/P1-requirements.md（23 条 BDD，验收依据）
- {AGATE_WORKSPACE}/tasks/TAG0012-protocol-mechanism-fixes/P4-implementation.md（改动记录，定位起点）
- {AGATE_WORKSPACE}/tasks/TAG0012-protocol-mechanism-fixes/P5-test-results/unit.md（回归证据）
- git HEAD 下的实际协议文件（12 个改动文件，见 P4-implementation.md 清单）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree HEAD 已含 P5 commit（e40adac），P4 的 12 个协议文件改动 + P5 的验证结果均已
  落盘。无 UI/无 vision 能力需求（`ui_affected: false`）。
- P5 独立验证结果：909 passed + 2 skipped + 0 failed；consistency --strict 0 ERROR；
  28/28 锚点测试通过；shellcheck 0 issue。
</objective_info>
