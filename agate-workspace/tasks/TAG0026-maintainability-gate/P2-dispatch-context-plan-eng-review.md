...
独立评审 `P2-design.md`（architect 已产出，候选 A 选定），产出 `P2-review.md`：判定
approved / rejected，只审不改。你是单评审角色（C8 映射去重后仅 plan-eng-review），直接产出
P2-review.md，无需组长汇总。Header `agent: plan-eng-review`（≠main，硬要求）。

### 约束

1. **只审不写**：不得修改 P2-design.md；发现问题写进 P2-review.md，交主 Agent 回派 architect
   修改。
2. **评审重点（角色定义基础上，本任务特有风险点必须逐条核查）**：
   - **gate_p4 挂载点与返回约定**：设计声称新步骤落在既有第④步（staged 代码检查 :893-905）
     之后、骨架 WARNING（:907-925）之前；声称不新增 return 2、violations 为空时行为与现状
     等价。核对 check-gate.py:870-927 实际结构，验证该顺序声称是否成立——尤其"门槛 c 复用
     既有 ①②③ 检查"的声称：新步骤在 ①②③ 之后执行，BDD-9（数量对齐但评审未 approve 仍阻断）
     是否真的由顺序天然保证
   - **consistency 扫描面方案（R6）**：设计声称 `check_script_alignment`（:771-785）按**字面
     basename** 在 caller 文件找 `check-maintainability.py`，import 语句是裸模块名不含 `.py`，
     故挂载处注释必须含字面 `check-maintainability.py`。实测核实 check-protocol-consistency.py
     的 callers 校验机制（是否真按字面 basename 匹配、锚点登记位 :745-751 之后、GATE_SCRIPT_
     EXEMPT :791-794），验证 M3+R6 方案是否真能防 CHECK9-coverage
   - **R7（P3* 键被当测试命令执行）**：设计声称 `agate-read-gate-commands.py:60` 把所有
     `P3*` 非元键收集为测试命令。实测核实该行为，验证"gate_commands 禁止 P3_xxx 键"的规避
     是否必要且正确
   - **count_kf_entries 样例行防虚高（R8）**：设计声称模板样例行首用 `| # |` 不命中
     `^\|\s*[0-9]+\s*\|` 正则。实测核实正则行为
   - **_norm_rel 单源复用（R4）**：设计声称经 `_load_script("agate-risk-score")` 加载后复用
     `_norm_rel`。核对 agate-risk-score.py:46-54 的 _load_script 模式与 :86-88 的 _norm_rel，
     验证复用链是否可行（import 路径、模块名、函数可见性）
   - **god-file 行数计算（R3）**：设计声称 before=`git show HEAD:{path}`（新增文件=0）、
     after=`git show :{path}`（staged 版本）。核对 git plumbing 语义：staged-but-not-committed
     文件 `git show :path` 是否真能取到内容；`HEAD` 不存在（首次提交）时的行为
   - **gate_commands 契约**：11 键声明是否完整（P3 运行器 / P5 全量 / consistency /
     count_tests / ruff / shellcheck）、无 `&&` 链、per-key timeout 档位合理（P5=600s）、
     无 P3_xxx 键（R7）；P5 命令是否与 AGENTS.md 的 worktree 验证命令一致
     （consistency 必须用 worktree 自己的脚本）
   - **多方案探索（v0.6 nudge）**：§2 是否 ≥2 候选 + 权衡 + 选择理由自洽（候选 B 是否被
     约束 5 直接排除、候选 C 的分层代价论证是否成立）
   - **实现就绪度**：files_to_read 是否覆盖实现所需全部上下文（含行段），implementer 能否
     无需额外探索即可实现；§5 测试落点与 13 条 BDD 是否一一对齐（M9=1..6/11/12/13、
     M10=7..10 + 回归面）
   - **minimal_validation**：是否含"纯代码逻辑，无外部系统依赖"声明 + 理由（§8 是否满足）
   - **影响面梳理（§1）**：改什么（M1-M10 落点+BDD 关联）/ 不改什么（显式列出+理由）/
     风险（R1-R10 配缓解）三部分是否齐全、证据是否客观（行号可核对）
3. **实质锚点要求**：结论必须引用具体锚点（文件:行号 / BDD 编号 / gate_commands 键名），
   不接受裸 approved。
4. **DEBT 格式强制**：若提出"后续应重构/存在架构债"，必须用标准 DEBT 条目格式
   （模板 `agate/assets/templates/tech-debt-template.md`，evidence 必填），在评审意见中给出
   条目内容（登记由主 Agent 执行）。

### 上游关联

- P1-requirements.md（13 条 BDD）是设计验收对照物；P0-brief.md 是范围与风险基线
- 落地计划 `docs/design-notes/rm-ag0046-maintainability-gate-plan.md`（v3）§2/§4.1 是实现
  蓝图——设计与其不一致处须指出（除非有实测依据的修正）
- P2-progress.md 记录了 architect 的设计轨迹，可交叉核对

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0026-maintainability-gate/P2-design.md`（评审对象）
2. `agate-workspace/tasks/TAG0026-maintainability-gate/P1-requirements.md`（BDD 对照）
3. `agate-workspace/tasks/TAG0026-maintainability-gate/P0-brief.md`
4. `agate/scripts/check-gate.py`（:25-58 / :174 / :870-927 / :930-985 / :1335-1346）
5. `agate/scripts/agate-risk-score.py`（:41-59 / :86-88 / :202-229）
6. `agate/scripts/check-protocol-consistency.py`（:697-830 一带：锚点表 / callers 校验 /
   豁免表 / CHECK9）
7. `agate/scripts/agate-read-gate-commands.py`（:60 一带：P3* 键收集行为）
8. `agate/scripts/agate-summary.py`（:42-50 _DRIFT_SCRIPTS）
9. `AGENTS.md`（项目约定）

### 产出文件字段

用 `FILE={AGATE_WORKSPACE}/tasks/TAG0026-maintainability-gate/P2-review.md agate-md-field-set --list`
查看本阶段应填字段；`FILE=... agate-md-field-set <key> <value>` 逐个写入；写入失败照错误提示
修正，不要手写 frontmatter；仍失败则报告主 Agent，不要绕开 set。
Header 关键字段：`status`（draft → 评审后改 approved/rejected）、`agent: plan-eng-review`、
`phase: P2`、`task_id: TAG0026`。
</dispatch_guide>
...
<objective_info>
### 客观查证信息（主 Agent 预核实的锚点，供交叉验证）
- `check-gate.py`（全 1355 行）：`_STAGED_EXCLUDE_RE` :174；`gate_p4` :870-927（①review 存在→1
  ②status 非 approved→1 ③agent 缺→2 / main→1 ④staged 代码检查→1 ⑤骨架 WARNING 不阻断
  ⑥return 0）；gate_p5 known-failures :930-985；分发映射 :1335-1346；import 兜底区 :25-58
- `agate-risk-score.py`：`_load_script` :46、`_norm_rel` :86、`score_task` :202、`_impact_high` :139
- `agate_common.py`：`count_kf_entries` :1015-1017（`^\|\s*[0-9]+\s*\|`）
- P2-design 声称的以下行号**主 Agent 未逐行核实**，评审时以实际读取为准：
  check-protocol-consistency.py :745-751 / :771-785 / :791-794 / :797-830 / :807-811 / :841-846；
  agate-summary.py :42-50；conftest.py :264-302 / :305-312 / :358-365 / :374-394；
  agate-read-gate-commands.py :60
- 设计 frontmatter：`candidate_count: 3` / `dispatch_plan: {mode: serial}` / `ui_affected: false` /
  `domains: [backend]`（无 frontend，无 UI 设计节要求）
- 环境：worktree dogfooding——改代码在 worktree；consistency 必须用 worktree 自己的脚本；
  ruff 用 `~/.venvs/agate-dev/bin/ruff`（锁 0.16.4 与 CI 对齐）；测试基线全量 pytest 全绿 +
  count-tests 1308（2026-08-30 实测，只增不减）
</objective_info>
