...
执行 P2-design.md §4 固化的 gate_commands 全套命令（11 键），逐条判定 pass/fail，
产出 `P5-test-results/unit.md`（逐命令结果 + failed 计数）与
`P5-test-results/fail-list.txt`（失败清单，可为空文件）。

### 命令清单（P2-design.md §4 固化，在 worktree 根 /home/kity/oclab/agateon/.worktrees/agate-TAG0026 执行）

```yaml
gate_commands:
  P5: "python3 -m pytest agate/tests/ -q --tb=no"                      # timeout 600s
  P5_consistency: "python3 agate/scripts/check-protocol-consistency.py --strict-errors-only"  # 120s（必须用 worktree 自己的脚本）
...
（P3 键为 TDD 运行器声明，P5 阶段不执行；P5_timeout_seconds=600 / 其余 per-key 见 P2 §4。
禁 && 链——每条独立跑、独立记录。）

### 约束

1. **逐条独立执行**：按上表顺序，每条独立跑（禁 && 短路），完整命令与 exit code 逐条记录
   到 unit.md（含命令原文 + 耗时 + 汇总输出尾部）。
2. **全量测试口径**：P5 命令是全量 pytest（含非本任务测试）。若有预存失败（与本次改动
   无关的失败），在 unit.md 标注"预存失败：X（与本次改动无关）"并单独列出处；
   本任务相关失败 = 真失败，逐条列 fail-list.txt。
3. **count_tests 数字**：记录实测数字（不写死），对比 P0 基线 1308（2026-08-30）应只增不减
   （P3 新增 27 用例）。
4. **环境**：全程在 worktree 根执行；不启动任何服务、不触碰生产环境；所有 bash 命令
   timeout 包裹（per-key timeout × 1.5 上限）。跑测试用
   `timeout 600 python3 -m pytest ...`（Bash 工具 timeoutMs 同步给足）。
5. **产出路径硬约束**（Write 工具）：
   - /home/kity/oclab/agateon/.worktrees/agate-TAG0026/agate-workspace/tasks/TAG0026-maintainability-gate/P5-test-results/unit.md
   - /home/kity/oclab/agateon/.worktrees/agate-TAG0026/agate-workspace/tasks/TAG0026-maintainability-gate/P5-test-results/fail-list.txt
6. **unit.md 证据签名**：末尾必须有 test runner 输出签名行（pytest 汇总如
   `====== N passed, M failed in Ts ======`），主 Agent 会 grep `^(PASSED|FAILED|passed|failed)` 计数验证。
7. **不修代码**：P5 是只读验证——发现问题记录报告，不修复。
8. **PROD 隔离**：不触碰生产；返回时报告 [PROD_NOT_TOUCHED] 或 [PROD_TOUCHED]。

### 上游关联

- P2-design.md §4 gate_commands（验证契约，P2 固化后不可改）
- P4 commit f7e7b9f（被验证的实现）
- P0-brief.md env_constraints（worktree dogfooding 纪律）

### 输入文件

1. `agate-workspace/tasks/TAG0026-maintainability-gate/P2-design.md`（§4 命令契约）
2. `agate-workspace/tasks/TAG0026-maintainability-gate/P0-brief.md`
3. `AGENTS.md`（工具纪律：分片跑技术、输出控制）

执行技术提示（AGENTS.md 工具纪律）：全量 pytest 可分片跑（unit/regression/integration
各 `-n auto`）后合并判定，但**判定口径以全量命令的汇总结果为准**（P2 §4 说明）——
分片只是控制单命令时长，最终 unit.md 记录的数字必须覆盖全量套件。
</dispatch_guide>
...
<objective_info>
### A. 环境（主 Agent 已核）
- worktree 根：/home/kity/oclab/agateon/.worktrees/agate-TAG0026（branch feat/TAG0026-maintainability-gate，HEAD f7e7b9f = P4 实现 commit）
- 被验证实现：check-maintainability.py（新增）+ check-gate.py gate_p4 三重门槛 + 锚点登记 + _DRIFT_SCRIPTS + 模板 + 卡片两处 + maintainability.yaml
- P0 基线：全量 pytest 全绿、consistency 0 ERROR（--strict-errors-only）、count-tests 1308（2026-08-30）
- P3 新增 27 用例（test_check_maintainability.py 14 + test_check_gate_p4_maintainability.py 13）→ count-tests 预期 ≥1335（实测为准）
- ruff：~/.venvs/agate-dev/bin/ruff（锁 0.16.4）；shellcheck 已装
- consistency 必须用 worktree 自己的脚本（检查对象是 worktree 协议文件）
</objective_info>
