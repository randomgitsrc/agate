...
### 目标
独立评审 `P1-requirements.md`（analyst 已产出，13 条 BDD），产出 `P1-review.md`：判定
approved / needs-revision / rejected，只审不改。

### 约束

1. **只审不写**：不得直接修改 P1-requirements.md；发现问题写进 P1-review.md，交主 Agent 回派
   analyst 修改。
2. **实质锚点要求**：结论必须逐条引用 BDD 编号 + 覆盖维度标注（数据/前端/多端/边界/兼容），
   不接受裸 "approved"。
3. **重点核查项（本任务特有风险点，在角色定义检查清单基础上重点验证，不是额外清单）**：
   - **挂载阶段 P4 不挂 P6**（v3 评审 B1 修正）：逐条 BDD 检查是否出现任何"P6 挂载"或
     "P4/P6 均可"措辞；BDD-13 是否真正验证"P4 能读到代码 diff、P6 读不到"的数据源对齐，
     而非笼统描述
   - **三重门槛不得退回"登记即放行"**（v3 评审 B3 修正，最深一处）：BDD-7/8/9/10 是否逐条
     对应「登记缺失阻断 / 数量不足阻断 / 数量对齐但评审未 approve 仍阻断 / 三重全满足才放行」；
     检查 BDD-9 是否把"评审未 approve"与"数量对齐"正确并列为阻断条件，而不是让数量对齐单独放行
   - **不新增第八道 provenance 审计**（v3 评审 B2 修正）：BDD-10 是否只写"放行 + 登记内容
     不进审计范围"，没有出现"登记进审计"类措辞
   - **阈值 N=1000 仅供参考可配置**：BDD-5/BDD-6 是否体现"默认值仅供参考可配置 + 缺失兜底"，
     没有造成"协议断言该阈值"的错觉；配置路径是否 `agate-workspace/maintainability.yaml`
     （不是 `.agate/`）
   - **fuzzy-boundary 只覆盖 Python/TS**：是否明确"其它语言项目经 gate_commands 自行补充"，
     没有"支持所有语言"的超范围承诺
   - **移动代码假阳性是已知行为非缺陷**：BDD-12 的判定锚是否为"判定为 violation = PASS"
     （诚实承认假阳性），而非"自动识别移动而忽略 = PASS"
   - **check-gate.py 返回约定兼容**：BDD 措辞是否用 exit code（1/2/0）作为判定锚，而非
     verifier/implementer 的文字描述（BDD-9 红线对齐）
   - **同类扫描完整性**：正文 §6 是否含"命中清单 + 逐条本次处理/不处理 + 理由"（check-gate.py
     消费方 / count_kf_entries 复用 / known-violations 首次引入 / maintainability.yaml 首次引入 /
     check-maintainability.py 不存在 / ruff E722 关系），空白不算做过
   - **BDD 可二值判定**：每条 BDD 的 Given/When/Then 判定锚是否明确 PASS/FAIL，禁止中间态
   - **裁剪声明合理性**：risk_level=high 全阶段不裁，逐阶段理由是否站得住（P3 因 high 不可裁 /
     P7 因跨文件一致性保留 / P8 因 roadmap 回写强制），不是抄一遍卡片模板
   - **frontmatter 完整性**：risk_level / ceremony / phases / packages / domains /
     capability_requirements 是否已声明且合法；ceremony=standard 时 phases 是否含 P7

### 上游关联

- P1-requirements.md 由 analyst 产出，基于 P0-brief.md + 落地计划
  `docs/design-notes/rm-ag0046-maintainability-gate-plan.md`（v3，独立评审修复 5 项后定稿）
- P1-progress.md 记录了 analyst 的读取与产出轨迹，可交叉核对结论出处
- 设计地基 `docs/design-notes/design-maintainability-gate.md`（G0-G3 分级 + 决策 1/2/3）

### 输入文件（按顺序读）

1. `agate-workspace/tasks/TAG0026-maintainability-gate/P1-requirements.md`（评审对象）
2. `agate-workspace/tasks/TAG0026-maintainability-gate/P0-brief.md`
3. `docs/design-notes/rm-ag0046-maintainability-gate-plan.md`（重点第 0/2/4/4.1 节，核对 BDD
   是否覆盖设计文档给出的 13 条语义与三重门槛结构）
4. `AGENTS.md`（项目约定）

### 产出文件字段
用 `FILE={AGATE_WORKSPACE}/tasks/TAG0026-maintainability-gate/P1-review.md agate-md-field-set --list`
...
<objective_info>
- P1-requirements.md frontmatter：`risk_level: high` / `ceremony: standard` /
  `phases: [P1, P2, P3, P4, P5, P6, P7, P8]`（全流程不裁）/
  `packages: [agate-scripts, agate-tests, agate-phase-cards, agate-templates]` /
  `domains: [backend]` / `capability_requirements: []`（无 frontend，UX 分类框架要求不适用，
  vision 能力声明不适用）
- `.state.yaml` 已确认含 `judge.enabled: true`（P1 初始化写入）
- 设计文档第 4 节 13 条 BDD 语义是 P1-requirements.md §7 的直接来源，评审时逐条对照
- count_kf_entries（agate_common.py:1015-1017）按行首 `^\|\s*[0-9]+\s*\|` 计数，known-violations
  模板必须沿用该格式——这是 BDD-8 依赖的格式约定
- gate_p4 现有检查（check-gate.py:870-927）已在客观查证信息 B 节给出，评审时核对"新增一步"
  不改变既有检查语义
</objective_info>
