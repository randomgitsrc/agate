...
执行 P2-design.md gate_commands 的 P5 命令集，逐条判定，产出 P5-test-results/（unit.md / fail-list.txt）。TAG0020 实现对 judge 机制全部新增/修改测试 + 全量回归。

### P5 命令集（P2-design.md §4 声明，按此执行）

1. **P5（全量测试）**：`python3 -m pytest -q --tb=no -p no:cacheprovider --basetemp=agate-workspace/.pytest-tmp agate/tests/` —— 在 worktree 根运行
2. **P5_consistency**：`python3 agate/scripts/check-protocol-consistency.py --strict-errors-only` —— worktree 自己的脚本
3. **P5_count_tests**：`bash agate/tests/scripts/count-tests.sh`

> P5_timeout_seconds: 600（全量测试给 timeout 600s + 工具层 timeout 同步设）

### 客观查证信息（本机实测，硬约束）

- /tmp 只读 + ptmp 只读（Errno30）：P2 声明 basetemp = `agate-workspace/.pytest-tmp`（worktree 下可写，git 仓库内——影响 test_bdd_7 类"非 git 上下文"用例时如 TAG0019 先例，判定环境前提记录，非缺陷）
- 解释器 /usr/bin/python3（裸 pytest 命令 PATH 找不到）
- bash 一律外层 timeout（全量 600s，其余 90s）；单步串行不并行
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；check-protocol-consistency.py 用 worktree 自己的
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P5-test-results/`
- 分阶段落盘：关键步骤追加 P5-progress.md

### 产出规格

- `{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P5-test-results/unit.md`：failed 计数 + 运行签名（PASSED/FAILED/passed/failed 行）
- `{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P5-test-results/fail-list.txt`：failed id 逐行（`FAILED ` 前缀）
- **全量测试（含非本任务）**：须跑 agate/tests/ 全量；预存失败 → unit.md 标注 + known-failures.md 登记（若与本次无关）

### 门槛（什么算完成）

- P5-test-results/unit.md 存在 + failed 计数 + 签名（grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)' > 0）
- 三条命令逐条执行并记录
- 真失败（本次改动引入）→ 标注回 P4；环境前提 → 标注非缺陷
