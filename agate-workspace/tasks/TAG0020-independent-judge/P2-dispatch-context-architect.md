...
产出 `P2-design.md`：TAG0020「独立 Judge 机制（RM-AG0032）」方案设计。需求基线 10 条 BDD 已 approved（P1-requirements.md）。设计覆盖设计文档 `design-independent-judge.md` 的文件级改动清单（M1-M2 主体）：
1. `agate/assets/review-roles/judge.md`（新）——judge 角色定义（fresh context / 信息隔离 / 重验所有 BDD / 预算防死循环）
...
7. 测试：`test_check_judge_verdict.py` / `test_check_events.py` + 回归

### 约束

- **agate 哲学红线（BDD-9）**：不引入"LLM 当 gate 主判据"——judge verdict 叠加机械核对（层 2/3），exit code 才是门槛
- **历史任务兼容（BDD-2）**：旧任务无 judge 字段 → P6.5 gate 跳过（只对新任务生效，存量不挂）
- **机械可判定（BDD-1/4/5/6/7）**：verdict 机器可读（Header status + criteria_total/criteria_passed/verdict_evidence）；信息隔离黑名单路径引用集（P1 R1 已固化：P6-acceptance.md / P6·P5·P4-dispatch-context / verifier·implementer 产出文件禁注入 judge dispatch-context）
- **三层防造假（BDD-4/5/6/7）**：①信息隔离白名单（check-judge-verdict 校验）②证据交叉核对（BDD 计数对照/md5 去重/执行留痕/git 留痕）③append-only 事件账本（gate-events.jsonl 行间哈希链，append_event 统一写入）
- **三档预算（BDD-8）**：轮次≤2/token 100k（judge_token_budget 可覆盖）/时间 30min；超限 partial:true → needs-revision 不静默放行
- **挂靠现有机制**（BDD-3/10）：status 门槛映射（passed→approved/needs-revision→needs-revision/rejected→rejected）、dispatch-prompt 模板、专家组/double-judge——零新架构
- **事件账本与既有 gate 兼容**：gate-events.jsonl 哈希链需与 check-p6-provenance 六道审计共存（P0 风险 2）——设计字段交集
- **P5/P6 不可裁**；测试平台无关（/tmp 只读 --basetemp）；改动面大（12+ 文件）全触发 SELF-GATE

### 上游关联

P1-requirements.md（10 BDD approved）为输入权威；design-independent-judge.md（§3 角色设计 / §4 三层防造假 / §5 预算 / §6 状态机集成 / §7 文件改动清单 / §8 对标取舍）为设计提案；P1-progress.md 含三组同类扫描结论（review-roles 现状/dispatch-context 注入/check-p6-provenance 审计——verify judge 挂载点）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md（需求基线，权威）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P0-brief.md（环境约束与风险）
- /home/kity/oclab/dsh-workspace/agate-research/design-independent-judge.md（设计提案，含 §7 文件改动清单）
- /home/kity/oclab/agate/agate/assets/execution-roles/architect.md（角色定义，稳定版）
- /home/kity/oclab/agate/agate/assets/review-roles/ 现状（judge 挂靠参考）
- /home/kity/oclab/agate/agate/scripts/check-gate.py（P6.5 分支挂载点参考）
- /home/kity/oclab/agate/agate/scripts/check-p6-provenance.py 与 check-p6-evidence.py（六道审计参考——事件账本交集）
- /home/kity/oclab/agate/agate/state-machine.md / dispatch-protocol.md / phase-cards/P6-acceptance.md（P6.5 挂载点）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`（= ~/.agate）；改造对象（worktree）= `/home/kity/oclab/agate/.worktrees/agate-TAG0020`（含 TAG0019 v0.58.0 合并内容）。

### 客观查证信息

- /tmp 只读 + ptmp 只读：pytest `-p no:cacheprovider --basetemp=<可写>`；解释器 /usr/bin/python3
- 读卡片/角色/脚本用 ~/.agate 稳定版；check-protocol-consistency.py 若跑用 worktree 自己的
- bash 一律 timeout（30-90s）；读文件优先 read/grep 工具；单步串行
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P2-design.md`
- 分阶段落盘：关键步骤追加 P2-progress.md

### P2 产出规格（P2 卡片为准，关键点摘要）

- 影响面梳理（强制节，写在候选方案之前）：改什么（逐文件/小节 + BDD 编号）/ 不改什么（显式 + 理由）/ 风险在哪（每条配缓解；事件账本与 provenance 交集、历史兼容、白名单禁入项是高频风险）
- 候选方案 ≥2 + 权衡 + 选择理由 + frontmatter `candidate_count:`（与正文一致）+ 四字段（packages/domains/ui_affected/gate_commands）
- `files_to_read:`（P4 参考清单，控制上下文）+ `env_constraints:` + `minimal_validation:`（P2 最小验证——本任务纯代码逻辑/静态判定则声明"纯代码逻辑，无外部系统依赖"并写明依赖的内部函数）
- Header：phase: P2 / task_id: TAG0020-independent-judge / type: design / parent: P1-requirements.md / trace_id: TAG0020-P2-20260822 / status: draft / created: 2026-08-22 / agent: architect

### 评审预告

P2 完成后主 Agent 将派 plan-eng-review（domains=backend 强制）。方案须含候选权衡与影响面证据。

### 返回

只返回两行：① P2-design.md 路径；② 一句话摘要（≤30 字）。
</dispatch_guide>
