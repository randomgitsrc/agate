...
产出 `P1-requirements.md`（需求基线）：TAG0020「独立 Judge 机制（RM-AG0032）」——P6.5 新增验收独立裁判。核心：judge 角色用 fresh context 只凭标准（P1 BDD + P2 验收设计）逐条重验**所有** BDD，信息隔离白名单禁传实现者自述，配合三层防造假（信息隔离/证据交叉核对/append-only 事件账本）与三档预算，解决 LIMITATIONS-3（自写 gate 作者与评判者同为一人）。设计文档 `design-independent-judge.md` 含文件级改动清单。

需求覆盖交付物（M1-M2 主体，P0-brief 锁定）：
1. `agate/assets/review-roles/judge.md`（新）——judge 角色定义（fresh context / 信息隔离 / 重验所有 BDD / 预算/防死循环）
2. `agate/scripts/check-judge-verdict.py`（新）——verdict 门槛判定（BDD 计数对照 / 证据引用 / 信息隔离白名单校验）
...
6. `WORKFLOW.md` / `state-machine.md`（P6.5 转移 + 重试表）/ `dispatch-protocol.md`（信息隔离节）/ `phase-cards/P6-acceptance.md`（P6.5 门槛）/ `assets/templates/dispatch-prompt.md`（Judge 追加节）
7. 测试：`test_check_judge_verdict.py` / `test_check_events.py` + 回归（BDD 计数对照、哈希链、信息隔离白名单）

### 核心约束

- **不引入"LLM 当 gate 主判据"**：judge verdict 叠加机械核对，exit code 才是门槛（agate 哲学红线）
- **历史任务兼容**：旧任务无 judge 字段 → P6.5 gate 跳过（只对新任务生效，存量不挂）
- **机械可判定**：verdict 落盘机器可读（Header status: passed/rejected/needs-revision + criteria_total/criteria_passed/verdict_evidence 字段）
- **挂靠现有机制**：status 门槛映射沿用（passed→approved / needs-revision→needs-revision / rejected→rejected）、dispatch-prompt 模板、专家组机制——不新建架构
- **三档预算**：轮次≤2 / token 100k（judge_token_budget 可覆盖）/ 时间 30min；超限诚实降级 partial: true（判 needs-revision 不静默放行）
- **三层防造假**：①信息隔离白名单（dispatch-context 禁含 P6-acceptance.md 与实现者自述——check-judge-verdict 校验）②证据交叉核对（BDD 计数对照/md5 去重/执行留痕/ git 留痕）③append-only 事件账本（gate-events.jsonl 行间哈希链）
- 测试平台无关（agate 核心约束）；/tmp 只读

### 上游关联

P0-brief 已锁定；design-independent-judge.md（角色设计/三层防造假/预算/状态机集成 §6/文件改动清单 §7/与 oh-my-agent 对标 §8）为 P1 分析基础；TAG0018 实证（LLM 评审≈0 净收益）为立项锚；TAG0019（风险分路由 v0.58.0，已合并 main 并同步进本 worktree）提供 thin 档联动背景（thin 档跳过 LLM 评审后 judge 为质量兜底）。任务工作区 = `{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/`。

### 强制同类扫描（HANDOFF §5，扫描结论必须写进 P1-requirements.md 正文）

1. grep `assets/review-roles/` 现状与 status 门槛映射——judge 如何挂靠（status 表/角色文件头字段/各评审角色差异）
2. grep dispatch-context 现有注入内容——信息隔离白名单反向推导禁入项（P6-dispatch-context/check 脚本会注入什么，judge 必须禁什么）
3. grep `check-p6-provenance.py` 六道审计——事件账本（gate-events.jsonl）与其字段交集（审计 1-6 各消费什么，账本如何配合不冲突）

每条命中：记录命中数量 + 文件清单，逐条判定"本次处理 / 本次不处理 + 理由"。同类型未来新增 → 声明回归拦截手段并转 BDD。

### 产出规格（P1 卡片为准，关键点摘要）

- BDD 验收条件 ≥1 条（Given/When/Then 格式），编号 `#### BDD-NN:`
- frontmatter：`phase: P1` / `task_id: TAG0020-independent-judge` / `type: problems` / `parent: P0-brief.md` / `trace_id: TAG0020-P1-20260822` / `status: draft` / `created: 2026-08-22` / `agent: analyst` / `risk_level:` / `phases:` / `packages:` / `domains:`
- P0-brief 时效性质疑：按 P0 卡判据核对（TAG0019 已合并同步——确认不改变本任务方案前提），无漂移写"已核对 P0-brief 时效性，无漂移"
- 无未决 `[NEED_CONFIRM]`；无 `status: GAP`；`capability_requirements:` 三态声明

### 输入文件（按序读取，每读完一个追加 progress）

1. {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P0-brief.md
2. /home/kity/oclab/dsh-workspace/agate-research/design-independent-judge.md
3. /home/kity/oclab/agate/.worktrees/agate-TAG0020/HANDOFF-TAG0020.md
4. {agate_root}/agate/assets/review-roles/ 现状（扫描对象 1）
5. {agate_root}/agate/scripts/check-gate.py 与 pre-commit-gate.py（扫描对象 2：dispatch-context 注入/ gate 结构）
6. {agate_root}/agate/scripts/check-p6-provenance.py 与 check-p6-evidence.py（扫描对象 3：六道审计）
7. {agate_root}/agate/state-machine.md / WORKFLOW.md / dispatch-protocol.md（P6.5 挂载点参考）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace`；{agate_root}（稳定版，读卡片/角色/脚本用）= `/home/kity/oclab/agate/agate`（= ~/.agate）；改造对象（要 grep 的仓库本体）= `/home/kity/oclab/agate/.worktrees/agate-TAG0020`（注意 worktree 的 agate/ 已含 TAG0019 v0.58.0 合并内容，与稳定版可能不同——扫描以 worktree 为准）。

### 客观查证信息（本机实测，硬约束）

- /tmp 只读 + ptmp 只读（Errno30）：pytest 必须 `-p no:cacheprovider --basetemp=<可写目录>`；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；check-protocol-consistency.py 若跑用 worktree 自己的
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep 工具；单步串行不并行 bash
- 状态标记：`[PROD_NOT_TOUCHED]`
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md`
- 分阶段落盘：每读完一个输入文件/关键步骤，把发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-progress.md`
