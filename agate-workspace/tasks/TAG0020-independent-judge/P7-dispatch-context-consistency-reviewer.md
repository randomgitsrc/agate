...
产出 `P7-consistency.md`：TAG0020「独立 Judge 机制」跨文件一致性审查结论。对照 P1-P6 全部产出 + 实现（13 个协议/脚本文件改动）做交叉检查。

### 检查清单（P7 卡）

1. **DESIGN_GAP 配对**：P4-implementation.md / P4-progress.md 中的 DESIGN_GAP 声明 → 逐条转抄 + REVIEWED 标记（本任务已知：P4 修复轮 [SCOPE_GAP] ci-gate-backstop 已闭环；评审 INFORMATIONAL R-1/2/3——核查是否需登记 DEVIATION 而非 DESIGN_GAP）
2. **SCOPE+ 闭环**：P1 有 [SCOPE_RESOLVED] 标记？本任务 P4 无新增 SCOPE+（实现报告无）→ 核对 P1 存量
3. **跨文件一致性**：
   - P1 packages（agate）vs P4 实现路径（agate/scripts + agate 文档）吻合？
   - P1 BDD 数（10）vs P6 PASS 数（10）匹配？
   - P4 实现（judge 机制 17 文件）vs P2 方案（serial 三批）吻合？
   - P2 dispatch_plan serial vs P4 实际分批（scripts→docs→补丁）一致？
   - P2 gate_commands（P5 全量 + consistency + count）vs P5 实际执行一致？
   - **judge 机制与既有 gate 一致性**：check-gate gate_p65 / pre-commit 2i.1 / ci-gate-backstop 三处 P6.5 注入条件是否一致（_judge_enabled）；append_event 唯一写路径；gate-events.jsonl 与 check-p6-provenance 审计共存
   - 新脚本注册完整性：CHECK 9 锚点 / _DRIFT_SCRIPTS / AGENTS 角色清单 / role-system 名册 / dispatch-prompt 追加节 是否同步
4. **未决项清零**：P1 无残留 [NEED_CONFIRM]（分析时无 SUGGEST——P1 [NO_NEED_CONFIRM]）；无 [BLOCKER]、无 [DEVIATION-CRITICAL]
5. **CODE-MAP 核对**：{AGATE_WORKSPACE}/agents/CODE-MAP.md 若存在 → 对照 P4 新增文件核对表（check-judge-verdict.py / check-events.py / judge.md）；机制未采用则 code_map_new_files_count: 0（与 P4 的 [CODE_MAP_EXEMPT] 声明核对）

### 输入文件（全部 P1-P6 产出 + 实现）

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md / P2-design.md / P3-test-cases.md / P4-implementation.md / P4-progress.md / P4-review.md / P5-test-results/ / P6-acceptance.md / P6-evidence/
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/known-failures.md（预存失败登记，核查与本任务无关判断）
- /home/kity/oclab/agate/agate/assets/execution-roles/consistency-reviewer.md（角色定义）
- worktree 下 agate/scripts/（check-judge-verdict/check-events/agate_common/check-gate/pre-commit-gate/ci-gate-backstop/check-protocol-consistency/agate-summary 各新改处）+ agate 文档（judge.md/state-machine/WORKFLOW/dispatch-protocol/P6 卡/dispatch-prompt/role-system/LIMITATIONS/AGENTS）
- {AGATE_WORKSPACE}/agents/CODE-MAP.md（若存在）

路径说明：{AGATE_WORKSPACE} = /home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace；{agate_root} = /home/kity/oclab/agate/agate。

### 产出规格（P7 卡样例）

- Header：phase: P7 / task_id: TAG0020-independent-judge / type: consistency / parent: P2-design.md / trace_id: TAG0020-P7-20260822 / status: draft / created: 2026-08-22 / agent: consistency-reviewer
- frontmatter：blocker_count / deviation_count / deviation_critical_count / design_gap_count / design_gap_reviewed_count / code_map_new_files_count / code_map_reviewed_count
- 正文：逐条检查结果（含跨文件引用关键词 P2§packages / P4§impl-path）+ 实质锚点（DESIGN_GAP_REVIEWED 配对 / SCOPE 闭环）
- 无 [BLOCKER] / [DEVIATION-CRITICAL] 残留

### 环境纪律

- /tmp 只读；读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s（consistency 若跑给 120s）
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P7-consistency.md`
