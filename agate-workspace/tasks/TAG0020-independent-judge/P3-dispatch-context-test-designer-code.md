...
<dispatch_guide>
> ⚠️ 本文件是 P3 分工任务 B 的强制指令（测试代码撰写）。P3-test-cases.md 用例文档已由任务 A 完成。

### 目标

按已产出的 `P3-test-cases.md` 用例映射，在 worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate/tests/unit/` 下写测试代码（TDD 红灯批——被测模块未实现时用例必须失败）：
1. `test_check_judge_verdict.py`（新，BDD-1/4/5/9）
2. `test_check_events.py`（新，BDD-6/7）
3. `test_check_gate.py` 增补（P6.5 分支，BDD-1/2/3）
4. `test_agate_common.py` 增补（append_event/read_judge_verdict，BDD-6 写侧 + BDD-3）
5. `test_docs_assertions.py`（新或增补，BDD-4/8/10 文档条文断言）

### 约束（平台无关 + 环境）

- /tmp 只读 + ptmp 只读：pytest 用可写 basetemp `/home/kity/oclab/agate/.ptmp-scratch`（用后清理）；解释器 /usr/bin/python3
- 平台无关：不裸 PATH/不裸 python3/不用 /tmp 字面/POSIX symlink 假设；临时文件用 tmp_path fixture
- 不要硬编码被测实现细节——用例驱动"被测模块未实现→红灯"，import 失败=正确红灯；断言与数据矛盾=测试 bug 先修
- 参考风格：agate/tests/unit/test_check_pruning.py / test_agate_md_field_get.py + conftest fixture（run_cli/agate_scripts/python_exe/git_repo/task_dir）
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s
- [PROD_NOT_TOUCHED]；只能写测试文件，不碰被测模块（P4 implementer 负责）

### 落盘节奏（强制——每写完 1 个文件立即追加 P3-progress.md）

1. **每写完 1 个测试文件立即追加 progress（文件名 + 用例数）**，不要攒批
2. 全部写完后自跑红灯确认：`timeout 120s /usr/bin/python3 -m pytest <新测试文件> -q -p no:cacheprovider --basetemp=/home/kity/oclab/agate/.ptmp-scratch`——记录红灯原因（模块未实现=正确红灯；断言矛盾=先修测试）
3. 若任一步骤超 12 分钟无落盘，立即停止返回当前产出 + 卡点

### 返回

只返回两行：① 产出测试文件路径清单（逗号分隔）；② 一句话摘要（文件数 + 用例数 + 红灯确认结果）。
</dispatch_guide>
