...
- P1-requirements.md：/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace/tasks/TAG0020-independent-judge/P1-requirements.md
- 上轮 dispatch-context：P1-dispatch-context-analyst.md（复用其目标/约束/输入清单，不重写）
- P1-review.md（评审意见，权威）：/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace/tasks/TAG0020-independent-judge/P1-review.md（status: needs-revision）

### 修复目标（评审 2 条修改项，全部修复后才可自检通过）

**R1（实质，修订主因）— BDD-4 禁项 2 无机械检测锚点**：评审确认 BDD-4 的"禁项 2：实现者/验收者自述摘要"只有语义描述（dispatch-context 禁含 P6-acceptance.md 与实现者自述），无**可机械判定的锚点**。§4.2 识别了"上游关联注入 verifier 产出结论"（agate-extract-context 为主要防泄漏点）但未固化进 BDD-4 验收标准。
修订要求：
- BDD-4 改为**可字符串/正则匹配的黑名单路径引用集**：明确列出 judge dispatch-context 必须禁的路径模式（如 `P6-acceptance.md`、`P6-dispatch-context-*.md`、`P5-dispatch-context-*.md`、`P4-dispatch-context-*.md`、verifier/implementer 产出文件清单——以实际同类扫描到的文件名为准）
- 上游关联节声明"禁注入禁项"（agate-extract-context / 主 Agent 派发时不得把禁项传入 judge）
- 不能留"P2 定义"式开放项——白名单禁入项必须在 P1 固化（可判定验收），不在 P2 再议

**R2（轻）— §4.3 审计编号口径统一**：标题写"六道审计"但内容写"七道审计（审计 1-7）"（check-p6-provenance 实际六道 + 审计 7 P5 证据复用 = 共 7 道）。统一为一致口径（建议"七道审计（审计 1-7）"或按实际数量统一标题）。

### 约束（沿用）

- 只改 P1-requirements.md；BDD 编号保持连续；无行首 [NEED_CONFIRM]；frontmatter 不破坏
- bash 一律 timeout；读文件用 read/grep 工具；单步串行
- /tmp 只读（pytest 需 --basetemp）；读卡片/角色用 ~/.agate 稳定版
- [PROD_NOT_TOUCHED]；修改记录追加 P1-progress.md（每项一行：修了什么 + 证据）

### 产出

更新后的 P1-requirements.md（原地更新）+ 修改记录追加 P1-progress.md
