...
产出 `P8-release.md`：TAG0020「独立 Judge 机制」发布准备。版本：**v0.59.0**（minor——新增 P6.5 judge 机制功能，向后兼容）。

### 发布信息（已核实）

- 当前版本：v0.58.0（README badge / CHANGELOG [0.58.0] / git tag v0.58.0）
- 目标版本：**v0.59.0**（bump_type: minor）
- 版本引用文件：README.md version badge（:5）/ CHANGELOG.md / UPGRADING.md（**尚无 v0.59 章节——本任务需新增**）/ git tag
- UPGRADING v0.58.0 章节为 TAG0019 所加（"无破坏性变更"）；v0.59.0 章节需新增（judge 机制——同样无破坏性变更：P6.5 只对新任务 judge 字段生效，历史任务跳过）

### 你的产出内容（P8-release.md）

1. `bump_type: minor`（理由：新增 P6.5 judge 机制（P1-10 BDD 全过），向后兼容——P7 确认 BLOCKER=0；历史任务无 judge 字段跳过）
2. `debt_check: reviewed`（读 tech-debt.md 核对本任务相关债务——P4 评审 INFORMATIONAL R-1/2/3、budget_exhausted 粘性取舍若需登记债务则列出 id；或注明"无本任务新增技术债"）
3. 版本号变更确认清单：README badge v0.58.0→v0.59.0 / CHANGELOG [Unreleased]→[0.59.0] 新增小节 / **UPGRADING.md 新增 v0.59.0 章节**（内容：新增 judge 角色 + check-judge-verdict/check-events + P6.5 强门槛 + append-only 事件账本；无破坏性变更——历史任务无 judge 字段跳过 P6.5；已有项目升级 git pull + 重跑 install-hook.py）
4. **临时资源清单**（主 Agent READY 收尾）：本任务启动的临时服务/进程/数据/开发安装（预期：无服务；pytest basetemp 已清理；列出需确认项）
5. 发布检查命令表：基于 P2-design.md gate_commands（P5 全量 / consistency / count-tests）列主 Agent 应执行的验证命令
6. CHANGELOG 变更条目草稿：v0.59.0 小节摘要（judge 角色/check-judge-verdict/check-events/append_event/gate_p65/信息隔离白名单/三档预算/历史兼容）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P7-consistency.md（发布前一致性结论）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P2-design.md（packages + gate_commands）
- README.md / CHANGELOG.md / agate/UPGRADING.md（版本引用现状）
- {AGATE_WORKSPACE}/debt/tech-debt.md（debt_check）
- /home/kity/oclab/agate/.worktrees/agate-TAG0020/agate/assets/review-roles/judge.md（发布内容素材）

### 环境纪律

- /tmp 只读；读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P8-release.md`
