...
产出 `P6-acceptance.md` + `P6-evidence/`：TAG0020「独立 Judge 机制」10 条 BDD（P1-requirements.md）逐条验收。ui_affected: false（非 UI 任务）。每条 PASS（带证据引用）或 FAIL。

> 说明：本任务引入了 P6.5 judge 机制本身。TAG0020 是 judge 机制的实施任务（自身 .state.yaml 无 judge 字段属"存量开放"，P6.5 对无 judge 字段任务跳过——机制正确性已由 test_check_judge_verdict/test_check_events 全覆盖）。本 P6 验收走标准 verifier 自评流程。

### 输入

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md（10 BDD，权威验收口径）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P5-test-results/（P5 验证证据）
- /home/kity/oclab/agate/.worktrees/agate-TAG0020/agate/scripts/check-judge-verdict.py / check-events.py / agate_common.py / check-gate.py（实现对象）
- /home/kity/oclab/agate/.worktrees/agate-TAG0020/agate/tests/unit/test_check_judge_verdict.py 等（测试对照）

### 验收口径（P6 卡）

- **每条 BDD 只允许 PASS 或 FAIL**；所有 PASS 必须有证据文件引用（`- PASS BDD-NN: 描述 (证据路径)`）
- frontmatter 汇总：`pass: N` / `fail: 0` / `ui_affected: false`
- **P6-evidence/ 非空**，每个文件含实质内容
- **引用 P5 证据判定**：审计 7 读 .state.yaml p5_pass_commit（=ae8fe177）；P5→P6 间无非产出改动 → 可复用 P5 证据
- 每条 BDD 对照实际行为/输出判定（跑测试/读实现/文档断言），不凭测试名猜

### 本任务 10 BDD 速览（详细见 P1）

- BDD-1 verdict 门槛判定（criteria_total==P1 BDD 数，P6.5 gate 放行）
- BDD-2 历史任务兼容（无 judge 字段跳过 P6.5）
- BDD-3 挂靠零新架构（status 映射/dispatch-prompt 复用，不新增 phase 值）
- BDD-4 信息隔离白名单（黑名单路径禁注入机械校验）
- BDD-5 证据交叉核对（BDD 计数/md5/执行留痕/git 留痕）
- BDD-6 append-only 事件账本（行间哈希链防改写）
- BDD-7 check-events 审计（哈希链完整/时间戳单调/无空洞）
- BDD-8 三档预算（轮次≤2/token 100k/时间 30min；partial→needs-revision）
- BDD-9 哲学红线（LLM 不当 gate 主判据，exit code 才是门槛）
- BDD-10 文档同步（judge.md/dispatch-prompt/state-machine 等条文可 grep）

### 环境纪律

- /tmp 只读 + ptmp 只读：pytest 用可写 basetemp `agate-workspace/.pytest-tmp`（用后清理）；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s；单步串行
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P6-acceptance.md` + `P6-evidence/`
- 分阶段落盘：关键步骤追加 P6-progress.md

### 返回

只返回两行：① P6-acceptance.md 路径；② 一句话摘要（PASS 数 / FAIL 数）。
不要声称"验收已通过"——gate 由主 Agent 判定。
</dispatch_guide>
