...
评审 TAG0020「独立 Judge 机制」（RM-AG0032) 的 P2 方案设计 `P2-design.md`，产出 P2-review.md（Header status: approved / rejected）。C8 映射：domains 含 backend → plan-eng-review 必派（单角色，直接产出 P2-review.md，无组长汇总）。

### 评审重点（plan-eng-review 通用 + TAG0020 特性）

通用（plan-eng-review 角色定义）：
- 数据流清晰（judge verdict → check-judge-verdict 机械核对 → 事件账本 append → gate exit code）
- 状态机完整（P6 → P6.5 → P7；P6.5(needs-revision) → P6 弹回 + 重试上限）
- 接口契约明确（verdict 文件字段 / gate-events.jsonl 行格式 / append_event 签名）
- 错误边界（预算耗尽 partial:true 不静默放行；历史任务无 judge 字段跳过）
- 测试策略（check-judge-verdict / check-events 单测 + 哈希链回归）
- 技术债记录（如提出"后续应重构"须 DEBT 格式）
- **多方案探索**：P2-design.md §1 ≥2 候选 + 权衡 + 选择理由（architect 报告 3 候选）
- **实现就绪度**：files_to_read 覆盖实现所需上下文；gate_commands 声明（P5 命令可执行）
- **P2 最小验证**：minimal_validation 字段（纯代码逻辑声明 + 理由）

TAG0020 特性专审：
1. **agate 哲学红线（BDD-9）**：方案是否确保"LLM 不当 gate 主判据"——judge verdict 叠加机械核对（check-judge-verdict/check-events），exit code 才是门槛
2. **信息隔离白名单（BDD-4）**：黑名单路径引用集（P6-acceptance.md / dispatch-context / verifier·implementer 产出）是否在 judge dispatch-context 禁注入中机械可校验（check-judge-verdict 校验实现）
3. **事件账本与 provenance 交集**：gate-events.jsonl 哈希链（append-only）与 check-p6-provenance 六道审计的字段交集是否设计清楚无冲突（P0 风险 2）
4. **历史任务兼容（BDD-2）**：无 judge 字段 → P6.5 跳过逻辑是否闭环
5. **三档预算（BDD-8）**：轮次/token/时间上限 + partial 诚实降级 semantics 是否可判定
6. **挂靠零新架构（BDD-3/10）**：status 门槛映射/dispatch-prompt/专家组复用是否真零新架构

### 上游关联

P1-requirements.md（10 BDD approved）；design-independent-judge.md（§4 三层防造假 / §5 预算 / §6 状态机 / §7 文件清单）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P2-design.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md（需求基线）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P0-brief.md（环境约束与风险）
- /home/kity/oclab/dsh-workspace/agate-research/design-independent-judge.md（设计提案）
- /home/kity/oclab/agate/agate/assets/review-roles/plan-eng-review.md（角色定义）
- /home/kity/oclab/agate/agate/scripts/check-p6-provenance.py（审计交集核验）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`。

### 客观查证信息

- /tmp 只读 + ptmp 只读：pytest `--basetemp=<可写> -p no:cacheprovider`；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate 稳定版；bash 一律 timeout（30-90s）；读文件优先 read/grep 工具
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P2-review.md`
- 分阶段落盘：关键步骤追加 P2-progress.md

### 评审产出要求

P2-review.md Header：phase: P2 / task_id: TAG0020-independent-judge / type: review / parent: P2-design.md / trace_id: TAG0020-P2-20260822 / status: approved|rejected / created: 2026-08-22 / agent: plan-eng-review
输出结构：架构问题（阻塞级）/ 架构问题（非阻塞）/ 测试缺口 / 锁定决策。结论须引用设计锚点，禁裸 approved。
