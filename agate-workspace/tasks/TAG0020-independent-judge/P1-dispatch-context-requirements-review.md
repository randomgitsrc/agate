...
评审 TAG0020「独立 Judge 机制」（RM-AG0032) 的 P1 需求基线 `P1-requirements.md`，产出 `P1-review.md`（Header status: approved / rejected / needs-revision）。本任务核心：P6.5 新增验收独立裁判 judge（fresh context 信息隔离 + 三层防造假 + append-only 事件账本 + 三档预算），解决 LIMITATIONS-3（自写 gate 作者与评判者同为一人）。

### 约束

- **只审不写**：不直接改 P1-requirements.md，产出评审意见由主 Agent 回派 analyst 修改
- **评审必须引用 BDD 编号锚点**：approved 结论必须逐条引用 BDD 编号 + 覆盖维度清单，禁裸 "approved"
- **重点审（本任务特性）**：
  1. 同类扫描是否扎实（三组：review-roles 现状与 status 门槛映射 / dispatch-context 注入内容→白名单禁入项反推 / check-p6-provenance 六道审计与事件账本交集）——命中数量 + 文件清单 + 逐条判定
  2. **agate 哲学红线**是否固化进 BDD："不引入 LLM 当 gate 主判据"（judge verdict 叠加机械核对，exit code 才是门槛）
  3. **历史任务兼容**是否 BDD 化：旧任务无 judge 字段 → P6.5 gate 跳过
  4. **三层防造假**是否可判定验收：信息隔离白名单 / 证据交叉核对（BDD 计数对照+md5+执行留痕+git 留痕）/ append-only 哈希链
  5. **三档预算**是否可判定：轮次≤2/token 100k/时间 30min；超限 partial:true → needs-revision 不静默放行
  6. capability_requirements 三态 + risk_level/phases/packages/domains frontmatter 合法
- **裁剪合理性**：跳过的阶段理由是否充分；risk_level 是否与实际风险匹配
- **P1 纯净性**：无掺入解决方案设计（P6.5 具体 implement 细节应留 P2）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-requirements.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P0-brief.md（任务简报）
- /home/kity/oclab/dsh-workspace/agate-research/design-independent-judge.md（设计提案）
- /home/kity/oclab/agate/.worktrees/agate-TAG0020/HANDOFF-TAG0020.md（任务范围 + 强制扫描）
- /home/kity/oclab/agate/agate/assets/review-roles/requirements-review.md（角色定义，稳定版）
- /home/kity/oclab/agate/agate/phase-cards/P1-requirements.md（产出规格 + gate 规则）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0020/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`；改造对象（worktree）= `/home/kity/oclab/agate/.worktrees/agate-TAG0020`。

### 客观查证信息

- /tmp 只读 + ptmp 只读：pytest 需 `--basetemp=<可写目录> -p no:cacheprovider`；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate 稳定版；bash 一律 timeout（30-90s）；读文件优先 read/grep 工具
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0020-independent-judge/P1-review.md`
- 分阶段落盘：关键步骤追加 P1-progress.md

### 返回

只返回两行：① P1-review.md 路径；② Status: <approved|rejected|needs-revision>（与 Header 一致）。
</dispatch_guide>
