...
<dispatch_guide>
> ⚠️ 本文件是 P1 需求修改轮第 2 次（⑩迭代循环第 3 轮）的强制指令。增量模式：只修本轮目标。

### 上轮产出与复审结论
- P1-requirements.md：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-requirements.md（已含 REV1 修复：R1 口径 36、R2 BDD-7 四要素并入）
- 复审意见（rev2，权威）：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-review.md（status: needs-revision）
- 复审唯一的修改项（R3）：**check-pruning.py 检查编号引用错误**——已核实源码 check-pruning.py:113-125：检查 3=P6 不可裁 / 检查 4=P4 不可裁 / 检查 5=P5 不可裁。但需求 §1:59 与 BDD-7 When:196 两处写"检查 4/5（P5/P6 不可裁剪）"= 实指 P4/P5，会把 P3 测试设计/P4 实现引向错误的闸。**正确引用应为：检查 3（P6）+ 检查 5（P5）**（P5/P6 双闸兜底）。

### 修复目标（唯一）
修正 P1-requirements.md 中全部 check-pruning 检查编号引用：
1. §1 相关句（原"检查 4/5"）→ 改为"检查 3（P6）+ 检查 5（P5）"
2. BDD-7 When 行（原"check-pruning 检查 4/5 兜底"）→ 改为"check-pruning 检查 3（P6）+ 检查 5（P5）兜底"
3. 全文件 grep `检查 4/5`、`检查 4、5`、`4/5` 等相关字样，确认无其他错引残留（如 §5 裁剪理由若引用编号也一并核对）
4. 不改 BDD 语义、不重排编号 1-15、不改 frontmatter

### 约束（沿用）
- 只改 P1-requirements.md；BDD Given/When/Then 可二值判定；无行首 [NEED_CONFIRM]
- bash 一律 timeout；读文件用 read/grep 工具；单步串行
- /tmp 只读（pytest 需 --basetemp）；读卡片/角色用 ~/.agate 稳定版
- [PROD_NOT_TOUCHED]；修改记录追加 P1-progress.md

### 产出
更新后的 P1-requirements.md（原地更新）+ 修改记录（P1-progress.md 追加一行：修了什么 + 证据行号）
