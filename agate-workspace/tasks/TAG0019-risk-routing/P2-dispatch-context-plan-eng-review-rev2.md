...
<dispatch_guide>
> ⚠️ 本文件是 P2 方案复审轮（⑩迭代循环第 2 轮）的强制指令。增量模式：确认上轮 rejected 的 BLK-1/NB 修订到位，全量复评后 approved/rejected。

### 上轮结论与修订声明
- 上轮评审：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P2-review.md（status: rejected：BLK-1 + NB-1~4 + 3 测试缺口）
- architect 已修订 P2-design.md（39797→39816 字节；修订记录见 P2-progress.md「rev1」节）：
  - **BLK-1** → P5_platform 收窄为变更文件集（agate-risk-score.py / check-routing.py + 5 个新/改测试文件，:283）；§0.3 R5 改"变更文件集 0 命中"（:103）；§4 记录既有存量命中不阻塞（:288）；§5 完成标志同步（:340）
  - **NB-1** → full→P7 不可裁：D2 补"ceremony: full 的 phases 必须含 P7"（:208）、D4 requirements-review 逐信号核对项（:242）、§2.6 删"通常伴随"改三重保证（:254-257）
  - **NB-2** → 错误边界补两条（:231-233）：P1 缺失 → exit 2 对齐 check-pruning；算分异常（run_git 失败/agate_common 不可导入 → score_task 输出 git_ok:false）+ thin 声明 → exit 1 fail-closed
  - **NB-3** → 影响面判据精确化（:179）：扫描对象=暂存区改动文件集、搜索面=repo_root 排除 task_dir/tests、命中判据=≥1 行引用且非 F 自身；R9 同步（:107）
  - **NB-4** → files_to_read 补 requirements-review:48-52 / role-system:54-70（:311,313）；§0.2 交代 WORKFLOW:290 权威在 role-system C8 + P3/P7 正交（:90-92）
  - **测试缺口** → §3 补分支测试清单 + full-P7 文档断言（:266-271）

### 复审任务
1. 独立复核 BLK-1：gate_commands.P5_platform 是否为变更文件集（非全树）？§0.3 R5/§4/§5 措辞是否一致？（记："变更文件集 0 命中"、存量命中不阻塞）
2. 独立复核 NB-1~4：full→P7 声明层+评审层保证是否闭环（D2+D4+§2.6）；error 边界 exit 语义（P1 缺失 exit 2 / 算分异常 fail-closed）是否可判定；影响面判据是否二值可判（BDD-5）；files_to_read/Not Modify 交代是否消除 P1-P2 不一致
3. 全量复评（防修订引入新不一致）：候选方案/四字段/gate_commands 其余 key/minimal_validation/candidate_count/影响面三部分
4. 产出更新版 P2-review.md（覆盖更新，Header status: approved / rejected）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-design.md（复审对象，已修订）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-review.md（上轮意见）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-progress.md（rev1 修订记录）
...
### 客观查证信息
- /tmp 只读（pytest --basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider）
- 读卡片/角色用 ~/.agate 稳定版；bash 一律 timeout；读文件用 read/grep 工具
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-review.md`（覆盖更新）
- 分阶段落盘：关键步骤追加 P2-progress.md

### 返回
只返回两行：① P2-review.md 路径；② Status: <approved|rejected>（与 Header 一致）。
</dispatch_guide>
