...
评审 TAG0019「风险分路由」（RM-AG0031）的 P1 需求基线 `P1-requirements.md`，产出 `P1-review.md`（Header status: approved / rejected / needs-revision）。本任务核心：把任务仪式深度从"agent 自报复杂度"改为"客观信号脚本算分"（agate-risk-score.py）+ ceremony 档位（thin/standard/full）+ fail-closed 声明 checklist + requirements-review 角色增"审声明"职责。

### 约束

- **只审不写**：不直接改 P1-requirements.md，产出评审意见由主 Agent 回派 analyst 修改
- **评审必须引用 BDD 编号锚点**：approved 结论必须逐条引用 BDD 编号 + 覆盖维度清单（数据/前端/多端/边界/兼容），禁裸 "approved"
- **重点审（本任务特性）**：
  1. 同类扫描是否扎实（三组 grep：check-pruning.py 判定逻辑 / 全仓 risk_level|ceremony|裁剪消费点 / 平台差异对 gate 语义影响）——命中数量 + 文件清单 + 逐条判定；"只此一处"也要显式写出
  2. fail-closed 语义是否固化进 BDD（不声明 = standard；thin 需 checklist 否则回退）
  3. P5/P6 不可裁是否被保护（薄化仪式不薄化验证）
  4. 复用约束（check-pruning.py 判定逻辑复用，不新写一套）是否固化为可判定验收
  5. capability_requirements 三态 + risk_level/phases/packages/domains frontmatter 是否齐全合法
- **裁剪合理性**：跳过的阶段理由是否充分；risk_level 是否与实际风险匹配
- **P1 纯净性**：无掺入解决方案设计 / 实现细节

### 上游关联

analyst 已产出 P1-requirements.md（路径见输入文件）。P0-brief 四字段已锁定；design-risk-routing.md 为设计提案（M1-M4 落地节奏）。任务工作区 = `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/`。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P0-brief.md（任务简报与风险声明）
- /home/kity/oclab/dsh-workspace/agate-research/design-risk-routing.md（设计提案：三原则/档位表/fail-closed 语义）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/HANDOFF-TAG0019.md（任务范围 + 强制扫描要求）
- /home/kity/oclab/agate/agate/assets/review-roles/requirements-review.md（角色定义，稳定版）
- /home/kity/oclab/agate/agate/phase-cards/P1-requirements.md（产出规格 + gate 规则，稳定版）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`（= ~/.agate）。

### 客观查证信息（本机实测，硬约束）

- /tmp 只读：pytest 必须 `-p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp`
- 读卡片/读角色文件用 ~/.agate（=/home/kity/oclab/agate/agate 稳定版），不用 worktree 副本
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep 工具；单步串行不并行
- 状态标记：`[PROD_NOT_TOUCHED]`
- 产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-review.md`
- 分阶段落盘：关键步骤追加写入 `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-progress.md`

### 返回

只返回两行：① P1-review.md 路径；② Status: <approved|rejected|needs-revision>（与文件 Header status 一致）。
</dispatch_guide>
