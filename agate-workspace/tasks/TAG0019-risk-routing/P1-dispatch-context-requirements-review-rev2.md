...
<dispatch_guide>
> ⚠️ 本文件是 P1 需求复审轮（⑩迭代循环第 2 轮）的强制指令。增量模式：重点验证上轮 needs-revision 的 2 条修改项是否已修复，其余按角色定义完整评审。

### 上轮结论与修改项
- 上轮评审：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-review.md（status: needs-revision，2 条修改项 R1/R2）
- analyst 已修复（修复记录见 P1-progress.md「REV1 R1 / REV1 R2 / REV1 selfcheck」）：
  - **R1** → 扫描 2 拆分口径：纯 `risk_level` 口径 = .md 36 / .py 70（与独立实测一致）；C8 单独 = .md 20；原 55 澄清为 `risk_level|C8` 合并去重并集（36+20−1，重叠行 P4-implementation.md:86）；§3.2 写明可复现 grep 命令口径
  - **R2** → P5/P6 保留并入 BDD-7 四要素（申请 ceremony:thin + 逐信号 checklist + 跳过风险评估 + P5/P6 保留，任一缺回退 standard；check-pruning 检查 4/5 双闸兜底）；BDD 编号保持 1-15 连续；§1 范围边界与 §5 已同步引用 BDD-7

### 复审任务
1. 独立复核 R1：按 §3.2 的 grep 口径在 worktree（/home/kity/oclab/agate/.worktrees/agate-TAG0019）复跑，确认纯 risk_level .md=36 可复现；如口径仍不可复现 → 维持 needs-revision
2. 独立复核 R2：BDD-7 四要素 + check-pruning 双闸描述是否可二值判定、覆盖"薄化仪式不薄化验证"；BDD 编号 1-15 连续；§1/§5 引用一致
3. 全量复评：15 条 BDD 可判定性 + 同类扫描三组 + 隐含需求覆盖 + frontmatter 合法性（风险：revision 可能引入新不一致——BDD 修改后相邻编号/§5 同步是否完整）
4. 产出更新版 P1-review.md（Header status: approved / rejected / needs-revision）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（复审对象，已更新）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-review.md（上轮意见，核对修复是否到位）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-progress.md（REV1 修复记录）
...
### 客观查证信息
- /tmp 只读：pytest 需 `--basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider`
- 读卡片/角色用 ~/.agate 稳定版；bash 一律 timeout（30-90s）；读文件用 read/grep 工具
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-review.md`（覆盖更新）
