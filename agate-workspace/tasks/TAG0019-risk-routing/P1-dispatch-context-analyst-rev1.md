...
<dispatch_guide>
> ⚠️ 本文件是 P1 需求修改轮（⑩迭代循环第 1 次）的强制指令。增量模式：复用上轮产出与派发上下文，只修本轮目标。

### 上轮产出
- P1-requirements.md：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-requirements.md
- 上轮 dispatch-context：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-dispatch-context-analyst.md（复用其目标/约束/输入清单，不再重写）
- P1-review.md（评审意见，权威）：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-review.md

### 修复目标（评审 2 条修改项，全部修复后才允许 self-check 通过）

**R1 — 扫描 2 risk_level .md 命中数口径**：P1-requirements.md 扫描 2 声明 risk_level .md 命中 55 处，独立实测（worktree agate/ 子树）为 36 处。处理二选一：
- 修正数字为实测一致值，或
- 保留原值但必须写明 grep 命令口径（含扫描范围/pattern/排除项）证明 55 可复现
无论哪种，最终数字必须与读者在 worktree（/home/kity/oclab/agate/.worktrees/agate-TAG0019）可复现的实测一致。
（注意：.py=70、check-pruning=40、ceremony=0 三组与独立实测一致，不用动。）

**R2 — "P5/P6 不可裁"补 BDD 固化**："薄化仪式不薄化验证"目前只有 §1/§5 声明层，BDD 集无一条把"thin 档不得薄化 P5/P6 验证"固化为可判定验收项。处理二选一（评审建议）：
- 并入 BDD-7 的 thin checklist 要素（缺 P5/P6 保留要素 → 回退 standard），或
- 并入 BDD-14 范围（full 档 P7 不可裁之外，补 thin 档 P5/P6 保留判据）
修改后 BDD 编号须保持连续（新增则重排编号，或并入既有 BDD 的 Then 判据——推荐并入避免 BDD 计数膨胀），并同步 §5 裁剪声明引用。

### 约束（与上轮一致，不重写）
- 只改 P1-requirements.md；不碰评审文件、不新增交付物
- BDD Given/When/Then 可二值判定、无行首 [NEED_CONFIRM]、frontmatter 不破坏（risk_level=medium / phases 全集 / packages / domains 保持）
- bash 一律 timeout；读文件用 read/grep 工具；单步串行
- /tmp 只读（pytest 需 --basetemp=/home/kity/oclab/dsh-workspace/ptmp）；读卡片/角色用 ~/.agate 稳定版
- 状态标记：`[PROD_NOT_TOUCHED]`

### 产出
修改后的 /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P1-requirements.md（原地更新，Header 的 trace_id 追加 -rev1 或保留原值均可，status 保持 draft）
修改记录追加写入 P1-progress.md（每个修复项一行：修了什么 + 证据）

### 返回
只返回两行：① P1-requirements.md 路径；② 一句话摘要（≤30 字，注明 R1/R2 处理方式）。
</dispatch_guide>
