...
执行 P2-design.md gate_commands 的 P5 命令集，逐条判定通过/失败，产出 P5-test-results/（unit.md / fail-list.txt）。TAG0019 实现对 P3 测试（test_agate_risk_score / test_check_routing / test_docs_assertions / test_check_frontmatter ceremony / test_agate_md_field_get ceremony / test_pre_commit_hook 2j.1）+ 全量回归。

### P5 命令集（P2-design.md §4 声明，按此执行）

1. **P5（全量测试）**：`python3 -m pytest -q --tb=no -p no:cacheprovider --basetemp=<可写Basetemp>` —— 在 worktree 根运行全量 `agate/tests/`
2. **P5_consistency**：`python3 /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/scripts/check-protocol-consistency.py --strict-errors-only` —— 必须用 worktree 自己的脚本
3. **P5_platform**：`python3 /home/kity/oclab/agate/agate/scripts/check-platform-assumptions.py <7 文件变更集>` —— 文件清单：agate-risk-score.py / check-routing.py / test_agate_risk_score.py / test_check_routing.py / test_check_frontmatter.py / test_agate_md_field_get.py / test_pre_commit_hook.py
4. **P5_count_tests**：`bash agate/tests/scripts/count-tests.sh`

### 客观查证信息（本机实测，硬约束）

- **/tmp 只读 + ptmp 只读（Errno30）**：P2 声明的 `--basetemp=/home/kity/oclab/dsh-workspace/ptmp` 不可写 → 改用**可写 basetemp `/home/kity/oclab/agate/.ptmp-scratch`**（worktree 下，用后清理）。⚠️ 该目录位于 git 仓库内——这正是已知环境前提（P4 评审 I1），影响仅 1 条用例：
  - `test_check_routing.py::test_bdd_7_thin_score_anomaly_git_ok_false_exit_1`：设计意图在"非 git 上下文"触发 git_ok:false，但本沙箱可写 basetemp 全在 git 仓库内 → run_git 必成功 → 该用例失败（exit 0 而非 1）。**实现逻辑已两轮 GIT_DIR=/nonexistent 探针验证正确**（git_ok:false + thin → exit 1 fail-closed）。处置：fail-list.txt 记录该用例 + 标注"环境前提 I1（沙箱无 git 仓库外可写 basetemp），实现已探针验证，非缺陷"
- 解释器 /usr/bin/python3（裸 pytest 命令 PATH 找不到）
- bash 一律 timeout（P5 全量测试用 timeout 300s，其余 90s）；单步串行不并行
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P5-test-results/`
- 分阶段落盘：关键步骤追加 P5-progress.md

### 产出规格

- `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P5-test-results/unit.md`：测试运行汇总（failed 数量 + 输出签名行 PASSED/FAILED/passed/failed）
- `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P5-test-results/fail-list.txt`：failed 测试 id 逐行（`FAILED ` 前缀），含 I1 标注
- **全量测试（含非本任务测试）**：须跑 `agate/tests/` 全量；发现预存失败 → unit.md 标注"预存失败：X（与本次改动无关）" + 登记 known-failures.md（若与本次无关）

### 门槛（什么算完成）

- P5-test-results/unit.md 存在，含 failed 计数 + 运行签名（grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)' > 0 才有效）
- 四条命令逐条执行并记录结果
- 真失败（本次改动引入）→ 明确标注回 P4；环境前提（I1）→ 标注非缺陷

### 返回

只返回两行：① P5-test-results/ 路径；② 一句话摘要（各命令 pass/fail + failed 数）。
</dispatch_guide>
