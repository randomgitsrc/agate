...
产出 `P1-requirements.md`（需求基线）：TAG0019「风险分路由（RM-AG0031）」——把任务仪式深度从"agent 自报复杂度"改为"客观信号脚本算分"，压成本曲线不降质量地板。需求覆盖五个交付物（M1-M2 主体）：
1. 新增 `agate/scripts/agate-risk-score.py`——客观信号算分（文件类型/敏感路径/改动规模对齐 pruning 源码数≤5/域映射/影响面）→ risk_score + tier
...
5. M3（thin 档跳过 LLM 评审）实证验收锚：以 TAG0018"LLM 评审≈0 净收益"为基线，前后对比评审轮数 vs 真实发现数

产出必须满足 P1 gate：≥1 条 BDD（Given/When/Then）、无行首 `[NEED_CONFIRM]`、无 `status: GAP`、BDD 编号锚点 `BDD-[0-9]`、含同类扫描结论、P0-brief 时效性质疑记录。

### 约束

- **复用不重造**（P0-brief 强制）：check-pruning.py 的源码数≤5 / coupling_checklist / 跳过风险判定逻辑直接复用，不新写一套
- **核心不符**：thin 档 = 申请 + 逐信号 checklist + 跳过风险评估，缺一不可回退 standard；不声明 = standard（fail-closed）
- **P5/P6 不可裁**：薄化的是"仪式"（评审/折叠），不是"验证"（测试/验收）
- **测试平台无关**：不得硬编码单平台假设；/tmp 只读需 --basetemp
- **SELF-GATE 触发面**：改动 `agate/**/*.md` 与 `agate/scripts/*.py` 均触发 self-gate；P1 需求不必包含 self-gate 流程本身（协议既有机制），但 BDD 中应声明触发文件清单以利 P8
- **同类扫描（强制，HANDOFF §5）**：
  1. grep `check-pruning.py` 的源码数≤5 / coupling_checklist / 跳过风险判定——哪些可复用/需扩展
  2. grep 全仓 `risk_level` / `ceremony` / 裁剪 / check-gate 消费点——P1 卡/P2 卡/state-machine/dispatch-protocol 等哪些文件跟着改
  3. grep 平台差异（opencode/Claude Code/DSH）对 gate 语义的影响——agate-risk-score.py 是否受平台影响
  - 每条命中：记录命中数量 + 文件清单，逐条判定"本次处理 / 本次不处理 + 理由"；同类型未来新增 → 声明回归拦截手段并转 BDD
- 需求基线是"活的、向前累加"：BDD 用 Given/When/Then 写"做完后应表现成什么样"，不写实现方案

### 上游关联

P0-brief 已锁定（task/issues/known_risks/env_constraints 四字段）。设计文档 design-risk-routing.md（三原则/三档 ceremony 表/M1-M4 落地节奏）是 P1 分析的直接基础。TAG0018 实证（LLM 评审≈0 净收益、机械 gate 全胜）为 M3 验收锚的基线数据。任务工作区 = `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/`（含 P0-brief.md + .state.yaml phase=P1）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P0-brief.md（任务简报与风险声明，权威）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/.state.yaml（任务状态 phase=P1）
- /home/kity/oclab/dsh-workspace/agate-research/design-risk-routing.md（设计提案：三原则/档位表/落地节奏）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/HANDOFF-TAG0019.md（任务范围 + 强制扫描要求）
- /home/kity/oclab/agate/agate/assets/execution-roles/analyst.md（角色定义，稳定版）
- /home/kity/oclab/agate/agate/scripts/check-pruning.py（同类扫描对象 1：既有判定逻辑）
- /home/kity/oclab/agate/agate/phase-cards/P1-requirements.md 与 P2-design.md（同类扫描对象 2：risk_level/裁剪消费点）
- /home/kity/oclab/agate/agate/state-machine.md 与 dispatch-protocol.md（同类扫描对象 2 续：裁剪/评审映射消费点）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace`；{agate_root}（稳定版，读卡片/角色/脚本用）= `/home/kity/oclab/agate/agate`（= ~/.agate）；改造对象（要 grep 的仓库本体）= `/home/kity/oclab/agate/.worktrees/agate-TAG0019`。注意：grep 同一段代码要区分"稳定版 ~/.agate（工具）"与"worktree（改造对象）"两个副本——扫描以 worktree 为准（那是要被改的）。

### 客观查证信息（本机实测，硬约束）

- /tmp 只读：pytest 必须 `-p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp`；解释器用 `/usr/bin/python3`（裸 pytest 命令 PATH 找不到）
- 跑 gate/读卡片/读角色文件用 `~/.agate`（=/home/kity/oclab/agate/agate）；`check-protocol-consistency.py` 若需跑用 worktree 自己的（/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/scripts/check-protocol-consistency.py）
- bash 一律外层 `timeout`（30-90s，按命令预期耗时）；读文件优先 read/grep/glob 工具；单步串行不并行 bash
- 状态标记：`[PROD_NOT_TOUCHED]`（本任务不涉及生产环境）
- 产出路径是硬约束：写 `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md`
- 分阶段落盘：每读完一个输入文件/关键步骤，把发现追加写入 `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-progress.md`
