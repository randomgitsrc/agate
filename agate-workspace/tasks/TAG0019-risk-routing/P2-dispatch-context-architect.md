...
产出 `P2-design.md`：TAG0019「风险分路由（RM-AG0031）」方案设计。需求基线 15 条 BDD 已 approved（P1-requirements.md）。设计覆盖五个交付物（M1-M2 主体）：
...
3. `check-pruning.py` 扩展为 check-routing（或新增 CHECK）：校验 ceremony 声明 vs risk_score 与 checklist（BDD-10 复用不重造：必须复用 check-pruning 同源判定）
4. `requirements-review` 角色增"审声明"职责（风险分级/裁剪声明 vs diff 证据）
5. M3 实证验收锚度量协议（BDD-12，非实施 M3 主体）

### 约束

- **复用不重造**（P0/P1 强制）：check-pruning.py 的 `_md_field`/`_read_p1`/`_staged_source_count` 与源码数>5 判定（check-pruning.py:134-136）直接复用；check-routing 与 check-pruning 同源判定（BDD-10）
- **fail-closed 语义**（BDD-7/8/9）：thin = 申请（ceremony: thin）+ 逐信号 checklist（coupling_checklist 流式格式）+ 跳过风险评估 + P5/P6 保留，缺一回退 standard；不声明 = standard；声明薄于算分 → 拦截（单向 fail-closed）
- **P5/P6 不可裁**：check-pruning 检查 3（P6）+ 检查 5（P5）双闸兜底（BDD-7）
- **平台无关**（BDD-13）：新脚本须过 check-platform-assumptions R1-R5（硬编码 PATH/裸解释器/[[ -L ]]/临时目录字面/裸外部工具 零命中）；路径相对化/CRLF 归一化先例对齐
- **消费点同步**（BDD-15）：scripts/README.md 工具清单、tests/README.md 用例映射、agate-summary.py 漂移清单、WORKFLOW.md gate 表、UPGRADING.md 需同步新机制
- P1 SUGGEST-1（可采纳）：check-routing 建议扩展现有 check-pruning.py 而非独立新脚本——你评估采纳与否并写入方案权衡
- 测试平台无关（agate 核心约束）；/tmp 只读 pytest 需 --basetemp

### 上游关联

P1-requirements.md（15 BDD approved）为输入权威；P0-brief.md 环境约束；P1-progress.md 含同类扫描三组的消费点清单（risk_level/ceremony 消费点：agate-frontmatter-check.py:31-50、agate-md-field-get.py:122/187、check-pruning.py:93/128、role-system C8:52-70、P2 卡:182-186、P4 卡:84-86、dispatch-protocol:740/931、requirements-review.md:50、WORKFLOW 裁剪矩阵:237-263、CONTEXT:19/29）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（需求基线，权威）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P0-brief.md（环境约束与风险）
- /home/kity/oclab/dsh-workspace/agate-research/design-risk-routing.md（设计提案：三原则/档位表/M1-M4）
- /home/kity/oclab/agate/agate/assets/execution-roles/architect.md（角色定义，稳定版）
- /home/kity/oclab/agate/agate/scripts/check-pruning.py（复用对象：判定逻辑 + 契约）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/scripts/check-pruning.py（改造对象 worktree 版本，确认与稳定版同源）
- /home/kity/oclab/agate/agate/phase-cards/P1-requirements.md（ceremony 字段要加的卡）/ P2-design.md（本卡规格）
- /home/kity/oclab/agate/agate/assets/review-roles/requirements-review.md（"审声明"职责要加的角**
- /home/kity/oclab/agate/agate/scripts/check-gate.py 与 pre-commit-gate.py（gate 消费点：check-routing 挂载处）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`（= ~/.agate）；改造对象（worktree）= `/home/kity/oclab/agate/.worktrees/agate-TAG0019`。

### 客观查证信息（本机实测，硬约束）

- /tmp 只读：pytest 必须 `-p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp`；解释器 `/usr/bin/python3`
- 读卡片/角色/脚本用 ~/.agate 稳定版（=/home/kity/oclab/agate/agate）；check-protocol-consistency.py 若跑用 worktree 自己的
- bash 一律外层 timeout（30-90s）；读文件优先 read/grep 工具；单步串行不并行 bash
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-design.md`
- 分阶段落盘：关键步骤追加 `{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-progress.md`

### P2 产出规格（P2 卡片为准，关键点摘要）

- 候选方案 ≥2 + 权衡 + 选择理由 + frontmatter `candidate_count:`（与本正文一致）
- **影响面梳理（强制节，写在候选方案之前）**：改什么（逐文件/小节 + BDD 编号）、不改什么（显式 + 理由）、风险在哪（每条配缓解）；有客观证据（grep 命中/消费方代码/gate 校验口径）
- 四字段：`packages:` `domains:` `ui_affected:`（本任务 false）`gate_commands:`（P5 用 pytest 命令，注意 --strict-errors-only 拆独立 key 不塞 && 链）
- `files_to_read:`（P4 implementer 参考清单，控制上下文，宁少勿多）
- `env_constraints:`（确认/细化 P0-brief 环境约束）
- `minimal_validation:`（P2 最小验证：本任务为纯代码逻辑/静态判定则声明"纯代码逻辑，无外部系统依赖"+写明依赖的内部函数）
- Header：phase: P2 / task_id: TAG0019-risk-routing / type: design / parent: P1-requirements.md / trace_id: TAG0019-P2-20260821 / status: draft / created: 2026-08-21 / agent: architect

### 评审预告

P2 完成后主 Agent 将派 plan-eng-review（domains=backend 强制）。你的方案须含候选权衡与影响面证据，便于方案评审。
