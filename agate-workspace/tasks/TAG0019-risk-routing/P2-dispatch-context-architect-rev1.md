...
<dispatch_guide>
> ⚠️ 本文件是 P2 设计修改轮（⑩迭代循环第 1 次）的强制指令。增量模式：复用上轮产出与派发上下文，只修本轮目标。

### 上轮产出与评审结论
- P2-design.md：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P2-design.md（方案 B：独立 check-routing.py + importlib 同源复用）
- P2-review.md（评审意见，权威）：/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace/tasks/TAG0019-risk-routing/P2-review.md（status: rejected）
- 上轮派发上下文：P2-dispatch-context-architect.md（复用其目标/约束/输入清单）

### 修复目标（评审修订要求）

**必改（BLK-1，阻塞级）**：gate_commands.P5_platform 收窄为**本任务变更文件集**——
- 原声明：全树双 target（`<worktree>/agate/scripts <worktree>/agate/tests`）→ 基线实测即有存量命中（agate-install.py:326,330,396 R2；check-platform-assumptions.py:22,38 自伤 R3/R1；3 个 sh hook R2；pre-commit-gate.py:61 / install-hook.py:128 R2），P5 照单执行必 exit 1
- 改为：`check-platform-assumptions.py` 只扫**本任务新增/修改文件**——`agate/scripts/agate-risk-score.py`、`agate/scripts/check-routing.py` + 新增/修改的测试文件（P3 test-designer 产出后确认清单）
- 验收口径（满足 BDD-13）：变更文件集 exit 0；既有 scripts 树存量命中不阻塞（记入评审备查）
- 同步修订：§0.3 R5 缓解措辞（"全树扫描 0 命中" → "变更文件集 0 命中"）、§5 实现完成标志对应行

**建议改（非阻塞 4 项，评审建议本轮修订）**：
- NB-1：BDD-14"P7 不可裁"落地为声明层+评审层保证——① P1 卡 ceremony 说明补"声明 ceremony: full 的任务 phases 必须含 P7"；② D4 requirements-review 核对项补"declaration ceremony: full → phases 含 P7"逐信号核对；③ §2.6 删"通常伴随…自然被拦截"，改显式声明保证层级
- NB-2：check-routing 错误边界分支语义声明完整——① P1 缺失分支：与 check-pruning 同链语义对齐（exit 2 或显式声明该分支语义）；② 算分异常分支：fail-closed 语义（run_git 失败 → thin 声明不通过 exit 1，或显式记录"降级为 standard 判定"）
- NB-3：影响面信号判据精确化（反向引用扫描范围/命中判据写清，保证 BDD-5 二值可判）
- NB-4：files_to_read 补文档格式先例（至少 requirements-review.md:48-52、role-system.md:54-70）；§0.2 Not Modify 显式交代"阶段总览 WORKFLOW:290 不处理（权威在 role-system C8）"与"P3/P7 卡不细化（thin 档仪式薄化与阶段去留正交）"，消除 P1-P2 清单不一致

### 约束（沿用）
- 只改 P2-design.md；candidate_count 与正文一致；不破坏四字段与 gate_commands 其余 key
- 影响面梳理三部分保持客观证据；minimal_validation 结论不推翻（方案 B 复用成立已获评审锁定）
- bash 一律 timeout；读文件用 read/grep 工具；单步串行
- /tmp 只读（pytest --basetemp）；读卡片/角色用 ~/.agate 稳定版
- [PROD_NOT_TOUCHED]；修改记录追加 P2-progress.md（每项一行：修了什么 + 证据行号）

### 产出
更新后的 P2-design.md（原地更新，Header status 保持 draft，trace_id 追加 -rev1 或保留均可）+ 修改记录

### 返回
只返回两行：① P2-design.md 路径；② 一句话摘要（≤30 字，注明 BLK-1/NB 处理情况）。
</dispatch_guide>
