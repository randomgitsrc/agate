...
<dispatch_guide>
> ⚠️ 本文件是 P1 需求复审轮第 3 次（⑩迭代循环第 4 轮）的强制指令。目标：确认 R3 已修复，全量复评通过后产出 approved。

### 迭代历史
- 第 1 轮：needs-revision（R1 命中数口径、R2 P5/P6 BDD 固化缺失）
- 第 2 轮：needs-revision（R1 已通过；R3 新发现——check-pruning 检查编号引用错误）
- 第 3 轮（本轮）：analyst 声称已修复 R3，待你独立复核 + 全量复评

### R3 修复声明（待你独立核实）
- 两处（§1:59、BDD-7 When:196）已从"检查 4/5（P5/P6 不可裁剪）"改为"检查 3（P6 不可裁）+ 检查 5（P5 不可裁）"
- analyst 自检：全文件 grep '检查 4/5|4/5' 残留=0；其余检查编号引用（检查 7、检查 9、检查 1/6）与源码一致
- 修复记录：P1-progress.md「REV2 R3 / REV2 selfcheck」

### 复审任务
1. 独立复核 R3：read check-pruning.py:113-125 确认检查 3=P6/4=P4/5=P5；grep P1-requirements.md 确认两处已改且无残留错引
2. 全量复评（防迭代引入新不一致）：15 条 BDD 可判定性 + 编号 1-15 连续 + 同类扫描三组 + 隐含需求覆盖 + frontmatter 合法性 + 裁剪合理性 + P1 纯净性
3. 产出最终 P1-review.md（覆盖更新，Header status: approved / needs-revision）

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（复审对象）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-review.md（上轮意见）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-progress.md（REV2 修复记录）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/scripts/check-pruning.py（R3 核实对象）
- /home/kity/oclab/dsh-workspace/agate-research/design-risk-routing.md（设计提案）
- /home/kity/oclab/agate/agate/assets/review-roles/requirements-review.md（角色定义）
...
### 返回
只返回两行：① P1-review.md 路径；② Status: <approved|needs-revision>（与 Header 一致）。
</dispatch_guide>
