...
评审 TAG0019「风险分路由」（RM-AG0031）的 P2 方案设计 `P2-design.md`，产出 P2-review.md（Header status: approved / rejected）。C8 映射：domains 含 backend → plan-eng-review 必派（单角色，直接产出 P2-review.md，无组长汇总）。

### 评审重点（TAG0019 特性叠加 plan-eng-review 通用要点）

通用（plan-eng-review 角色定义）：
- 数据流清晰（agate-risk-score.py 输入 diff → 算分 → 输出 risk_score/tier/证据行的异常路径）
- 接口契约明确（check-routing.py 与既有 check-pruning.py、pre-commit-gate 挂载点的契约）
- 错误边界（fail-closed：声明薄于算分 → 拦截；脚本异常 → 降级/回退语义）
- 测试策略（单元覆盖各信号判定；P5 gate_commands 覆盖集成）
- 技术债记录（如提出"后续应重构"须 DEBT 格式）
- **多方案探索**：P2-design.md §1 是否 ≥2 候选 + 权衡 + 选择理由（candidate_count: 3，方案 A/B/C）
- **实现就绪度**：files_to_read 是否覆盖实现所需全部上下文；方案是否清晰到 implementer 无需步骤计划即可实现
- **P2 最小验证**：minimal_validation 字段（本任务声明纯代码逻辑 + confirmed，若不是则打回）

TAG0019 特性专审：
1. **复用不重造（BDD-10）**：方案 B"独立 check-routing.py + import 复用 check-pruning"是否真复用（import 同源函数 _md_field/_read_p1/_staged_source_count/源码数>5），还是表面 import 实际重写？对拍/同源要求是否落实为可判定验收
2. **fail-closed 三 BDD（BDD-7/8/9）**：ceremony 声明校验（thin 四要素缺一回退 standard / 不声明=standard / 单向 fail-closed 拦截）在方案中是否闭环
3. **P5/P6 不可裁（BDD-7）**：check-routing 与既有 check-pruning 检查 3（P6）+ 检查 5（P5）双闸关系是否清楚
4. **平台无关（BDD-13）**：gate_commands.P5_platform 显式传 agate/scripts + agate/tests 两 target 是否覆盖新脚本 R1-R5；路径/CRLF 归一化先例是否引用
5. **消费点同步（BDD-15）**：6 脚本注册点 + 10 文档同步清单是否与 P1 同类扫描清单一致、无遗漏
6. **影响面边界**：不改 check-gate.py / 不改 check-pruning 既有 8 检查 / M3 主体与 M4 不在范围——边界是否合理

### 上游关联

P1-requirements.md（15 BDD approved）为方案输入；P1-progress.md 含同类扫描消费点清单；design-risk-routing.md 为设计提案。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-design.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（需求基线）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P0-brief.md（环境约束与风险）
- /home/kity/oclab/dsh-workspace/agate-research/design-risk-routing.md（设计提案）
- /home/kity/oclab/agate/agate/assets/review-roles/plan-eng-review.md（角色定义）
- /home/kity/oclab/agate/agate/scripts/check-pruning.py（复用对象源码核验）

路径说明：{AGATE_WORKSPACE} = `/home/kity/oclab/agate/.worktrees/agate-TAG0019/agate-workspace`；{agate_root}（稳定版）= `/home/kity/oclab/agate/agate`。

### 客观查证信息

- /tmp 只读（pytest --basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider）；解释器 /usr/bin/python3
- 读卡片/角色用 ~/.agate 稳定版；bash 一律 timeout（30-90s）；读文件用 read/grep 工具
- [PROD_NOT_TOUCHED]；产出路径硬约束：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-review.md`
- 分阶段落盘：关键步骤追加 P2-progress.md

### 评审产出要求

P2-review.md Header：phase: P2 / task_id: TAG0019-risk-routing / type: review / parent: P2-design.md / trace_id: TAG0019-P2-20260821 / status: approved|rejected / created: 2026-08-21 / agent: plan-eng-review
输出结构：架构问题（阻塞级）/ 架构问题（非阻塞）/ 测试缺口 / 锁定决策。结论须引用设计锚点（方案编号/小节/文件），禁裸 approved。

### 返回

只返回两行：① P2-review.md 路径；② Status: <approved|rejected>（与文件 Header 一致）。
</dispatch_guide>
