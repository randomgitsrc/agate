...
产出 `P6-acceptance.md` + `P6-evidence/`：TAG0019「风险分路由」15 条 BDD（P1-requirements.md）逐条验收，每条 PASS（须带证据引用）或 FAIL。ui_affected: false（非 UI 任务）。

### 输入

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P1-requirements.md（15 条 BDD，权威验收口径）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P5-test-results/（P5 验证证据）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/scripts/agate-risk-score.py 与 check-routing.py（实现对象）
- /home/kity/oclab/agate/.worktrees/agate-TAG0019/agate/tests/unit/test_agate_risk_score.py 等测试（BDD 对照）

### 验收口径（P6 卡）

- **每条 BDD 只允许 PASS 或 FAIL**；所有 PASS 必须有证据文件引用（`- PASS BDD-NN: 描述 (证据路径)`）
- 行格式要求：PASS 行最小格式为 `- PASS BDD-NN: {描述} ({证据路径})`；行首 `- PASS`/`- FAIL`
- frontmatter 汇总：`pass: N` / `fail: 0` / `ui_affected: false`（sample 见 P6 卡）
- **P6-evidence/ 必须非空**，每个文件含实质内容（不接受 1 行文本充数）
- **引用 P5 证据判定**：审计 7 将读取 .state.yaml p5_pass_commit（=c7a5355，P5 通过点）；若 P5→P6 间无非产出文件改动 → 可复用 P5 证据（P5-test-results/），不必重跑全量
- 验收记录**验收时的事实**：BDD 满足则 PASS + 引用证据；不满足写 FAIL（不写"修复后 PASS"）
- **功能验证 ≠ 凑格式**：每条 BDD 对照实际行为/输出判定，不凭测试名猜

### 本任务 15 BDD 速览（用于对照，详细见 P1）

- BDD-1..5 算分脚本（输出三要素/文件类型/敏感路径/规模/域映射影响面）
- BDD-6..10 ceremony+check-routing（合法值/fail-closed×2/单向拦截/复用同源）
- BDD-11 requirements-review 审声明职责（文档断言）
- BDD-12 M3 验收锚度量协议（文档断言）
- BDD-13 平台假设零命中（platform 扫描）
- BDD-14 full 档强制评审 P7 不可裁（文档断言）
- BDD-15 消费点文档同步（consistency 0 ERROR）

### 环境纪律

- /tmp 只读 + ptmp 只读：pytest 用可写 basetemp `/home/kity/oclab/agate/.ptmp-scratch`（用后清理）
- 读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s；单步串行
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P6-acceptance.md` + `P6-evidence/`
