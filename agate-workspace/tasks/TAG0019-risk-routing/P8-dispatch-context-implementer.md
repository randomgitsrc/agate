...
<dispatch_guide>
> ⚠️ 本文件是 P8 发布阶段的 releaser（implementer P8 模式）派发指引。你的产出是 P8-release.md——**不执行 git commit / tag / bump**（主 Agent 亲自执行）。

### 目标

产出 `P8-release.md`：TAG0019「风险分路由」发布准备。版本：**v0.58.0**（minor，新增 ceremony 机制功能）。

### 发布信息（已核实）

- 当前版本：v0.57.0（README badge / CHANGELOG [0.57.0] / git tag v0.57.0）
- 目标版本：**v0.58.0**（bump_type: minor——新增 ceremony 字段 + agate-risk-score.py + check-routing.py 机制，向后兼容）
- UPGRADING.md 已有 v0.58.0 占位章节（docs-sync 已写，需确认内容与实际发布一致）
- 版本引用文件：README.md version badge（:5）+ CHANGELOG.md + UPGRADING.md 章节 + git tag

### 你的产出内容（P8-release.md）

1. `bump_type: minor`（理由：新增 ceremony 机制功能，非破坏性向后兼容——P7 已确认无 BLOCKER/DEVIATION-CRITICAL）
2. `debt_check: reviewed`（读 `{AGATE_WORKSPACE}/debt/tech-debt.md` 核对本任务相关债务——P4 评审遗留 F4/F6 若已登记则列出 id；无则注明"无本任务新增技术债"）
3. 版本号变更确认清单：README badge v0.57.0→v0.58.0、CHANGELOG [Unreleased]→[0.58.0] 新增小节（含 TAG0019 变更条目）、UPGRADING v0.58.0 章节内容核对
4. **临时资源清单**（主 Agent READY 收尾用）：本任务启动的临时服务/进程/数据/开发安装——评估并列出（预期无服务；pytest basetemp 临时目录已清理；ptmp-scratch 若残留列出）
5. 发布检查命令表：基于 P2-design.md gate_commands（P5 全量 / consistency / platform / count-tests）列主 Agent 应执行的验证命令
6. CHANGELOG 变更条目草稿：v0.58.0 小节摘要（新增 ceremony 三档 + check-routing gate + agate-risk-score.py + requirements-review 审声明 + M3 指标；修复 CHECK 9 锚点 / tmp 字面）

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P7-consistency.md（发布前一致性结论）
- {AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P2-design.md（packages + gate_commands）
- README.md（version badge 位置）
- CHANGELOG.md（[0.57.0] 之后加 [0.58.0] 小节）
- agate/UPGRADING.md（v0.58.0 章节核对）
- {AGATE_WORKSPACE}/debt/tech-debt.md（debt_check）

### 环境纪律

- /tmp 只读；读卡片/角色用 ~/.agate（=/home/kity/oclab/agate/agate）；bash 一律 timeout 90s
- [PROD_NOT_TOUCHED]；产出路径：`{AGATE_WORKSPACE}/tasks/TAG0019-risk-routing/P8-release.md`
- 分阶段落盘：关键步骤追加 P8-progress.md

### 返回

只返回两行：① P8-release.md 路径；② 一句话摘要（bump_type + debt_check + 临时资源数）。
</dispatch_guide>
