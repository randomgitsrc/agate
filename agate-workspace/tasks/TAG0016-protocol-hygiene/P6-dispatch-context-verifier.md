...
把 P1-requirements.md 全部 19 条 BDD 逐条实跑验收，产出 P6-acceptance.md + P6-evidence/。本任务
`ui_affected: false`，不需要 Playwright/vision-analyst，不需要截图。功能任务（非 refactor），走
标准验收口径，不走三段式回归口径。

**铁律：先验证，后结论**——每条 BDD 必须先跑命令/读文件看到客观结果，再写 PASS/FAIL，不能"看代码
推断应该满足"。无法验证的标 FAIL，不标 PASS。

### 约束（19 条 BDD 逐条验证方式建议，你可以按实际情况调整命令，但每条都必须有真实执行的证据）

1. **BDD-1（职责声明表落地）+ BDD-19（同一处声明行）**：对 4 个文件（WORKFLOW.md/
   dispatch-protocol.md/state-machine.md/platform-notes.md）跑
   `grep -n "职责边界" agate/{文件}` 确认文件头 20 行内含声明行，内容与 P2-design.md §0 表
   对应条目一致。证据：`bdd-1-19.log`（4 次 grep 输出拼接）。
2. **BDD-2（平台适配收敛）**：`grep -n "## 平台适配" -A5 agate/WORKFLOW.md
   agate/dispatch-protocol.md` 确认已收窄为摘要+指针，`grep -c "." agate/platform-notes.md`
   确认权威源内容完整未被误删。证据：`bdd-2.log`。
3. **BDD-3（阶段门槛表分工声明）**：grep 两处分工声明句存在。证据：`bdd-3.log`。
4. **BDD-4（派发 prompt 单一权威源）**：`wc -l agate/dispatch-protocol.md` 确认「派发 prompt
   模板」节已收窄（对照 M6 记录 <20 行骨架），`head -5 agate/assets/templates/dispatch-prompt.md`
   确认文件头已改为"本文件是权威来源"声明。证据：`bdd-4.log`。
5. **BDD-5（重试上限表单一来源）**：`grep -n "重试上限" -A3 agate/rules/state-transitions.md`
   确认已是纯指针句（不含数值表）。证据：`bdd-5.log`。
6. **BDD-6（内联 MAX 与权威表不同步可被检测）**：本条判据字面是"CHECK 12 运行后报 ERROR"——
   引用已跑通的 `test_check_protocol_consistency.py::test_bdd_9_check12_mismatched_inline_max_reports_error`
   测试作为真实执行证据（该测试构造真实不一致场景并断言 CHECK 12 报错，是这条 BDD 行为的活证明），
   附带跑一次 `python3 -m pytest agate/tests/unit/test_check_protocol_consistency.py -k
   mismatched_inline_max -v` 拿到实时结果。证据：`bdd-6.log`。
7. **BDD-7（Pre-commit 清单模式不误伤）**：确认 3 处正确指针未变（`grep -n "Pre-commit 检查"
   agate/dispatch-protocol.md agate/state-machine.md agate/git-integration.md`）+
   `python3 agate/scripts/check-protocol-consistency.py --strict` 实跑确认 **0 ERROR**（不要求
   0 WARNING——308+ 条既有 WARNING 是本仓库长期存量债务，与本任务无关，已在 P5 阶段核实并登记
   DEBT0012，不影响本条 BDD 判定）。证据：`bdd-7.log`。
8. **BDD-8（协议实际内容与职责声明表一致，P6 人工抽查）**：P1 明确定义这是 P6 阶段的人工抽查
   动作。抽查 WORKFLOW.md、dispatch-protocol.md 各至少 1 处曾被认定"职责定位混乱"的内容段落
   （如平台适配摘要归属、派发编排机制归属），核对是否与 P2-design.md §0 职责声明表口径一致。
   把抽查过程和结论写进 P6-acceptance.md 正文（不是只写 PASS，要写"抽查了哪段、对照哪条职责
   声明、结论是什么"）。证据：`bdd-8-manual-review.md`（人工核对记录）。
9. **BDD-9（CHECK 12 新增跨文件一致性检测）**：`python3 -m pytest
   agate/tests/unit/test_check_protocol_consistency.py -k bdd_9 -v` 实跑，附
   `grep -n "CHECK 12" agate/scripts/check-protocol-consistency.py` 确认已注册进 CHECKS 列表。
   证据：`bdd-9.log`。
10. **BDD-10（不误报既有正确模式）**：`python3 -m pytest
    agate/tests/unit/test_check_protocol_consistency.py -k bdd_10 -v` 实跑。证据：`bdd-10.log`。
11. **BDD-11（全量重跑点审计表存在）**：`grep -n "## 全量重跑点审计" -A10
    agate/dispatch-protocol.md` 确认小节存在且含四个重跑点描述。证据：`bdd-11.log`。
12. **BDD-12（P6 引用 P5 证据无改动校验标准）**：`python3 -m pytest
    agate/tests/unit/test_check_p6_provenance.py -k "bdd_12 or critical1" -v` 实跑（含
    CRITICAL-1 修复后的 fail-closed 场景）。证据：`bdd-12.log`。
13. **BDD-13（不可复用边界）**：`python3 -m pytest agate/tests/unit/test_check_p6_provenance.py
    -k bdd_13 -v` 实跑。证据：`bdd-13.log`。
14. **BDD-14（P8 重跑范围精简）**：`grep -n "复用同一份 P5-test-results" -B3 -A3
    agate/phase-cards/P8-release.md` 确认条件化表述存在且保留"至少一次客观验证动作"底线。
    证据：`bdd-14.log`。
15. **BDD-15（xdist 试点仅锚定 CI）**：`grep -n "xdist" -A5 .github/workflows/protocol-tests.yml`
    确认观测步骤存在、`continue-on-error: true`、不影响 job exit code。**明确写清楚**：本条
    判据是"步骤存在且不影响门禁"，不是"已验证加速效果"——本地/本次验收都不产出"已验证加速"
    的结论。证据：`bdd-15.log`。
16. **BDD-16（xdist 不破坏并行隔离规则）**：`grep -n "资源密集型默认串行" -A8
    agate/dispatch-protocol.md` 确认判据描述仍包含 xdist 相关表述、未被 M23 改动削弱。证据：
    `bdd-16.log`。
17. **BDD-17（回归基线不破坏）**：引用 P5-test-results/unit.md 的实跑结果（966 passed/0
    failed/2 skipped + consistency 0 ERROR + count-tests 968≥961），不需要在 P6 重新跑一遍
    全量（P5→P6 之间本任务自己没有代码改动，P6 阶段引用 P5 证据本身就是 BDD-12/13 这套新机制
    的自我验证——但由于 `p5_pass_commit` 字段是"开发中的新功能"，本次不实际使用该机制，只是
    直接引用 P5-test-results/ 路径作为本条 BDD 的证据来源，说明"回归基线由 P5 已验证"）。证据：
    引用 `{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P5-test-results/unit.md`（相对
    P6-evidence/ 的路径写法见下方"证据路径"说明）。
18. **BDD-18（Windows 兼容仅增量声明）**：人工核对本任务全部 P6-acceptance.md 正文表述（含
    本文件自己）未出现"已在 Windows 实测验证"类措辞；`grep -n "Windows" agate/platform-notes.md
    | head -20` 确认 Windows 原生安装指南章节未被去重误删/误改。证据：
    `bdd-18-manual-review.md`。
19. **BDD-19**：与 BDD-1 共用同一处声明行/证据（见约束 1），P1-requirements.md 已声明"与 BDD-1
    合并测试"，P6-acceptance.md 里可以合并写一条 PASS 同时引用两个 BDD 编号，或分别各写一行
    PASS 引用同一份证据文件（任选，只要两条 BDD 编号都有独立的 PASS/FAIL 行，不能只写一条漏了
    另一个编号）。

### 证据路径说明

- `P6-evidence/` 下的证据文件按上面 `bdd-N.log` / `bdd-N-manual-review.md` 命名，每条 PASS 行
  括号内引用对应文件（相对 `P6-evidence/` 路径）。
- BDD-17 需要引用 P5-test-results/ 里的文件——**复制**（不是移动）
  `P5-test-results/unit.md` 到 `P6-evidence/bdd-17-p5-regression-evidence.md`（保留 P5 目录
  原文件不动，P6-evidence/ 下放一份拷贝供 PASS 行引用，避免跨目录相对路径引用失效）。

### 不要做的事

- 不要改动任何 `agate/*.md` 或 `agate/scripts/*.py`——P6 是纯验收阶段，发现问题标 FAIL 回 P4，
  不能自己动手改代码或改协议文档
- 不要为了凑 PASS 数量而弱化判据（如把"不误报"简化成"没跑到误报的具体路径就算过"）
- 19 条 BDD 全部要有 PASS/FAIL 结果，不能挑验，不允许"调整/跳过/覆盖"等中间态

### 上游关联

P5 技术验证已通过（966 passed / 0 failed / 2 skipped，consistency 0 ERROR，count-tests 968）。
P4 全部完成（3 批次实现 + SELF-GATE 修复 + P4-review 两轮 approved）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（19 条 BDD 全文，验收依据）
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P5-test-results/unit.md（P5 证据，BDD-17 引用）
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-design.md（§0 职责声明表，BDD-1/8/19 对照）
- agate/tests/unit/test_check_protocol_consistency.py、test_check_p6_provenance.py（BDD-6/9/10/
  12/13 引用的测试文件）
...
<objective_info>
- worktree HEAD：4abf959（P5 已 commit），工作区干净。
- ui_affected: false，无需 vision-analyst/Playwright/截图。
- change_type 未声明 refactor，走标准功能验收口径（非三段式回归口径）。
</objective_info>
