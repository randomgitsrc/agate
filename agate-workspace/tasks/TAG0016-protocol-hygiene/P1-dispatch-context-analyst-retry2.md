...
P1-requirements.md 内容已 approved（P1-review.md status: approved，两轮 BDD-12 核查通过），但主
Agent 预跑 `check-gate.py P1` 时 exit 1，**不是内容问题，是格式违规**：

- 上轮产出：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md
- gate 诊断：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-gate-diagnosis.md（完整读一遍）

### 唯一要改的地方

第 282 行（「5. 待确认清单」说明段落）出现了字面文本 `` `[NEED_CONFIRM]` ``（"...故不标
`[NEED_CONFIRM]`"）。这命中 dispatch-protocol.md 明文禁止的反模式——在正文里引用标记文本做否定
描述。check-gate.py 对 P1 的检测逻辑是字面子串匹配：只要正文任何位置出现 `[NEED_CONFIRM]` 这个
字面串且没有合规的行首阻塞声明，就判定格式不合规，不管这句话本身是不是在说"不需要"。

**只需要把这一句改写成不包含字面 `[NEED_CONFIRM]` 子串的表述**，保留原意即可，例如：
- "故不使用该阻塞标记"
- "不判定为需人工确认的阻塞项"
- 或任何你觉得清楚、但不重复出现 `[NEED_CONFIRM]` 这个方括号字面串的表述

第 280 行的 `[NO_NEED_CONFIRM]`（行首、无反引号包裹）本身是合规声明，**不要动这一行**。

### 不要做的事

- 不要碰任何 BDD（1-19 条全部已 approved，包括 BDD-12/BDD-13）
- 不要碰 frontmatter
- 不要碰第 280 行的 `[NO_NEED_CONFIRM]` 声明本身
- 不要重新产出整份文件，只在第 282 行附近做定点文本替换
- 改完后自己搜一遍全文确认没有其他地方也字面出现了 `[NEED_CONFIRM]`（如果有，一并改掉，不要留漏网的）
...
<objective_info>
- `grep -n "NEED_CONFIRM" P1-requirements.md` 当前只有两处命中：第 280 行 `[NO_NEED_CONFIRM]`
  （合规，不动）+ 第 282 行 `` `[NEED_CONFIRM]` ``（违规，需改写）。改完后应只剩第 280 行一处
  命中。
</objective_info>
