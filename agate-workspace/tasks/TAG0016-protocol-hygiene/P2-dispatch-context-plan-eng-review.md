...
独立评审 {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-design.md（architect 刚产出），产出
P2-review.md。本次触发评审的原因：P1 声明 `risk_level: high`，按 C8 映射表硬规则必须派
plan-eng-review（不因为"觉得不需要"而省略）。domains 不含 frontend，不触发
plan-design-review；P1 无 `[NEED_CONFIRM]`，不触发 plan-ceo-review——只有你一个评审角色，评审
结果直接写 P2-review.md，不需要组长汇总。

### 约束

1. 本任务不是产品功能开发，是 agate 协议自身的改造（Markdown 协议文档去重 + 2 个 Python gate
   脚本新增逻辑），`ui_affected: false`。评审重点里的"前后端约定"类问题不适用，改为关注"协议
   文档之间的约定"（谁是权威源、指针句格式是否统一）。
2. **重点核查候选方案是否真实（不是稻草人）**：P2-design.md §2（CHECK 12 设计）和 §3
   （BDD-12/13 provenance 机制设计）各自给出了 3 个候选 + 权衡表，§4 给出整体路线级 2 个候选
   （方案 A/B，`candidate_count: 2`）。请核实这些候选是否满足"某些维度上比方案一更好"（真实
   替代方案的判据），而不是"方案二的缺点只是不如方案一"。
3. **重点核查"自指悖论"论证是否站得住**（P2-design.md §3.2）：architect 论证"P5 commit 无法把
   自己的哈希写进自己携带的文件里，等价解法是写入父提交哈希"，并声称"父提交哈希与 P5 commit 自身
   哈希在 diff exclusion 判定上等价"。请判断这个等价性论证的逻辑是否成立（关键前提：P5 commit
   本身只改 `agate-workspace/tasks/` 下的产出文件，这一前提是否总是成立、是否有反例）。
4. **重点核查批次设计（§5 dispatch_plan）的合理性**：三批 serial（doc-dedup → 
   check12-anti-recurrence → test-evidence-provenance），architect 给出的理由是"批次间隐性依赖 +
   SELF-GATE 触发 + 归因复杂度"。请评估这个 serial 选择是否有更好的并行方案被忽略（如
   `test-evidence-provenance` 与 `doc-dedup` 文件不重叠，architect 自己也承认"理论上可并行"但
   仍选 serial）——这属于工程判断题，不是对错题，请给出你的专业意见，不通过时需说明具体依据。
5. **测试策略核查**：P2-design.md §6 gate_commands.P3 提到新建
   `agate/tests/unit/test_protocol_dedup_audit.py` 作为"一个断言审计测试覆盖批量机械改动"的
   策略（呼应 HANDOFF-TAG0016.md 建议）。请评估这个测试策略是否足够（是否遗漏了某类改动没有
   测试覆盖，如职责声明表落地 M3/M7/M10/M12 的存在性校验）。
6. **DEBT 记录检查**：P2-design.md §3.4 提到"候选 C（commit message 派生）技术上更优雅但依赖无
   gate 强校验的自然语言约定，记录为技术债，不在本任务展开"——这句话不是标准 DEBT 条目格式
   （模板 `assets/templates/tech-debt-template.md`）。请判断：这是否构成"提出了'后续应重构/存在
   架构债'"从而必须用标准格式登记到 `{AGATE_WORKSPACE}/debt/tech-debt.md`？还是只是候选方案权衡
   过程中的一句带过、不构成正式提债？按角色定义"review 可不提债，一旦提必须用标准格式"的规则
   判断，若认为需要标准格式登记，在评审意见里明确指出（不阻塞的话可作为非阻塞建议）。
7. 评审结论按角色定义「输出结构」四段式（架构问题-阻塞级 / 架构问题-非阻塞 / 测试缺口 / 锁定
   决策），Header status 字段为 approved/rejected。

### 上游关联

architect 返回摘要：整体路线 = 结构化白名单锚点 CHECK 12（延续 CHECK4/9/11 模式）+ serial 三批
（doc-dedup → check12-anti-recurrence → test-evidence-provenance）+ `.state.yaml` 新增可选字段
`p5_pass_commit`（P6/P8 证据复用）+ CI-only xdist 观测步骤。BDD-9/10（CHECK 12 算法）和 BDD-12
（provenance 存储位置）各自做了真实 3 候选权衡，均基于真实代码读取（CHECK4/9/11 实现、
`agate-state-yaml-check.py` schema 逻辑）和真实 git 命令验证（diff exclusion 前缀语义）。
`dispatch_plan` 已声明（risk_level=high 触发批次设计硬规则）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-design.md（评审对象，全文约 400 行）
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（对照 BDD 覆盖是否完整）
- {agate_root}/assets/review-roles/plan-eng-review.md（你的角色定义，已通过角色文件读取）
- agate/scripts/check-protocol-consistency.py（核查 CHECK 4/9/11 复用论证是否准确，可选抽查）
- agate/scripts/agate-state-yaml-check.py（核查"无 unknown-field 拒绝"论证是否准确，可选抽查，
  全文仅 58 行）
...
<objective_info>
- P2-design.md 当前 frontmatter：candidate_count=2，packages（8 项，同 P1）、
  domains=[protocol-docs, gate-scripts, test-infra]，ui_affected=false，
  dispatch_plan={mode: serial, parallel_limit: 1, batches: 3 项}。
- worktree HEAD：546b093（P1 commit），P2-design.md 尚未 commit。
</objective_info>
