...
P1-requirements.md 已产出（19 条 BDD），requirements-review 已独立评审，结论 `needs-revision`。
**只修复评审指出的问题，不要重做已通过的部分**——18 条 BDD 评审结论是"可用"，只有 1 条需要修订。

- 上轮产出路径：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（直接在这个
  文件上修改，不要新建文件）
- 上轮 dispatch-context：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-dispatch-context-analyst.md
  （复用其约束，本文件不重复）
- 评审意见：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-review.md（完整读一遍，尤其
  「AG0026 核心机制可判定性评审」节和「补充意见」节）

### 修复目标（唯一必须处理项）

**BDD-12** 的 Then 子句括号部分——"读取 P5 gate 通过时的 commit hash（记录来源为 `.state.yaml`
或 `P5-test-results/` 的 provenance 信息）"——review 实地核查两处候选存储位置后发现：
`.state.yaml` 现有 schema 完全不含 commit hash 字段；`P5-test-results/` 里各任务的 commit 提及
是格式不统一的自由文本，不是可稳定 parse 的结构化字段。这句话读起来像"字段已存在，直接读取即
可"，但实际是需要新增的机制，会误导 P2 architect 把这件事当成读现成字段而非设计新字段。

按 P1-review.md「处理建议」二选一修改 BDD-12（任选其一，不要求两个都做）：
- (a) 明确写成"当前 `.state.yaml` 与 `P5-test-results/` 均无结构化 commit hash 字段，P2 需新增
  字段（属 schema 变更，需声明对存量任务文件的兼容处理方式）"，把这件事显式标注为留给 P2 的
  设计边界，不是可读取的既成事实
- (b) 若认为"具体存哪个字段"纯粹是"怎么做"、应完全留给 P2 设计，则把括号里"记录来源
  为...的 provenance 信息"这个断言性语气去掉，改成"P2 需设计一个可靠的 commit hash 记录/读取
  机制（当前两处候选存储位置均无此字段，非直接复用）"

同时按 P1-review.md「隐含需求覆盖·数据维度」的提醒，在 BDD-12 附近补一句说明：若该字段最终落
在 `.state.yaml`，需声明"字段可选、缺失时回退到强制重跑"（不要求存量任务的 `.state.yaml` 回填
新字段），避免 check-state-yaml.py 未来把新字段设为必填导致 TAG0001~TAG0015 等历史归档任务的
`.state.yaml` 被动校验时报错。这句可以作为 BDD-12 Given/Then 之外的一行补充说明，不需要拆成
独立 BDD。

BDD-13（依赖 BDD-12）review 判定"本身没有新增缺口，BDD-12 修订后无需跟着改"——**不需要动
BDD-13**，除非你判断 BDD-12 的具体修法影响了 BDD-13 的表述（若无影响则保持不变）。

### 可选项（补充意见，非阻塞，可顺手处理也可跳过）

P1-review.md「补充意见」节：BDD-2 直接把 platform-notes.md 预设为权威源，与 BDD-3/BDD-4 把
"指定哪份是权威源"留给 P2 的风格不一致。review 判断这不是问题（3.1 节已给出充分证据），但建议
补一句"本条不留给 P2 二次判断的理由"。**这条不影响 approve 判定，做不做都可以**，做的话在
BDD-2 或 3.1 节末尾加一行即可，不要求返工。

### 不要做的事

- 不要重新产出整份 P1-requirements.md（会导致 frontmatter/trace_id 等已通过评审的部分被无意
  改动）
- 不要碰其余 18 条已判定"可用"的 BDD（BDD-19 review 提示"第二段是背景说明不是独立判据"，这是
  提醒不是打回项，不需要改）
- 不要因为修 BDD-12 就顺带调整 domains/packages/risk_level/phases 等 frontmatter 字段（这些未
  被评审指出问题）
...
<objective_info>
- P1-review.md Header：status: needs-revision，agent: requirements-review（非 main，满足
  agent≠main 要求）。
- review 已完成的复核（可直接采信，不需要重跑）：同类扫描 3.4/3.5/3.6 三条抽查全部与 P1 正文
  判定一致；BDD-2/BDD-5/BDD-9 判据未绑定行号；BDD-14 无"文字游戏"；裁剪评审核对 WORKFLOW.md
  风险矩阵后确认"无裁剪，全部走 P1-P8"理由站得住。这些结论不需要你重新验证。
</objective_info>
