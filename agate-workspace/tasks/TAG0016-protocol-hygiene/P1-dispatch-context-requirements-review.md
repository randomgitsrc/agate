...
独立评审 {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（analyst 刚产出，
19 条 BDD，0 个 NEED_CONFIRM），产出 P1-review.md。只审不改——发现问题写进评审意见，不直接改
P1-requirements.md。

### 约束

1. 本任务不含 frontend/UI 维度（domains = [protocol-docs, gate-scripts, test-infra]），
   requirements-review.md 角色文件里的"frontend 任务 UI/UX 机制评审要点"整节不适用，跳过即可，
   不要因为它缺失而打回。
2. 隐含需求覆盖清单里的"数据/前端/多端"三个维度对本任务字面意义不大（协议文档改造，不是产品
   功能），评审时应把这四个通用维度**按本任务语境重新解读**后再判定覆盖情况，而不是机械对照
   字面意义打"遗漏"：
   - 数据维度 → 协议文档/gate 脚本改动是否考虑了"已有任务的 .state.yaml/历史产出文件"是否受影响
   - 前端维度 → 不适用（无 UI），可标"不适用"
   - 多端维度 → 协议是否要在多平台（OpenCode/Claude Code/Codex）一致生效
   - 边界维度 → 去重后锚点失效、防复发 gate 误报/漏报的边界情况
   - 兼容维度 → 是否破坏现有正确的"权威源+指针"模式（P1 正文 2. 节已列，需要核实 BDD 是否真覆盖了这条）
3. **重点核查 P1 正文「同类扫描」一节的可信度**——这是本任务 P0-brief 明确强调的强制要求
   （不愿意一轮一轮来回改）。评审时请至少抽查 1-2 条 grep 命令（P1 正文 3.1-3.6 节给出的具体
   grep 命令）在 worktree 里重新跑一遍，确认命中结果与 P1 正文描述的判定结论一致，而不是只读文字
   叙述就采信。**在 worktree 自己的 agate/ 目录跑，不要用 ~/.agate**。
4. **重点核查 BDD 判据是否真的不绑定具体行号**——P1 正文多处自称"判据绑定标题/内容存在与否，
   不绑定行号"，请抽查几条 BDD（如 BDD-2/BDD-5/BDD-9）确认 Then 子句确实没有写死行号作为判定
   依据（写文件名+标题/关键词是可以的，写死具体行号数字作为判据不可以，因为协议文档后续会继续
   演进导致行号漂移）。
5. **BDD-12/BDD-13（AG0026 核心机制）的可判定性是评审重点**：这两条定义"P6 引用 P5 证据"的
   git diff 校验标准，请核实 Then 子句里描述的判定流程（取 P5 通过 commit hash → git diff 到
   P6 发起时点 → 排除产出文件路径 → 非空即不可复用）是否表述完整、无歧义，是否存在"P5 通过
   commit hash 从哪里读取"这类关键信息缺失的情况（P1 正文写"记录来源为 .state.yaml 或
   P5-test-results/ 的 provenance 信息"，请判断这个表述对 P2 architect 是否足够可操作，不够
   具体的话应打回要求 analyst 补充或标注留给 P2 设计的边界）。
6. **BDD-14（P8 放宽）容易被写成"文字游戏"**：请核实这条 BDD 是否清楚区分了"现状是什么"
   （已核实：P8 目前跑一次 P5 gate，不是两次）和"要改什么"（精简这一次的范围/触发方式），避免
   放行一条模糊到无法验收的 BDD。
7. **裁剪说明评审**：本任务声明"无裁剪，全部走 P1-P8"，请核对给出的理由（跨模块 + 机制交叉，
   ≥5 协议文件 + ≥2 子系统交互）是否站得住，若认为可以裁剪某阶段（如 P3），要在评审意见里给出
   具体理由，不是无根据地要求补充裁剪。
8. 评审结论必须引用具体锚点（BDD 编号 + 覆盖维度标注），裸 "approved" 不算数（角色文件「实质
   锚点要求」节）。

### 上游关联

analyst 返回摘要：P1-requirements.md 路径，19 条 BDD，0 个待确认项（[NO_NEED_CONFIRM]）。
analyst 在正文「同类扫描」节报告核实结论：P0-brief 六条已知重复中，①②③⑤成立（但③分叉程度
比预期更严重，⑤配对文件与 P0-brief 猜测不同）、④不成立（已是正确模式）、⑥定性问题非结构化；
另系统扫描新发现一类未在 P0-brief 之列的重复模式（8 张阶段卡片内联 MAX 数字散落，对应
BDD-6/BDD-9）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P0-brief.md（对照范围锁定是否被遵守）
- {agate_root}/assets/review-roles/requirements-review.md（你的角色定义，已通过角色文件读取）
- agate/WORKFLOW.md、agate/dispatch-protocol.md、agate/state-machine.md、
  agate/platform-notes.md、agate/rules/state-transitions.md（抽查同类扫描 grep 结论时的核对
  对象，读 worktree 自己这份，不要读 ~/.agate）
...
<objective_info>
- P1-requirements.md 当前状态：frontmatter risk_level=high，phases 含全部 P0-P8，
  domains=[protocol-docs, gate-scripts, test-infra]，capability_requirements 四条均
  status: available，正文含 [NO_NEED_CONFIRM] 声明。
- worktree 当前 HEAD 仍为 d48b742（P1-requirements.md 尚未 commit，工作区/暂存区改动为
  P1-requirements.md + 两份 dispatch-context 文件本身）。
</objective_info>
