...
上一轮你（或另一个 requirements-review 实例）评审 P1-requirements.md 后结论 `needs-revision`，
唯一实质问题是 BDD-12 的 Then 子句括号部分把"P5 通过 commit hash 的记录来源"描述得像已存在的
可读字段，实测两处候选位置均无此字段。analyst 已修订。

- 上轮评审：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-review.md（完整读一遍，了解
  上一轮发现的问题和已确认没问题的部分）
- 上轮 dispatch-context：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-dispatch-context-requirements-review.md
  （复用其约束，本文件不重复）
- 本轮修复说明：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-dispatch-context-analyst-retry1.md
  （analyst 收到的修复指引，可对照看改动是否覆盖到位）

### 本轮复审范围（收窄，不要重新审全部 19 条）

1. **只需重点核查 BDD-12 的修订是否解决了上一轮指出的问题**：Then 子句括号部分是否已去掉
   "字段已存在、可直接读取"的既成事实语气，是否已显式声明这是需要 P2 新增的 schema 变更；是否
   补充了"字段可选、缺失回退强制重跑"的存量兼容说明。
2. 确认 BDD-13 未被意外改动破坏原有清楚的判定逻辑（analyst 被要求"除非确认 BDD-12 修法影响了
   它的表述，否则不动 BDD-13"）。
3. 确认 BDD 总数仍是 19 条、编号连续、其余 17 条 BDD 内容与上一轮评审时一致（未被误改）。
4. BDD-2/3.1 节可能新增了一句"权威源归属不留给 P2 二次判断"的说明（上一轮补充意见，非阻塞项，
   analyst 可选择做也可选择不做）——若已加上，确认表述合理即可，不构成 approve/reject 的判据；
   若未加，也不因此打回（上一轮已明确"不要求本轮修订"）。
5. **不需要重新做完整的同类扫描 grep 复核、不需要重新逐条评审全部 19 条 BDD 的覆盖维度、不需要
   重新评审裁剪说明**——上一轮已完成且结论稳定（3.4/3.5/3.6 抽查一致、行号绑定核查通过、BDD-14
   非文字游戏、裁剪理由站得住），这些结论在本轮直接沿用，除非你发现 analyst 的改动意外波及了这些
   已确认部分。

### 判定标准

BDD-12 修订到位（括号语气已从"既成事实"改为"需 P2 设计的 schema 变更" + 补了存量兼容说明）→
`status: approved`。若认为修订仍不到位（如仍暗示字段已存在，或存量兼容说明缺失/不清楚）→
`status: needs-revision`，具体指出还差什么。
...
<objective_info>
- P1-requirements.md 当前 BDD 总数：19（`grep -c "^#### BDD-" P1-requirements.md` = 19，主 Agent
  已核实）。
- BDD-12 修订后原文（主 Agent 已读取确认存在）：Then 子句末尾新增"『P5 gate 通过时的 commit
  hash』当前无处可直接读取——经核实 `.state.yaml` 现有 schema...不含 commit hash 字段，
  `P5-test-results/` 里的 commit 提及是格式不统一的自由文本...这是一次需要 P2 新增的 schema
  变更...不是读取既有字段"，并在 BDD-12 之后新增一段"补充说明（数据维度，若字段落在
  `.state.yaml`）：新增的 commit hash 字段须声明为可选，缺失时回退到强制重跑（不要求
  TAG0001~TAG0015 等存量归档任务的 .state.yaml 补填该字段）"。
- BDD-2 判定段末尾新增一句："本条不留给 P2 二次判断权威源归属——上述独家内容量对比...已经充分，
  无需 P2 重新调查判断。"
</objective_info>
