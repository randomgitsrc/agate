...
执行 P2-design.md §6 固化的 `gate_commands.P5`，产出 P5-test-results/unit.md（+ fail-list.txt）。
本任务 `ui_affected: false`，不需要 E2E/Playwright，不需要 e2e.md。

### 约束

1. **gate_commands.P5 的确切命令**（P2-design.md §6，直接执行这条，不要自己拼凑）：
   ```
   ...
   ```
   三条命令用 `&&` 串联，任一失败即整体失败。`P5_timeout_seconds: 180` 已声明，跑这条命令时
   设 shell 层超时 `timeout 200s`（180 × 1.5 ≈ 270，但三条命令实测总耗时约 100-110s，180s
   声明已含余量，再加保守系数用 200s 足够，不需要严格套用 ×1.5 公式导致超时设得过长）。
2. **这是全量测试**，不是只跑本任务改动相关的文件——`gate_commands.P5` 本身就是全量套件命令，
   直接执行即可，不需要额外判断"要不要全量"。
3. **worktree 环境**：在 worktree 自己的 `agate/` 目录下跑，不要用 `~/.agate`（HANDOFF-TAG0016.md
   §2 双工作区纪律：改代码/跑测试用 worktree 自己的）。
4. **预期基线**（供你核对实跑结果是否符合预期，不是让你不跑直接抄）：P4 阶段主 Agent 已多次
   独立验证过 966 passed / 0 failed / 2 skipped，`check-protocol-consistency.py --strict`
   0 ERROR（308 条与本任务无关的既有 WARNING，不阻塞），`count-tests.sh` 应显示总用例数
   ≥ 961（P4 阶段最后一次统计为 961，本轮修复轮又新增了 3 条测试，预期在此基础上增加）。
   若你的实跑结果与此有出入（尤其是 failed>0），如实记录，不要因为"应该是绿的"就默认写通过。
5. **`.state.yaml` 的 `p5_pass_commit` 字段**：这是 TAG0016 本任务正在开发的**新机制**（P2 §3.4/
   phase-cards/P5-verification.md M20），但本次编排会话用的是 `~/.agate`（稳定版工具，尚未包含
   TAG0016 的改动），所以**不要求你在本次 P5 派发里实际写入 `p5_pass_commit` 字段**——这是
   "开发这个新功能的过程"，不是"使用这个新功能"，自我指涉但不矛盾：稳定版工具编排 TAG0016 的
   开发工作，TAG0016 完成合并后，未来的任务才会用上这个新字段。不要因为读到 P2 设计里提到这个
   字段就误以为本次也要照做。
6. **[PROD_TOUCHED] 声明**：本任务全程不涉及生产环境，验证完成后写 `[PROD_NOT_TOUCHED]`（负向
   格式本身，不要在句子里重复引用这个字面标记文本）。
7. **预存失败判断**：若发现失败，先判断是否与本任务改动无关（预存失败）——但按上面的基线预期，
   本轮应该是全绿，若出现失败大概率是本任务引入的真实回归，需要如实记录并说明具体是哪个测试、
   什么错误，不要笼统写"有失败"。

### 上游关联

P4 已完成（3 个实现批次 + SELF-GATE 修复轮 + P4-review 修复轮，均已 commit）。P4-review.md
status: approved（2 轮，0 阻塞）。SELF-GATE protocol-alignment-review 全部 ALIGNED/RESOLVED。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P0-brief.md
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P1-requirements.md（BDD 条件、范围声明）
- {AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-design.md（§6 gate_commands.P5 确切命令）
...
<objective_info>
- worktree HEAD：0738075（P4 全部完成，含 self-gate-skip 补记 commit），工作区干净。
- 预期基线：966+ passed / 0 failed / 2 skipped；consistency --strict 0 ERROR；count-tests
  ≥ 961。
</objective_info>
