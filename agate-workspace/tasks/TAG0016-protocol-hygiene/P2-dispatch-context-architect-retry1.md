...
P2-design.md 已产出，plan-eng-review 独立评审后结论 `rejected`（1 个阻塞项 + 3 个非阻塞建议）。
**候选方案本身（CHECK 12 选候选 2、BDD-12/13 选候选 A、整体路线选方案 A、serial 批次划分）全部
通过评审，不需要重新设计或重新权衡**——只需要按下面 4 点做定点修订。

- 上轮产出：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-design.md（直接在这个文件上
  修改，不要新建文件）
- 上轮 dispatch-context：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-dispatch-context-architect.md
  （复用其约束，本文件不重复）
- 评审意见：{AGATE_WORKSPACE}/tasks/TAG0016-protocol-hygiene/P2-review.md（完整读一遍）

### 修复目标 1（阻塞项，必须处理）

§3.2「自指悖论」的等价性论证——"父提交哈希与 P5 commit 自身哈希在 diff exclusion 判定上等价"——
review 用真实 git 历史找到反例：`git show --name-only 5bdcd90`（历史 `wf(TAG0001-P5):` commit）
证明"P5 commit 只改产出文件"这个前提不总成立——该 commit 混入了对 `agate/scripts/agate-debt-check.py`
的真实修复（非产出文件）。这意味着若某次 P5 commit 混入非产出文件改动，父提交哈希与 P5 commit
自身哈希在 diff exclusion 判定上**不再等价**。

按 review「架构问题（阻塞级）」给出的修复方向，在 §3.2（或 §1.3 风险表新增一条 R9）补充：
(a) 承认该前提的边界条件——P5 commit 若混入非产出文件改动，等价性不再成立；
(b) 说明失败方向是保守/安全的——不会产生"应重跑却被误判为可复用"的安全漏洞，只会产生"该被判定
可复用却被误判为需要重跑"这种保守方向的误判（多跑一次，不会少跑该跑的验证）；
(c) 给出轻量缓解——如在 M20（P5-verification.md 写入 `p5_pass_commit` 的插入点）附近明确写一句
"P5 commit 不得混入非产出文件改动，若发现顺手修复的必要性，应先回 P4 走正常流程，不要混入 P5
commit"，或在 §1.3 风险表新增一条 R9 记录该残余风险。

**这是文档补充，不需要重新设计 §3 的机制本身**（候选 A 的选择依然成立）。

### 修复目标 2-4（非阻塞，一并处理，避免再走一轮评审）

2. **§5 批次设计论证文本事实错误**：原文称 `test-evidence-provenance` 批次"与 `doc-dedup` 文件
   不重叠，理论上可并行"——review 核对后发现 M7（`dispatch-protocol.md` 文件头新增职责边界声明，
   属 `doc-dedup` 批次）与 M16（`dispatch-protocol.md` 新增小节「## 全量重跑点审计」，属
   `test-evidence-provenance` 批次）**同时修改 `dispatch-protocol.md`**，两批次实际有文件重叠。
   订正这段论证文本（不改变最终 serial 决策，甚至这个重叠是支持 serial 的更硬理由——可以把这点
   写进去替换掉原来"理论上可并行"的表述）。

3. **§11 完成标志"8 份协议文档"计数与实际列名不一致**：原文列出 6 个文件名（WORKFLOW.md /
   dispatch-protocol.md / state-machine.md / platform-notes.md / rules/state-transitions.md /
   dispatch-prompt.md）却称"8 份"；且按 §1.1 M-表，实际新增"§0 职责边界声明行"这一具体格式的
   落点只有 M3/M7/M10/M12 四个文件（WORKFLOW.md/dispatch-protocol.md/state-machine.md/
   platform-notes.md）——`rules/state-transitions.md`（M11）和 `dispatch-prompt.md`（M8）拿到的
   是各自不同性质的指针改写/矛盾声明修正，不是同一格式的"职责边界"声明行。请统一口径：明确写清
   "职责边界"声明行具体落在哪几个文件（建议就是 M3/M7/M10/M12 这 4 个），§11 完成标志和 §0
   职责声明表的措辞需要跟这个口径一致，不要再用"8 份"这类会让 P4/P6 产生范围分歧的数字。

4. **§3.4 候选 C"记录为技术债"未落地为标准 DEBT 条目**：review 判断这句话构成正式提债（不是权衡
   表里一带而过的陈述），需要二选一处理：
   - (a) 补一条标准 DEBT 条目到 `{AGATE_WORKSPACE}/debt/tech-debt.md`（模板见
     `agate/assets/templates/tech-debt-template.md`，category: protocol，source: review，
     priority: low，evidence 指向 P2-design.md §3.4）
   - (b) 把 §3.4 原文"记录为技术债"的措辞改为不构成正式提债的表述（如"候选 C 保留作为未来若
     commit message 格式被 gate 化后的重新评估选项，不登记为债务"）
   任选一种处理，不要求两个都做。

### 不要做的事

- 不要重新设计 CHECK 12 算法选择、BDD-12/13 存储位置选择、整体路线方案选择、serial 批次划分
  ——这些评审已认同，全部保持不变
- 不要碰 frontmatter（candidate_count/packages/domains/ui_affected/dispatch_plan 均未被评审
  指出问题）
- 不要重新产出整份 P2-design.md，用 Edit 定点修改对应章节
...
<objective_info>
- P2-review.md Header：status: rejected，agent: plan-eng-review（非 main）。
- review 已确认无异议、不需要重新验证的部分：CHECK 12 候选 2 选择、BDD-12/13 候选 A 选择、
  整体路线方案 A 选择、serial 批次拆分的最终决策（论证文本需订正但决策本身认同）。
</objective_info>
