...
### 目标
产出 P1-requirements.md：把 P0-brief.md 列出的 5 条已核实缺陷（DEBT0010/DEBT0011/DEBT0012/DEBT0014/RM-AG0028-DEBT0015）转成需求基线——BDD 验收条件 + 同类扫描结论 + 裁剪声明，供 P2 architect 设计。
...
2. **回归基线不可破**：现有 950 pytest 全绿 + `check-protocol-consistency.py --strict` 0 ERROR 是回归底线，需求条款不能隐含"允许现有测试变红"。
3. **Windows 兼容是增量声明，不是实测承诺**：DEBT0014 相关 BDD 不得写成"已验证 Windows 行为"，只能声明"静态修复 + Linux 回归 + CI matrix 验证"。
4. **五条 issue 域重叠——按"文件→改动"归并 BDD，禁止同一文件被两条不相关 BDD 各自认领一次**：
   - DEBT0010（4 个 gate 解析脚本）+ RM-AG0028/DEBT0015（env_constraints 语义边界）都会touch `gate_commands` 语义——**这两条必须在同一 BDD 分组下整体表述"gate_commands 是执行机制、env_constraints 是声明性字段"这条边界**，不能拆成两组互不提及的 BDD，避免 P2 设计口径冲突。
   - DEBT0014 三个薄壳（`pre-commit-gate.sh`/`commit-msg-self-gate.sh`/`pre-push-gate.sh`）的改动点结构相同，需求应以"三薄壳同批改"表述，不要拆成三条互不关联的 BDD。
   - DEBT0011（SELF-GATE.md 审查文件命名）与 DEBT0010/DEBT0012/DEBT0014/DEBT0015 无域重叠，可独立成组。
5. **强制同类扫描清单**（每条都要在 P1 正文写出"命中数量 + 文件清单 + 逐条处理判定"，"只此一处"也要显式写）：
   - `_timeout_seconds` 全仓 grep（找 DEBT0010 之外第五处遗漏消费点）
   - `agate-alignment-review-{date}` 及同类纯日期命名模式 grep（DEBT0011 同类命名引用）
   - `--strict` 在 gate_commands 声明 / 协议文档（phase-cards、dispatch-protocol.md 等）中的所有使用点（DEBT0012 影响面）
   - `env_constraints` 全协议引用点 + 各引用点对应的 gate_commands 消费情况（DEBT0015 影响面）
   - `command -v` / 薄壳探测循环相关 grep（防同类平台探测陷阱，不限于已知 3 薄壳）
   - `python3` / `WindowsApps` / `Store` 关键词 grep（防同类跨平台兼容性陷阱）
6. **不确定是否超出 P0-brief 锁定范围时先标 `[NEED_CONFIRM]`，不要擅自扩大范围**（P0-brief 已明确锁定 5 条 issue，DEBT0013/DEBT0009 明确排除在外）。
7. `risk_level` 建议 `medium`（协议核心 gate 脚本 + 3 薄壳 + 多份协议文档同批改动，非单文件小改），但最终由你判断并说明理由。
8. `domains`：本任务改动对象是 agate 协议自身（Python gate 脚本 + bash 薄壳 + Markdown 协议文档），不是常规业务前后端；按实际改动性质声明（不强制 frontend/backend 字面套用，可结合 agate 项目惯例判断，之前同类协议任务如 TAG0016/TAG0013 的 domains 声明可作参考）。

### 上游关联
首次派发，无上游 subagent 摘要可引用。P0-brief.md 由主 Agent（PM 视角）亲自写就，已完成 P0 阶段的"同类/影响面预判"粗粒度版本（见 P0-brief known_risks 最后一条），本次 P1 要在此基础上把清单做实（見「同类扫描」约束）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P0-brief.md（主 Agent 任务简报，5 条 issue 精确描述 + known_risks + env_constraints，P1 的主要输入）
- /home/kity/oclab/agate/.worktrees/agate-TAG0017/HANDOFF-TAG0017.md（交接单，含 5 条缺陷的文件:行号级证据、双工作区纪律细节、核心约束 1-6 条、已知风险与止损）
- /home/kity/oclab/agate/.worktrees/agate-TAG0017/AGENTS.md（项目开发约定，权威入口，实际内容见其指向的 agate/AGENTS.md）
- {agate_root}/WORKFLOW.md（"需求与验收机制"节）
- 涉及的具体脚本/文档（用于同类扫描 grep，按需读取，不强制全文通读）：
  ...
  - agate/SELF-GATE.md（DEBT0011 派发模板）
  - agate/scripts/check-protocol-consistency.py（DEBT0012 main() 尾部）
  - agate/scripts/pre-commit-gate.sh / commit-msg-self-gate.sh / pre-push-gate.sh（DEBT0014，约 L11-13）
  - agate/scripts/agate-extract-context.py（DEBT0015，约 L107-109）
  - agate/phase-cards/P2-design.md（DEBT0010/DEBT0015 的 gate_commands 声明规则所在）
</dispatch_guide>
...
<objective_info>
- 环境：worktree 基线已验证（950 pytest 全绿 + consistency 0 ERROR --strict，2026-08-19 验证，交接单第 9 节）
- 任务目录：{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/
- 关键标识：无外部服务/URL 依赖，纯本地脚本 + 文档改动任务
</objective_info>
