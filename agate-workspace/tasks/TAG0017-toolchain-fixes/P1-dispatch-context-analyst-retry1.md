...
<dispatch_guide>
> ⚠️ 修复轮——不要重写完整目标/约束/上游关联，只处理下方「修复目标」列出的具体问题，其余节沿用上轮 dispatch-context 与已产出的 P1-requirements.md。

### 上轮产出路径
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（本轮直接在此文件上订正，不新建文件）

### 上轮 dispatch-context（约束沿用，不重复）
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-dispatch-context-analyst.md

### 评审结论（本轮修复依据）
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-review.md（status: needs-revision，见「待订正清单」节）

### 修复目标（仅这两项，其余节 requirements-review 已核实达标，不要动）

**1. 第 3.3 节（`--strict` DEBT0012 影响面）**：
- 算术错误：正文结论句写"确认 8 个历史任务已踩过或规避过这个反模式"，但正文列出的历史任务只有 6 个命中 + 1 个规避 = 7 个，8 与 7 不符，需订正为与实际枚举一致的数字。
- 分类缺口：review 独立 grep 发现 TAG0005（L250）/ TAG0010（L273）/ TAG0011（L382）三个历史任务的 `gate_commands` 也声明了含 `--strict` 的 `P5_consistency` 命令，但声明形态是独立 `P5_consistency` key（而非 TAG0004 式同一字符串内 `&&` 链式拼接），且 P5-verification.md 未明确规定多个 `P5_*` key 最终执行时是否又被拼成一条 `&&` 链，是否构成同一类短路风险存在解释空间。**要求**：给这三个命中补一个显式分类桶（"独立 key 拆分声明"），并对其是否属于同一反模式/是否需纳入 P2 卡片新增指引覆盖范围做出判定说明——判定结论本身你可自主决定（同类或不同类均可），但不能像本轮一样完全不提。

**2. 第 3.4 节（`env_constraints` DEBT0015 影响面）**：
- 算术错误：正文写"命中 12 处引用"，但随后列出的文件清单实际有 13 个（review 已逐一清点：dispatch-protocol.md / state-machine.md / WORKFLOW.md / P0-orchestrator.md / P1-requirements.md / P2-design.md / P4-implementation.md / analyst.md / architect.md / dispatch-context.md / dispatch-prompt.md / task-files.md / agate-extract-context.py），需订正为与实际枚举一致的数字。
- 分类缺口：review 独立 grep 全仓还命中 `agate/rules/state-transitions.md`（P0-brief 四字段自查清单提及）、`agate/tests/conftest.py`、`agate/tests/fixtures/*/P0-brief.md`（5 个 fixture）、`agate/tests/unit/test_check_retrospective.py`，均未出现在"12 处引用"清单或任何分类桶中。**要求**：为这些测试基础设施类命中补一句归类说明（结论可以维持"本次不处理"，但必须写明为什么不算数——如"清单勾选项/测试字面量引用，非协议语义消费点"），保持与 3.1/3.2/3.3 节同等的分类完整性，不能完全不提及。

### 不要做的事
- 不要改动第 3.1/3.2/3.5/3.6 节（review 已核实通过）
- 不要改动 BDD-1 至 BDD-12（review 已逐条核实 PASS，结构健全）
- 不要改动第 0/2/5/6/7 节及 frontmatter（review 未提出异议）
- 只在第 3.3、3.4 节做订正性补充，不新增/删除 BDD、不改变任何已核实通过的判定结论

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（待订正对象）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-review.md（订正依据，重点读「同类扫描核查」与「待订正清单」两节）
</dispatch_guide>
...
<objective_info>
- 本轮是 retries[P1] 第 1 轮（P1 MAX=3），修复后需重新派 requirements-review 复评
</objective_info>
