...
<dispatch_guide>
> ⚠️ 以下派发指引是本次任务的强制指令。本任务非 UI 任务（`ui_affected: false`），非 refactor 任务（功能任务，缺省口径），单批次（无需并行拆分）。

### 目标
把 P1-requirements.md 的全部 12 条 BDD 逐条实跑验收，产出 P6-acceptance.md + P6-evidence/。

### 约束
1. **双工作区纪律**：只读写 worktree，不碰主 checkout 或 `~/.agate`。
2. **非 UI 任务，无需截图/vision**：所有证据是命令输出日志/文件内容断言（test-output.log 类），不涉及 Playwright/screenshots/vision-analyst。
3. **12 条 BDD 逐条实跑**（不允许"看代码推断应该满足"）：
   - BDD-1~4：跑对应的 pytest 用例（`agate/tests/unit/test_agate_common.py`、`test_gate_key_suffix_audit.py`、`test_check_tdd_red.py`、`test_agate_gate_missing_cmds.py`、`test_agate_gate_p5_count.py`、`test_agate_read_p5_commands.py` 中的具体 BDD 对应用例），确认全部通过（P3 阶段是红灯，P4 后应为绿灯，P6 需重新实跑确认，不能直接引用 P3 的红灯结论）
   - BDD-5/6/9(文档半)：跑 `agate/tests/unit/test_p2p4_boundary_docs.py`，或直接读取对应协议文档段落做断言验证
   - BDD-7/8：跑 `agate/tests/unit/test_self_gate_naming_docs.py`
   - BDD-9(代码半)：跑 `agate/tests/unit/test_check_protocol_consistency.py -k "strict_errors_only"`
   - BDD-10/11：跑 `agate/tests/integration/test_pre_commit_hook.py -k "bdd_10_probe or bdd_11_agate_python"`
   - BDD-12：跑 `agate/tests/unit/test_windows_python_probe_docs.py`
4. **P5 已确认全量测试 1011 passed（含以上全部用例）**——P6 可复用 P5 的技术验证结论作为"测试通过"这一事实的依据，但 BDD 验收要求"翻译成人能看懂的行为结果"，不是简单抄一遍 pytest 输出。每条 BDD 的 PASS 描述应说清楚"验证了什么行为"，证据文件可以是针对该 BDD 的定向 pytest 输出片段（`pytest -k <关键词> -v > P6-evidence/bdd-N.log`）。
5. **证据文件要求**：每条 BDD 的证据文件必须非空、含实质输出（不是 1 行文本充数）。可以多条 BDD 共享同一证据文件（如同批次的多条 BDD 共用一次 pytest 输出），但每条 PASS 行必须显式引用。
6. **无 [NEED_CONFIRM] 待确认项**：写 `[NO_NEED_CONFIRM]`。
7. **[PROD_NOT_TOUCHED]**：本任务不涉及生产环境。

### 上游关联
P5 verifier 摘要：4/4 gate_commands.P5 命令 exit 0（pytest 1011 passed/2 skipped/0 failed；consistency --strict-errors-only 0 ERROR；count-tests 1013 用例达标；shellcheck 0 warning）。`.state.yaml` 已记录 `p5_pass_commit: 17a3a5d6ea00f968352dbeba1c5bf5b22488e786`。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（全部 12 条 BDD 原文，验收依据）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P5-test-results/unit.md（P5 技术验证结论，可作为参考但需针对性重跑）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P4-implementation.md（实现改动清单，理解每条 BDD 对应哪些文件）
</dispatch_guide>
...
<objective_info>
- 环境：worktree（1011 passed 基线已确认，本次 P6 针对每条 BDD 做定向验证 + 证据落盘）
- 非 UI 任务：无需 vision-analyst/Playwright/截图
</objective_info>
