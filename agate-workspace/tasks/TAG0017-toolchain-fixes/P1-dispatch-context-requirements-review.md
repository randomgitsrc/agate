...
### 目标
独立评审 {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md，产出 P1-review.md（只审不写，不直接改需求文件）。

### 约束
1. 只审不写——不得用 Edit/Write 修改 P1-requirements.md，只能产出独立的 P1-review.md。
2. 本任务 domains 不含 frontend（`protocol-docs, gate-scripts`），checklist 中「frontend 任务 UI/UX 机制评审要点」节不适用，可跳过，无需硬要求补 UX BDD/vision 声明。
3. **重点核查「同类扫描」结论落地质量**（这是本任务 P0-brief 反复强调的强制要求，也是历史任务反复复发的失分点）：P1-requirements.md 第 3 节声明了 6 类同类扫描（`_timeout_seconds` / `agate-alignment-review-{date}` / `--strict` / `env_constraints` / `command -v` 探测 / `python3`+`WindowsApps`+`Store`），逐类核查是否都给出了「命中数量 + 文件清单/分类 + 逐条本次处理/不处理判定」，而不是只写结论句。
4. **重点核查"按文件→改动归并 BDD"是否真的做到**：五条 issue（DEBT0010/11/12/14/15）是否被正确归并为 4 个功能分组，尤其检查功能分组 1（DEBT0010 + RM-AG0028/DEBT0015 整合）是否把"`gate_commands` 是执行机制、`env_constraints` 是声明性字段"这条语义边界讲清楚、没有自相矛盾，而不是两条 issue 各自表述互不关联。
5. **重点核查 DEBT0014 相关 BDD（BDD-10/11/12）是否守住"不夸大 Windows 实测"的边界**：验证方式表述是否清楚区分"Linux 模拟回归 + CI matrix 验证"与"已在真实 Windows 环境实测"，若发现措辞暗示已实测 Windows → 应打回。
6. 评审结论必须引用具体产物锚点（BDD 编号 + 覆盖维度清单），裸 "approved" 不算数——按角色定义「实质锚点要求」执行。

### 上游关联
P1 analyst 首轮产出摘要：需求基线已建立，12 条 BDD（4 功能分组归并 5 条 issue），0 个待确认项（`[NO_NEED_CONFIRM]`），1 处轻微 `[P0_STALE]` 已记录（P0-brief task 字段"4 个"文案与实际 5 条 issue 不一致的计数漂移，判定非严重，已记录不阻塞）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P0-brief.md（需求应忠实反映的上游任务简报）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-progress.md（analyst 落盘的同类扫描原始 grep 命令与输出，核查第 3 节结论是否有据）
</dispatch_guide>
...
<objective_info>
- 独立核验（主 Agent 已做）：DEBT0010 的核心断言（4 个脚本只排除 `_formatter`、未排除 `_timeout_seconds`）已用 grep 核实为真（`endswith("_formatter")` 命中 4 处，`_timeout_seconds` 排除逻辑 0 处命中）
- 独立核验（主 Agent 已做）：`_timeout_seconds` 全仓 grep 命中 48 个文件（analyst 声明"约 44 个"，同量级，未见明显失实）
</objective_info>
