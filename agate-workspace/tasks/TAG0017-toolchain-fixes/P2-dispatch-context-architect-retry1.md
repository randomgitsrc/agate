...
<dispatch_guide>
> ⚠️ 修复轮——不要重写方案内容，只修正下方「修复目标」列出的具体问题。方案主体（候选方案选择、批次划分、gate_commands）已被 plan-eng-review 评审确认"无需重新设计，订正后可直接进入下一轮评审"。

### 上轮产出路径
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-design.md（本轮直接在此文件上订正，不新建文件）

### 上轮 dispatch-context（约束沿用，不重复）
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-dispatch-context-architect.md

### 评审结论（本轮修复依据）
{AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-review.md（status: rejected，BLOCKER-1）

### 修复目标（仅一项）

**BLOCKER-1：`SELF-GATE.md` 路径前缀在文档内自相矛盾**

`SELF-GATE.md` 的真实路径是**仓库根目录**（无 `agate/` 前缀）。P2-design.md 全文档内该文件出现 4 处，其中 2 处写对了（无前缀），2 处仍错误地带着 `agate/` 前缀：
- §1.1「改什么」表格（约 L40）：`agate/SELF-GATE.md` L53/54、L133/143、L183/193 → 应改为 `SELF-GATE.md`
- §7 files_to_read 的 `fg2-self-gate-naming` 批次（约 L216）：`agate/SELF-GATE.md:48-60,125-145,175-195` → 应改为 `SELF-GATE.md:48-60,125-145,175-195`

正确写法参照同文档内已经写对的两处（§3「实现完成的标志」约 L149、§6 dispatch_plan 说明的 `fg2-self-gate-naming` 文件列约 L189），统一改为 `SELF-GATE.md`（不带 `agate/` 前缀）。

**修复后自检（强制）**：用 grep 搜索整份 P2-design.md，确认不再有 `agate/SELF-GATE.md` 字符串残留（`/usr/bin/grep -n "agate/SELF-GATE" {P2-design.md 路径}` 应无命中）。

### 不要做的事
- 不要改动方案主体的其他任何内容（候选方案、影响面梳理其余部分、gate_commands、dispatch_plan、minimal_validation、frontmatter）——plan-eng-review 已明确这些部分"无需重新设计"
- 不要新增或删除 BDD 覆盖范围
- 只做路径前缀的字符串订正

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-design.md（待订正对象）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-review.md（订正依据，读「必查项核验结论」与「架构问题（阻塞级）」两节）
</dispatch_guide>
...
<objective_info>
- 本轮是 retries[P2] 第 1 轮（P2 MAX=3），修复后需重新派 plan-eng-review 复评
</objective_info>
