...
<dispatch_guide>
> ⚠️ 以下派发指引是本次任务的强制指令。单批次（无需并行拆分——`gate_commands.P5` 是全量测试套件，资源密集型默认串行）。

### 目标
执行 P2-design.md §5 声明的全部 `gate_commands.P5` 命令，产出 P5-test-results/。

### 约束
1. **双工作区纪律**：只读写 worktree（`/home/kity/oclab/agate/.worktrees/agate-TAG0017`），不碰主 checkout 或 `~/.agate`。
2. **严格按 P2-design.md §5 声明的命令执行，不自行修改命令**：
...
3. **无 UI 改动**（`ui_affected: false`），不需要 E2E/Playwright。
4. **全量测试套件**：`P5` 命令本身就是全量（`agate/tests/`，非本任务专属子集），如实记录 failed 数量，若有预存失败（与本任务无关）按角色文件「预存失败的处理」流程登记 `known-failures.md`。
5. **生产环境隔离**：本任务不涉及生产环境/生产数据库/生产 API，改动对象是 agate 协议脚本与文档本体，验证环境即本 worktree（开发环境）。触发写 `[PROD_TOUCHED]`，未触发写 `[PROD_NOT_TOUCHED]`（预期后者）。
6. **无待确认项**：若验证中未遇到需要人工确认的不可逆操作，写 `[NO_NEED_CONFIRM]`。

### 上游关联
P4 review approved（含 1 轮 review-fix：`--strict` → `--strict-errors-only` 同步修正）；self-gate（protocol-alignment-review）A1-A7 全 ALIGNED（1 轮修复：`agate/scripts/README.md` 补充文档）。主 Agent 在 P4 阶段已多次独立验证全量 `pytest agate/tests/` 为 1011 passed, 2 skipped, 0 failed；`check-protocol-consistency.py --strict-errors-only` exit 0（0 ERROR + 314 WARNING）。本次 P5 verifier 应独立重新执行确认，不能假设 P4 阶段的验证结果直接适用（P4 是 self-authored gate，P5 是 external-output-gate，两者独立）。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P0-brief.md（环境约束）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（BDD 条件、范围声明）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-design.md（§5 gate_commands 声明，命令原文）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P4-implementation.md（已完成的实现改动清单，供理解验证对象）
</dispatch_guide>
...
<objective_info>
- 环境：worktree（dogfooding，check-protocol-consistency.py 必须用 worktree 自己的脚本，不用 ~/.agate 稳定版——因为检查对象是 worktree 里的协议文件本身）
- 已知基线：314 条历史 WARNING（叙事文件死链，非本任务引入，`--strict-errors-only` 模式下不算失败）
</objective_info>
