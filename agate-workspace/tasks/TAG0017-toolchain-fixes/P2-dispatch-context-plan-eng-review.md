...
### 目标
独立评审 {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-design.md，产出 P2-review.md（单评审角色，直接产出，不需要组长汇总）。

### 约束
1. domains 声明为 `[protocol-docs, gate-scripts]`（不含字面 backend/frontend），本任务按项目惯例（参照 TAG0013/TAG0016 同类协议工具链任务）视同 backend 性质的工程改动，触发 plan-eng-review——这是主 Agent 已做的映射判断，你不需要重新判断是否该触发评审，只需正常执行评审职责。
2. **必查项（主 Agent 独立核验发现的具体问题，请在评审中确认是否已订正）**：P2-design.md §1.1「改什么」表格与 §7 files_to_read 中多处引用 `agate/SELF-GATE.md`（如 L40 附近），但独立核验确认该文件实际路径是仓库根目录 `SELF-GATE.md`（无 `agate/` 前缀）——`ls agate/SELF-GATE.md` 不存在，`ls SELF-GATE.md`（根目录）存在且内容与设计文档引用的行号范围（L48-60/L125-145/L175-195）吻合。这是一个真实的路径前缀错误，若 P4 implementer 按 files_to_read 给的错误路径去读文件会直接找不到文件。**请在评审中明确核实此项，若未订正应判 rejected 或 needs-revision 并指出**（不要因为其他部分质量高就放过这个具体路径错误）。
3. 除上述必查项外，按角色定义「评审重点」逐项常规评审：数据流/状态机/接口契约/错误边界/测试策略/技术债记录、多方案探索（≥2 候选 + 权衡）、实现就绪度（files_to_read 是否覆盖实现所需全部上下文）、P2 最小验证（minimal_validation 字段是否合规）。
4. 本任务无 UI/前端受影响（`ui_affected: false`），无需按 frontend 维度评审。
5. dispatch_plan 声明了 5 批 static-batch 并行拆批——评审时确认批次边界是否真的互不重叠（P2-design.md §6 已自称"两两不相交"，请独立核验该结论，尤其关注 §1.3 R1 提到的"`fg1-doc-boundary` 吸收了原本会与其他批次重叠的文档改动"这一处理是否真的消除了重叠，而不是换个说法但实际仍冲突）。

### 上游关联
architect 首轮摘要：8 个候选方案（4 功能分组各 2 个），已声明 dispatch_plan（5 批 static-batch），覆盖全部 12 条 BDD。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-design.md（评审对象）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P1-requirements.md（需求基线，核对方案是否覆盖全部 12 条 BDD）
- {AGATE_WORKSPACE}/tasks/TAG0017-toolchain-fixes/P2-progress.md（architect 落盘的读码过程记录，核查关键行号声明是否有据）
</dispatch_guide>
...
<objective_info>
- 独立核验（主 Agent 已做）：`check-protocol-consistency.py` main() 实际位于 L1076-1134 附近，与设计文档引用一致；`agate_common.py` 已有 `probe_python()` 函数（不同于本任务新增的 `is_gate_meta_key`，两者不冲突）；`is_gate_meta_key` 全仓 grep 0 命中，确认是真正新增，非重复定义
- 独立核验（主 Agent 已做，见「必查项」）：`agate/SELF-GATE.md` 路径错误，实际应为根目录 `SELF-GATE.md`
</objective_info>
