...
> 你是 TAG0013（agate 脚本一致性批）的 P5 技术验证者（verifier）。**只读验证，不修改代码。**

### 目标

执行 P2-design.md gate_commands.P5 全部命令，产出 P5-test-results/（unit.md + fail-list.txt），判定技术验证通过/失败。

### 约束

- 只读验证；不修改任何代码/测试/文档；不 commit
- 全量测试套件必须运行（含非本任务测试），发现预存失败 → 按 P5 卡登记 known-failures.md（从 `{agate_root}/assets/templates/known-failures-template.md` 拷贝模板），并在 unit.md 标注
- 紧凑输出模式（pytest --tb=no），只保留通过/失败汇总+失败清单
- 结果文件必须真实来自实际执行（exit code + 输出摘要），不编造
- 自查≠gate：不声称"P5 已过"

### 上游关联

- P4 实现完成：768 passed, 2 skipped（66s）；consistency 0 ERROR；ruff 通过；count-tests 770
- P2-design.md §5 gate_commands 固化（P4-P6 不能改）

### 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P2-design.md`（gate_commands §5 权威来源）
2. `{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P4-implementation.md`（实现说明）
3. 角色定义：`agate/assets/execution-roles/verifier.md`

### 客观查证信息（执行基准）

P2-design.md §5 gate_commands：
- `P3: python3 -m pytest agate/tests/ -q --tb=short`
- `P5: python3 -m pytest agate/tests/ -q --tb=no`
- `P5_consistency: python3 agate/scripts/check-protocol-consistency.py`（判据 0 ERROR）
- `P5_count: bash agate/tests/scripts/count-tests.sh`
- 补充：ruff check（py 变更静态检查，AGENTS.md 开发约定）——`~/.venvs/agate-dev/bin/ruff check agate/scripts/check-protocol-consistency.py agate/scripts/commit-msg-self-gate.py agate/scripts/check-retrospective.py`
- 注意：consistency 用 worktree 自己的脚本（检查对象是 worktree 协议文件），不用 ~/.agate

### 产出要求

产出目录：`{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P5-test-results/`
- `unit.md`：测试执行结果（failed 数量、通过/失败汇总、测试运行器输出签名 `PASSED|FAILED|passed|failed|ok|not ok` 行）
- `fail-list.txt`：failed 测试 id 逐行（无失败可为空文件）
- 若发现预存失败：`known-failures.md` + unit.md 标注

### 返回给我

- P5-test-results/ 路径
- 每条 gate_commands.P5 命令的 exit code + failed 计数
- 任何预存失败（如有）
</dispatch_guide>
