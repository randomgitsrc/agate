...
> 你是 TAG0013（agate 脚本一致性批）的 P6 验收者（verifier）。**逐条 BDD 对照验收，产出 P6-acceptance.md + P6-evidence/。**

### 目标

对照 P1-requirements.md 全部 11 条 BDD（含 SCOPE+），逐条执行验收（PASS/FAIL 二值），产出 P6-acceptance.md + P6-evidence/ 证据目录。

### 约束

- 只验收；不修改代码/测试/文档；不 commit
- **BDD 二值规则**：每条 BDD 结果只允许 PASS 或 FAIL，不允许中间态。任何 FAIL → gate 不通过
- **P6 BDD 结果格式**：行首 `- PASS BDD-NN: {描述} ({证据路径})`，gate 用 grep -cE '^\s*- (PASS|FAIL)' 匹配
- **BDD 覆盖完整性**：P1 有 11 条 BDD（BDD-1..11）→ P6 必须有 11 条验收结果（PASS 或 FAIL），挑验 = gate 不通过
- **每条 PASS 必须有对应证据文件**（存入 P6-evidence/）：test-output.log / 断言日志 / 执行输出。无证据的 PASS = gate 不通过
- **证据引用格式**：每条 PASS 括号内引用证据文件路径（相对 P6-evidence/ 目录）。hook 会检查引用路径真实存在
- 本任务无 UI（ui_affected: false）→ 不需要截图/vision
- 证据日志格式（M1.3a）：执行日志末行 `EXIT_CODE: <n>`（0 成功）
- 自查≠gate：不声称"验收已通过"

### 验收依据（关键）

- **BDD-1/4/5 依赖 CHECK 10 实现行为**：用 worktree 自己的 check-protocol-consistency.py 实跑验证（0 ERROR）
- **BDD-2**：构造假协议文件含 `check-nonexistent-script.py` → 应 ERROR——可用最小假协议树临时目录模拟（不动真实文件）
- **BDD-3**：豁免 5 类——可在假协议树中构造 UPGRADING/formatters/hook/count-tests/scripts-README 引用验证 0 漂移
- **BDD-6/7/8**：git 暂存区语义——用临时 git 仓库 + commit-msg-self-gate.sh 验证 README 触发 / AGENTS 触发 / CHANGELOG 豁免
- **BDD-9**：既有 4 用例不回归——由 P5 全量 pytest 结果 + test_commit_msg_self_gate.py 覆盖
- **BDD-10/11**：check-retrospective.py 实跑（构造异常模式 / 无异常模式）
- 每条 PASS 附实际执行输出到 P6-evidence/（test-output.log 或独立 .log 文件）

### 输入文件

1. `{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P1-requirements.md`（11 条 BDD + SCOPE+）
2. `{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P2-design.md`（实现语义 + gate_commands）
3. `{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/P5-test-results/unit.md`（P5 全量结果）
4. 角色定义：`agate/assets/execution-roles/verifier.md`
5. 被测脚本（worktree 内）：`agate/scripts/check-protocol-consistency.py`、`agate/scripts/commit-msg-self-gate.py`、`agate/scripts/check-retrospective.py`

### 客观查证信息（执行基准）

- P5 全量：768 passed, 2 skipped, 0 failed；consistency 0 ERROR（279 WARNING 基线）；count 770
- 当前 count-tests.sh = 770（P2 §5 基线 751，新增 19 用例后 770）
- worktree 路径：`/home/kity/oclab/agate/.worktrees/agate-batch`

### 产出要求

产出目录：`{AGATE_WORKSPACE}/tasks/TAG0013-script-consistency/`
- `P6-acceptance.md`（frontmatter 含 pass/fail/ui_affected 汇总 + 11 条 BDD 逐条 PASS/FAIL）
- `P6-evidence/`（非空，每条 PASS 引用真实证据文件）

### 返回给我

- P6-acceptance.md 路径
- 验收结果计数（应与 frontmatter pass/fail 一致）
- 每条未通过 BDD 的具体原因（如有）
</dispatch_guide>
