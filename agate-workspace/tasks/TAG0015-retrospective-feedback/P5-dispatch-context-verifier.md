...
执行 P2-design.md §5 固化的 `gate_commands.P5`，产出 P5-test-results/unit.md（含 passed/failed
计数）+ P5-test-results/fail-list.txt（failed 测试 id 逐行，无失败可为空文件）。

### 约束

1. **准确执行固化命令，不要自行更换**：
   `python3 -m pytest agate/tests/ -q --tb=no && python3 agate/scripts/check-protocol-consistency.py --strict`
   （两条命令用 `&&` 链接，`P5_timeout_seconds: 180`——预期耗时 100-110s 左右，外层 shell
   timeout 按 ×1.5 设为 `timeout 270s`）。
2. **本任务无 UI（`ui_affected: false`）**，不需要 E2E/Playwright，跳过 `gate_commands.P5_e2e`
   相关要求。
3. **全量测试套件基线**：本任务改动前基线是 909 passed + 2 skipped；P4 净增 23 个测试后预期
   基线是 **932 passed + 2 skipped + 0 failed**（P4 commit 208a1ec 后主 Agent 已独立验证过
   这个数字，你的实跑应该复现同一结果）。
4. **【重要，务必读】关于 `test_check_pruning.py` 的已知环境相关性**：该文件的部分用例
   （`test_p2_6e_prune_p7_coupling_checklist_exit_0`/`test_p2_52_yaml_list_phases_exit_0`/
   `test_p2_52b_yaml_list_phases_p3_pruned_low_exit_0`）依赖当前仓库的**真实 git 暂存区**
   （`check-pruning.py:56 _staged_source_count` 用 `git diff --cached --name-only` 判定
   "源码文件数≤5"裁剪条件），不是隔离在测试自己的 tmp fixture 内——这是该测试自身的隔离缺口
   （非本任务引入，主 Agent 在 P4 阶段已排查确认根因并记入 commit message）。**你执行 P5 时
   git 暂存区应该是干净的**（P4 已 commit，暂存区已清空），若你观察到这三个用例失败，先检查
   `git status --porcelain | wc -l` 是否意外有大量文件被暂存（如你自己的操作不慎 `git add` 了
   什么），排除这个环境因素后再判断是否为真失败——不要在暂存区不干净的情况下把这类失败误判为
   本任务的真实回归。
5. **无生产环境接触**：本任务纯协议文档 + Python 脚本改动，无数据库/浏览器/外部服务依赖，
   直接跑测试命令即可，无需准备任何 debug 环境。
6. **签名要求（N5 残余风险缓解）**：P5-test-results/unit.md 必须含可被
   `grep -cE '^(PASSED|FAILED|passed|failed|ok|not ok)'` 命中的 test runner 输出签名（如
   pytest 汇总行 `932 passed, 2 skipped in 100.86s`），不能只写"全部通过"这类无法机器校验的
   摘要。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P2-design.md（§5 `gate_commands`
  声明，权威命令来源）
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P0-brief.md（env_constraints）

### 门槛（什么算完成）
P5-test-results/unit.md 含 passed/failed 计数 + test runner 输出签名；
P5-test-results/fail-list.txt 存在（本任务预期为空文件，因为预期 failed=0）；两条
`gate_commands.P5` 命令均已实际执行且 exit 0。
</dispatch_guide>
...
<objective_info>
- P2-design.md §5 `gate_commands.P5` 原文：
  `python3 -m pytest agate/tests/ -q --tb=no && python3 agate/scripts/check-protocol-consistency.py --strict`，
  `P5_timeout_seconds: 180`。
- 主 Agent 在 P4 commit（208a1ec）后已独立验证：`pytest agate/tests/ -q --tb=no` →
  932 passed + 2 skipped，0 failed；`check-protocol-consistency.py --strict` → 0 ERROR
  （约 300+ WARNING，与基线一致，非新增 ERROR）。当前 git 暂存区应为空（P4 已 commit）。
</objective_info>
