...
对 P1-requirements.md 的 20 条 BDD 逐条做用户视角验收（本任务无 UI，`ui_affected: false`，
"用户"是未来读这些协议文件的主 Agent/subagent，"行为"是协议文本/脚本能否让读者据以正确行动）。
产出 P6-acceptance.md + P6-evidence/，每条 BDD 一个独立证据文件。

### 验收方法（沿用本仓库 TAG0012 P6 先例，风格参照
`agate-workspace/tasks/TAG0012-protocol-mechanism-fixes/P6-acceptance.md`）

1. **不复用 P5 结论**——P5 的 pytest 全绿只证明"关键词/断言存在"，不证明"内容语义正确、真的能
   指导行为"。本轮对 20 条 BDD 逐条打开 HEAD 下的实际文件（协议文档/脚本），读实际内容，逐句
   对照 P1-requirements.md 的 Given/When/Then 判定。
2. **证据形式**：
   - 文档类 BDD（BDD-1~8/12~16，13 条）：`P6-evidence/bdd-NN-*.md`，每个文件含①Then 子句
     逐项核对（每项写"Then 要求什么 → 实际文本怎么写的（引用文件路径+行号）→ 是否满足"）②从
     实际文件摘录的原文片段
   - 脚本行为类 BDD（BDD-9/10/11/17~20，7 条）：`P6-evidence/bdd-NN-*.md`，除文本摘录外，须
     附**独立实跑命令的输出**（不是转抄 P3/P4/P5 的测试断言，而是主 Agent/verifier 自己在本轮
     手动构造场景跑一次，如构造一个含 DEBT/roadmap 关联信号的假 task_dir 跑
     `check-retrospective.py` 观察实际 stderr 输出；构造一份样例 retrospective.md 跑
     `agate-feedback.py` 观察实际 JSON/Markdown 输出），这样证据独立于自动化测试代码本身，
     不会因为测试代码本身有 bug 而两边一起骗过
3. **本轮共享命令输出**落 `P6-evidence/shared-p6-command-output.log`（非转抄 P5）：
   ...
   （P5 909→932 净增 23 的全量回归结果作为"未破坏既有行为"的旁证引用，不替代任何一条 BDD 的
   语义验证）

### BDD 分组导航（P1 §4 六大类，P6 逐条验收时按此定位改动文件）

- 4.1（BDD-1~8）：`agate/assets/templates/retrospective-template.md`（新路径）+
  `agate/phase-cards/P8-release.md`（BDD-8 挂钩点）+ `agate-workspace/roadmap/roadmap.md`
  （BDD-8 路径同步）
- 4.2（BDD-9~11）：`agate/scripts/check-retrospective.py`
- 4.3（BDD-12~13）：`agate/state-machine.md`
- 4.4（BDD-14）：`agate/loop-orchestration.md` + `agate/assets/templates/task-files.md`
  （核实"不改文本但不矛盾"的判断，以及 task-files.md 辅助文件表新增的两行）
- 4.5（BDD-15）：`agate/AGENTS.md`
- 4.6（BDD-16）：`docs/reviews/retrospective-tag00{08,10-0011×2,13,14}-*.md`（5 个文件标注）
- 4.7（BDD-17~20）：`agate/scripts/agate-feedback.py`

### 约束

1. **不得凑数或挑验**——20 条 BDD 必须逐条给出 PASS 或 FAIL，不允许"调整/跳过/覆盖"。若发现
   某条 BDD 实际未达标，如实标 FAIL（阶段卡明确："验收报告记录的是验收时的事实，不是修复后的
   状态"）。
2. **BDD-13 的验收对象是协议文档本身，不是运行时产物**——P2-design.md 已明确 BDD-13 的验收范围
   收窄到"state-machine.md/task-files.md 是否定义了 L2 checkpoint 两件套规则"，不要求本任务
   期间真的产出过 `P{n}-checkpoint.md` 实际文件（这是未来任务落地使用的机制，本任务只负责把
   规则写清楚）。
3. **BDD-17~20 的证据构造**：`agate/scripts/agate-feedback.py` 是纯 CLI 工具，可以直接构造一份
   符合 `retrospective-template.md` frontmatter 格式的样例 `.md` 文件（含
   `mechanism_issues`/`execution_issues`/`feedback_ready`/`## agate 反馈` 节），存到
   `/tmp/` 或 `P6-evidence/` 下，跑 `AGATE_FEEDBACK=on python3 agate/scripts/agate-feedback.py
   {样例文件路径}` 观察实际输出，把命令+输出存进对应证据文件。
4. **本任务无生产环境接触**，验收全程只读文件 + 跑本地脚本，不涉及数据库/网络/浏览器。
5. **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`，路径相对 `P6-evidence/`。总结行
   （如 `**Summary**: 20/20 PASS`）不能用 `- PASS`/`- FAIL` 开头。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P1-requirements.md（20 条 BDD 原文，
  验收判据来源）
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P2-design.md（改动落点参照，理解
  "改成什么样才算对"）
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P4-implementation.md（实现细节 +
  [DESIGN_GAP] 记录 + ADR-007 修复说明）

### 门槛（什么算完成）
P6-acceptance.md：frontmatter `pass`/`fail`/`ui_affected: false` 齐全；20 条 BDD 逐条 PASS/FAIL
+ 证据引用；P6-evidence/ 非空，每个文件含实质内容（非 1 行占位）。
</dispatch_guide>
...
<objective_info>
- HEAD 状态：P5 commit（ae7dc57）后，本任务七类改动已全部落地并通过 P4 review + SELF-GATE
  语义对齐审查 + P5 全量验证（932 passed + 2 skipped + 0 failed，consistency --strict 0 ERROR）。
- 工作目录：/home/kity/oclab/agate/.worktrees/agate-TAG0015。只读验收，不修改任何协议/脚本
  文件，不碰主 checkout 与 `~/.agate`。
- 20 条 BDD 编号锚点：`grep -n "^#### BDD-" P1-requirements.md` 可快速定位每条 BDD 的
  Given/When/Then 原文位置。
</objective_info>
