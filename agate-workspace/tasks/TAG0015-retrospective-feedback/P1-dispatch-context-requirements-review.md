...
## 重试 #1（复核订正，本节优先于下方原始派发指引）

你上一轮的判定是 `needs-revision`：20 条 BDD 中 19 条 approved，仅 BDD-14 因 `task-files.md`
裸路径引用（真实路径 `agate/assets/templates/task-files.md`）判需修订，见你自己产出的
`P1-review.md`「BLOCKER」节。analyst 已针对性订正，**不是重写全文**。本轮复核**只需聚焦以下
改动点**，不必重新逐条核对已判 approved 的 19 条 BDD（除非你发现订正过程中意外破坏了它们的语义）：

1. 用 `grep -n "task-files.md" P1-requirements.md` 确认所有命中（第 52/69/150/154/155/219/225
   行及其偏移后的位置）都已带上完整路径 `agate/assets/templates/task-files.md`，无裸名残留。
2. 确认 BDD-14 的 Given/When/Then 语义未被意外改动（只是路径字符串订正，行为要求应与你上一轮
   approved 的其余 19 条 BDD 保持同一严谨程度）。
3. 确认第 8 节裁剪说明（risk_level 理由）与第 9 节 packages 范围声明已就 `task-files.md` 的包
   归属给出自洽理由（analyst 选择归入 `assets/templates` 包，理由是"任务产出文件命名规范参考表
   而非流程规则文件，物理路径本就在 assets/templates/ 下"——确认这个理由站得住，不要求必须是
   这个答案，但需自洽）。
4. 用 `grep -c "^#### BDD-" P1-requirements.md` 确认仍是 20 条（BDD 编号未被破坏/重排）。

若以上 4 点均已妥善解决 → status: approved，按角色文件「实质锚点要求」补齐 approved 结论应引用
的锚点（可复用上一轮 approved 的 19 条 BDD 判定表，不必重新展开全部理由，但仍需列出 BDD 编号
1-20 完整清单 + 说明 BDD-14 本轮复核通过）。
...
独立评审 analyst 产出的 P1-requirements.md（20 条 BDD，按"文件 → 改动"归并组织，覆盖 RM-AG0020
复盘机制统一（八项残缺 + 项目资产沉淀增量）+ RM-AG0021 跨项目反馈机制（四项修复 + 触发方式修正）
两条已核实的 agate 协议机制缺口）。只审不改，产出 P1-review.md，交主 Agent 判定门槛。
...
2. **重点审查以下四类风险点**（P0-brief known_risks 已预警，是本任务最容易出错的地方）：
   a. **BDD 是否真的按"文件 → 改动"归并、无重复定义**——检查 P0-brief 8+4 条缺口是否都能在 20
      条 BDD 里找到落点，尤其确认"同类扫描"第 3 节里"本次不处理"的判定理由是否站得住（不是为了
      图省事划走），以及第 7 节"范围外观察"的三项是否真的在范围外（不是变相漏做）。
   b. **AG0020 → AG0021 依赖声明是否自洽**——P1-requirements.md 第 5 节声明 BDD-6/BDD-7 是
      BDD-17 的输入依赖，检查这个依赖关系在 BDD 正文里是否真的体现（BDD-17 的 Given 是否引用了
      BDD-6/BDD-7 的产出，而不是凭空假设一份"已有 frontmatter 的复盘文档"）。
   c. **"新增机制设计"类 BDD（BDD-13 的 L2 会话 checkpoint）是否越权写死具体格式**——P1 只应界定
      "P2 设计必须回答哪些问题"（四个问题：落盘时机/文件命名/与 orchestrator-log 关系/防 compact
      策略），不应替 P2 拍板具体触发时机或文件名。若发现已经写死具体方案，应打回。
   d. **BDD-16（存量 4 份复盘迁移决策）与 BDD-9/10（脚本路径提示）的路径字符串是否逐字一致**——
      第 5 节声明"三者路径字符串必须逐字一致，P7 据此交叉核对"，检查 BDD-9/BDD-16 正文引用的
      `tasks/{Txxx}/retrospective.md` 写法是否确实逐字相同（大小写/占位符格式/斜杠）。
3. **BDD 的 Given/When/Then 是否可二值判定**——协议文档类 BDD 常见问题是判据落在"内容读起来
   是否合理"这种主观标准上；核实每条 BDD 的 Then 是否落在"可被 grep 命中的关键词/章节存在性"
   或"字段格式合法性"/"exit code"等客观可判定的锚点上，而非"表述清晰""逻辑合理"这类主观描述。
   BDD-9/BDD-11/BDD-17~20 涉及脚本行为，重点确认 Then 子句是否给出了可执行的验证方式（能跑出
   PASS/FAIL 的命令或断言，不是"应该正确处理"这类空话）。
4. **同类扫描覆盖度是本任务的核心验收点之一**——检查 P1-requirements.md 第 3 节"同类扫描"表格
   （9 行扫描目标）是否体现了对 objective_info 起点之外的独立深挖（不是照抄主 Agent 预跑的 grep
   结果原样复述），尤其确认 analyst 自行发现的 `AGENTS.md:11` 措辞冲突（BDD-15，不在 P0-brief
   原始 8+4 条缺口清单里）是否真的是"目标方案的必然连带"而不是范围蔓延——评审需要判断这条纳入
   是否合理（若认为不合理，应指出理由，而不是默认接受）。
5. **裁剪评审**：analyst 声明"不裁剪任何阶段"（`phases` 全量 P1-P8），需要核实第 8 节裁剪说明的
   理由是否充分（尤其"phases 是任务级字段不支持按 BDD 拆分裁剪"这个论证是否站得住，还是应该按
   BDD 子集裁剪而非整体不裁）。
6. **两项 [SUGGEST:] 是否已给出可直接采纳的具体方案**（BDD-10 用 DEBT/roadmap 关联做检测代理、
   BDD-16 存量复盘保留原位+标注）——确认建议本身逻辑自洽、不是回避决策的模糊说法。
7. 评审产出必须按角色文件「实质锚点要求」逐条引用 BDD 编号（1-20）+ 覆盖维度标注，不允许裸
   "approved"。

### 上游关联

analyst 在同类扫描中发现的关键结论（供评审时交叉核对，不代替自行核实）：
- `check-retrospective.py` 现有设计哲学是"只提醒不阻断"（exit 0 恒成立）——BDD-10（触发标的扩展）
  和 BDD-4（产出流向强制约定）都刻意保留了这个既有契约，没有把提醒机制升级为阻断式 gate。评审
  需确认这个"不升级为阻断"的选择在 BDD 里说明清楚，且没有暗中改变脚本对外契约。
- `postmortem-template.md` 目前不被任何 `agate/` 核心协议文件引用（"游离"状态）——BDD-8 要求
  迁移后至少一个核心协议文件显式引用新路径。评审需确认 BDD-8 的验证方式（grep 新路径字符串）
  是可执行的客观判据。
- orchestrator-log 语义扩展分布在 4 个文件 6 处（state-machine.md/loop-orchestration.md/
  task-files.md/WORKFLOW.md），analyst 判定 WORKFLOW.md:91 是"纯路径级提及不含语义"本次不处理，
  其余 3 处纳入 BDD-12/BDD-14——评审需确认这个"哪些算语义描述、哪些算纯路径提及"的切分是否合理。

### 输入文件
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P1-requirements.md（评审对象，226 行）
- {AGATE_WORKSPACE}/tasks/TAG0015-retrospective-feedback/P0-brief.md（核对 8+4 条缺口是否被完整
  承接，无遗漏无越权扩大）
- {agate_root}/assets/review-roles/requirements-review.md（角色定义，含「实质锚点要求」）

### 门槛（什么算完成）
P1-review.md 含：Header 完整、`status:` 初始 draft，评审结论后改为 `approved` / `rejected` /
`needs-revision`；结论逐条引用 BDD 编号（1-20）+ 覆盖维度标注（同类扫描覆盖 / 依赖关系自洽 /
裁剪合理性 / 客观可判定性 四个维度至少各有一条引用）；若判 needs-revision/rejected，需指出具体
哪条 BDD、哪个字段、缺什么，不允许笼统打回。
</dispatch_guide>
...
<objective_info>
- P1-requirements.md 客观事实（评审前可直接核实，不用重新统计）：`grep -c "^#### BDD-" ` 得 20；
  frontmatter 含 `risk_level: medium`、`phases: [P1..P8]`（全量不裁）、`packages:` 6 项、
  `domains: [process]`、`implicit_coupling: true`；`grep -n "NEED_CONFIRM"` 只命中
  `[NO_NEED_CONFIRM]` 一处（第 203 行），另有 2 处 `[SUGGEST:]`（BDD-10、BDD-16 各一处）。
- 本次是首次进入 P1 评审（retry: 0），无历史评审意见需要承接。
</objective_info>
