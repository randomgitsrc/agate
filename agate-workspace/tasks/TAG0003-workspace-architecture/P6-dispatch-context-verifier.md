...
### 目标
对 TAG0003 P4 实现做 P6 验收：逐条对照 P1 的 20 条 BDD 验收条件，在 worktree 实测验证，产出 P6-acceptance.md（每条 BDD 二值 PASS/FAIL + 证据引用）+ P6-evidence/（执行日志/断言文件）。

### 约束
- **只验证不修改**：不修改 agate/ 任何文件。验收记录的是验收时的事实（PASS 必须基于证据文件实际输出）。
- 本任务是 **agate 协议自身改造**（dogfooding）：验收对象是 worktree `agate/`（新版协议）；**禁止触碰 `~/.agate`**（稳定版 v0.40.2 开发工具）。跑测试/验证用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
- **ui_affected: false**（P2 声明）→ 无 UI/截图需求，无 vision-analyst。
- **BDD 二值规则**：每条 BDD 只允许 PASS 或 FAIL，不允许"调整/跳过/覆盖"。任何 FAIL → 如实记录（gate 会拦）。
- **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`——行首 `- PASS` 大写、括号内引用证据文件路径（相对 P6-evidence/ 目录）。FAIL 同理 `- FAIL BDD-NN: ...`。
- **证据要求**：每条 PASS 必须有对应证据文件（验证脚本执行日志/断言记录），存 P6-evidence/。证据文件须有实质内容（非 1 行文本充数），日志末行含 `EXIT_CODE: <n>` 格式。P6-evidence/ 必须非空。
- **frontmatter 汇总**：P6-acceptance.md 文件头 frontmatter 必须声明 `pass:`/`fail:`/`ui_affected:` 三个机器字段。
- **BDD 覆盖完整性**：P1 有 20 条 BDD → P6 必须有 ≥20 条验收结果（PASS 或 FAIL），不能挑验。
- **验收路径提示**（来自 P2-design.md §4 BDD 覆盖对照 + P4-review 观察项，逐条对照）：
  - BDD-1（新项目初始化 8 子目录）：orchestrator-template.md 接入节 mkdir 命令存在（grep 可判）+ SETUP.md 步骤。
  - BDD-2（默认 agate-workspace/）：`agate-workspace-resolve.sh` 无 .agate.env 时输出 AGATE_WORKSPACE=.../agate-workspace（实跑可判）。
  - BDD-3（项目外路径）：`.agate.env` 绝对路径 → 解析器输出外部路径且项目根不建默认目录（实跑可判）。
  - BDD-4（无 .agate.env 不报错）：实跑解析器 exit 0 + 默认位置。
  - BDD-5（路径含空格）：fixture 含空格路径实跑解析器/迁移工具（或引用 WR.5 测试证据）。
  - BDD-6（迁移工具迁入工作区）：fixture 仓库实跑 agate-migrate-workspace.sh，docs/tasks → 工作区 tasks（实跑 + 文件清单对照）。
  - BDD-7（不丢失状态与阶段产出）：迁移前后文件清单对比（fixture，.state.yaml 必须随迁）。
  - BDD-8（保留 git 历史）：git log --follow 在新路径可追溯（fixture + MW.9 测试证据）。
  - BDD-9（迁移幂等）：重复运行迁移工具 no-op（实跑 + MW 测试证据）。
  - BDD-10（旧布局获得迁移指引）：orchestrator-template.md 旧布局检测+指引存在（grep 可判）。
  - BDD-11（orchestrator 从工作区读 project.md）：orchestrator-template.md 路径为 {AGATE_WORKSPACE}/agents/project.md（grep 可判）。
  - BDD-12（orchestrator 从工作区读任务看板）：路径为 {AGATE_WORKSPACE}/tasks/active-tasks.md（grep 可判）。
  - BDD-13（状态机与 gate 行为不变）：全量 bats 回归绿（P5 已跑 631/0 + ST_WS 回归守卫——引用 P5-test-results/unit.md 作证据）。
  - BDD-14（需求进 roadmap）：WORKFLOW.md roadmap 循环规范 + roadmap-template.md 存在（grep 可判）+ 模拟执行创建条目。
  - BDD-15（roadmap 拆任务进待开始）：WORKFLOW.md 规范 + 模拟拆分（条目 → scheduled + active-tasks 待开始）。
  - BDD-16（任务完成回写 roadmap）：WORKFLOW.md 闭环规范 + 模拟回写（done/cancelled）。
  - BDD-17（内容边界二值判据）：WORKFLOW.md 判据正式规则存在 + 对偶自洽（grep + 双场景核对）。
  - BDD-18（归档迁入工作区 archived/ 幂等）：fixture docs/archived → 工作区 archived（实跑）。
  - BDD-19（无 docs/tasks 时迁移工具正常）：fixture 空源实跑 no-op exit 0（实跑 + MW 测试证据）。
  - BDD-20（一致性白名单与用例数基线全绿）：`python3 agate/scripts/check-protocol-consistency.py` 0 ERROR + `bash agate/tests/scripts/count-tests.sh` 625 无漂移（实跑，P1-review 观察项 1 口径：既有 603 换血不改数 + 新增允许增长）。
- **roadmap 模拟闭环提示**（P4-review 观察项 4）：BDD-14/15/16 按文档规范**模拟执行**闭环（创建条目→拆任务→写待开始→回写 done），不依赖真实跑完一个完整 P0-P8 任务。模拟执行的证据（如 roadmap.md 示例片段）存入 P6-evidence/。
- **自查≠gate**：你产出验收记录，最终 gate 由主 Agent 亲自跑。不要声称"验收已通过"。

### 上游关联
- P1：20 条 BDD（BDD-1..20），risk_level=high。
- P2：方案 A（单点解析器 + git mv 迁移 + roadmap 循环 + 内容边界判据），ui_affected=false。
- P4：9 脚本（2 新 + 7 改）+ 16 文档 + roadmap-template + 8 测试 fixture 换血；review approved（F1/F2/MW.9 闭环）。
- P5：全量验证绿（bats 631/0 + consistency 0 ERROR + shellcheck 0 + count 625）。
- 3 项 SCOPE+ 已回补 P1 scope_resolved。

### 输入文件
- docs/tasks/TAG0003-workspace-architecture/P1-requirements.md（20 条 BDD——**必读**，验收对象）
- docs/tasks/TAG0003-workspace-architecture/P2-design.md（§4 BDD 覆盖对照 + 验收路径——**必读**）
- docs/tasks/TAG0003-workspace-architecture/P5-test-results/unit.md（回归证据——必读）
- docs/tasks/TAG0003-workspace-architecture/P4-review.md（观察项：roadmap 模拟闭环/BDD-20 口径——必读）
- docs/tasks/TAG0003-workspace-architecture/P0-brief.md（环境约束——必读）
- AGENTS.md（项目约定——必读）
- agate/scripts/agate-workspace-resolve.sh + agate-migrate-workspace.sh（验证对象——必读）
- agate/orchestrator-template.md + agate/WORKFLOW.md + agate/SETUP.md + agate/UPGRADING.md（文档 BDD 验证对象——必读）
- agate/assets/templates/roadmap-template.md（BDD-14/15/16 验证对象——必读）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=2bf9221=P5 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
- 版本隔离三条铁律：跑 gate/读卡片用 `~/.agate`（原版规则）；跑测试/验证用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
- 测试基线：全量 bats 631 用例（count-tests 625 + sanity 6）；consistency 0 ERROR；shellcheck 0。
- 已核实查证：解析器默认输出 `{project_root}/agate-workspace`；迁移工具用 `git -c core.hooksPath=/dev/null` + pathspec 限定自动 commit（F1 修复后）；orchestrator-template 已切 {AGATE_WORKSPACE} 路径；WORKFLOW.md 含内容边界判据 + roadmap 循环规范。
- 验证临时目录：可用 /tmp/opencode/ 下建 fixture 仓库（迁移/解析器实跑用），产出证据落 P6-evidence/。
</objective_info>
