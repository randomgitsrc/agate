...
### 目标
执行 TAG0003 P4 实现的技术验证（P5 模式）：从 P2-design.md 读取 gate_commands.P5 全部命令并执行，产出 P5-test-results/（unit.md + fail-list.txt + 各补充命令结果）。你**执行命令并记录结果**，不修改任何代码。

### 约束
- **只读验证**：不修改 agate/ 任何文件、不修改测试、不修复 bug。发现失败只记录，不回改。
- 本任务是 **agate 协议自身改造**（dogfooding）：验证对象是 worktree `agate/`（新版协议）；**禁止触碰 `~/.agate`**（稳定版 v0.40.2 开发工具）。跑测试用 worktree 本体（load.bash 反推 AGATE_ROOT 到 worktree）。
- **必须执行 P2 gate_commands.P5 全部命令**（不是子集——T060 教训：只跑子集会掩盖失败）：
  ```yaml
  gate_commands:
...
- **输出要求**：
  ...
  - 补充命令结果写入 unit.md 各节：consistency（0 ERROR 达标）/ shellcheck（0 error）/ count-tests（用例数 624 无漂移）
  - 日志文件末行须有 `EXIT_CODE: <n>`（M1.3a 格式约定）
...
### 上游关联
- P4 已 commit（9aaab53）：9 脚本（2 新 + 7 改）+ 16 文档 + 8 测试 fixture + roadmap-template；P4-review approved（F1/F2/MW.9 修复闭环）。
- P3 测试：agate-workspace-resolve.bats（9 用例）+ agate-migrate-workspace.bats（含 MW.9）+ check-state-transition.bats（47 用例含 ST_WS）。
- 测试基线：全量 bats 624 用例；consistency 0 ERROR；shellcheck 0；count-tests 624 无漂移（P4 修复轮自验）。

### 输入文件
- docs/tasks/TAG0003-workspace-architecture/P2-design.md（gate_commands §5——必读，命令权威来源）
- docs/tasks/TAG0003-workspace-architecture/P0-brief.md（环境约束——必读）
- AGENTS.md（测试约定/脚本约定——必读）
- docs/tasks/TAG0003-workspace-architecture/P4-review.md（实现评审记录——选读，了解修复内容）
</dispatch_guide>
...
<objective_info>
- 环境状态：worktree 是改造对象（分支 dev/workspace，HEAD=9aaab53=P4 commit）；`~/.agate` → 主 checkout 是稳定版 v0.40.2 开发工具（禁止改动）。
...
- 已核实查证：全量 bats 基线 624 用例；count-tests.sh 基线 624。
</objective_info>
