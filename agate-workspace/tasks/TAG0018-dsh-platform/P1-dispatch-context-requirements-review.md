...
独立评审 `P1-requirements.md`（TAG0018 agate 原生支持 DSH 平台的需求基线），产出 `P1-review.md`，Header 含 `status: approved` 或 `rejected`（含 needs-revision 理由）。你是独立评审角色，不得由需求作者自审。

### 约束

- **独立评审**：只读需求文件与简报，按需求质疑模式逐条核对 BDD 可验收性；不预判 PASS/FAIL（产出文件避免行首 `- PASS`/`- FAIL` 格式，被 provenance 审计拦截）
- **核对项**：① BDD 是否可验收（Given/When/Then 完整、可机器判定，而非模糊描述）② 是否覆盖 P0-brief 全部 issues/known_risks ③ 核心约束是否固化（不发明新结构、身份薄协议厚、测试平台无关、tool-fs-search 必填配置回归）④ BDD 编号锚点连续无缺号 ⑤ 无行首 [NEED_CONFIRM]、无 status: GAP ⑥ 裁剪声明（P7 裁剪 + coupling_checklist）是否合理
- **status 语义**：approved（可推进 P2）/ rejected（有 BLOCKER）/ needs-revision（需补充，计入重试）
- 评审结论落到 `P1-review.md` 的 `status` 字段，主 Agent 只读该字段判定

### 上游关联

P1-requirements.md 由 analyst 产出（19 条 BDD，risk_level=low，裁剪 P7）。P0-brief 已锁定；实机验证证据与草稿实现路径见派发上下文输入文件。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-requirements.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-progress.md（analyst 查证证据与同类扫描原始结果）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P0-brief.md（任务简报，对照基准）
- /home/kity/oclab/agate/agate/assets/review-roles/requirements-review.md（角色定义）
- /home/kity/oclab/agate/agate/WORKFLOW.md（P1 阶段门槛定义）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；P1 产出已就位（P1-requirements.md 25643 字节，19 条 BDD）
- 关键标识：worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018
- 查证结果：P1 gate 预检 = "P1-review.md 不存在——P1 评审不可裁"（预期，待本评审补齐）
</objective_info>
