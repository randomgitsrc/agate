...
执行 TAG0018 的 P5 技术验证：跑 P2-design.md `gate_commands` 声明的全部命令（P5 / P5_consistency / P5_count 三个独立 key），产出 `P5-verification.md`，记录每条命令的 exit code 与输出摘要。

### 约束

- **gate_commands 权威**：P2-design.md §5 声明的命令与 `_timeout_seconds` 为唯一执行依据，P4-P6 不得修改（P2 已固化）
  - P5: `python3 -m pytest agate/tests/unit/ agate/tests/regression/ agate/tests/integration/ -q`（或按 tests/README 分片；timeout 120s；**注意本环境沙箱对 /tmp 只读——pytest 需 `--basetemp=/home/kity/oclab/dsh-workspace/ptmp -p no:cacheprovider`**）
  - P5_consistency: `python3 agate/scripts/check-protocol-consistency.py --strict-errors-only`（timeout 60s）
  - P5_count: `bash agate/tests/scripts/count-tests.sh`（timeout 30s）
- **判定**：全部命令 exit 0 + failed==0 才算 P5 通过；任何非零 → 记录失败并返回主 Agent（不自行深入修复，P5 修复流程按 dispatch-protocol）
- **平台注意**：本环境 Linux；/tmp 只读（用 --basetemp）；长命令外层 timeout；测试平台无关性（test_dsh_preset.py 只读仓库内文件）在 CI 类环境同样成立（P2 isolation_check 声明）
- 产出文件避免行首 `- PASS`/`- FAIL`（provenance 审计拦截）

### 上游关联

P1 BDD-1~19 / P2 gate_commands（P5/P5_consistency/P5_count）/ P3 红灯 8 用例 / P4 实现（8/8 绿 + 906 unit 无新失败，P4 review approved）。基线：worktree 全量 pytest 1028+8=1036 passed + 2 skipped（P4 后）；consistency 0 ERROR / ~317 WARNING；count-tests 1038。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P2-design.md（gate_commands 权威，§5）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P4-implementation.md（实现摘要）
- /home/kity/oclab/agate/agate/assets/execution-roles/verifier.md（角色定义，P5 模式）
- /home/kity/oclab/agate/agate/phase-cards/P5-verification.md（阶段卡片）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018；/tmp 只读（pytest 需 --basetemp + -p no:cacheprovider）
- 关键标识：分支 feat/TAG0018-dsh-platform；P4 已提交（bf69754 + 153c0a2）
- 查证结果：P1~P4 gate 全过；实现 8/8 绿
</objective_info>
