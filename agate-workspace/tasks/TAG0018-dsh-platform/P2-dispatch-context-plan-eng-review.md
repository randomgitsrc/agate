...
独立评审 `P2-design.md`（TAG0018 agate 原生支持 DSH 平台的方案设计），产出 `P2-review.md`，Header 含 `status: approved` / `rejected` / `needs-revision`。你是独立评审角色（工程维度），不是设计者。

### 约束

- **独立评审**：按工程评审视角核对设计，产出文件避免行首 `- PASS`/`- FAIL` 格式（provenance 审计拦截）
- **核对项**：① 六项交付物设计是否满足 P1 BDD-1~19（逐条）② gate_commands 设计是否合规（无 `&&` 短路链、`--strict-errors-only` 独立 key、timeout_seconds 合理）③ 核心约束（不发明新结构 / 身份薄协议厚 / 测试平台无关）④ 权衡取舍锚点与 D-1~D-5 决策合理性 ⑤ 影响面梳理（改 7 / 不改 7 / 风险 6）⑥ candidate_count: 1 与 design_trivial 声明一致
- **status 语义**：approved（可推进 P3）/ rejected（有 BLOCKER）/ needs-revision（需补充，计入重试）
- 评审结论落到 `P2-review.md` 的 `status` 字段，主 Agent 只读该字段判定

### 上游关联

P2-design.md 由 architect 产出（design_trivial 单候选，逐条映射 BDD-1~19，吸收 P1-review 5 条建议）。P1 已 approved（19 条 BDD）。risk_level: low、domains: [cli, docs]、裁剪 P7。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P2-design.md（评审对象，权威）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P2-progress.md（architect 查证证据）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-requirements.md（需求基线，验收对照）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-review.md（P1 评审建议，验证 P2 是否吸收）
...
- /home/kity/oclab/agate/agate/phase-cards/P2-design.md（P2 门槛与 gate_commands 规范）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；P2-design.md 机械条件已自检通过（四字段 / 独立 key / timeout / ui_affected: false）
- 关键标识：worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018；基线 count-tests 1030
- 查证结果：P1 gate exit 2（已过）；P2 待本评审补齐 P2-review.md
</objective_info>
