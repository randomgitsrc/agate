...
执行 TAG0018 的 P6 验收：对 P1 BDD-1~19 **逐条实际验证**（行为验收），产出 `P6-acceptance.md`（每条 BDD 的判定 PASS / NC / FAIL + 证据引用）与 `P6-evidence/` 证据目录，满足 P6 gate（check-gate P6 exit 2：FAIL=0/NC=0/证据非空）+ check-p6-provenance.py exit 0（六道客观审计）。

### 约束

- **P6 验收口径**（WORKFLOW.md P6 节）：P1 是"约定"，P6 是"兑现验证"——逐条把 BDD 条件实际跑一遍，把结果翻译成人能看懂的行为描述。ui_affected: false → 无 UI 截图要求（R1a 截图 barrier 不适用），证据形式 = 命令输出 / 文件检查 / grep 结果
- **证据要求**：
  - 每条 BDD 的判定必须引用 `P6-evidence/` 下真实证据文件（证据-结论对应，provenance 审计 1）
  - 证据文件真实存在、非空、md5 逐字节去重（check-p6-evidence.py）
  - dispatch-context 内容约束（provenance 审计 2）：P6-acceptance.md 与 dispatch-context 无矛盾
  - BDD 总数对照（provenance 审计 3）：P1 BDD 数 = P6 声明数（19）
  - EXIT_CODE 一致性（审计 5）：声称 exit 0 的命令必须在证据中留下 exit code
  - evidence JSON 与 PASS/FAIL 声明一致性（审计 6）
- **P6 验收不可裁剪**；provenance 审计不过 → gate 拦截
- **命令执行注意**：/tmp 只读（pytest 用 --basetemp + -p no:cacheprovider）；长命令外层 timeout；本环境 Linux
- **产出文件避免行首 `- PASS`/`- FAIL`**（provenance 预判检测）——判定放表格/正文内联

### 上游关联

P1 BDD-1~19（验收标准）/ P2 gate_commands（P5 已全过）/ P4 实现（6 交付物 + test 8/8）/ P5 全量通过（1036 passed / 0 ERROR / 1038 用例 / platform-assumptions 0 命中）。P4-review approved（0 BLOCKER）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-requirements.md（BDD 权威，19 条）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P2-design.md（设计与 gate_commands）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P5-verification.md（P5 证据，可引用）
- /home/kity/oclab/agate/agate/assets/execution-roles/verifier.md（角色定义，验收模式）
- /home/kity/oclab/agate/agate/phase-cards/P6-acceptance.md（阶段卡片：验收口径/证据规范）
- /home/kity/oclab/agate/agate/scripts/check-p6-evidence.py 与 check-p6-provenance.py（gate 判定依据）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；worktree 基线全绿（1036 passed / 0 ERROR / 1038 用例）；P5 已提交（40a9046）
- 关键标识：交付物在 /home/kity/oclab/agate/.worktrees/agate-TAG0018/agate/{assets/templates/dsh/,SETUP.md,platform-notes.md,tests/unit/test_dsh_preset.py}
- 查证结果：P1~P5 gate 全过；P4-review approved
</objective_info>
