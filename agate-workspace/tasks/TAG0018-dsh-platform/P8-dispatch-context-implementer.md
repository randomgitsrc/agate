...
执行 TAG0018 的 P8 发布准备：版本 bump v0.56.0 → **v0.57.0**，产出 `P8-release.md`（含 `bump_type:` 声明），完成发布检查清单。

### 约束

- **版本引用文件清单**（AGENTS.md 发布 checklist，agate 仓库特有）：README.md badge（v0.56.0 → v0.57.0）+ **README.zh-CN.md badge 同步** + CHANGELOG.md（[Unreleased] → 新增 [0.57.0] 章节，Keep a Changelog 格式：新增=DSH 平台支持 + 关键机制 + 技术债/修复）+ UPGRADING.md 新增 v0.57.0 章节（本版本无破坏性变更——新增平台支持，注明"DSH 接入见 SETUP.md 步骤 2-DSH"）
- **CHANGELOG [0.57.0] 内容**：TAG0018 agate 原生支持 DSH 平台（RM-AG0030）——assets/templates/dsh/（agent.cordis.yml + preset.yml + SKILL.md）、SETUP.md「步骤 2-DSH」、platform-notes.md DSH 条目、test_dsh_preset.py 回归（8 用例，含 tool-fs-search 必填配置守护）
- **P8 gate 判定**（check-gate.py P8）：`bump_type:` 命中；version 双路径检查（暂存区或最近 5 commit，WARNING）；CHANGELOG 双路径检查（WARNING）；`git log v0.56.0..HEAD --oneline` 对照 CHANGELOG 无遗漏
- **bump 后重跑 P5 gate_commands**（P8 卡要求）：`python3 -m pytest agate/tests/unit/test_dsh_preset.py -q -p no:cacheprovider --basetemp=/home/kity/oclab/dsh-workspace/ptmp` 8/8 绿 + consistency 0 ERROR
- **产出 P8-release.md**：bump_type: minor + 版本对照 + 发布检查清单逐项结果
- **commit 纪律**：bump 变更单独 commit（phase=P8）；UPGRADING.md 属 `agate/**/*.md` 触发 self-gate → commit message 带 `self-gate-skip:` 或 `self-gate-review:` 标记（self-gate review 由主 Agent 另行派发）

### 上游关联

P6 验收 19/19 PASS（0ccbfd7）。当前版本 v0.56.0（README badge + CHANGELOG + git tag）。全部改动已提交（P0~P6：4da6bfe…0ccbfd7）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P6-acceptance.md（验收结果，CHANGELOG 内容依据）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-requirements.md（BDD 摘要）
- /home/kity/oclab/agate/agate/assets/execution-roles/implementer.md（角色定义，P8/releaser 模式）
- /home/kity/oclab/agate/agate/phase-cards/P8-release.md（发布检查清单）
- /home/kity/oclab/agate/README.md、README.zh-CN.md、CHANGELOG.md、agate/UPGRADING.md（bump 对象）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；P6 已提交（0ccbfd7）；当前版本 v0.56.0 → 目标 v0.57.0
- 关键标识：分支 feat/TAG0018-dsh-platform；worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018
- 查证结果：P1~P6 gate 全过；全量 1036 passed / 0 ERROR / 1038 用例
</objective_info>
