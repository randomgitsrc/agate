...
执行 TAG0018 的 **self-gate review**（SELF-GATE.md 触发面：本任务改动了 `agate/**/*.md` 与新增模板文件）：审查分支 `feat/TAG0018-dsh-platform` 相对 main 的全部协议改动，产出成果文件 `docs/reviews/agate-alignment-review-2026-08-21-TAG0018.md`（结构化审查报告）。

### 约束

- **审查范围**（git diff main...HEAD 中 agate/** 相关）：
  - 新增 `agate/assets/templates/dsh/{agent.cordis.yml, preset.yml, SKILL.md}`
  - 修改 `agate/SETUP.md`（步骤 2-DSH 章节）、`agate/platform-notes.md`（DSH 条目）、`agate/tests/README.md`（映射行）、`agate/UPGRADING.md`（v0.57.0 章节）
  - 新增 `agate/tests/unit/test_dsh_preset.py`
- **审查要点（A1-A7 结构）**：协议文档与 gate 脚本对齐（新模板是否被一致性检查覆盖）、文档间互链一致性（SETUP↔platform-notes↔模板路径）、SELF-GATE 触发面声明准确性、命名/路径约定符合性（不发明新结构）、版本引用一致性（README badge/CHANGELOG/UPGRADING）、回归测试有效性
- **写入前检查**（DEBT0011 要求）：确认 `docs/reviews/agate-alignment-review-2026-08-21-TAG0018.md` 不存在（防跨任务覆盖）
- **产出文件避免行首 `- PASS`/`- FAIL`**

### 上游关联

P4 实现已过 review（approved）；P6 验收 19/19；P8 bump 完成（v0.57.0）。分支 9 commits（P0~P6 + bump）。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P6-acceptance.md（验收证据）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P8-release.md（发布清单）
- /home/kity/oclab/agate/agate/assets/review-roles/protocol-alignment-review.md（角色定义）
- /home/kity/oclab/agate/SELF-GATE.md（触发面与流程）
- 审查对象：git diff main...HEAD（worktree 内 `git diff main...HEAD -- agate/ README.md CHANGELOG.md`）
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018；分支 feat/TAG0018-dsh-platform
- 关键标识：main = 1272b0c（v0.56.0）；bump 目标 v0.57.0
- 查证结果：P1~P6 gate 全过；consistency 0 ERROR（319 WARNING，零新增）
</objective_info>
