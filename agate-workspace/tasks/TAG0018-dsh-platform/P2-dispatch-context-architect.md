...
产出 `P2-design.md`：TAG0018（agate 原生支持 DSH 平台）的方案设计。`design_trivial: true`（P1 已声明）——P0-brief 已锁定接入方案（符号链接 + agent-preset），P2 把草稿结构正式化即可，**只写 1 个候选方案**（design_trivial 允许），但必须满足 P2 gate 全部机械条件。

### 约束

- **P2 gate 机械条件**（check-gate.py P2）：P2-design.md 含 `packages:` / `domains:` / `ui_affected:` / `gate_commands:` 四行（≥4 命中）+ 权衡/取舍锚点（design_trivial 可用"选择"+理由简化）；`ui_affected: false`（无 UI）
- **`gate_commands.P5` 声明**（TAG0017 DEBT0012 教训）：不得把 `--strict` 放进 `&&` 链路中间；用 `--strict-errors-only`。每个命令声明 `_timeout_seconds` 字段（RM-AG0016 运行时管控）：pytest 单文件 ~5-10s、全量分片 ~120s
- **不发明新结构**（P0-brief 强制）：平台接入 = SETUP.md 文档化符号链接 + 唯一 install-hook.py；无 platforms/ 目录、无 per-platform installer
- **身份薄、协议厚**：preset persona 只写薄身份，行为规范指向 `{agate_root}/orchestrator-template.md`
- **测试平台无关**：test_dsh_preset.py 只校验仓库内文件
- 设计以 P1 BDD（BDD-1~19）为验收基线，逐条标注设计如何满足；参考实现（agate-copy 草稿）已实机验证，设计可引用但以 BDD 为准

### 上游关联

P1-requirements.md 已 approved（19 条 BDD，requirements-review 无 BLOCKER，5 条非阻塞建议：BDD-12 标题断言串缺右括号、P0_STALE 措辞澄清、preset.yml 可选元数据语义、BDD-18 基线数值钉死、待实机验证标注处理——P2 设计可吸收）。risk_level: low、domains: [cli, docs]、packages: [agate]、裁剪 P7。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-requirements.md（需求基线，权威）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P1-review.md（评审结论与建议）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P0-brief.md（任务简报）
- /home/kity/oclab/agate/agate/assets/execution-roles/architect.md（角色定义）
- /home/kity/oclab/agate/agate/WORKFLOW.md（P2 门槛）与 phase-cards/P2-design.md（阶段卡片，含 gate_commands 声明规范）
...
<objective_info>
- 环境状态：Linux；P1 已提交（03fdd8c）；worktree 基线 1028 passed / 0 ERROR
- 关键标识：分支 feat/TAG0018-dsh-platform；DSH preset 实机已验证（tool-fs-search 必填配置已修复）
- 查证结果：P1 BDD-1~19 全部 approved；裁剪 P7（coupling_checklist 已按 check-pruning 格式修正）
</objective_info>
