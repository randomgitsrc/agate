...
产出 `P1-requirements.md`：agate 原生支持 DSH 平台（deepseek-harness）的需求基线与 BDD 验收条件。需求覆盖六个交付物：① `assets/templates/dsh/agent.cordis.yml`（orchestrator agent-preset）② `assets/templates/dsh/preset.yml` ③ `assets/templates/dsh/SKILL.md`（agate-protocol skill）④ `SETUP.md`「步骤 2-DSH」章节 ⑤ `platform-notes.md` DSH 条目 ⑥ `tests/unit/test_dsh_preset.py` 回归测试。产出必须满足 P1 gate：≥1 条 BDD、无行首 `[NEED_CONFIRM]`、无 `status: GAP`、BDD 编号锚点 `BDD-[0-9]` 全覆盖。

### 约束

- **不发明新结构**（P0-brief 强制要求）：平台接入 = SETUP.md 文档化符号链接 + 唯一 install-hook.py。不得引入 `platforms/` 目录、不得引入 per-platform installer 脚本（install-dsh.py 已废弃）。P1 必须 grep 全仓核实无其他平台目录先例可复用，并在 BDD 中固化该约束的验收
- **身份薄、协议厚**：preset 的 persona 只写薄身份（你是谁 + 会话开始三步 + DSH 工具映射），行为规范指向 `{agate_root}/orchestrator-template.md`，模板随 `~/.agate` 升级自动更新——等价官方符号链接性质，不得把模板全文复制进 persona
- **测试平台无关**：`test_dsh_preset.py` 只校验仓库内模板文件（YAML 结构 / 必填配置 / 文档章节在位），不依赖真实 DSH 实例（CI 无 DSH）；不得假设符号链接语义、不得写 /tmp
- **回归必须覆盖实机缺陷**：agent.cordis.yml 的 tool-fs-search 行必须带 `config.sampleOverCapGlobResults: false`（DSH schemastery 必填无默认值，缺失导致 preset 挂载失败、DSH fail-closed 拒绝创建会话——2026-08-21 实机复现并修复）
- **SELF-GATE 触发面**：改动 `agate/**/*.md` 与 `agate/scripts/*.py` 均触发 self-gate，P1 需求无需包含 self-gate 流程本身（那是协议既有机制），但 BDD 中应声明触发文件清单以利 P8
- **同类扫描**：grep `assets/templates/` 现有模板命名风格、grep `SETUP.md` 各平台章节形态（OpenCode/Claude Code 步骤 2）、grep `platform-notes.md` 平台条目结构，BDD 需求与现有形态对齐
- 需求基线是"活的、向前累加"：BDD 用 Given/When/Then 写"做完后应表现成什么样"，不写实现方案

### 上游关联

P0-brief 已锁定（task/issues/known_risks/env_constraints 四字段）。实机验证证据（2026-08-21，DSH GUI）：preset 软链安装 → 热发现 → 选择器出现「agate 编排者 · 自定义」→ 设置持久化 → 新会话以 agate 编排者人格启动（探针回复实证）；发现并修复 tool-fs-search 缺必填配置。草稿实现与回归测试已在 `agate-copy` 中 TDD 红/绿验证，P1 以草稿为参考输入（路径见下），但 BDD 以"行为/结构验收"为准，不绑定草稿实现细节。

### 输入文件

- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/P0-brief.md（任务简报与风险声明，权威）
- {AGATE_WORKSPACE}/tasks/TAG0018-dsh-platform/.state.yaml（任务状态）
- /home/kity/oclab/agate/agate/assets/execution-roles/analyst.md（角色定义）
- /home/kity/oclab/agate/agate/WORKFLOW.md（P1 阶段门槛）
- /home/kity/oclab/agate/agate/SETUP.md（现有平台接入章节形态：步骤 2 OpenCode/Claude Code）
- /home/kity/oclab/agate/agate/platform-notes.md（平台条目结构）
- /home/kity/oclab/agate/agate/assets/templates/（模板命名与风格）
- 【参考实现（已实机验证，非权威）】/home/kity/oclab/dsh-workspace/agate-copy/agate/assets/templates/dsh/（agent.cordis.yml / preset.yml / SKILL.md）
- 【参考实现（已 TDD 验证，非权威）】/home/kity/oclab/dsh-workspace/agate-copy/agate/tests/unit/test_dsh_preset.py
- 【参考实现（已实机验证，非权威）】/home/kity/oclab/dsh-workspace/agate-copy/agate/SETUP.md 与 platform-notes.md 的 DSH 章节草稿
</dispatch_guide>
...
<objective_info>
- 环境状态：Linux；DSH 实机验证已完成（2026-08-21）；worktree 基线 pytest 全绿待确认（沙箱放开后重跑中）
- 关键标识：worktree = /home/kity/oclab/agate/.worktrees/agate-TAG0018；分支 feat/TAG0018-dsh-platform
- 查证结果：仓库无 platforms/ 目录、install 脚本仅 install-hook.py 一个（grep 待 P1 复核）
</objective_info>
