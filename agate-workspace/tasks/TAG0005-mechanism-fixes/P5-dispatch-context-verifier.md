...
执行 P2-design.md 声明的 gate_commands.P5 全部命令（1 主 2 辅），验证 TAG0005 六处修复后全量测试全绿，产出 P5-test-results/。

### 约束

- **P5 命令集（P2-design.md §3 固化，全部执行，不可只跑子集）**：
  ```yaml
  gate_commands:
...
- **P5_consistency 必须用 worktree 自己的脚本**（I13 双工作区纪律）：`python3 agate/scripts/check-protocol-consistency.py --strict`（以 worktree 为 cwd 即 worktree 脚本）
- **全量测试**：P5 命令即全量 bats（含 sanity/unit/regression/integration）——务必全量执行并确认全绿。发现预存失败（与本任务无关）→ 登记 known-failures.md + 在 unit.md 标注；不得擅自跳过。
- **compact 输出**：bats 默认紧凑；consistency `--strict` 已紧凑；shellcheck 默认紧凑。
- **判定**：三条命令全部 exit 0 才算通过。任何失败 → 记录 fail-list，判定真失败/环境问题。
- **产出**：
  - `P5-test-results/unit.md`（含测试运行器输出签名：`ok`/`not ok` 行 + 总数 + failed 标注 + 三条命令各自 exit code）
  - `P5-test-results/fail-list.txt`（无失败时为空文件）
  - 命令执行日志落盘（如 `P5-test-results/consistency.log` / `shellcheck.log`），末行 `EXIT_CODE: <n>`
- **格式约束**：约束节避免行首 `- PASS`/`- FAIL`（provenance 预检检测）。unit.md 内容用 bats 输出签名（ok/not ok 计数）。

### 上游关联

- `P2-design.md` §3 gate_commands（P5 命令集）
- `P4-implementation.md` / `P4-review.md`（实现说明 + approved）

### 输入文件

- `agate-workspace/tasks/TAG0005-mechanism-fixes/P2-design.md`（§3 gate_commands）
...
- P4 自查已跑：全量 726 bats 全绿 + consistency 0 ERROR + shellcheck 0 error——P5 需独立重跑确认（external-output-gate，不信 P4 自查）
- 注意：P5 的 WARNING「1 个主命令 + 2 个辅助命令」会由 check-gate.sh P5 输出——这正是 RM-AG0011 修复后的新文案，in-situ 验证
</objective_info>
