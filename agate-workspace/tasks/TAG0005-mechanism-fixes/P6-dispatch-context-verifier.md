...
产出 `P6-acceptance.md` + `P6-evidence/`，逐条验收 P1 的 16 条 BDD（PASS/FAIL 二值），证据落盘。

### 约束

- **验收口径**：功能任务标准口径。16 条 BDD 逐条实际验证（非文本声称），每条 PASS 必须有对应证据文件引用。
- **证据策略**（本任务为协议/脚本/文档修复，无 UI）：
  - 脚本行为类 BDD（BDD-3/4/5/6/10/11/16 + BDD-15 守卫）：跑实际命令/测试，输出落盘为证据（如 `bdd-3-p5-count.log`、`bdd-16-check-debt.log`）
  - 文档断言类 BDD（BDD-1/2/7/8/9/12/13/14）：grep/rg 断言输出落盘为证据（如 `bdd-1-c8-tables.log`、`bdd-9-review-instruction.log`）
  - 每条证据文件含实际命令输出（非 1 行文本充数），末行 `EXIT_CODE: <n>`
- **BDD 全覆盖**：P1 有 16 条 BDD（BDD-1..16）→ P6 必须有 16 条验收结果（PASS 或 FAIL），编号连续，不许挑验。
- **PASS 行格式**：`- PASS BDD-NN: {描述} ({证据路径})`，路径相对 P6-evidence/ 目录。勿用表格/emoji/其他格式。
...
- `P1-requirements.md`（16 BDD 验收条件 + 同类扫描结论）
- `P2-design.md`（§2.1-2.6 各修复设计 + §7 完成标准）
- `P5-test-results/`（P5 全绿证据，可引用）

### 输入文件

- `agate-workspace/tasks/TAG0005-mechanism-fixes/P1-requirements.md`（BDD，验收主要依据）
- `agate-workspace/tasks/TAG0005-mechanism-fixes/P2-design.md`（完成标准 §7）
- `agate-workspace/tasks/TAG0005-mechanism-fixes/P5-test-results/`（P5 证据）
- `agate-workspace/tasks/TAG0005-mechanism-fixes/P3-test-cases.md`（测试契约，验证方法）
- 按需核验的实际对象：
  - `agate/role-system.md`、`agate/rules/review-mapping.md`、`agate/phase-cards/P2-design.md`（BDD-1）
  - `agate/scripts/check-gate.sh`（BDD-2）
  - `agate/scripts/agate-gate-p5-count.py`、`agate/scripts/check-gate.sh`（BDD-3/4/5）
  - `agate/scripts/agate-read-p5-commands.py`（BDD-6）
  - `agate/scripts/agate-render-dispatch-prompt.sh`、`agate/assets/templates/dispatch-prompt.md`（BDD-7/8/9）
  - `agate/tests/unit/agate-render-dispatch-prompt.bats`（BDD-10/11）
  - `agate/dispatch-protocol.md`（BDD-12/13/14）
  - `agate/scripts/check-debt.sh`（BDD-16）
...
<objective_info>
- 环境状态：worktree `/home/kity/oclab/agate/.worktrees/agate-TAG0005-0009`；P5 已全绿（726 bats / consistency 0 ERROR / shellcheck 0 error）
- 证据目录：`agate-workspace/tasks/TAG0005-mechanism-fixes/P6-evidence/`（须创建，非空，每条 PASS 引用其中文件）
- P1 BDD 清单：BDD-1..16（见 P1-requirements.md §3），分组：RM-AG0010(1-2) / RM-AG0011(3-6) / RM-AG0012①(7-9) / RM-AG0012②(10-11) / RM-AG0003(12-14) / 守卫(15-16)
</objective_info>
