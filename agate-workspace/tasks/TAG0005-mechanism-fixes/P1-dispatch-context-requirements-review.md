...
评审 `P1-requirements.md`，按 requirements-review 角色检查清单逐项审查，产出 `P1-review.md`（status: approved / rejected / needs-revision，含 BDD 编号锚点引用 + 覆盖维度标注）。

### 约束

- **评审对象**：`agate-workspace/tasks/TAG0005-mechanism-fixes/P1-requirements.md`（analyst 新产出，15 条 BDD）
- **重点核查**（本任务是 agate 协议本体修复，以下为 analyst 已做但需独立复核的点）：
  1. RM-AG0010 方向决策（C8 补 backend P2 评审，非 gate 豁免）是否被 BDD-1/BDD-2 正确锁定
  2. RM-AG0012② 的判定（缺陷已修仅补测试）是否正确——可抽查 `agate/scripts/agate-render-dispatch-prompt.sh` L63-69 确认 exit 2 存在
  3. BDD 是否全部可二值判定、编号连续、无中间态
  4. 同类扫描结论（「静默 exit 0」/「无条件注入评审指令」/「P5 前缀计数」三组）是否可信——可抽查全仓 grep
  5. P1 纯净性：BDD 是否混入解决方案设计（如 BDD-3 提到「输出能区分主命令与辅助命令的数量」是否算实现细节；BDD-7 提到「render 脚本」是否算实现绑定）
- **只审不写**：不直接改 P1-requirements.md，产出评审意见。
- **格式约束**：评审文件避免行首 `- PASS`/`- FAIL`（provenance 预判检测）。用「通过/失败」或加引号；结论引用 BDD 编号锚点。

### 上游关联

- `P1-requirements.md`（评审对象，analyst 产出）
- `P0-brief.md`（任务简报，缺陷范围）
- 用户 RM-AG0010 方向决策：**C8 补 backend P2 评审**（2026-08-13 已确认）

### 输入文件

- `agate-workspace/tasks/TAG0005-mechanism-fixes/P1-requirements.md`（评审对象）
- `agate-workspace/tasks/TAG0005-mechanism-fixes/P0-brief.md`（范围基准）
- 按需核验：
  - `agate/scripts/agate-render-dispatch-prompt.sh`（RM-AG0012② 核实）
  - `agate/scripts/check-gate.sh`（P2 L157-173 / P5 L253-258）
  - `agate/scripts/agate-gate-p5-count.py` / `agate/scripts/agate-read-p5-commands.py`
  - `agate/role-system.md` / `agate/rules/review-mapping.md` / `agate/phase-cards/P2-design.md`（C8 表）
  - `agate/dispatch-protocol.md`（L105-129）
  - `agate/assets/templates/dispatch-prompt.md`（L9-13）
...
- analyst 产出 15 条 BDD（BDD-1/2: RM-AG0010；BDD-3/4/5/6: RM-AG0011；BDD-7/8/9: RM-AG0012①；BDD-10/11: RM-AG0012②；BDD-12/13/14: RM-AG0003；BDD-15: 同类扫描守卫）
- RM-AG0012② 已实测：render 脚本对不存在角色 EXIT=2（stderr 报错）
</objective_info>
