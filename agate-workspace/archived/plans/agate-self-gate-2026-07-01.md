...
CHECK 9 只能确认"关键词存在"，不能确认"语义一致"。已知的三类假阳性：

1. **值不匹配**：文档说"MAX_RETRY = 2"，脚本有 `MAX_RETRY=3` → CHECK 9 PASS（关键词存在），但语义不一致
2. **语义降级**：文档说"强制 PAUSED"，脚本有 `echo 警告` → CHECK 9 PASS，但语义不一致
3. **关键词在错误位置**：关键词出现在脚本里，但不在检查逻辑里——可能在不同检查块（如 `P6` 在检查 3、`跳过风险` 在检查 7，但检查 7 的条件不含 P6），或在 echo 错误消息文本里（如 `权衡` 只出现在 `echo "...需权衡..."` 字符串里，不在 grep 条件里）→ CHECK 9 PASS，但偏差原封不动

第 3 类是最隐蔽的——实测中 #7 和 #10 就是这种假阳性（见 §2.5）。**CHECK 9 PASS 的可信度有限，语义一致性由 Layer 1（LLM 审查）保证。**
...
| WARN # | 描述 | 能否加锚点 | 理由 |
|--------|------|-----------|------|
| #3 | md5 去重声称 hook 强制但未实现 | ✅ 已加 | `md5` / `去重` 关键词在 check-p6-evidence.sh 中完全缺失，CHECK 9 正确报 WARN |
...
| #7 | 裁剪"跳过风险"检查遗漏 P6 | ❌ 只靠 LLM（假锚点已删除）| `P6` 和 `跳过风险` 都在脚本里存在，但在不同检查块——关键词存在 ≠ 在正确位置，CHECK 9 会假 PASS |
| #8 | P8 裁剪文档说"+ 理由"，脚本只检查 internal_only | ❌ 只靠 LLM | "理由"是自由文本，关键词存在不等于理由存在 |
| #9 | BDD 总数文档内部矛盾（= vs ≥）| ❌ 只靠 LLM | 文档内部矛盾需要跨文件语义理解 |
| #10 | P2 候选方案文档说"权衡+选择理由"，脚本只查数量 ≥2 | ❌ 只靠 LLM（假锚点已删除）| `权衡`/`选择理由` 只出现在错误消息文本里，不在检查逻辑里——关键词在 echo 字符串里 ≠ 被用作检查条件，CHECK 9 会假 PASS |

**结论**：8 条 WARN 中 1 条可加锚点（#3），7 条只能靠 LLM 层。标注盲区本身有价值——维护者知道 CHECK 9 全绿不等于语义对齐，7 类偏差只能靠 LLM 审查覆盖。

**⚠️ 假阳性风险**：#7 和 #10 原本标"✅ 已加"，实测发现是假锚点——关键词在脚本里存在但不在检查逻辑里（#7 在不同检查块，#10 在 echo 消息文本）。假锚点比没锚点更有害：给维护者"CHECK 9 覆盖了"的虚假安全感。删除这两条锚点，改回"只靠 LLM"。
...
用结构对比验证审计发现的 2 个 ERROR 场景（不需要 LLM）：

| # | 用例 | 方法 | 预期 |
|---|------|------|------|
| 1 | MAX_RETRY 文档说 2 脚本写 3 | 从 state-machine.md 提取各阶段 MAX_RETRY 值，从 check-state-transition.sh 提取 `MAX_RETRY=` 赋值，比较一致 | FAIL（不一致）|
| 2 | 回退跳变文档说强制 PAUSED 脚本降级 WARNING | 从 state-machine.md 提取"强制 PAUSED"，从 check-state-transition.sh 检查是否有 `exit 1`（而非只有 `echo`）| FAIL（脚本无 exit 1）|
...
1. **建 self-gate 机制** — CHECK 9 + 角色文件 + 派发模板（先把刀磨好）
2. **用新机制修审计发现的 2 个 ERROR**（#1 MAX_RETRY + #2 回退跳变）— 用 CHECK 9 + LLM 审查验证修复
3. **全量测试 + consistency + shellcheck**
4. **版本号 v0.8.0**

---

## 9. 递归适用：本方案自身也走 self-gate

### 9.1 原则

**self-gate 机制自身的实施也必须走 self-gate 流程。** 制法者不守法，和 agate 要防的"主 Agent 跳过 gate"是同一个问题。

本方案的实施会改动：
- `agate/scripts/check-protocol-consistency.py`（加 CHECK 9）
- `agate/assets/review-roles/protocol-alignment-review.md`（新角色文件）
- `agate/dispatch-protocol.md`（加派发模板）
- `agate/AGENTS.md`（加检查清单）
- `agate/LIMITATIONS.md`（更新局限 5）
- `agate/orchestrator-template.md`（Hardening 节）

每一项都命中 self-gate 触发条件（`agate/scripts/*` + `agate/**/*.md`），因此实施时必须：
1. 跑 check-protocol-consistency.py（含新加的 CHECK 9）确认无 ERROR
2. 派发 protocol-alignment-review subagent 审查本次变更
3. 读审查报告，MISALIGNED 必须修复
4. 跑全量 bats 确认无退化

### 9.2 对其他 plan 的约束

**任何针对 agate 的变更 plan，在实施时都必须走 self-gate 流程。** 包括但不限于：

- `agate-hotfix-evidence-2026-07-01.md`（hotfix 协议 + evidence L3 + 诊断优先）
- 本方案自身
- 未来所有改 agate 协议本体或脚本的 plan

实施者（主 Agent）在开始实施任何 agate 变更前，应确认 self-gate 机制已就绪。如果 self-gate 尚未实现（如本方案还在 plan 阶段），实施者至少要手动执行 self-gate 的等价检查：
1. 跑现有 check-protocol-consistency.py（即使还没有 CHECK 9）
2. 人工逐项核对"文档描述的规则 vs 脚本实现"是否一致（对照附录 A 审计清单的 A1-A6）
3. 跑全量 bats

### 9.3 实施顺序的递归约束

§8 的实施顺序"先建机制再修 ERROR"正是递归适用的体现——先建好 self-gate 机制，后续所有 agate 变更（含修 ERROR、实施 hotfix-evidence plan）都能用新机制审查。如果先修 ERROR 再建机制，等于跳过 self-gate 直接改——违反本节原则。
