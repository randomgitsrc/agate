...
P6 --[P6-acceptance.md 有效 AND P1 的每条 BDD 条件标记为 - PASS 或 - FAIL（行首标记格式，二值，不允许"调整/跳过/覆盖"）AND P6 验收条数 = P1 BDD 总数 AND 无 - FAIL AND 无 [NEED_CONFIRM]]--> P7
...
P7 --[! grep -qE '^\s*-?\s*\[BLOCKER\]' P7-consistency.md AND ! grep -qE '^\s*-?\s*\[DEVIATION-CRITICAL\]' P7-consistency.md]--> P8
