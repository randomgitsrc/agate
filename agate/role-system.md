...
| 任意 | **high** | **plan-eng-review 必须派发**（P2.1 硬规则，hook 对 agent=main 输出 WARNING 建议派发独立 subagent）|
...
主 Agent 派发 subagent 时，在 prompt 里指定角色定义文件（**实际派发使用 `assets/templates/dispatch-prompt.md` 完整模板**——含分阶段落盘、环境隔离、PROD_TOUCHED、dispatch-context.md、阶段特定提示等。本节只展示最简骨架）：
...
见 `assets/templates/custom-role.md`（权威来源，含分阶段落盘 / dispatch-context.md / PROD_TOUCHED 等现代角色必备节）。
