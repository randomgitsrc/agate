...
主 Agent 派发 subagent 时，在 prompt 里指定角色定义文件（**实际派发使用 `assets/templates/dispatch-prompt.md` 完整模板**——含分阶段落盘、环境隔离、PROD_TOUCHED、dispatch-context、阶段特定提示等。本节只展示最简骨架）：
...
见 `assets/templates/custom-role.md`（权威来源，含分阶段落盘 / dispatch-context / PROD_TOUCHED 等现代角色必备节）。
