...
#   check-debt.sh --retreat-coverage      # 回退覆盖比对（只读 WARNING）
# exit 0 = 通过（FILE 模式：schema 合法 / 文件不存在 / 无 yaml 块；覆盖模式：比对完成无缺失，或无 retreat 提交等有意跳过分支）
# exit 1 = FILE 模式：schema 非法或校验器异常（fail-closed）
# exit 2 = 覆盖模式：依赖加载失败（agate-workspace-resolve.sh 缺失/source 失败），需主 Agent 自判
#
...
#     source: retreat 条目的 evidence 引用比对，缺失打 WARNING；依赖加载失败 exit 2（需主 Agent 自判），
#     无 retreat 提交等有意跳过分支仍 exit 0（不阻断）。
...
if [ "$MODE" = "--retreat-coverage" ]; then
    ...
    if [ -f "$SCRIPT_DIR/agate-workspace-resolve.sh" ]; then
        source "$SCRIPT_DIR/agate-workspace-resolve.sh" "$REPO_ROOT" \
            || { echo "GATE DEBT: 无法加载 agate-workspace-resolve.sh" >&2; exit 2; }
    else
        echo "GATE DEBT: 缺少 agate-workspace-resolve.sh，无法解析工作区，回退覆盖比对无法执行" >&2
        exit 2
    fi
