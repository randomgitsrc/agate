...
vision_analysis:
  ...
  bdd_results:
    - condition: "Given 用户进入条目列表 When 列表为空 Then 显示空状态提示文字和图标"
      result: "pass"                    # 只允许 pass 或 fail（二值，不允许中间态）
...
若被派发执行视觉分析，**先自查当前环境能否调用视觉能力**（视觉模型 / 视觉分析 skill / 图像读取
工具）：
- 能 → 正常执行（按任务所选证据形式分析：截图 / 帧序列逐帧描述帧间差异与时序 / 渲染输出对比
  描述结果差异）
- 不能 → 明确报告 `[CAPABILITY_GAP]` 并走降级路径（文档条文 / 像素检测 / 人工复核记录），
  **不静默假设**、不编造观察结果（字节比对失败时必须诚实上报，不用 naturalWidth>0 糊弄）

**分析对象按渲染形态适配**（不写死视觉工具，仅作机制描述）：常规布局型分析截图；渲染组件型可分析
帧序列（`frames/`，逐帧描述帧间差异/时序，判定是否与 BDD 时序判据对齐）或渲染输出对比
（`renders/`，描述实际输出 vs 参考的差异并给出 diff 度量解读）；时序特效型分析时序截图
（`screenshots/{bdd-id}-t{N}.png`，逐时刻描述状态与 BDD 时序断言对齐）。帧序列/输出对比的
结构化描述同样写入 vision YAML 的 bdd_results（结果只允许 pass/fail）。
...
**截图质量标准**
分析截图时，若发现操作类 BDD 的多张截图内容完全相同（可能是同一截图重复使用），在 anomalies 中标注 `type: screenshot_duplicate`。操作类 BDD 截图必须互不相同（md5 去重），查询类 BDD 可不截图。
