...
agate 协议里散落在正文的机器读取字段（P1/P2/P6/P7 共约 40+ 个）此前靠正则从全文 grep 提取，长期存在格式摩擦——全角冒号、缩进错误、总结行误判等问题，v0.30.2 → v0.35.0 连续 5 个版本打同类补丁仍未根治。v2.0 需要一种机制把这些字段变成可靠的机器可读格式。P2 设计阶段对比了两条候选路径，完整权衡矩阵与选择理由见 `docs/tasks/T001-v2.0-structured/P2-design.md` §1。
