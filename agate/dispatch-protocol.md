...
**P7 例外**：一致性检查天然需要跨文件对照，不受输入文件数限制。consistency-reviewer 角色文件明确列出输入文件和关注点，dispatch-context.md 提供关键决策摘要减少提取量。
