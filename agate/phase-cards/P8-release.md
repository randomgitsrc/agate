...
- **P5 验证（TAG0016 BDD-14 精简为条件化表述，底线不变——至少一次客观验证动作不可省）**：
  若 `check-p6-provenance.py` 审计 7（BDD-12）判定 P8 发起时点距 P5 通过点（`.state.yaml`
  的 `p5_pass_commit`）之间无代码改动 → 复用同一份 `P5-test-results/`（不重新执行命令）；
  否则完整重跑 `gate_commands.P5`（exit 0 + failed==0）
