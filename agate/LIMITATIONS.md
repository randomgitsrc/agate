...
T026 事故：主 Agent 编造 11/16 条 BDD 验收结果（不跑验证脚本直接写 PASS），`grep PASS` 仍然通过——因为文件内容和 grep 条件都是主 Agent 自己写的，作者和裁判是同一人。缓解措施：P6 gate 追加 `P6-evidence/` 目录非空检查（外部产出作为必要条件），但**证据存在 ≠ 证据与结论对应**——这是必要非充分条件，self-authored gate 的伪造风险只能缓解，无法根治。
...
**降级缓解（v2 客观行为审计，已实现）**：
- P6 验收：`scripts/check-p6-provenance.sh` 三道客观审计
  - 证据-结论对应：每条 PASS 的证据引用路径必须存在 + PASS 数 ≤ 证据文件数（空证据拦截）+ 每个证据文件必须被引用（空 png 充数拦截）
...
- 已知局限：主 Agent 可伪造证据文件（如空 png 充数），但成本远高于填一行自报字段。CI git blame 单 author WARNING 作为兜底审计。证据-结论对应检查只验证引用存在性和数量，不验证证据内容真实性（如截图是否为真实 UI 截图 vs 纯色 png）
