...
档位 C（"主 Agent 自动跑完 P1-P8"）在 hardening 后承担新的硬约束：**每次阶段 commit 都要先过 pre-commit hook**。这是 /loop 推进时必须严格遵守的客观边界。
...
单步函数推进（如 P4→P5）：
  ...
  │    └─ pre-commit hook 触发
  │         ├─ exit 0（通过）→ 自动进入 P5
  │         ├─ exit 1（拦截）→ PAUSED，主 Agent 分析错误并修复后重试
  │         └─ exit 2（WARNING）→ 通过（记入日志）
...
- `risk=high` + `agent=main` 自审触发 WARNING（仅信息层，不阻塞——但主 Agent 应考虑暂停并派发独立 subagent 评审）
