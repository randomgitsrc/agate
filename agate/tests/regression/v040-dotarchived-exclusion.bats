...
# 是 ".archived"（带前导点，如 docs/tasks/{Txxx}/.archived/{timestamp}-{phase}/），从未被排除过
...
@test "R-DA.1 iter_md_files 排除路径含 .archived（带点）分量的文件" {
    ...
    mkdir -p "$tmpdir/docs/tasks/T001-fake/.archived/20260101-000000-P6"
    echo "not: valid: yaml: [" > "$tmpdir/docs/tasks/T001-fake/.archived/20260101-000000-P6/bad.md"
    mkdir -p "$tmpdir/docs/tasks/T001-fake"
    echo "# live file" > "$tmpdir/docs/tasks/T001-fake/live.md"
...
@test "R-DA.2 iter_md_files 仍排除路径含 archived（不带点）分量的文件（既有行为不回归）" {
    ...
    mkdir -p "$tmpdir/docs/archived/tasks/T001-fake"
    echo "not: valid: yaml: [" > "$tmpdir/docs/archived/tasks/T001-fake/bad.md"
