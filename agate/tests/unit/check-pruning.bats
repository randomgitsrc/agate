#!/usr/bin/env bats
# tests/unit/check-pruning.bats — 19 用例覆盖 check-pruning.py
...
# （不再是 v0.35 的正文追加）；check-pruning.py 的读取行为须与 v0.35 保持一致
...
@test "P2.2 check-pruning.py 裁剪 P2 期望 exit 1" {
    local dir
...
@test "P2.3a check-pruning.py 裁剪 P2 + legacy_p2_pruned 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P3 P4 P5 P6 P7 P8)
...
@test "P2.3b check-pruning.py 裁剪 P2 + design_trivial 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P3 P4 P5 P6 P7 P8)
...
@test "P2.3c check-pruning.py 裁剪 P2 + follows_existing_pattern 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P3 P4 P5 P6 P7 P8)
...
@test "P2.4 check-pruning.py 裁剪 P6 期望 exit 1" {
    local dir
...
@test "P2.4a check-pruning.py 裁剪 P6 + no_behavior_change 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P7 P8)
...
@test "P2.5c check-pruning.py P4 裁剪 期望 exit 1" {
    local dir
...
@test "P2.5d check-pruning.py P5 裁剪 期望 exit 1" {
    local dir
...
@test "P2.5 BDD-9: check-pruning.py 旧格式（--legacy-fields，risk_level 在正文非 frontmatter）risk=high 裁剪 P3 期望 exit 1（回退路径行为与 v0.35 一致）" {
    local dir
...
@test "P2.5b check-pruning.py risk=medium 裁剪 P3 期望 exit 1（P1-8: 仅 low 可裁）" {
    local dir
...
@test "P2.6a check-pruning.py 裁剪 P7，源文件数 > 5 期望 exit 1" {
    local dir
...
@test "P2.6b check-pruning.py 裁剪 P7，源文件数 ≤ 5 + coupling_checklist 通过" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P8)  # P7 不在声明
...
@test "P2.6c BDD-1: check-pruning.py 裁剪 P7 + frontmatter implicit_coupling 字段 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P8)  # P7 不在声明
...
@test "P2.6d check-pruning.py 裁剪 P7 无 coupling_checklist 期望 exit 1" {
    local dir
...
@test "P2.6e check-pruning.py 裁剪 P7 + coupling_checklist 放行" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P8)
    add_p1_field "$dir" "coupling_checklist" "[api-schema: checked, data-model: checked]"
    add_pruning_excuse "$dir" P7 "小改动" "低"
...
@test "P2.7 check-pruning.py 裁剪 P8 无 internal_only 期望 exit 1" {
    local dir
...
@test "P2.7a BDD-1: check-pruning.py 裁剪 P8 + frontmatter internal_only: true + internal_only_reason 放行" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P7)
...
@test "P2.8 check-pruning.py 裁剪理由缺'跳过风险' 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P3 P4 P5 P6 P7 P8)
    # 写裁剪理由但不写跳过风险
...
@test "P2.12 check-pruning.py P6 裁剪无跳过风险 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P7 P8)
    add_p1_field "$dir" "no_behavior_change" "true"
    run "$PYTHON" "$AGATE_SCRIPTS/check-pruning.py" "$dir"
...
@test "P2.12a check-pruning.py P6 裁剪 + no_behavior_change + 跳过风险 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P7 P8)
    add_p1_field "$dir" "no_behavior_change" "true"
    add_pruning_excuse "$dir" P6 "无行为变更" "低"
...
@test "P2.13 check-pruning.py 裁剪 P8 有 internal_only 无 reason 期望 exit 1" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P7)
    add_p1_field "$dir" "internal_only" "true"
    add_pruning_excuse "$dir" P8 "内部任务" "低"
...
@test "P2.14 check-pruning.py 裁剪 P8 + internal_only + internal_only_reason 期望 exit 0" {
    local dir
    dir=$(create_task_dir P0 P1 P2 P3 P4 P5 P6 P7)
    add_p1_field "$dir" "internal_only" "true"
    add_p1_field "$dir" "internal_only_reason" "内部工具，无外部用户"
...
@test "P2.9 check-pruning.py 裁剪声明 vs 实际有产出文件 + 无 override 期望 exit 1" {
    local dir
...
@test "P2.9a check-pruning.py 裁剪 P2 + override + 产出文件 期望 exit 1 (P2 不可裁)" {
    local dir
    dir=$(create_task_dir P0 P1 P3 P4 P5 P6 P7 P8)
    echo "actual design" > "$dir/P2-design.md"
...
@test "P2.10 check-pruning.py 无 P1 文件 期望 exit 2" {
    local dir
    dir=$(mktemp -d "$BATS_TEST_TMPDIR/task-XXXXXX")
    run "$PYTHON" "$AGATE_SCRIPTS/check-pruning.py" "$dir"
    [ "$status" -eq 2 ]
...
@test "P2.11 check-pruning.py 全合规 (happy path) 期望 exit 0" {
    local dir
    dir=$(create_task_dir)
    run "$PYTHON" "$AGATE_SCRIPTS/check-pruning.py" "$dir"
    [ "$status" -eq 0 ]
