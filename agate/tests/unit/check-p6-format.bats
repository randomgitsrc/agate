...
@test "F1 check-p6-format.py --check: clean file → exit 0" {
...
EOF
    run "$PYTHON" "$AGATE_ROOT/scripts/check-p6-format.py" --check "$TASK_DIR/P6-acceptance.md"
    [ "$status" -eq 0 ]
}

@test "F2 check-p6-format.py --check: lowercase pass → exit 1" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- pass BDD-1: verified (evidence/log.json)
EOF
...
@test "F3 check-p6-format.py --fix: lowercase pass → auto-fix → exit 0" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- pass BDD-1: verified (evidence/log.json)
EOF
    run "$PYTHON" "$AGATE_ROOT/scripts/check-p6-format.py" --fix "$TASK_DIR/P6-acceptance.md"
    [ "$status" -eq 0 ]
    grep -q '^\- PASS BDD-1' "$TASK_DIR/P6-acceptance.md"
}

@test "F5 check-p6-format.py --check: no P6 file → exit 0" {
    TASK_DIR=$(create_task_dir)
    run "$PYTHON" "$AGATE_ROOT/scripts/check-p6-format.py" --check "$TASK_DIR/P6-acceptance.md"
    [ "$status" -eq 0 ]
}

@test "F_BDD17.1 BDD-17: check-p6-format.py --check 行首 - PASS|FAIL BDD-NN: 格式被识别为有效逐条结果" {
...
@test "F8 check-p6-format.py --check: lowercase fail: → exit 1" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- fail: BDD-2 broken
EOF
...
@test "F9 check-p6-format.py --fix: lowercase fail with space → auto-fix" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- fail BDD-3: timeout
EOF
...
@test "F10 check-p6-format.py --fix: 'failure' NOT matched (word boundary)" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
...
@test "F12 check-p6-format.py --fix: summary line - PASS：34 → **Summary**: PASS: 34" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- PASS BDD-1: verified (evidence/log.json)
- PASS：34
...
@test "F13 check-p6-format.py --fix: POSIX locale 下全角冒号总结行仍被归一化（locale 回归）" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- PASS BDD-1: verified (evidence/log.json)
- PASS：34
- FAIL：0
EOF
    run env LC_ALL=POSIX LANG= "$PYTHON" "$AGATE_ROOT/scripts/check-p6-format.py" --fix "$TASK_DIR/P6-acceptance.md"
    [ "$status" -eq 0 ]
    grep -q '^\*\*Summary\*\*: PASS: 34' "$TASK_DIR/P6-acceptance.md"
    grep -q '^\*\*Summary\*\*: FAIL: 0' "$TASK_DIR/P6-acceptance.md"
    ! grep -q '^- PASS：34' "$TASK_DIR/P6-acceptance.md"
...
@test "F_P6FMFIX.1 check-p6-format.py --fix: frontmatter 的 pass:/fail: 字段不被正文归一化 sed 误伤，仍为合法 YAML" {
...
@test "F_P6FMFIX.2 check-p6-format.py --fix: frontmatter 存在时正文总结行仍被归一化为 **Summary** 格式" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
---
phase: P6
...
@test "F_P6FMFIX.3 check-p6-format.py --fix: 无 frontmatter 闭合边界的畸形文件回退按正文整体处理（不误判为已切分）" {
...
@test "bdd-12 check-p6-format.py --fix LC_ALL=C 小写 fail 全角冒号总结行归一化（line 69 路径，M5）" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- PASS BDD-1: verified (evidence/log.json)
- fail：3
...
@test "bdd-13 check-p6-format.py --fix+--check LC_ALL=C 半角冒号总结行行为不变（v0.40.3 回归，M5）" {
    TASK_DIR=$(create_task_dir)
    cat > "$TASK_DIR/P6-acceptance.md" <<'EOF'
- PASS BDD-1: verified (evidence/log.json)
- FAIL: 3
EOF
    ...
    [ "$status" -eq 0 ]
    grep -q '^\*\*Summary\*\*: FAIL: 3' "$TASK_DIR/P6-acceptance.md"
    run env LC_ALL=POSIX LANG= "$PYTHON" "$AGATE_ROOT/scripts/check-p6-format.py" --check "$TASK_DIR/P6-acceptance.md"
    [ "$status" -eq 0 ]
}
