...
EOF
    ...
    run bash -c "cd '$repo' && bash -x '$AGATE_SCRIPTS/check-state-transition.sh' .state.yaml 2>&1"
    echo "ST4-DIAG-STATUS: $status" >&2
    printf 'ST4-DIAG-TRACE: %s\n' "$output" | grep -E 'old_phase|new_phase|diff=|exit|case|GATE STATE' | head -20 >&2
