...
setup() {
    CARD_CMD="$AGATE_SCRIPTS/agate-next-card.py"
...
@test "P0: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P0 | tail -n +5 | sha256sum | awk '{print $1}')"
...
@test "P1: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P1 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P2: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P2 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P3: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P3 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P4: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P4 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P5: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P5 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P6: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P6 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P7: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P7 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "P8: CLI body sha256 == 卡片文件 sha256（防漂移前提）" {
    ...
    actual_hash="$("$PYTHON" "$CARD_CMD" P8 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$actual_hash" = "$expected_hash" ]
}
...
@test "字节稳定性：连续两次调用 P3 sha256 一致" {
    local hash1 hash2
    hash1="$("$PYTHON" "$CARD_CMD" P3 | sha256sum | awk '{print $1}')"
    hash2="$("$PYTHON" "$CARD_CMD" P3 | sha256sum | awk '{print $1}')"
    [ "$hash1" = "$hash2" ]
}
...
@test "CWD 在项目目录仍能解析 AGATE_ROOT" {
    local hash
    hash="$(cd "$BATS_TEST_TMPDIR" && "$PYTHON" "$CARD_CMD" P3 | sha256sum | awk '{print $1}')"
    [ -n "$hash" ]
}
...
@test "软链接场景：脚本被 symlink 调用时 readlink -f 解析正确" {
    ...
    mkdir -p "$link_dir"
    ln -sf "$AGATE_SCRIPTS/agate-next-card.py" "$link_dir/card"
    hash="$("$PYTHON" "$link_dir/card" P3 | sha256sum | awk '{print $1}')"
    local expected_hash
    expected_hash="$("$PYTHON" "$AGATE_SCRIPTS/agate-next-card.py" P3 | sha256sum | awk '{print $1}')"
    [ "$hash" = "$expected_hash" ]
}
...
@test "跨 checkout 路径：通过不同根路径调用 CLI 全量 hash 一致（P0-P8）" {
    ...
    ln -sf "$AGATE_SCRIPTS/agate-next-card.py" "$link_a/card"
    ln -sf "$AGATE_SCRIPTS/agate-next-card.py" "$link_b/card"
    for phase in P0 P1 P2 P3 P4 P5 P6 P7 P8; do
        hash_a="$("$PYTHON" "$link_a/card" "$phase" | sha256sum | awk '{print $1}')"
        hash_b="$("$PYTHON" "$link_b/card" "$phase" | sha256sum | awk '{print $1}')"
        [ "$hash_a" = "$hash_b" ] || {
...
@test "2 个参数 期望 exit 1" {
    run "$PYTHON" "$CARD_CMD" P3 extra
    [ "$status" -eq 1 ]
    [[ "$output" == *"需要 1 个参数"* ]]
}

@test "phase=P9 期望 exit 2" {
    run "$PYTHON" "$CARD_CMD" P9
    [ "$status" -eq 2 ]
    [[ "$output" == *"不在 P0-P8 范围内"* ]]
}

@test "phase=小写 p3 期望 exit 2（case-sensitive）" {
    run "$PYTHON" "$CARD_CMD" p3
    [ "$status" -eq 2 ]
}
...
@test "NC_ROOT.1 AGATE_ROOT 环境变量覆盖" {
    ...
    hash_default="$("$PYTHON" "$CARD_CMD" P3 | sha256sum | awk '{print $1}')"
    hash_override="$(AGATE_ROOT="$AGATE_ROOT" "$PYTHON" "$CARD_CMD" P3 | sha256sum | awk '{print $1}')"
...
@test "NC_ROOT.2 协议目录不在 git 仓库内时仍能工作" {
    ...
    cp "$AGATE_ROOT/scripts/agate-next-card.py" "$tmp_root/scripts/"
    hash_main="$("$PYTHON" "$CARD_CMD" P3 | tail -n +5 | sha256sum | awk '{print $1}')"
    hash_tmp="$(AGATE_ROOT="$tmp_root" "$PYTHON" "$tmp_root/scripts/agate-next-card.py" P3 | tail -n +5 | sha256sum | awk '{print $1}')"
    [ "$hash_main" = "$hash_tmp" ]
}
# ========== TAG0004 Q1 路径归一化（BDD-21/22） ==========

@test "bdd-21 agate-next-card.py Windows 盘符/反斜杠 AGATE_ROOT 前缀剥离稳定（Q1）" {
    ...
    mkdir -p "$dir/$root_agate/phase-cards"
    cp "$AGATE_ROOT/phase-cards/P3-tdd.md" "$dir/$root_agate/phase-cards/"
    run bash -c "cd '$dir' && AGATE_ROOT='$root_agate' '$PYTHON' '$AGATE_SCRIPTS/agate-next-card.py' P3"
    [ "$status" -eq 0 ]
    # 修复前：${CARD_FILE#$AGATE_ROOT/} 把反斜杠当转义 → 剥离失败 → 相对路径含盘符前缀；修复后：phase-cards/P3-tdd.md
    [[ "$output" == *"路径：phase-cards/P3-tdd.md"* ]]
}

@test "bdd-22 agate-next-card.py Linux 常规路径前缀剥离字节不变（Q1 回归）" {
    run "$PYTHON" "$AGATE_SCRIPTS/agate-next-card.py" P3
    [ "$status" -eq 0 ]
    [[ "$output" == *"路径：phase-cards/P3-tdd.md"* ]]
}
