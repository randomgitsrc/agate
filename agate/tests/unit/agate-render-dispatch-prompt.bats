...
@test "RP.1: rejects missing arguments" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py"
    [ "$status" -eq 1 ]
}

@test "RP.2: rejects invalid phase" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P9 architect "$TEST_TASK_DIR"
    [ "$status" -eq 2 ]
}

@test "RP.3: rejects nonexistent task dir" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P2 architect "/nonexistent"
    [ "$status" -eq 2 ]
}

@test "RP.4: placeholder replacement for phase/role/task_id" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P2 architect "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
...
@test "RP.6: P4 without --rollback selects P4 normal appendix" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P4 implementer "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
    [[ "$output" == *"上下文控制"* ]]
    [[ "$output" != *"回退诊断"* ]]
}
...
@test "RP.7: P4 with --rollback selects P4 rollback appendix" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P4 implementer "$TEST_TASK_DIR" --rollback
    [ "$status" -eq 0 ]
    [[ "$output" == *"回退诊断"* ]]
    [[ "$output" != *"上下文控制"* ]]
}
...
@test "RP.8: P5/P6 share same appendix (screenshot quality)" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P5 verifier "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
    [[ "$output" == *"截图质量标准"* ]]
    [[ "$output" != *"上下文控制"* ]]
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P6 verifier "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
    [[ "$output" == *"截图质量标准"* ]]
}

@test "RP.9: P8 selects P8 appendix (READY check)" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P8 implementer "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
    [[ "$output" == *"READY 收尾检查"* ]]
    [[ "$output" != *"上下文控制"* ]]
}
...
@test "RP.10: role with special characters produces safe filename" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P2 "design-review" "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
...
@test "RP.12: --rollback ignored for non-P4 phases" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P2 architect "$TEST_TASK_DIR" --rollback
    [ "$status" -eq 0 ]
    [[ "$output" == *"P2 最小验证"* ]]
    [[ "$output" != *"回退诊断"* ]]
}
...
@test "RP.16: P3 renders P3 self-check appendix" {
    local output
    output=$(AGATE_ROOT="$AGATE_ROOT" "$PYTHON" "$AGATE_SCRIPTS/agate-render-dispatch-prompt.py" P3 test-designer "$TEST_TASK_DIR")
    [[ "$output" == *"P3 自检"* ]]
}

@test "RP.17: 角色文件不存在 -> exit 2 + stderr 报错（回归锁定 v0.23.0 修复，BDD-10/11）" {
    run "$PYTHON" "$AGATE_ROOT/scripts/agate-render-dispatch-prompt.py" P2 nonexistent-role "$TEST_TASK_DIR"
    [ "$status" -eq 2 ]
    [[ "$output" == *"角色文件不存在"* ]]
}
...
@test "bdd-20 agate-render-dispatch-prompt.py AGATE_ROOT 含 & 时替换按字面插入（其他-c）" {
    local root
    ...
    run env AGATE_ROOT="$root" "$PYTHON" "$AGATE_SCRIPTS/agate-render-dispatch-prompt.py" P4 implementer "$TEST_TASK_DIR"
    [ "$status" -eq 0 ]
