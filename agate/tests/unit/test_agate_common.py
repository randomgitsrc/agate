...
_APPEND_FIRST = (
    ...
    "line = open(td + '/gate-events.jsonl').readline().rstrip('\\n'); "
    "obj = json.loads(line); "
...
_APPEND_TWO = (
    ...
    "lines = open(td + '/gate-events.jsonl').readlines(); "
...
_APPEND_TS_FALLBACK = (
    ...
    "open(td + '/gate-events.jsonl', 'w').write(json.dumps(tail, sort_keys=True) + '\\n'); "
    "append_event(td, {'event': 'state_transition', 'phase': 'P6.5', 'from': 'P6', 'to': 'P7'}); "
    "lines = open(td + '/gate-events.jsonl').readlines(); "
    "second = json.loads(lines[1]); "
